...
class ModelCatalogProduct extends Model {
	public function getAdditionalProductes()
	{

		
	}
	public function addProduct($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', tax_class_id = '" . $this->db->escape($data['tax_class_id']) . "', sort_order = '" . (int)$data['sort_order'] . "', date_added = NOW(),is_main_sku='".(int)$data['is_main_sku']."',main_sku='".$this->db->escape($data['main_sku'])."',item_grade='".$this->db->escape($data['item_grade'])."'");
	...
	public function editProduct($product_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$data['price'] . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', tax_class_id = '" . $this->db->escape($data['tax_class_id']) . "', sort_order = '" . (int)$data['sort_order'] . "', date_modified = NOW(),is_main_sku='".(int)$data['is_main_sku']."',main_sku='".$this->db->escape($data['main_sku'])."',item_grade='".$this->db->escape($data['item_grade'])."',is_kit='".(int)$data['is_kit']."' WHERE product_id = '" . (int)$product_id . "'");
	...
	public function getProductsBySKU($sku) {
		$query = $this->db->query("SELECT DISTINCT *, (SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = 'product_id=" . (int)$product_id . "') AS keyword FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE LCASE(p.sku) = '" . strtolower($sku) . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
	...
	public function getProducts($data = array()) {
		if ($data) {
			...
			if (!empty($data['filter_product_id'])) {
				$sql .= " AND LCASE(p.product_id) = '" . $this->db->escape(utf8_strtolower($data['filter_product_id'])) . "'";
			}
	...
	public function getOutOfStockProducts()
	{
		...
		$query = $this->db->query("SELECT p.sku,d.name,p.quantity FROM `".DB_PREFIX."product` p,`".DB_PREFIX."product_description` d WHERE p.product_id=d.product_id AND p.quantity<=0 AND p.status=1 AND p.location IN ('','0')");	
		return $query->rows;
	}
	
	public function updateMainProductImage($data)
	{
...
$products = $this->getProductsBySKU($sku);
		...
		foreach($products as $product)
		{
			...
			$image = $dir.$sku.'-'.$this->changeImageName($product_details['name']).'-1.'.end(explode(".",$data[1]));
		...
		$query = $this->db->query("UPDATE `".DB_PREFIX."product` SET image='".$image."' WHERE product_id='".$product_details['product_id']."'");
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_image WHERE product_id = '" . (int)$product_details['product_id'] . "'");
		}
		...
		if($data1['image']!='')
		{
			...
			$image = $dir.$sku.'-'.$this->changeImageName($product_details['name']).'-'.($i+1).'.'.end(explode(".",$data1['image']));
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "product_image SET product_id = '" . (int)$product_details['product_id'] . "', image = '" . $this->db->escape(html_entity_decode($image, ENT_QUOTES, 'UTF-8')) . "', sort_order = '" . $i . "'");
		$i++;
		}
		}
	...
	public function updateLowerGradeImage($data)
	{
...
$products = $this->getProductsBySKU($sku);
		...
		$query = $this->db->query("UPDATE `".DB_PREFIX."product` SET is_main_sku='".(int)$data[1]."',main_sku='".$this->db->escape($data[2])."',item_grade='".$this->db->escape($data[3])."' WHERE product_id='".$product_details['product_id']."'");
	...
	public function changeImageName($data)
	{
		//$data = trim($data);
		//$array_search = array(" ","_","&","\"","'","+","=","*","@","/",'quot;');
		//$name = str_replace($array_search,"-",($data));
		//return $name;
   ...
   return preg_replace('/[^A-Za-z0-9\-]/', '', $string); // Removes special chars.
	...
	public function updateSubProductImage($datas,$product_id,$datam)
	{
		$this->db->query("DELETE FROM " . DB_PREFIX . "product_image WHERE product_id = '" . (int)$product_id . "'");
		...
		if($data['image']!='')
		{
			$dir = 'data/'.$this->changeImageName($datam[0]).'/'.$this->changeImageName($datam[1]).'/';
			
			if (!file_exists('../image/'.$dir)) {
    mkdir('../image/'.$dir, 0775, true);
}
			
			$image = $dir.$this->changeImageName($datam[1]).'-'.$this->changeImageName($datam[2]).'-'.($i+1).'.'.end(explode(".",$data['image']));
			
			if(copy('../image/'.$data['image'], '../image/'.$image)){
				
				
			}
			else
			{
			file_put_contents('import_csv.log', PHP_EOL."Main Image:".$data[4].PHP_EOL."Renamed Image: ".$image."Date Time: ".date('d-m-Y h:i:s').PHP_EOL.PHP_EOL."****************************************", FILE_APPEND);	
			}
			
		
		$this->db->query("INSERT INTO " . DB_PREFIX . "product_image SET product_id = '" . (int)$product_id . "', image = '" . $this->db->escape(html_entity_decode($image, ENT_QUOTES, 'UTF-8')) . "', sort_order = '" . $i . "'");
		$i++;
		}
		}
