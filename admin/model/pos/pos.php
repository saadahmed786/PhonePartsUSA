...
class ModelPosPos extends Model {
	...
	// print_r($data);exit;
		...
		// Restock products before subtracting the stock later on
        	...
        	$check_ref_order_id = $order_query->row;
        	$check_ref_order_id = $check_ref_order_id['ref_order_id'];
        	if($check_ref_order_id)
        	{
        		$inv_order_id = $check_ref_order_id;
        	}
        	else
        	{
        		$inv_order_id = $order_id;
        	}
        	$this->db->query("UPDATE inv_orders SET order_status='".$_order_status."', auth_qc='".$data['auth_qc']."', auth_manager='".$data['auth_manager']."' WHERE order_id='".$inv_order_id."'");
        	...
        	if (isset($data['order_product'])) {
        		...
        		foreach ($data['order_product'] as $order_product) {
        			...
        			$this->db->query("UPDATE inv_orders_items SET product_unit='".(float)$order_product['price']."',product_price='".(float)$order_product['total']."',product_qty='".(int)$order_product['quantity']."' WHERE product_sku='".$this->db->escape($order_product['model'])."' and order_id='".$inv_order_id."'");
        			$remainingItems[] = $order_product['model'];
        			$order_product_id = $this->db->getLastId();
        		...
        		$this->db->query("DELETE FROM inv_orders_items WHERE product_sku NOT IN ('".implode('\',\'', $remainingItems)."') and order_id='".$inv_order_id."'");
        	}
		...
		// Update order total			 
        	...
        	$this->db->query("UPDATE inv_orders SET order_price='".(float)$total."',paid_price='".(float)$data['pos_total']."' WHERE order_id = '" . $inv_order_id . "'");
        	return $order_id;
        }
	...
	// add for Browse begin
        ...
        public function getCustomer($customer_id){
        	...
        	$query = $this->db->query("SELECT a.* FROM `" . DB_PREFIX . "customer` a WHERE a.customer_id = '".$customer_id."'");
        	return $query->row;
        }
        ...
        public function getCustomerTax($customer_id)
        {
        ...
        public function get_today_card($user_id){
        	$query = $this->db->query('SELECT sum(pos_total) as total FROM `' . DB_PREFIX . 'order` WHERE user_id = "' . $user_id . '" AND payment_method ="Card" AND DATE(date_modified) ="'.date('Y-m-d').'"');
        	$row = $query->row;

        	$query = $this->db->query('SELECT sum(card_split) as total FROM `' . DB_PREFIX . 'order` WHERE user_id = "' . $user_id . '" AND payment_method ="Cash,Card" AND DATE(date_modified) ="'.date('Y-m-d').'"');
        	$row2 = $query->row;
        ...
        public function get_today_paypal($user_id){
        	$query = $this->db->query('SELECT sum(pos_total) as total FROM `' . DB_PREFIX . 'order` WHERE payment_method in("PayPal","Paypal Express") AND DATE(date_modified) ="'.date('Y-m-d').'" and order_status_id IN(3,11) AND user_id = "'. $user_id .'"');
        	$row = $query->row;
        	return $row['total'];
        }
        ...
        public function get_today_cash($user_id){
        	$query = $this->db->query('SELECT sum(pos_total) as total FROM `' . DB_PREFIX . 'order` WHERE user_id = "' . $user_id . '" AND payment_method="Cash" and order_status_id IN(3,11) AND DATE(date_modified) ="'.date('Y-m-d').'"');
        	$row = $query->row;

        	$query = $this->db->query('SELECT sum(cash_split) as total FROM `' . DB_PREFIX . 'order` WHERE user_id = "' . $user_id . '" AND payment_method="Cash,Card" and order_status_id IN(3,11) AND DATE(date_modified) ="'.date('Y-m-d').'"');
        	$row2 = $query->row;
        ...
        public function makeVoidReport($data)
        {
        	...
        	$this->db->query("INSERT INTO `".DB_PREFIX."void_product` SET order_id='".(int)$data['order_id']."',product_id='".(int)$data['product_id']."',quantity='".(int)$data['quantity']."',amount='".(float) $amount."',user_id='".$this->user->getId()."',reason_id='".(int)$data['reason_id']."',date_added=NOW() ");
        ...
        public function storeStatusSet($status)
        {
        	$this->db->query('UPDATE `' . DB_PREFIX . 'setting` SET `value` = "'. (int) $status .'" WHERE `group` = "POS" AND `key` = "store_status"');
        	if (!$status) {
        		$this->db->query('UPDATE `' . DB_PREFIX . 'drawer` SET `status` = "Unassigned", user_id="0", close_drawer_id="0"');
        	}
        }
    }
    ?>