...
class ModelModuleExcelportproduct extends ModelModuleExcelport {
	public function importXLSProductsFull($language, &$allLanguages, $file, $importLimit, $addAsNew = false) {
		...
		$chunkFilter = new CustomReadFilter(array('Products' => array('A', ($this->productSize*$progress['importedCount'] + 2), 'Q', ($this->productSize*($progress['importedCount'] + $importLimit) + 1))), true); 
		...
		do {
			...
			if (!empty($product_name)) {
				...
				$product_id = $addAsNew ? '' : $productSheetObj->getCell(PHPExcel_Cell::stringFromColumnIndex($source[0] + $map['product_id'][0]) . ($source[1] + $map['product_id'][1]))->getValue();
				...
				if (version_compare(VERSION, '1.5.6', '>=')) {
					$profile_customer_groups = explode(',', strval($productSheetObj->getCell(PHPExcel_Cell::stringFromColumnIndex($source[0] + $map['profiles'][0]) . ($source[1] + $map['profiles'][1]))->getValue()));
					
					foreach ($profile_customer_groups as $profile_customer_group) {
						$profile_customer_group = explode(':', trim($profile_customer_group));
						
						if (!empty($profile_customer_group[0]) && !empty($profile_customer_group[1])) $product_profile[] = array(
							'profile_id' => trim($profile_customer_group[0]),
							'customer_group_id' => trim($profile_customer_group[1])
						);
					}
				}
	...
	public function importXLSProductsLight($language, &$allLanguages, $file, $importLimit, $addAsNew = false) {
		...
		require_once(IMODULE_ROOT.'vendors/phpexcel/CustomReadFilter.php');
		$chunkFilter = new CustomReadFilter(array('Products' => array('A', ($progress['importedCount'] + 2), 'AM', (($progress['importedCount'] + $importLimit) + 1))), true); 
		...
		do {
			...
			if (!empty($product_name)) {
				...
				$product_id = $addAsNew ? '' : $productSheetObj->getCell(PHPExcel_Cell::stringFromColumnIndex($source[0] + $map['product_id']) . ($source[1]))->getValue();
	...
	public function exportXLSProductsFull($language, $store, $destinationFolder = '', $productNumber, $export_filters = array()) {
		...
		if (version_compare(VERSION, '1.5.6', '>=')) {
			for ($i = 0; $i < count($profiles); $i++) {
				$metaSheetObj->setCellValueExplicit(PHPExcel_Cell::stringFromColumnIndex($profilesStart[0]) . ($profilesStart[1] + $i), $profiles[$i]['profile_id'], PHPExcel_Cell_DataType::TYPE_STRING);
		...
		if ($products->num_rows > 0) {
			foreach ($products->rows as $myProductIndex => $row) {
				...
				if (empty($row['profiles'])) $row['profiles'] = '';
	...
	public function getQuery($filters = array(), $store = 0, $language = 1, $count = false) {
		...
		$select = $count ? "COUNT(*)" : "*, pd.name as name, pd.description as description, pd.meta_description as meta_description, pd.meta_keyword as meta_keyword, " . (version_compare(VERSION, '1.5.4', '<') ? 'pt.tag as tag' : 'pd.tag as tag') . ", p.*";
	...
	public function addProduct($product_id = '', $data, &$allLanguages) {
		...
		foreach ($data['product_description'] as $language_id => $value) {
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_description SET product_id = '" . (int)$product_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', description = '" . $this->db->escape($value['description']) . "'" . $extra_insert . " ON DUPLICATE KEY UPDATE name = '" . $this->db->escape($value['name']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', description = '" . $this->db->escape($value['description']) . "'" . $extra_insert);
		}
