...
class ControllerCatalogProduct extends Controller {
  ...
  public function exportOos()
  {
	
	$this->load->model('catalog/product');
	
	$data = $this->model_catalog_product->getOutOfStockProducts();
	
header("Content-Type: text/csv");
header("Content-Disposition: attachment; filename=OutOfStockData-".date('d-m-Y').".csv");
// Disable caching
...
fputcsv($output, $row); // here you can change delimiter/enclosure
 ...
 fclose($output);
  ...
  public function export_lower_grade()
  {
...
header("Content-Type: text/csv");
header("Content-Disposition: attachment; filename=LowerGradeProducts-".date('d-m-Y').".csv");
// Disable caching
header("Cache-Control: no-cache, no-store, must-revalidate"); // HTTP 1.1
header("Pragma: no-cache"); // HTTP 1.0
header("Expires: 0"); // Proxies
$headings = array('SKU','Is Main SKU','Main SKU','Item Grade');
$output = fopen("php://output", "w");
...
foreach($data as $row)
{
	if($row['sku']!='')
	{
	$xdata[] = array('sku'=>$row['sku'],'is_main_sku'=>$row['is_main_sku'],'main_sku'=>$row['main_sku'],'item_grade'=>$row['item_grade']);
	}
//fputcsv($output, $row); // here you can change delimiter/enclosure
}
fputcsv($output, $headings); // here you can change delimiter/enclosure
foreach($xdata as $row)
{
 ...
 fclose($output);
  ...
  public function exportSubImages()
  {
	
	$this->load->model('catalog/product');
	
	$datam = $this->model_catalog_product->getProducts(array('filter_status'=>1));
	$i=1;
	...
	{
		$images = $this->model_catalog_product->getProductImages($data['product_id']);
		//if(count($images)>0)
		//{
...
fputcsv($output, $row); // here you can change delimiter/enclosure
 ...
 fclose($output);
	...
	public function importSubImages() {
    	$this->load->language('catalog/product');

    	$this->document->setTitle('Import Sub Images CSV'); 
    ...
    while ($data = fgetcsv($handle,1000,",","'")) {
        ...
        if ($data[0]!='') {
			...
			$this->model_catalog_product->updateMainProductImage($data);  
		...
		file_put_contents('import_csv.log', PHP_EOL."Filename:".$_FILES['csv']['name'].PHP_EOL."data: ".json_encode($data)."Date Time: ".date('d-m-Y h:i:s').PHP_EOL.PHP_EOL."****************************************", FILE_APPEND);	
		}
		}
		$k++;
    } 
	...
	$this->data['action'] = $this->url->link('catalog/product/importSubImages', 'token=' . $this->session->data['token'] . $url, 'SSL');
	...
	$this->data['breadcrumbs'] = array();
    	...
    	$this->template = 'catalog/import_image_form.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	public function import_lower_grade() {
    	$this->load->language('catalog/product');

    	$this->document->setTitle('Import Lower Grade CSV'); 
    ...
    while ($data = fgetcsv($handle,1000,",","'")) {
        ...
        if ($data[0]!='') {
			...
			$this->model_catalog_product->updateLowerGradeImage($data);  
	...
	//exit;
	$this->session->data['success'] = 'CSV Successfully Imported!';
		 $this->redirect($this->url->link('catalog/product/import_lower_grade', 'token=' . $this->session->data['token'] . $url, 'SSL'));	
			}
	
	
	$this->data['cancel'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . $url, 'SSL');
	$this->data['action'] = $this->url->link('catalog/product/import_lower_grade', 'token=' . $this->session->data['token'] . $url, 'SSL');
	...
	$this->data['breadcrumbs'] = array();
    	...
    	$this->template = 'catalog/import_lower_grade.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
  	...
  	private function getList() {				
		...
		$this->data['export_out_of_stock'] = $this->url->link('catalog/product/exportOos', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['export_sub_images'] = $this->url->link('catalog/product/exportSubImages', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$this->data['import_sub_images'] = $this->url->link('catalog/product/importSubImages', 'token=' . $this->session->data['token'] . $url, 'SSL');
  	...
  	private function getForm() {
		...
		if (isset($this->request->post['is_main_sku'])) {
      		$this->data['is_main_sku'] = $this->request->post['is_main_sku'];
    	} elseif (!empty($product_info)) {
			$this->data['is_main_sku'] = $product_info['is_main_sku'];
		} else {
      		$this->data['is_main_sku'] = 0;
    	}
		
		
		if (isset($this->request->post['main_sku'])) {
      		$this->data['main_sku'] = $this->request->post['main_sku'];
    	} elseif (!empty($product_info)) {
			$this->data['main_sku'] = $product_info['main_sku'];
		} else {
      		$this->data['main_sku'] = '';
    	}
		
		
		if (isset($this->request->post['item_grade'])) {
      		$this->data['item_grade'] = $this->request->post['item_grade'];
    	} elseif (!empty($product_info)) {
			$this->data['item_grade'] = $product_info['item_grade'];
		} else {
      		$this->data['item_grade'] = '';
    	}
		
		if (isset($this->request->post['is_kit'])) {
      		$this->data['is_kit'] = $this->request->post['is_kit'];
    	} elseif (!empty($product_info)) {
			$this->data['is_kit'] = $product_info['is_kit'];
		} else {
      		$this->data['is_kit'] = '';
    	}
		...
		if (isset($this->request->post['retail_price'])) {
      		$this->data['retail_price'] = $this->request->post['retail_price'];
    	} elseif (!empty($product_info)) {
			$this->data['retail_price'] = $product_info['retail_price'];
		} else {
