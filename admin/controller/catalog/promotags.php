...
class ControllerCatalogPromoTags extends Controller {
	private $error = array();
  	...
  	private function getList() {
		...
		foreach ($results as $result) {
			...
			if ($result['pimage'] && file_exists(DIR_IMAGE . $result['pimage'])) {
				$pimage = $this->model_tool_image->resize($result['pimage'], 50, 50);
			} else {
				$pimage = $this->model_tool_image->resize('no_image.jpg', 50, 50);
			}
			
			$this->data['promotags'][] = array(
				'promo_tags_id' 	=> $result['promo_tags_id'],
				'promo_text'    	=> $result['promo_text'],
				'promo_direction'   => $result['promo_direction'],
				'promo_link'     	=> $result['promo_link'],
  	...
  	private function getForm() {
		...
		if (isset($this->request->post['promo_direction'])) {
      		$this->data['promo_direction'] = $this->request->post['promo_direction'];
    	} elseif (isset($promotags_info)) {
			$this->data['promo_direction'] = $promotags_info['promo_direction'];
		} else {	
      		$this->data['promo_direction'] = '';
    	}
		
		
		if (isset($this->request->post['promo_link'])) {
      		$this->data['promo_link'] = $this->request->post['promo_link'];
    	} elseif (isset($promotags_info)) {
			$this->data['promo_link'] = $promotags_info['promo_link'];
		} else {	
      		$this->data['promo_link'] = '';
    	}
		
		if (isset($this->request->post['image'])) {
			$this->data['image'] = $this->request->post['image'];
		} elseif (isset($promotags_info)) {
			$this->data['image'] = $promotags_info['image'];
		} else {
			$this->data['image'] = '';
		}
		
		if (isset($this->request->post['pimage'])) {
			$this->data['pimage'] = $this->request->post['pimage'];
		} elseif (isset($promotags_info)) {
			$this->data['pimage'] = $promotags_info['pimage'];
		} else {
			$this->data['pimage'] = '';
		}
		...
		if (isset($promotags_info) && $promotags_info['image'] && file_exists(DIR_IMAGE . $promotags_info['image'])) {
			$this->data['thumb'] = $this->model_tool_image->resize($promotags_info['image'], 100, 100);
		} else {
			$this->data['thumb'] = $this->model_tool_image->resize('no_image.jpg', 100, 100);
		}
		
		if (isset($promotags_info) && $promotags_info['pimage'] && file_exists(DIR_IMAGE . $promotags_info['pimage'])) {
			$this->data['thumb1'] = $this->model_tool_image->resize($promotags_info['pimage'], 100, 100);
		} else {
  	...
  	private function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $promotags_id) {
			...
			$promotags_top_left = $this->model_catalog_promotags->getTotalProductsByPromoTagsTopLeftId($promotags_id);
			$promotags_bottom_right = $this->model_catalog_promotags->getTotalProductsByPromoTagsBottomRightId($promotags_id);
			$promotags_bottom_left = $this->model_catalog_promotags->getTotalProductsByPromoTagsBottomLeftId($promotags_id);
			
			if ($promotags_top_right) {
	  			$this->error['warning'] = sprintf($this->language->get('error_promotags'), $promotags_top_right);	
			}

			if ($promotags_top_left) {
	  			$this->error['warning'] = sprintf($this->language->get('error_promotags'), $promotags_top_left);	
			}	
			
			if ($promotags_bottom_right) {
	  			$this->error['warning'] = sprintf($this->language->get('error_promotags'), $promotags_bottom_right);	
			}	
			
			if ($promotags_bottom_left) {
	  			$this->error['warning'] = sprintf($this->language->get('error_promotags'), $promotags_bottom_left);	
			}	
	  	} 
