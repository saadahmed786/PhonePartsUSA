...
class ControllerSaleOrder extends Controller {
    ...
    public function prepare_order() {
        ...
        $products = $this->request->post['product_items'];
        ...
        $order_info['shipping_code'] = $this->request->post['shipping_code'];
        $order_info['shipping_method'] = $this->request->post['shipping_method'];
        ...
        foreach ($products as $product) {
            $product_det = explode("-", $product);
            $product_info = $this->model_catalog_product->getProduct($product_det[0]);
            $data['order_product'][] = array(
        ...
        $json['success'] = 'Successful Order is maintained.';
    ...
    public function getOrderProducts() {
        ...
        $product_details = $this->model_sale_order->getOrder($order_id);





        if (strtotime($product_details['date_added']) < strtotime('-30 days')) {
            $isOld = true;
        } else {
            $isOld = false;
        }
        ...
        if ($product_details['payment_code'] == 'pp_standard' or $product_details['payment_code'] == 'authorizenet_aim' or $product_details['payment_code'] == 'paypal_express') {
            $allowedShippingMethod = 15;

            $orderHistories = $this->model_sale_order->getOrderHistoriesByStatusID($order_id, $allowedShippingMethod);

            if (!$orderHistories and $product_details['order_status_id'] != $allowedShippingMethod) {
                echo 'Store Credit cannot be issued for this Order~error';
                exit;
            }
        } else if ($product_details['payment_code'] == 'cod') {
            $allowedShippingMethod = 3;

            if ($product_details['order_status_id'] != $allowedShippingMethod) {
                echo 'For Cash at Store Pickup Payments, the order must be shipped~error';
                exit;
            }
        } else {
            echo 'The Store Credit can only be generated for Paypal, Authorize.net &amp; Cash at Pickup Payment Methods~error';
            exit;
        }
        ...
        foreach ($products as $product_related) {
            ...
            for ($i = 1; $i <= $product_related['quantity']; $i++) {
                if (!$isOld) {

                    $price = $product_related['price'];
                } else {
                    $price = $this->model_catalog_product->getProduct($product_related['product_id']);
                ...
                <div id="product-related<?php echo $product_related['product_id']; ?>" class="<?php echo $class; ?>"> <input type="checkbox" id="checkbox-<?php echo $i; ?>" name="product_items[]" value="<?php echo $product_related['product_id'] . '-' . $price; ?>" onclick="updateAmount()" class="amount_checkbox" /> <?php echo $product_related['name']; ?>  (<?php echo '$' . number_format($price, 2); ?>)
        ...
        foreach ($order_total as $order_total) {
            ...
            if ($order_total['code'] == 'shipping') {
                $shipping_method = $order_total['title'] . ' (' . $order_total['text'] . ')';
                $shipping_price = $order_total['value'];
            }
        }
        echo $shipping_method . '<input type="hidden" id="shipping_price" value="' . $shipping_price . '" />';
    }

    public function getOrderProductsWithReason() {
        $this->load->model('sale/order');
        $this->load->model('catalog/product');
        ...
        } else if ($product_details['payment_code'] == 'in_store') {
        ...
        foreach ($products as $product_related) {
            ...
            for ($i = 1; $i <= $product_related['quantity']; $i++) {
                ...
                <div id="product-related<?php echo $product_related['product_id']; ?>" class="<?php echo $class; ?>"> <input type="checkbox" id="checkbox-<?php echo $k; ?>" name="product_items[]" value="<?php echo $product_related['product_id'] . '-' . $price; ?>" onclick="updateAmount()" class="amount_checkbox" /> <?php echo $product_related['model'] . ' - ' . $product_related['name']; ?>  (<?php echo '$' . number_format($price, 2); ?>) <select name="reason[<?php echo $product_related['product_id']; ?>]" id="reason-<?php echo $k; ?>" onchange="">
...
public function tni_verification() {
    $telephone = $this->request->get['telephone'];
    $order_id = $this->request->get['order_id'];


    $request1 = '?phone=' . urlencode($telephone) . '&l=' . urlencode($this->config->get('config_fraud_key'));
    $curl = curl_init('https://api.maxmind.com/app/phone_id_http' . $request1);
    ...
    if (isset($data['phoneType'])) {
        $tni_phoneType = $data['phoneType'];
    } else {
        $tni_phoneType = '';
    }

    if (isset($data['refid'])) {
        $tni_refid = $data['refid'];
    } else {
        $tni_refid = '';
    }
    if (isset($data['city'])) {
    ...
    if (isset($data['countryname'])) {
        $tni_countryname = $data['countryname'];
    } else {
        $tni_countryname = '';
    }

    if (isset($data['latitude'])) {
        $tni_latitude = $data['latitude'];
    } else {
        $tni_latitude = '';
    }

    if (isset($data['longitude'])) {
        $tni_longitude = $data['longitude'];
    } else {
        $tni_longitude = '';
    }
    if (isset($data['err'])) {
    ...
    $this->db->query("UPDATE `" . DB_PREFIX . "order_fraud` SET tni_phoneType='" . $this->db->escape($tni_phoneType) . "'
        ,tni_refid='" . $this->db->escape($tni_refid) . "'
        ,tni_city='" . $this->db->escape($tni_city) . "'
        ,tni_state='" . $this->db->escape($tni_state) . "'
        ,tni_zip='" . $this->db->escape($tni_zip) . "'
        ,tni_countryname='" . $this->db->escape($tni_countryname) . "'
        ,tni_latitude='" . $this->db->escape($tni_latitude) . "'
        ,tni_longitude='" . $this->db->escape($tni_longitude) . "'
        ...
        <table class="form">
            ...
            if ($tni_phoneType) {
                ...
                switch ($tni_phoneType) {
                    case 0:
                    $phoneType = 'Undetermined';
                    $riskLevel = 'Medium';
                    break;
                    ...
                    break;
                    case 8:
                    $phoneType = 'Invalid Number';
                    $riskLevel = 'High';
                    break;
                    case 9:
                    $phoneType = 'Restricted Number';
                    $riskLevel = 'High';
                    break;
                    case 10:
                    $phoneType = 'Personal';
                ...
                <tr>
                    <td>Zip:</td>
                    <td><?php echo $tni_zip; ?></td>
                </tr>
                <?php } ?>


                <?php if ($tni_countryname) { ?>
                <tr>
                    <td>Country:</td>
                    <td><?php echo $tni_countryname; ?></td>
                </tr>
                <?php } ?>
                ...
                <tr>
                    <td>Latitude:</td>
                    <td><?php echo $tni_latitude; ?></td>
                </tr>
                <?php } ?>


                <?php if ($tni_longitude) { ?>
                <tr>
                    <td>Longitude:</td>
                    <td><?php echo $tni_longitude; ?></td>
                </tr>
                <?php } ?>
        ...
        public function getForm() {
            ...
            if (isset($this->request->post['company'])) {
                $this->data['payment_company'] = $this->request->post['company'];
            } elseif (!empty($order_info)) {
                $this->data['payment_company'] = $order_info['company'];
            } else {
            ...
            if (isset($this->request->post['shipping_company'])) {
                $this->data['shipping_company'] = $this->request->post['company'];
            } elseif (!empty($order_info)) {
                $this->data['shipping_company'] = $order_info['company'];
            } else {
        ...
        public function info() {
            ...
            $order_mod_history = $this->model_sale_order->getModHistory($order_id);
            if ($order_info) {
                $this->load->language('sale/order');
                ...
                $this->data['heading_title'] = $this->language->get('heading_title');
                $this->data['order_mod_history'] = $order_mod_history;
                ...
                $this->data['ref_order_id'] = $order_info['ref_order_id'];
            ...
            // Maintain the orders to be visible against the order no
            // Fraud
                ...
                if ($fraud_info) {
                    ...
                    $this->data['tni_phoneType'] = $fraud_info['tni_phoneType'];
                    $this->data['tni_refid'] = $fraud_info['tni_refid'];
                    $this->data['tni_city'] = $fraud_info['tni_city'];
                    $this->data['tni_state'] = $fraud_info['tni_state'];
                    $this->data['tni_zip'] = $fraud_info['tni_zip'];
                    $this->data['tni_countryname'] = $fraud_info['tni_countryname'];
                    $this->data['tni_latitude'] = $fraud_info['tni_latitude'];
                    $this->data['tni_longitude'] = $fraud_info['tni_longitude'];
                } else {
                    $this->data['maxmind_id'] = '';
                }
                $this->data['xuser_group_id'] = $this->user->getUserGroupId();

                $this->data['employee_allowed_order_status'] = array(3, 7);
        ...
        public function history() {
            ...
            if ($this->request->server['REQUEST_METHOD'] == 'POST') {
                ...
                if (!$this->data['error']) {
                    ...
                    if ($this->request->post['order_status_id'] == 15) {
                        $this->db->query("UPDATE inv_orders SET order_status='Processed' WHERE order_id='" . (int) $this->request->get['order_id'] . "'");
                    } elseif ($this->request->post['order_status_id'] == 3) {
                        $this->db->query("UPDATE inv_orders SET order_status='Shipped' WHERE order_id='" . (int) $this->request->get['order_id'] . "'");
                    } elseif ($this->request->post['order_status_id'] == 21) {
                        $this->db->query("UPDATE inv_orders SET order_status='On Hold' WHERE order_id='" . (int) $this->request->get['order_id'] . "'");
                    } elseif ($this->request->post['order_status_id'] == 7) {
                        $this->db->query("UPDATE inv_orders SET order_status='Canceled' WHERE order_id='" . (int) $this->request->get['order_id'] . "'");
                    }
                    $this->db->query("UPDATE " . DB_PREFIX . "order SET date_modified=NOW() WHERE order_id='" . (int) $this->request->get['order_id'] . "'");
            ...
            foreach ($results as $result) {
                $this->data['histories'][] = array(
                    ...
                    'store_credit' => ($result['store_credit']),
                    'code' => ($result['code']),
                    'amount' => ($result['amount']),
                    'date_added' => date($this->language->get('date_format_short'), strtotime($result['date_added'])),
                    'user_name' => $this->model_sale_order->getHistoryUser($result['order_history_id'])
                    );
            }
        ...
        public function invoice() {
            ...
            foreach ($orders as $order_id) {
                ...
                $checkForRefundInvoice = $this->db->query("SELECT order_id,ref_order_id FROM " . DB_PREFIX . "order WHERE ref_order_id='" . ($this->db->escape($order_id)) . "' ");
                $checkForRefundInvoice = $checkForRefundInvoice->row;

                $ref_order_id = '';
                if ($checkForRefundInvoice) {
                    $order_id = $checkForRefundInvoice['order_id'];
                    $ref_order_id=$checkForRefundInvoice['ref_order_id'];
                }
                if(!$ref_order_id)
                {
                 $checkForRefundInvoice2 = $this->db->query("SELECT order_id,ref_order_id FROM " . DB_PREFIX . "order WHERE order_id='" . ($this->db->escape($order_id)) . "' ");
                 $checkForRefundInvoice2 = $checkForRefundInvoice2->row;

                 if ($checkForRefundInvoice2) {
                    $order_id = $checkForRefundInvoice2['order_id'];
                    $ref_order_id=$checkForRefundInvoice2['ref_order_id'];
                }
            ...
            if ($order_info) {
                ...
                //Void Report Items
                $void_items = $this->model_sale_order->getVoidProductsByOrderId($order_id);

                foreach ($void_items as $void_item) {
                    $product_data[] = array(
                        'name' => $void_item['name'],
                        'model' => $void_item['sku'],
                        'option' => array(),
                        'quantity' => $void_item['quantity'],
                        //'quantity' => 0,
                        'price' => $void_item['void_type'],
                        //'price'    => $this->currency->format(0, $order_info['currency_code'], $order_info['currency_value']),
                        'total' => $this->currency->format(0, $order_info['currency_code'], $order_info['currency_value'])
                        );
                }


                // End Void Report Items
                ...
                $vouchers = $this->model_sale_order->getOrderVouchersAndProducts($order_id);

                //$vouchers = $this->model_sale_order->getOrderVouchers($order_id);

                foreach ($vouchers as $voucher) {
                    $product_ids = explode(',', $voucher['product_ids']);
                    foreach ($product_ids as $key => $value) {
                        $product_ids[$key] = substr($value, 0, strpos($value, '-'));
                    }
                    $product_ids = array_count_values($product_ids);
                    $vProducts = array();
                    foreach ($product_ids as $key => $value) {
                        $vProductSku = $this->model_sale_order->getProductSku($key);
                        $vProducts[] =  $vProductSku[0]['model'] . ' x' . $value;
                    }
                    $voucher_data[] = array(
                        'description' => 'Voucher Issued (<strong>' . $voucher['code'] . '</strong>) / Amt: $' . number_format($voucher['amount'], 2) . ' / ' . implode(', ', $vProducts) . ' ' . $voucher['rtitle'],
                        'amount' => $this->currency->format($voucher['amount'], $order_info['currency_code'], $order_info['currency_value'])
                        );
                }
                ...
                foreach ($total_data as $total => $row) {
                    if ($total_data[$total]['code'] == 'voucher') {
                        if (strtolower($order_info['payment_method']) == 'replacement' or $ref_order_id) {
                            unset($total_data[$total]);
                        } else {
                            $code = str_replace('Voucher(', '', str_replace(')', '', $total_data[$total]['title']));
                            $history_da = $this->model_sale_order->getUsedVoucherHistoryByCode($code);
                            $voucher_used = 0;
                            foreach ($history_da as $history) {
                                $voucher_used = $voucher_used + (float) str_replace('-', '', $history['amount']);
                            }

                            $total_data[$total]['title'] = $total_data[$total]['title'] . ' - ' . number_format(((float) $history_da[0]['famount'] - $voucher_used), 2);
                        }
                    }
                    if ($row['code'] = 'total') {
                        $totalTotal = $row['value'];
                    }
                }
                ...
                $total_data = $this->model_pos_slip->getTotal($total_data, $order_id, true);

                $this->data['orders'][] = array(
                    'order_id' => ($ref_order_id ? $ref_order_id : $order_id),
                    ...
                    'card_paid' => $order_info['card_paid'],
                    'cash_paid' => $order_info['cash_paid'],
                    'paypal_paid' => $order_info['paypal_paid'],
                    'change_due' => $order_info['change_due'],
                    'testData' => $product_ids
                    );
            }
        }
    ...
    public function update_product() {
        ...
        $this->model_sale_order->updateOrderProductAndHistory2($order_id, $this->request->post);
    }
    ...
    public function process_order() {
        ...
        $order_info = $this->model_sale_order->getOrder($order_id);
        $type_check = $this->model_xhelper_order->orderTypeCheck($order_id); //
        $zones = $this->model_localisation_zone->getZones();


        $this->data['type_check'] = $type_check;
        $this->data['order_id'] = $order_id;
        $this->data['previous_total'] = $order_info['total'];
        $this->data['new_total'] = $_SESSION['order_changes']['total'];
        $this->data['payment_address_1'] = $order_info['payment_address_1'];
        $this->data['payment_city'] = $order_info['payment_city'];
        $this->data['payment_zone'] = $order_info['payment_zone'];
        $this->data['payment_postcode'] = $order_info['payment_postcode'];
        ...
        $this->template = 'sale/process_order.tpl';
    ...
    public function cash_payment() {
        $json = array();
        $order_id = $this->request->get['order_id'];
        //$record = $this->db->query("UPDATE `".DB_PREFIX."order` SET old_total=total,total_edits=total_edits+1 WHERE order_id='".(int)$order_id."'");	
        $this->load->model('sale/order');
        $this->model_sale_order->updateOrderProductAndHistory($order_id);

        $json['success'] = 'Sucessfuly Cash Received!';
    ...
    public function order_statuses_ajax() {
        $order_status_id = $this->request->get['order_status_id'];
        $structure_statuses = array(
            15 => array(21, 3, 7), // Processed it can be changed to On-Hold, Shipped or Canceled
            24 => array(3, 7), // Store Pick Up can be changed to Shipped, Canceled
            21 => array(21, 15, 7), // On Hold can be changed to On Hold,Processed &  Canceled
            3 => array(11, 22), // Shipped can be changed to Refunded, Store Credit
            1 => array(7), // If Status is pending it can be canceled
            11 => array(7), // IF Status is refunded it can be canceled
            );
        $this->load->model('localisation/order_status');
        //$this->data['available_statuses']=array();

        foreach ($structure_statuses[$order_status_id] as $status_id) {
            $result = $this->model_localisation_order_status->getOrderStatus($status_id);
            ?>
            <a  class="button" onclick="changeStatus('<?php echo $result['order_status_id']; ?>')"><?php echo $result['name']; ?></a>

            <?php
        }
        if ($order_status_id == 21) {
            ?>
            <!--<a  class="button" onclick="changeStatus(15,true)">Print and Process</a>-->
            <?php
        }
        exit;
    }
