...
class ControllerSaleCoupon extends Controller {
	private $error = array();
  	...
  	private function getForm() {
    	...
    	if (isset($this->request->post['maximum_amount'])) {
      		$this->data['maximum_amount'] = $this->request->post['maximum_amount'];
    	} elseif (!empty($coupon_info)) {
			$this->data['maximum_amount'] = $coupon_info['maximum_amount'];
		} else {
      		$this->data['maximum_amount'] = '';
    	}
		...
		if (isset($this->request->post['customer_group'])) {
			...
			$customer_group = $this->model_sale_customer_group->getCouponProducts($this->request->get['coupon_id']);
		...
		foreach ($customer_group as $group) {
			$group_id = $this->model_sale_customer_group->getCustomerGroup($group);
			
			if ($group_id) {
				$this->data['customer_group'][] = array(
					'customer_group_id' => $group_id['customer_group_id'],
					'name'       => $group_id['name']
				);
			}
		}
