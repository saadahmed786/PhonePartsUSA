  ...
  <div class="box">
    ...
    <div class="content">
      <div class="vtabs"><a href="#tab-order"><?php echo $tab_order; ?></a><a href="#tab-payment"><?php echo $tab_payment; ?></a>
        ...
        {
        ?>
        <a href="#tab-edit-order" id="edit_order_tab">Edit Order</a>
       <?php
       }
       ?>
        <?php if ($maxmind_id) { ?>
      ...
      <div id="tab-order" class="vtabs-content">
        <table class="form">
          <tr>
            ...
            <td>#<?php echo ($ref_order_id?$ref_order_id:$order_id); ?></td>
          </tr>
          <tr>
      ...
      <div id="tab-product" class="vtabs-content">
        <table class="list">
          ...
          <?php foreach ($totals as $totals1) { ?>
          <tbody id="totals">
            <tr>
              <td colspan="4" class="right"><?php echo $totals1['title']; ?>:</td>
              <td class="right"><?php echo $totals1['text']; ?></td>
            </tr>
          </tbody>
          <?php } ?>
        </table>
        ...
        if(trim($order_mod_history['username'])!='')
        
        {
        
        ?>
        <div style="text-align: center;font-weight:bold;color:red;font-size: 16px">
        Modified <?php echo date('m/d/Y',strtotime($order_mod_history['date_modified']));?> at <?php echo date('h:ia',strtotime($order_mod_history['date_modified']));?> by <?php echo $order_mod_history['username'];?>
        
        </div>
        <?php
        if($order_mod_history['new_status']!='')
        {
        ?>
        <div style="text-align: center;font-size: 12px">
        Order Status changed from <span style="color:red;font-weight:bold"><?php echo $order_mod_history['old_status'];?></span> to <span style="color:red;font-weight:bold"><?php echo $order_mod_history['new_status'];?> </span>
        </div>
        <?php
        }
        if($order_mod_history['product_id']!=0)
        {
        ?>
        <div style="text-align: center;font-size: 12px">
        Products &amp; Quantities are modified
        </div>
        <?php
        }
        ?>
      ...
      <div id="tab-edit-order" class="vtabs-content" <?php if($order_status_id!='15'){  echo 'style="display:none"';  } ?>>
      <form name="edit-order-frm" id="edit-order-frm">
        <table class="list">
          <thead>
            <tr>
              <td class="left"><?php echo $column_product; ?></td>
          ...
          ?>
            ...
            <tr>
              <td class="left"><a href="<?php echo $product['href']; ?>"><?php echo $product['name']; ?></a>
                ...
                <?php } ?></td>
              <td class="left"><?php echo $product['model']; ?></td>
              <td class="right"><input type="number" style="width:50px" id="equantity_<?php echo $j;?>"  name="equantity<?php echo $j;?>" min="0" max="<?php echo $product['product_quantity']+1;?>" value="<?php echo $product['quantity']; ?>" onChange="UpdateValue('<?php echo $j;?>',this.value)"></td>
              <td class="right"><?php echo $product['price']; ?><input type="hidden" name="eunit_<?php echo $j;?>" id="eunit_<?php echo $j;?>" value="<?php echo str_replace('$','',$product['price']);?>" >
              
              <input type="hidden" name="etotal_<?php echo $j;?>" id="etotal_<?php echo $j;?>" value="<?php echo str_replace('$','',$product['total']);?>" >
               <input type="hidden" name="eproduct_id_<?php echo $j;?>" id="eproduct_id_<?php echo $j;?>" value="<?php echo $product['product_id'];?>" >
               <input type="hidden" name="eorder_product_id_<?php echo $j;?>" id="eorder_product_id_<?php echo $j;?>" value="<?php echo $product['order_product_id'];?>" >
              
              </td>
              <td class="right" id="tr_total_<?php echo $j;?>"><?php echo $product['total']; ?></td>
            </tr>
            <?php 
            $j++;
            } ?>
        ...
        <input type="hidden" id="TotalProducts" name="TotalProducts" value="<?php echo $j;?>">
        
        </form>
        
        
       <div style="text-align:right"> 
       <a id="btn-add-product" class="button">Add Product</a>
       <a id="submit-edit-order"  class="button">Save</a>
       
       </div>
        <div style="text-align:right;margin-top:15px;display:none"> 
       <a id="save-all-order" class="button">Submit</a>
      ...
      <div id="tab-history" class="vtabs-content">
        ...
        <table class="form">
          <tr style="display:none" >
            ...
            <td><select name="order_status_id" <?php /*if($order_status_id==3 and $_SESSION['user_id']!=1) echo 'disabled';*/ ?>>
                <?php foreach ($order_statuses as $order_statuses) { ?>
                <?php if ($order_statuses['order_status_id'] == $order_status_id) { ?>
                <option value="<?php echo $order_statuses['order_status_id']; ?>" selected="selected"><?php echo $order_statuses['name']; ?></option>
                <?php } else {
                /*if($xuser_group_id=='11')
                {
                
                if(in_array($order_statuses['order_status_id'],$employee_allowed_order_status))
                {
                 ?>
          ...
          <tr>
            ...
            <td><textarea name="comment" cols="40" rows="8" style="width: 99%"></textarea>
              <div style="margin-top: 10px; text-align: left;"><a id="button-history" class="button">Comment</a></div>
              
              <div style="margin-top: 10px; text-align: right;" id="order_history_msg">
      ...
      <div id="tab-fraud" class="vtabs-content">
        <table class="form">
          ...
          <td colspan="2" style="font-weight:bold;font-size:14px;">Telephone Number Identification (<a href="https://www.maxmind.com/en/telephone-number-identification-phone-type-codes" target="_blank">View Documentation</a>) <?php
          
          if(!$tni_phoneType)
          {
          ?> <a class="button" onclick="tniVerification('<?php echo $telephone;?>')" id="request_verification_button">Request Verification</a>
          <?php
          }
          ?>
          <div id="request_verification">
          ...
          <td colspan="2" style="font-weight:bold;font-size:14px;">--------------------------------------------------------------------------------------------</td>
          </tr>
...
function tniVerification(telephone)
{
	$.ajax({
		url: 'index.php?route=sale/order/tni_verification&token=<?php echo $token; ?>&telephone='+telephone+'&order_id=<?php echo $this->request->get['order_id'];?>',
		type:'POST',
		
		beforeSend: function() {
			$('#request_verification_button').after('<img src="view/image/loading.gif" class="loading" style="padding-left: 5px;" />');	
		},
		complete: function() {
			$('.loading').remove();
		},
		...
		success: function(data) {
			$('#request_verification').html(data).fadeIn('slow');
			}
	});
	
}
$(document).ready(function(e) {
    populateStatusesButtons();
});
function changeStatus(order_status_id,isPrint)
{
	
	 isPrint = typeof isPrint !== 'undefined' ? isPrint : false;
	
if(!confirm('Are you sure want to apply thing change?'))
{
	return false;
}
	$('select[name=order_status_id] option[value='+order_status_id+']').attr('selected','selected');
	$('#button-history').click();
	$('#order_history_msg').fadeOut('slow');
	populateStatusesButtons();
	if(order_status_id=='15' && isPrint==true)
	{
		//UberAutoPrint();
	}
	return false;
...
function populateStatusesButtons()
{
	
 $.ajax({
		url: 'index.php?route=sale/order/order_statuses_ajax&token=<?php echo $token; ?>&order_status_id='+$('select[name=order_status_id]').val(),
		type:'POST',
...
function UberAutoPrint()
{
 ...
 $.ajax({
		url: '<?php echo HTTPS_CATALOG;?>index.php?route=checkout/manual/autoprint_confirm&token=<?php echo $token; ?>&order_status_id=15&order_id=<?php echo $this->request->get['order_id'];?>&comment='+$('textarea[name=comment]').html(),
		type:'POST',
...
$('#button-history').live('click', function() {
	$.ajax({
		...
		success: function(html) {
			...
			//{
			?>
			if($('select[name=\'order_status_id\']').val()==3)
			{
			$('select[name=\'order_status_id\']').attr('disabled','disabled');
			$('#edit_order_tab').hide();	
			}
			<?php
			//}
...
function UpdateValue(i,qty)
{
	
	var total = qty*parseFloat($('#eunit_'+i).val());
	
$("#tr_total_"+i).html('$'+total.toFixed(2));	
$("#etotal_"+i).val(total);
...
for(i=0;i<TotalProducts;i++)
{
	...
	SubTotal+=parseFloat($('#etotal_'+i).val());
	
}

}
$("#submit-edit-order").click(function(e) {
	var datastring = $('#edit-order-frm').serialize();
	
    $.ajax({
		url: 'index.php?route=sale/order/update_product&token=<?php echo $token; ?>&order_no=<?php echo $order_id;?>',
		type:'POST',
		...
		success: function(data) {
			//alert('Products Modified');
			//location.reload();
				$('#dialog').remove();
	$('#content').prepend('<div id="dialog" style="padding: 3px 0px 0px 0px;"><iframe src="index.php?route=sale/order/process_order&token=<?php echo $token; ?>&order_id=<?php echo $order_id;?>" style="padding:0; margin: 0; display: block; width: 100%; height: 100%;" frameborder="no" scrolling="auto"></iframe></div>');
...
$("#btn-add-product").click(function(e) {
	...
	var it = $("#TotalProducts").val();
	it = parseInt(it);
	it2 = it+1;
	
$("#TotalProducts").val(it2);
	$("#edit-order-frm #products").append('<tr><td class="left"><input style="width:400px" class="auto-products" name="product-name_'+it+'" id="product-name_'+it+'"><small style="color:red;margin-left:10px" id="warning_'+it+'"></small></td><td class="left" id="model_'+it+'"></td><td class="right"><input type="number" style="width:50px" id="equantity_'+it+'"  name="equantity'+it+'" min="0" value="1" disabled onChange="UpdateValue(\''+it+'\',this.value)"></td><td class="right"><input type="hidden" name="eunit_'+it+'" id="eunit_'+it+'" value="" ><span id="auto-product_'+it+'">$0.00</span><input type="hidden" name="etotal_'+it+'" id="etotal_'+it+'" value="" ><input type="hidden" name="eproduct_id_'+it+'" id="eproduct_id_'+it+'" value="" ><input type="hidden" name="eorder_product_id_'+it+'" id="eorder_product_id_'+it+'" value="0" ></td><td class="right" id="tr_total_'+it+'">$0.00</td></tr>');
	
	
	$('.auto-products').autocomplete({
	...
	/*	$('#product-related' + ui.item.value).remove();
		...
		$("#product-name_"+it).val(ui.item.label);
		$("#eproduct_id_"+it).val(ui.item.value);
		$("#model_"+it).html(ui.item.model);
		var price = parseFloat(ui.item.price);
		if(ui.item.quantity>0)
		{
			$("#warning_"+it).html('');	
		$("#equantity_"+it).removeAttr('disabled');	
		$("#equantity_"+it).val(1);
		$("#equantity_"+it).attr('max',ui.item.quantity);
		$("#eunit_"+it).val(price.toFixed(2));
		$("#etotal_"+it).val(price.toFixed(2));
		
		$("#tr_total_"+it).html('$'+price.toFixed(2));
		$("#auto-product_"+it).html('$'+price.toFixed(2));
		...
		{
			$("#warning_"+it).html('(Not available in stock)');	
			$("#equantity_"+it).attr('disabled','disabled');	
		$("#equantity_"+it).val(1);
		
		
		$("#unit_"+it).val('0.00');
		$("#total_"+it).val('0.00');
		$("#tr_total_"+it).html('$0.00');
		$("#auto-product_"+it).html('$0.00');
		$("#equantity_"+it).removeAttr('max');
