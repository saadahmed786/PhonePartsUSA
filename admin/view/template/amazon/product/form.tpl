...
<div id="page-links">
    <table class="form" align="left">
        <tr>
            <td colspan="2"><h2><?php echo $item_links_header_text; ?></h2>
...
<table align="left" class="list" id="linkListTable">
    ...
    <thead id="tableThread2">
        <tr>
            ...
            <td class="left" width="35%"><?php echo $new_link_amazon_sku_column; ?></td>
    ...
    <tbody id="tableBody">
        ...
        <?php $i = 1; foreach($options as $option) { ?>
            <tr>
                ...
                <td>
                    <input id="amazonSku<?php echo $i; ?>" type="text">
                </td>
                <td class="center">
    ...
    <thead>
        <tr>
            ...
            <td width="45%"><?php echo $new_link_amazon_sku_column; ?></td>
            <td class="center" width="10%"><?php echo $new_link_action_column; ?></td>
        </tr>
    </thead>
                            ...
                            <td>
                                <?php foreach ($marketplaces as $mp) { ?>
                                    <div style="text-align: center; float: left; margin-right: 20px;">
                                        <label for="qck_marketplace_<?php echo $mp['code'] ?>"><?php echo $mp['name'] ?></label>
                                        <?php if($saved_marketplaces === false) { ?>
                                        <input id="qck_marketplace_<?php echo $mp['code'] ?>" <?php if (in_array($mp['id'], $default_marketplaces)) { ?> checked="checked" <?php } ?> type="radio" name="marketplace_ids[]" value="<?php echo $mp['id']; ?>">
                                        <?php } else { ?>
                                        <input id="qck_marketplace_<?php echo $mp['code'] ?>" <?php if (in_array($mp['id'], $saved_marketplaces)) { ?> checked="checked" <?php } ?> type="radio" name="marketplace_ids[]" value="<?php echo $mp['id']; ?>">
                                        <?php } ?>
                                    </div>
                                <?php } ?>
                        ...
                        <td>
                            <?php foreach ($marketplaces as $mp) { ?>
                                <div style="text-align: center; float: left; margin-right: 20px;">
                                    ...
                                    <?php if($saved_marketplaces === false) { ?>
                                    <input id="adv_marketplace_<?php echo $mp['code'] ?>" <?php if (in_array($mp['id'], $default_marketplaces)) { ?> checked="checked" <?php } ?> type="radio" name="marketplace_ids[]" value="<?php echo $mp['id']; ?>">
                                    <?php } else { ?>
                                    <input id="adv_marketplace_<?php echo $mp['code'] ?>" <?php if (in_array($mp['id'], $saved_marketplaces)) { ?> checked="checked" <?php } ?> type="radio" name="marketplace_ids[]" value="<?php echo $mp['id']; ?>">
                                    <?php } ?>
                                </div>
                            <?php } ?>
                        <span class="required" id="required_marketplaces"></span>
        ...
        <table class="list" align="left">
            <thead>
                <tr>
                    ...
                    <td width="22.5%"><?php echo $amazon_sku_column_text ;?></td>
    ...
    $.ajax({
            ...
            success: function(json) {
                ...
                for(i in json) {
                    ...
                    rows += '<td class="center"><a class="button" onclick="removeLink(this, \'' + json[i]['amazon_sku'] + '\')" ><span><?php echo $links_remove_text; ?></span></a></td>';
                    rows += '</tr>';
                }
...
function addNewLink(button, product_id, row_id) {
    var amazon_sku_field = $('#amazonSku' + row_id);
    ...
    $.ajax({
            ...
            dataType: 'json',
            async: 'false',
            data: 'product_id=' + encodeURIComponent(product_id) + '&amazon_sku=' + encodeURIComponent(amazon_sku_field.val()) + '&var=' + encodeURIComponent(var_field.val()),
            beforeSend: function() {
...
<script type="text/javascript">
    ...
    function upload() {
        $.ajax({
            ...
            success: function(data) {
                if(data == null) {
                    alert('Error. No response from amazon/product/uploadSaved.');
                    return;
