...
class Pos_cart {
	...
	public function getProducts($order_id) {
		//echo "Cart Order: ".$order_id."\n";
		//if (!$this->data[$order_id]) {
			
			foreach ($this->session->data['cart'][$order_id] as $key => $quantity) {
				...
				if ($product_query->num_rows) {
					...
					foreach ($this->session->data['cart'][$order_id] as $key_2 => $quantity_2) {
...
//		}
	...
	public function add($order_id,$product_id, $qty = 1, $option, $profile_id = '') {
		$key = (int)$product_id . ':';
		...
		if ((int)$qty && ((int)$qty > 0)) {
			if (!isset($this->session->data['cart'][$order_id][$key])) {
				$this->session->data['cart'][$order_id][$key] = (int)$qty;
			} else {
				$this->session->data['cart'][$order_id][$key] += (int)$qty;
			}
		}
	...
	public function remove($key,$order_id) {
		
		/*if (isset($this->session->data['cart'][$order_id][$key])) {
			unset($this->session->data['cart'][$order_id][$key]);
		}*/
		
		if (isset($this->session->data['cart'][$order_id][$key])) {
			if($this->session->data['cart'][$order_id][$key]>1)
			{
			$this->session->data['cart'][$order_id][$key] = (int)($this->session->data['cart'][$order_id][$key])-1;	
			}
			else
			{
				unset($this->session->data['cart'][$order_id][$key]);
...
public function getSubTotal($order_id) {
	...
	public function getSubTotal2($order_id) {
		$total = 0;
$array_keys = array_keys($this->session->data['cart']);

foreach($array_keys as $order_id)
{
		foreach ($this->getProducts($order_id) as $product) {
			$total += $product['total'];
		}
...
$array_keys = array_keys($this->session->data['cart']);
	...
	public function getTaxes2($order_id) {
		$tax_data = array();
$array_keys = array_keys($this->session->data['cart']);
...
{  
	...
	public function hasShipping($order_id) {
		$shipping = false;
