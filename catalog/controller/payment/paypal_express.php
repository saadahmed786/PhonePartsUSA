...
class ControllerPaymentPaypalExpress extends Controller {
	...
	protected function index() {
		...
		$this->data['action'] = ($store_url . 'index.php?route=payment/paypal_express/DoExpressCheckoutPayment');
		...
		if (file_exists(DIR_TEMPLATE . (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/payment/paypal_express.tpl')) {
            $this->template = (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/payment/paypal_express.tpl';
        } else {
	...
	public function SetExpressCheckout() {
		...
		if (isset($this->session->data['ppx']['payerid'])) {
			...
			if (method_exists($this->document, 'addBreadcrumb')) { //1.4.x
				...
				$this->redirect($store_url . 'index.php?route=checkout/ppx_checkout');
			}

        } elseif (isset($this->session->data['ppx']['token'])) {
        	$this->redirect($store_url . 'index.php?route=payment/paypal_express/GetExpressCheckoutDetails');
		}
		...
		$data['USER'] 			= trim($this->config->get('paypal_express_apiuser'));
		$data['PWD'] 			= trim($this->config->get('paypal_express_apipass'));
		$data['SIGNATURE'] 		= trim($this->config->get('paypal_express_apisig'));
		$data['VERSION'] 		= '60.0';
		$data['PAYMENTACTION'] 	= ($this->config->get('paypal_express_payment_action')) ? $this->config->get('paypal_express_payment_action') : 'Sale';
		$data['CURRENCYCODE']	= $currency;
		$data['RETURNURL'] 		= ($store_url . 'index.php?route=payment/paypal_express/GetExpressCheckoutDetails');
		$data['CANCELURL'] 		= ($store_url . 'index.php?route=checkout/cart');
		$data['NOTIFYURL'] 		= ($store_url . 'index.php?route=payment/paypal_express/callback');

		if ($this->config->get('paypal_express_survey')) {
			$data['SURVEYENABLE'] 		= '1';
		...
		if ($this->config->get('paypal_express_logo')) {
			$data['HDRIMG'] = $this->config->get('paypal_express_logo');
		...
		$data['LANDINGPAGE'] 	= ($this->config->get('paypal_express_landing') ? 'Billing' : 'Login'); // Billing or Login
	...
	public function GetExpressCheckoutDetails() {
		...
		if (isset($this->request->get['token'])) {
			...
			$this->redirect($store_url . 'index.php?route=payment/paypal_express/SetExpressCheckout');
		}
	...
	public function DoExpressCheckoutPayment() {
		...
		if (isset($nvp['ACK']) && $nvp['ACK'] == 'Success') {
			...
			if ($nvp['PAYMENTSTATUS'] == 'Completed' && $nvp['PAYERSTATUS'] == 'verified') { // verified status
				$this->model_checkout_order->confirm($order_info['order_id'], $this->config->get('paypal_express_order_status_id'), $order_info['comment']);
				$this->model_checkout_order->update($order_info['order_id'], $this->config->get('paypal_express_order_status_id'), "PP Transaction ID: " . $nvp['TRANSACTIONID'] . "\r\n" . $nvp['PAYERSTATUS'], FALSE);
			} elseif ($nvp['PAYMENTSTATUS'] == 'Completed') { // unverified status
				if (!$this->config->get('paypal_express_order_status_id')) { $this->config->set('paypal_express_unverified_order_status_id', $this->config->get('paypal_express_order_status_id')); }
				$this->model_checkout_order->confirm($order_info['order_id'], $this->config->get('paypal_express_unverified_order_status_id'), $order_info['comment']);
				$this->model_checkout_order->update($order_info['order_id'], $this->config->get('paypal_express_unverified_order_status_id'), "PP Transaction ID: " . $nvp['TRANSACTIONID'], FALSE);
			} else { // not Completed
	...
	private function fail($msg = false) {
		...
		if (method_exists($this->document, 'addBreadcrumb')) { //1.4.x
			...
			echo 'alert("'.$msg.'");';
			echo 'window.location="' . ($store_url  . 'index.php?route=checkout/ppx_checkout') . '";';
	...
	public function prepareOrder($data = array()) {
		...
		$this->session->data['payment_method']['id'] = 'paypal_express'; //v14x
		$this->session->data['payment_method']['code'] = 'paypal_express'; //v15x
		...
		if ($this->customer->isLogged()) {
			...
			if ($this->config->get('paypal_express_account')) {
	...
	function createAccount($pp_address = array()) {
		...
		if ($this->model_account_customer->getTotalCustomersByEmail($login_email)) {
			...
			$temp_string_passwd = 'paypal_express';
