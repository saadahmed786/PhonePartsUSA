...
class ControllerModuleBossZoom extends Controller {
	protected function index($setting) {
		if($setting['status']){
			...
			if (file_exists('catalog/view/theme/' . (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/stylesheet/cloud-zoom.1.0.3.css')) {
				$this->document->addStyle('catalog/view/theme/' . (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/stylesheet/cloud-zoom.1.0.3.css');
			} else {
			...
			if ($product_info['image']) {
				...
				if($results){
					...
					$this->data['popup'] = $this->model_tool_image->resize('data/image-coming-soon.jpg', $this->data['zoom_image_width'] ,$this->data['zoom_image_heigth'] );
					$this->data['thumb'] = $this->model_tool_image->resize('data/image-coming-soon.jpg', $this->data['thumb_image_width'], $this->data['thumb_image_heigth']);;
				}
			}
			//addition image
			if ($product_info['image']) {
			...
			if(!$results and !$product_info['image'])
			{
			$this->data['images'][] = array(
					'popup' => $this->model_tool_image->resize('data/image-coming-soon.jpg', $this->data['zoom_image_width'], $this->data['zoom_image_heigth']),
					'addition' => $this->model_tool_image->resize('data/image-coming-soon.jpg', $this->data['addition_image_width'], $this->data['addition_image_heigth']),
					'thumb'   => $this->model_tool_image->resize('data/image-coming-soon.jpg', $this->data['thumb_image_width'], $this->data['thumb_image_heigth']),
				);
				
			}
			
		}
		if (file_exists(DIR_TEMPLATE . (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/module/boss_zoom.tpl')) {
			$this->template = (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/module/boss_zoom.tpl';
		} else {
