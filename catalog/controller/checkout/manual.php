...
class ControllerCheckoutManual extends Controller {
	...
	public function shipping_method()
	{
		...
		$order_info = $this->model_account_order->getOrderAll($order_id);
		
		if(!$order_info)
		{
			$_order_info_query = $this->db->query("SELECT state from inv_orders_details where order_id='".$order_id."' ");
			$_order_info = $_order_info_query->row;
			$_zone_query  = $this->db->query("SELECT zone_id,country_id from oc_zone where LOWER(code)='".strtolower($_order_info['state'])."' or LOWER(name)='".strtolower($_order_info['state'])."' ");
			$_zone = $_zone_query->row;

			$order_info['shipping_zone_id'] = $_zone['zone_id'];
			$order_info['shipping_country_id'] = $_zone['country_id'];
		}
	...
	public function shipping_method_for_imp_old()
	{
	header('Access-Control-Allow-Origin: *');	
		
		$this->load->model('account/order');
		
		$zone = trim(urldecode($this->request->post['zone']));
		
		$zone_query = $this->db->query("SELECT zone_id FROM ".DB_PREFIX."zone WHERE lower(`name`) = '".$this->db->escape(strtolower($zone))."' and country_id in (38,223)");
		$zone_id = $zone_query->row;
		if(!$zone_id)
		{
		$zone_query = $this->db->query("SELECT zone_id FROM ".DB_PREFIX."zone WHERE lower(`code`) = '".$this->db->escape(strtolower($zone))."' AND country_id=223");
		$zone_id = $zone_query->row;
		}
	...
	public function shipping_method_for_imp()
	{
	header('Access-Control-Allow-Origin: *');	
		...
		$country_id = $zone_query->row['country_id'];
		if(!$zone_id)
		{
		$zone_query = $this->db->query("SELECT zone_id,country_id FROM ".DB_PREFIX."zone WHERE lower(`code`) = '".$this->db->escape(strtolower($zone))."' AND country_id=223");
		$zone_id = $zone_query->row['zone_id'];
		$country_id = $zone_query->row['country_id'];
		}
		...
		$results = $this->model_setting_extension->getExtensions('shipping');
					...
					foreach ($results as $result) {
						if ($this->config->get($result['code'] . '_status')) {
							...
							$quote = $this->{'model_shipping_' . $result['code']}->getQuote($address_data,true,$this->request->post['sub_total']); 
	...
	public function autoprint_confirm()
	{
		$order_id = $this->request->get['order_id'];
		...
		$this->model_checkout_order->autoprint_confirm($order_id,$comment);
