<?php
class ControllerCheckoutPPXCheckoutNew extends Controller {
	...
	public function index() {
		...
		// PO Box check
		if (($this->cart->hasShipping() && $this->config->get('paypal_express_new_no_po_box'))) {
		...
		$this->data['breadcrumbs'] = array();
      	...
      	$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('checkout/ppx_checkout_new', '', 'SSL'),
        	'separator' => $this->language->get('text_separator')
      	);
    	...
    	if ($this->customer->isLogged() && !isset($this->session->data['payment_address_id'])) {
			$this->redirect($this->url->link('payment/paypal_express_new/SetExpressCheckout', 'resetppx'));
		...
		if (($this->cart->hasShipping() && isset($this->session->data['shipping_method'])) || (!$this->cart->hasShipping())) {
			//$this->redirect($this->url->link('payment/paypal_express_new/DoExpressCheckoutPayment'));
		...
		if (file_exists(DIR_TEMPLATE . (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/checkout/ppx_checkout_new.tpl')) {
			$this->template = (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/checkout/ppx_checkout_new.tpl';
		} else {
			$this->template = 'default/template/checkout/ppx_checkout_new.tpl';
		}
