...
class ControllerOneCheckoutAddress extends Controller {
	...
	public function payment() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if (file_exists(DIR_TEMPLATE . (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/onecheckout/address.tpl')) {
				$this->template = (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/onecheckout/address.tpl';
			} else {
	...
	public function getinfo() {
		...
		if(substr(VERSION,0,5) == '1.5.0'){
			$onecheckout_fields = unserialize($this->config->get('onecheckout_fields'));
		}else{
			$onecheckout_fields = $this->config->get('onecheckout_fields');
		}
		...
		if(isset($payment_address)) {
			$json['paymentaddress'] = '';
			if(in_array('firstname',$onecheckout_fields)){
				$json['paymentaddress'] .= $payment_address['firstname'] . ' ';
			}
			if(in_array('lastname',$onecheckout_fields)){
				$json['paymentaddress'] .= $payment_address['lastname'] . '<br />';
			}else{
				$json['paymentaddress'] .= '<br />';
			}
			if(in_array('address1',$onecheckout_fields)){
				$json['paymentaddress'] .= $payment_address['address_1'] . ' ';
			}
			if(in_array('address2',$onecheckout_fields)){
				$json['paymentaddress'] .= $payment_address['address_2']  . '<br />';
			}else{
				$json['paymentaddress'] .= '<br />';
			}
			if(in_array('city',$onecheckout_fields)){
				$json['paymentaddress'] .= $payment_address['city'] . ' ';
			}
			if(in_array('postcode',$onecheckout_fields)){
				$json['paymentaddress'] .= $payment_address['postcode'] . '<br />';
			}else{
				$json['paymentaddress'] .= '<br />';
			}
			if(in_array('zone',$onecheckout_fields)){
				$json['paymentaddress'] .= $payment_address['zone'] . '<br />';
			}
			if(in_array('country',$onecheckout_fields)){
				$json['paymentaddress'] .= $payment_address['country'];
			}
		} else {
			$json['paymentaddress'] = '';
		}
		...
		if(isset($shipping_address)) {
			...
			if(in_array('firstname',$onecheckout_fields)){
				$json['shippingaddress'] .= $shipping_address['firstname'] . ' ';
			}
			if(in_array('lastname',$onecheckout_fields)){
				$json['shippingaddress'] .= $shipping_address['lastname'] . '<br />';
			}else{
				$json['shippingaddress'] .= '<br />';
			}
			if(in_array('address1',$onecheckout_fields)){
				$json['shippingaddress'] .= $shipping_address['address_1'] . ' ';
			}
			if(in_array('address2',$onecheckout_fields)){
				$json['shippingaddress'] .= $shipping_address['address_2']  . '<br />';
			}else{
				$json['shippingaddress'] .= '<br />';
			}
			if(in_array('city',$onecheckout_fields)){
				$json['shippingaddress'] .= $shipping_address['city'] . ' ';
			}
			if(in_array('postcode',$onecheckout_fields)){
				$json['shippingaddress'] .= $shipping_address['postcode'] . '<br />';
			}else{
				$json['shippingaddress'] .= '<br />';
			}
			if(in_array('zone',$onecheckout_fields)){
				$json['shippingaddress'] .= $shipping_address['zone'] . '<br />';
			}
			if(in_array('country',$onecheckout_fields)){
				$json['shippingaddress'] .= $shipping_address['country'];
			}
		} else {
			$json['shippingaddress'] = '';
		}
