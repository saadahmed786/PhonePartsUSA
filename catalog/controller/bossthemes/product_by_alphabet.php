...
class ControllerBossthemesProductByAlphabet extends Controller {
	public function index() { 
		...
		if ($char) {
			...
			if (file_exists(DIR_TEMPLATE . (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/bossthemes/product_by_alphabet.tpl')) {
				$this->template = (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/bossthemes/product_by_alphabet.tpl';
			} else {
