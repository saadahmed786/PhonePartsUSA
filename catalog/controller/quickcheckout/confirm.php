...
class ControllerQuickcheckoutConfirm extends Controller {
  public function show() {
		...
		if (!$redirect) {
			...
			if ($this->cart->hasShipping()) {
				if ($this->customer->isLogged()) {
					...
					if (isset($this->session->data['shipping_address_id'])) $shipping_address = $this->model_account_address->getAddress($this->session->data['shipping_address_id']);	
				} elseif (isset($this->session->data['guest'])) {
		...
		if (file_exists(DIR_TEMPLATE . (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/'.$quickcheckout.'/confirm.tpl')) {
			$this->template = (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/'.$quickcheckout.'/confirm.tpl';
		}else{
...
$this->data['text_checkout_payment_method'] = substr($this->language->get('text_checkout_payment_method'), strpos ( $this->language->get('text_checkout_payment_method') , ' ' ,strpos($this->language->get('text_checkout_payment_method'), '5') ));
		...
		if($config['quickcheckout_display']){$quickcheckout = 'quickcheckout';}else{ $quickcheckout= 'checkout';}
		if (file_exists(DIR_TEMPLATE . (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/'.$quickcheckout.'/payment.tpl')) {
        	$this->template = (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/'.$quickcheckout.'/payment.tpl';
		} else {
