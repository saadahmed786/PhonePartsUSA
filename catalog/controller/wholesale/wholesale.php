...
class ControllerWholesaleWholesale extends Controller {
	...
	public function index() {
		$this->redirect('https://phonepartsusa.com/businessaccount');
...
else
{
 $this->error['captcha'] = 'Captcha Error';
	...
	public function thanks() {
		$this->document->setTitle('Thank You!');
...
// }
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			// $data['owner_id'] = '21225';
			$data['owner_id'] = '4067';
			$data['work_number'] = '+1'.$data['work_number'];
			...
			// print_r($result);exit;
				...
				$data = array();
			$data['firstname']  = ucfirst($this->request->post['first_name']);
			$data['lastname']  = ucfirst($this->request->post['last_name']);
			$data['email']  = strtolower($this->request->post['email']);
			$data['password']  = '12345';
			$data['telephone']  = $this->request->post['work_number'];
			$data['business_name']  = $this->request->post['company']['name'];

			
		

			$this->db->query("INSERT INTO ".DB_PREFIX."customer_source SET source='".$this->db->escape($this->request->post['custom_field']['cf_how_did_you_hear_about_us'])."',email='".$this->db->escape($this->request->post['email'])."' ");

			$this->db->query("INSERT INTO ".DB_PREFIX."customer_source SET source='".$this->db->escape($this->request->post['custom_field']['cf_business_type'])."',email='".$this->db->escape($this->request->post['email'])."',type='business_type' ");


			$this->model_account_customer->addCustomer($data,0);
			$this->model_catalog_wholesale->addWholeSaleAccountNew($this->request->post);
			$this->model_catalog_wholesale->addImpAccount($this->request->post);
			}
			// echo '<pre>';
