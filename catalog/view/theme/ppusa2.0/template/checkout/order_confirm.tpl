...
<div id="confmOdr" class="tab-pane fade in active">
					      				...
					      				<table class="table sidebox-billing">
											<tbody>
												<tr>
													...
													else if($this->session->data['newcheckout']['payment_method']=='behalf')
													{	
														$first_head = 'Payment Mode: ';
														$second_head = 'Behalf';
													}
	...
	window.location='index.php?route=payment/pp_payflow_pro/DoExpressCheckoutPayment'
					    	}
					    	<?php
					    }
					    }
...
<?php
					      ...
					      else if($this->session->data['newcheckout']['payment_method']=='behalf')
					    {
					    	...
					    	function submitForm()
					    	{
					    		$.ajax({ 
		type: 'get',
		url: 'index.php?route=payment/behalf/confirm',
		dataType: 'json',
		beforeSend: function(){
			$('.alert-danger').hide();
			$('.button-place-order').attr('disabled',true);
