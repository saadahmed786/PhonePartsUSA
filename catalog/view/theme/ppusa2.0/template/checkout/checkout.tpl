	...
	$(document).on('click', '#button-guest', function(event) {
		$.ajax({
			...
			success: function(json) {
				...
				if (json['error']['source']) {
					
					$('#source').parent().parent().addClass('has-error');
				}

				if (json['error']['business_type']) {
					
					$('#business_type').parent().parent().addClass('has-error');
				}
...
$(document).on('click','input[name=payment_method]',function(){
	if($(this).val()=='pp_payflow_pro')
			{
				$('#confirm-payment-method').html('Complete Payment <i class="fa fa-angle-right"></i>');
			}
			else
			{
				$('#confirm-payment-method').html('Next Step <i class="fa fa-angle-right"></i>');
			}
	...
	$.ajax({
		...
		success: function(json) {
			...
			if (json['redirect']) {
				...
				// $('.alert-danger').show();
				$('.alert-danger').html(' <span class="glyphicon glyphicon-exclamation-sign" aria-hidden="true"></span>Please fill in the required fields marked in red.');
				$('html,body').animate({
					scrollTop: 0
				}, 700);
				$('.alert-danger').show(500);
		...
		$.ajax({
			...
			success: function(json) {
				...
				if (json['redirect']) {
					...
					if (json['error']['source']) {
						$('#source').after('<span class="error">' + json['error']['source'] + '</span>');
					}
					if (json['error']['business_type']) {
						$('#business_type').after('<span class="error">' + json['error']['business_type'] + '</span>');
					}	

					if (json['error']['company']) {
						$('#company').after('<span class="error">' + json['error']['company'] + '</span>');
					}	
