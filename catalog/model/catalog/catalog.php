...
class ModelCatalogCatalog extends Model {
	...
	public function loadManufacturers ($filter = array()) {
		...
		foreach ($filter['where'] as $prop) {
			$where[] = $this->db->escape($prop['column']) . ' ' . $this->db->escape($prop['operator']) . ' "' . $this->db->escape($prop['value']) . '"';
		}
	...
	public function loadSubModels ($device_id) {
		$query = "SELECT a.*, b.`carrier_id`, b.`id` FROM `inv_model_dt` a, `inv_model_carrier` b WHERE a.`sub_model_id` = b.`sub_model_id` AND model_id = '". (int)$device_id ."' ORDER BY `sub_model`";
		...
		foreach ($subModelsRecord as $sMRow) {
			$carrier = '';
			if ($sMRow['carrier_id']) {
				$carrier = $this->db->query("SELECT name FROM `inv_carrier` WHERE `id` = '". (int)$sMRow['carrier_id'] ."'")->row['name'];
			}
					// Array Sub Models
			$results[] = array(
				'id' => $sMRow['id'],
	...
	public function loadClassAttr ( $filter = array() ) {
		...
		$model_id = $this->db->escape(trim($filter['model_id'], ','));
		if ($model_id) {
		...
		if ($class_id) {
			$query .= " AND d.`class_id` = '".(int)$class_id."'";
		}
	...
	public function filterProducts( $filter = array() , $attr = false,$page = 1) {
		...
		if ($filter['manufacturers']) {
			...
			// foreach ($filter['manufacturers'] as $manu) {
				...
				if ($filter['manufacturers']) {
					...
					$_manuf = array();
					foreach($filter['manufacturers'] as $_m)
					{
						$_manuf[] = (int)$_m;
					}
					$manuf = implode('\',\'', $_manuf);
		...
		if ($filter['class_id']) {
			...
			foreach ($filter['class_id'] as $class_id) {
				$nQuery = '';
				if ($class_id) {
					$nQuery .= " AND d.`class_id` = '".(int)$class_id."'";
				}
				...
				if ($attrib_id) { 
					...
					$_attrib_id = array();
					foreach($attrib_id as $aid)
					{
						$_attrib_id[] = (int)$aid;
					}
					$attrib_id = implode('\',\'', $_attrib_id);
	...
	public function loadModelClasses ( $filter = array() ) {
		...
		$class_name = $this->db->escape($filter['class_name']);
		if ($class_name) {
	...
	public function getProductAttr( $sku ) {
		...
		$query .= " AND h.`sku` = '".$this->db->escape($sku)."'";
	...
	public function filterProductsByDeviceIds( $filter = array() , $attr = false) {
		...
		$model_id = $this->db->escape(trim($filter['model_id'], ','));
		if ($model_id) {
			$model_id = $this->db->escape(str_replace(',', "','", $model_id));
			$query .= " AND a.`model_id` in ('$model_id') ";
		}
