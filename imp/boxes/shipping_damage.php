...
$bid = $db->func_query_first_cell('SELECT id FROM inv_return_shipment_boxes WHERE box_type = "SDBox" ORDER BY id DESC');
if ($bid) {
	$link = "$host_path" . "boxes/boxes_edit.php?box_id=$bid&return=shipping_damage";
...
$_query = "select * from inv_return_shipment_boxes where $where and box_type = 'SDBox' order by date_added desc";
$count_query = "select count(id) as total from inv_return_shipment_boxes where $where and box_type = 'SDBox'";

$splitPage   = new splitPageResults($db , $_query , 25 , "shipping_damage.php",$page ,  $count_query);
...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <body>
		<div align="center"> 
		 ...
		 <h2 align="center">Shipping Damage Items</h2>
		...
		<div align="center">
			<a href="<?php echo $host_path;?>boxes/createbox.php?type=SDBox&return=shipping_damage">Create new Box</a> <br /><br />
					   ...
					   <td style="width:50px;"><?php echo $k+1;?></td>			   		
			   		   ...
			   		   <td>
			   		       <?php if(($rma_return['status'] == 'Issued' && $_SESSION['edit_pending_shipment']) || $_SESSION['login_as'] == 'admin'):?>	
                              		<a href="<?php echo $host_path;?>boxes/boxes_edit.php?box_id=<?php echo $rma_return['id']?>&return=shipping_damage">Edit</a>
                           <?php endif;?>	
                          
                           <?php if($_SESSION['delete_shipment']):?>
                          		|
                          		<a href="<?php echo $host_path;?>boxes/deletebox.php?action=delete&box_id=<?php echo $rma_return['id']?>&return=shipping_damage" onclick="if(!confirm('Are you sure ?')){ return false; }">Delete</a>
                           <?php endif;?>
			   		   </td>
			   		</tr>
			   <?php endforeach;?>
