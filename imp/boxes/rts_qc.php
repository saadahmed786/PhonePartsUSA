...
if($_POST['QCUpdate'] || $_POST['QcComplete']){
	...
	if($_POST['QcComplete'] && $_SESSION['qc_shipment']){
		$db->db_exec("update inv_return_shipment_boxes SET status = 'Completed',fb_added = '1' , date_completed = '".date('Y-m-d H:i:s')."'  where id = '$box_id'");
		
		//$shipment_name = $db->func_query_first_cell("select box_number from inv_return_shipment_boxes where id = '$box_id'");
		$box_date = $db->func_query_first_cell("select date_added from inv_return_shipment_boxes where id = '$box_id'");
		...
		$my_products = array();
		foreach ($_POST['products'] as $product) {
			$my_products[$product['product_sku']] = array(
				'product_sku'=>$product['product_sku'],
				'unit_price'=>$product['item_cost'],
				'cu_po'=>$product['rma_number'],
				'accept_all' =>$product['accept_all'],
				'quantity' => (int)$my_products[$product['product_sku']]['quantity'] + (int)$product['quantity'],
				'grade_a_qty'=>$product['grade_a_qty'],
				'grade_b_qty'=>$product['grade_b_qty'],
				'grade_c_qty'=>$product['grade_c_qty'],
				'grade_d_qty'=>$product['grade_d_qty'],
		...
		//     if ($inv_shipment_id) {
		// 			$product_id = $db->func_query_first_cell("SELECT product_id FROM oc_product WHERE sku='".$product['product_sku']."'");
		// 			$shipment_item = array();
		// 			$shipment_item['product_id']  = $product_id;
		// 			$shipment_item['product_sku'] = $product['product_sku'];
		// 			$shipment_item['unit_price']  = $product['item_cost'];
		// 			$shipment_item['cu_po']  = $product['rma_number'];
		// 			if ($product['accept_all'] == '1') {
		// 				$shipment_item['qty_shipped']  = $product['quantity'];
		// 			} else {
		// 				$shipment_item['qty_shipped']  = $product['grade_a_qty']+$product['grade_b_qty']+$product['grade_c_qty']+$product['grade_d_qty'];
		// 			}
		...
		foreach($my_products as $product_sku =>$product){
...
if($box_id){
	...
	$shipment_query  = "select sq.* , si.product_sku , si.quantity , si.price,si.rma_number
		...
		<script type="text/javascript">
			...
			function showDiv(val , product_sku){
				if(val){
					...
					jQuery('.product_'+product_sku+'_row').hide();
					jQuery('.row_'+product_sku).find('input[type="checkbox"]').prop('checked', true);
				}else{
					jQuery('.product_'+product_sku+'_row').show();
					jQuery('.row_'+product_sku).find('input[type="checkbox"]').prop('checked', false);
				}
			}
		</script>	
    </head>
			                 ...
			                 <?php foreach($box_items as $i => $product):?>
			                        <tr class="row_<?php echo $product['product_sku'];?>">
			                          ...
			                          <td align="center">
			                          	  ...
			                          	  <input type="hidden" name="products[<?php echo $i?>][item_cost]" value="<?php echo $product['item_cost'];?>" />
			                          	  <input type="hidden" name="products[<?php echo $i?>][quantity]" value="<?php echo $product['quantity'];?>" />
			                          	  <input type="hidden" name="products[<?php echo $i?>][rma_number]" value="<?php echo $product['rma_number'];?>" />
