...
else
{
	...
	if($data['is_mapped']==1 and $data['order_id'])
	{
		...
		$payment_method = 'Payflow';
		if($payment_source!='payflow')
		{
			$payment_method='PayPal';
				// $payment_source='paypal';
		}
		$db->db_exec("UPDATE inv_orders_details SET payment_method='".$payment_method."' where order_id='".$data['order_id']."'");

			 // addVoucher($data['order_id'],$payment_source,$data['amount']);
	}
}
...
if(trim($data['order_id']))
{
	...
	addVoucher($data['order_id'],$payment_source,$data['amount'],$data['transaction_id']);
					...
					add_accounting_voucher($accounts); // Credit PayPal Account
					...
					$tax_check = $db->func_query_first_cell("SELECT tax from inv_orders where order_id='".$data['order_id']."'");
