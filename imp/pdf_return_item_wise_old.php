	...
	<td    >
		...
		<table  cellpadding="0" cellspacing="1" border="0" class="item_table"     >
			<tr style="background-color:#3C3D3A;color:#fff;">
				<td style="width:35px;height:20px;padding:5px;text-align:center" class="no_border1" >#</td>
				
				<td style="width:85px;padding:5px;text-align:center" class="no_border1" >SKU</td>
				
				<td style="width:255px;padding:5px;text-align:center" class="no_border1" >Item Name</td>
				<td style="width:60px;padding:5px;text-align:center" class="no_border1" >Total</td>
				<td style="width:65px;padding:5px;text-align:center" class="no_border1" >Awaiting</td>
				<td style="width:65px;padding:5px;text-align:center;" class="no_border1"  >Completed</td>
			...
			foreach($inv_orders as $row)
			{
				...
				<tr>
					...
					<td  class="'.($bottom_order?'':'no_border').' center normal " style="height:10px;padding:5px">'.($row['sku']).'</td>
					<td  class="'.($bottom_order?'':'no_border').' normal " style="height:10px;padding:5px">'.trimName(secureItemName(getItemName($row['sku']))).'</td>
					<td  class="'.($bottom_order?'':'no_border').' center normal " style="height:10px;padding:5px">'.$row['count_sku'].'</td>
					<td  class="'.($bottom_order?'':'no_border').' center normal " style="height:10px;padding:5px">'.$db->func_query_first_cell("SELECT COUNT(b.sku) as count_sku FROM inv_returns a,inv_return_items b WHERE a.id=b.return_id ".($condition_sql?' AND ':'')."  $condition_sql AND b.sku='".$row['sku']."' AND a.rma_status='Awaiting'").'</td>
					<td  class="'.($bottom_order?'':'no_border').' center normal " style="height:10px;padding:5px">'.$db->func_query_first_cell("SELECT COUNT(b.sku) as count_sku FROM inv_returns a,inv_return_items b WHERE a.id=b.return_id ".($condition_sql?' AND ':'')."  $condition_sql AND b.sku='".$row['sku']."' AND a.rma_status='Completed'").'</td>
...
<tr>
	<td align="center" border="0">
		...
		<tr>
		<td  style="font-size:24px;" align="center" >Detailed Return Item Report</td>
		</tr>
		<tr>
...
foreach($inv_orders as $row)
{
	...
	$html.='
	<h2 style="font-size:14px">'.$row['sku'].' '.secureItemName(getItemName($row['sku'])).'</h2>
		<table  cellpadding="0" cellspacing="1" border="0" class="item_table"     >
			<tr style="background-color:#3C3D3A;color:#fff;">
				<td style="width:80px;height:20px;padding:5px;text-align:center" class="no_border1" >Order Date</td>
	<td style="width:60px;height:20px;padding:5px;text-align:center" class="no_border1">Order ID</td>
	<td style="width:80px;height:20px;padding:5px;text-align:center" class="no_border1">Return Reason</td>
	<td style="width:60px;height:20px;padding:5px;text-align:center" class="no_border1">How to Process</td>
	<td style="width:80px;height:20px;padding:5px;text-align:center" class="no_border1">Condition</td>
	<td style="width:80px;height:20px;padding:5px;text-align:center" class="no_border1">Decision</td>
	<td style="width:60px;height:20px;padding:5px;text-align:center" class="no_border1">Except</td>
	<td style="width:80px;height:20px;padding:5px;text-align:center" class="no_border1">Comments</td>
	</tr>';
...
;
	...
	$html.='<tr>
	<td class=" center normal " style="height:10px;padding:5px">'.americanDate($db->func_query_first_cell("SELECT order_date FROM inv_orders WHERE order_id='".$item['order_id']."'")).'</td>
	<td class=" center normal " style="height:10px;padding:5px">'.$item['order_id'].'</td>
	<td class=" center normal " style="height:10px;padding:5px">'.$item['return_code'].'</td>
	<td class=" center normal " style="height:10px;padding:5px">'.$item['how_to_process'].'</td>
	<td class=" center normal " style="height:10px;padding:5px">'.$item['item_condition'].'</td>
	<td class=" center normal " style="height:10px;padding:5px">'.$decision.'</td>
	<td class=" center normal " style="height:10px;padding:5px">'.($item['item_exception']?'x':'').'</td>
	<td class=" center normal " style="height:10px;padding:5px">'.($item['comment']).'</td>
	</tr>';
	$i++;
	$iterator++;
}
