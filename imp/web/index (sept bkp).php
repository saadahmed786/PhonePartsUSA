...
$orderArray = array();
$order_query = "Select * from oc_order where ( oc_order.order_status_id IN( 15 , 24 , 3 , 16 , 7 , 21 , 11) ) AND ((oc_order.date_modified >= '$startDate' OR is_synced=0))";
...
else
{
    ...
    $orderArray[$k] = array('order_id' => $order['order_id'],
        ...
        'bill_firstname'=>$order['payment_firstname'],
        'bill_lastname'=>$order['payment_lastname'],
...
function insertOrderinImp($orderArray) {
    ...
    foreach ($orderArray as $order) {
        ...
        if($ref_order_id)
        {
            $db->db_exec("UPDATE oc_order SET is_synced=1 WHERE ref_order_id='".$ref_order_id."'");    
        }
        else
        {
