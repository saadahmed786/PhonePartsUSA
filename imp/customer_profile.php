<?php
// echo 'under development, please come later';exit;
...
if(isset($_POST['action']) && $_POST['action']=='remove_consolidation')
{
	...
	$parent_id = $db->func_query_first("UPDATE inv_customers SET parent_id=0 WHERE trim(lower(email))='".$db->func_escape_string($email)."' ");
...
if(isset($_POST['action']) && $_POST['action']=='update_source')
{
	$email = trim(strtolower($_POST['email']));;
	

	$db->db_exec("DELETE FROM oc_customer_source WHERE trim(lower(email))='".$email."' and `type`='".$_POST['type']."'");
	if($_POST['source']!='')
	{
	$db->db_exec("INSERT INTO oc_customer_source set email='".$email."', `type`='".$_POST['type']."',source='".$_POST['source']."'");
	}
	$status = 1;
...
if(isset($_POST['action']) && $_POST['action']=='convert_contact')
{
	$email = trim(strtolower($_POST['email']));;
	

	$data = $db->db->func_query_first_cell("SELECT freshsales_data FROM inv_customers WHERE TRIM(LOWER(email))='".$db->func_escape_string($email)."'");
...
curl_setopt_array($curl, array(
  CURLOPT_URL => "https://phonepartsusa.freshsales.io/api/leads/".$array['leads']['leads'][0]['id']."/convert",
  ...
  CURLOPT_POSTFIELDS => json_encode(array('lead'=>$array['leads']['leads'][0])),
...
$customer_details['freshsales_data'] = curl_exec($curl);
$err = curl_error($curl);
...
$json['error'] = 1;
  ...
  // echo $response;
	$db->func_query("UPDATE inv_customers SET freshsales_data='' WHERE trim(lower(email))='".$db->func_escape_string($email)."'");
	$json['success'];
}
...
if($_POST['action']=='updateAccountPotential') {
	
	$account_user_id = $_SESSION['user_id'];
	$account_date = date('Y-m-d H:i:s');
	$account_potential = (float)$_POST['account_potential'];

	// echo "UPDATE inv_customers SET account_potential='".$account_potential."',account_potential_update_user='".$account_user_id."',account_potential_update_date='".$account_date."'  WHERE email='".$customer_email."'";exit;
	$db->db_exec("UPDATE inv_customers SET account_potential='".$account_potential."',account_potential_update_user='".$account_user_id."',account_potential_update_date='".$account_date."'  WHERE email='".$customer_email."'");

	$json = array();
	$json['success'] = 1;
	$json['account_potential_update_user'] = get_username($account_user_id);
	$json['account_potential_update_date'] = americanDate($account_date);
...
if($_POST['action']=='testAccount')
{
	$status = $_POST['status'];
	
	if($status=='true') $status = 1; else $status = 0;
	// $db->db_exec("UPDATE oc_customer SET dis_tax='".$dis_tax."' WHERE email='".$customer_email."' ");
	$db->db_exec("UPDATE inv_customers SET is_test_account='".$status."' WHERE email='".$customer_email."' ");	
	echo $status;
	exit;
}
...
if(isset($_GET['debug']))
{
	echo "select c.* , cg.name , count(order_id) as total , sum(total) as price
	...
	where c.email = '$customer_email'";exit;
}
...
$_temp_detail = $db->func_query_first("SELECT *,(select avg(d.total) from inv_customer_data_summary d where d.email=inv_customers.email and d.type='order') as avg_weekly_total FROM inv_customers where email='".$customer_details['email']."' limit 1");
...
$customer_details['account_potential'] = $_temp_detail['account_potential'];
$customer_details['account_potential_update_user'] = $_temp_detail['account_potential_update_user'];
$customer_details['account_potential_update_date'] = $_temp_detail['account_potential_update_date'];
$customer_details['avg_weekly_total'] = $_temp_detail['avg_weekly_total'];
if($_temp_detail)
{
...
$customer_details['tax_id'] = $db->func_query_first_cell("SELECT tax_id FROM inv_customers WHERE email='".$customer_details['email']."'");
$customer_source = $db->func_query_first_cell("SELECT source FROM oc_customer_source WHERE TRIM(LOWER(email))='".trim(strtolower($customer_details['email']))."' and `type`='hear'");
$customer_business_type = $db->func_query_first_cell("SELECT source FROM oc_customer_source WHERE TRIM(LOWER(email))='".trim(strtolower($customer_details['email']))."' and `type`='business_type'");

$customer_details['dis_tax'] = $db->func_query_first_cell("SELECT dis_tax FROM inv_customers WHERE email='".$customer_details['email']."'");;
$customer_details['is_test_account'] = $db->func_query_first_cell("SELECT is_test_account FROM inv_customers WHERE email='".$customer_details['email']."'");;
...
if ($_POST['update_shipping_address']) {
	if ($_POST['ship_address_type'] == 'oc') {
		...
		$update_array = array ();
		$update_array['firstname'] = $_POST['shipping_firstname'];
		$update_array['lastname'] = $_POST['shipping_lastname'];
...
// echo $query;exit;
if ($_GET['action'] == 'fetch_orders' && $_temp_detail) {
// echo $customer_details['parent_id'];exit;
	if($customer_details['parent_id']==0)
{
	$query="SELECT a.order_id,a.order_status,a.order_date,a.paid_price,b.shipping_cost,a.order_price from inv_orders a,inv_orders_details b,inv_customers c where a.order_id=b.order_id and a.email=c.email and  (c.parent_id='".$_temp_detail['id']."' or c.id='".$_temp_detail['id']."') order by a.order_date desc";
	//$contacts = $db->func_query("SELECT * FROM inv_customers WHERE parent_id='".$_temp_detail['id']."'");
}
else
{

	$query="SELECT a.order_id,a.order_status,a.order_date,a.paid_price,b.shipping_cost,a.order_price from inv_orders a,inv_orders_details b,inv_customers c where a.order_id=b.order_id and a.email=c.email and  (c.parent_id='".$_temp_detail['parent_id']."' or c.id='".$_temp_detail['parent_id']."') order by a.order_date desc";
...
if($customer_details['parent_id']==0)
{
	$rma_returns = $db->func_query("select a.* from inv_returns a,inv_customers c where lcase(a.email)=lcase(c.email) and (c.parent_id='".$_temp_detail['id']."' or c.id='".$_temp_detail['id']."') ORDER BY date_added DESC");
}
else
{

$rma_returns = $db->func_query("select a.* from inv_returns a,inv_customers c where lcase(a.email)=lcase(c.email) and (c.parent_id='".$_temp_detail['parent_id']."' or c.id='".$_temp_detail['parent_id']."') ORDER BY date_added DESC");
}
}
...
if (isset($_GET['load_credit_debit']) && $_temp_detail) {
	...
	if($customer_details['parent_id']==0)
{
	...
	$total_debit = $db->func_query_first_cell("SELECT SUM(a.paid_price) FROM inv_orders a,inv_customers c where lcase(a.email)=lcase(c.email) and (c.parent_id='".$_temp_detail['id']."' or c.id='".$_temp_detail['id']."') and lower(a.order_status)='shipped' ");
	$total_credit = $db->func_query_first_cell("SELECT SUM(a.order_price) FROM inv_orders a,inv_customers c where lcase(a.email)=lcase(c.email) and (c.parent_id='".$_temp_detail['id']."' or c.id='".$_temp_detail['id']."' ) and lower(a.order_status)='shipped' ");

$total3month_debit = $db->func_query_first_cell("SELECT SUM(a.paid_price) FROM inv_orders a,inv_customers c where lcase(a.email)=lcase(c.email) and (c.parent_id='".$_temp_detail['id']."' or c.id='".$_temp_detail['id']."') and lower(a.order_status)='shipped' and a.order_date >= ( NOW() - INTERVAL 3 MONTH) ");
$total6month_debit = $db->func_query_first_cell("SELECT SUM(a.paid_price) FROM inv_orders a,inv_customers c where lcase(a.email)=lcase(c.email) and (c.parent_id='".$_temp_detail['id']."' or c.id='".$_temp_detail['id']."') and lower(a.order_status)='shipped' and a.order_date >= ( NOW() - INTERVAL 6 MONTH) ");


$total3month_credit = $db->func_query_first_cell("SELECT SUM(a.order_price) FROM inv_orders a,inv_customers c where lcase(a.email)=lcase(c.email) and (c.parent_id='".$_temp_detail['id']."' or c.id='".$_temp_detail['id']."') and lower(a.order_status)='shipped' and a.order_date >= ( NOW() - INTERVAL 3 MONTH) ");

$total6month_credit = $db->func_query_first_cell("SELECT SUM(a.order_price) FROM inv_orders a,inv_customers c where lcase(a.email)=lcase(c.email) and (c.parent_id='".$_temp_detail['id']."' or c.id='".$_temp_detail['id']."') and lower(a.order_status)='shipped' and a.order_date >= ( NOW() - INTERVAL 6 MONTH) ");
...
else
{
	...
	$total_debit = $db->func_query_first_cell("SELECT SUM(a.paid_price) FROM inv_orders a,inv_customers c where lcase(a.email)=lcase(c.email) and (c.parent_id='".$_temp_detail['parent_id']."' or c.id='".$_temp_detail['parent_id']."') and lower(a.order_status)='shipped' ");
	$total_credit = $db->func_query_first_cell("SELECT SUM(a.order_price) FROM inv_orders a,inv_customers c where lcase(a.email)=lcase(c.email) and (c.parent_id='".$_temp_detail['parent_id']."' or c.id='".$_temp_detail['parent_id']."') and lower(a.order_status)='shipped' ");

$total3month_debit = $db->func_query_first_cell("SELECT SUM(a.paid_price) FROM inv_orders a,inv_customers c where lcase(a.email)=lcase(c.email) and (c.parent_id='".$_temp_detail['parent_id']."' or c.id='".$_temp_detail['parent_id']."') and lower(a.order_status)='shipped' and a.order_date >= ( NOW() - INTERVAL 3 MONTH) ");
$total6month_debit = $db->func_query_first_cell("SELECT SUM(a.paid_price) FROM inv_orders a,inv_customers c where lcase(a.email)=lcase(c.email) and (c.parent_id='".$_temp_detail['parent_id']."' or c.id='".$_temp_detail['parent_id']."') and lower(a.order_status)='shipped' and a.order_date >= ( NOW() - INTERVAL 6 MONTH) ");


$total3month_credit = $db->func_query_first_cell("SELECT SUM(a.order_price) FROM inv_orders a,inv_customers c where lcase(a.email)=lcase(c.email) and (c.parent_id='".$_temp_detail['parent_id']."' or c.id='".$_temp_detail['parent_id']."') and lower(a.order_status)='shipped' and a.order_date >= ( NOW() - INTERVAL 3 MONTH) ");

$total6month_credit = $db->func_query_first_cell("SELECT SUM(a.order_price) FROM inv_orders a,inv_customers c where lcase(a.email)=lcase(c.email) and (c.parent_id='".$_temp_detail['parent_id']."' or c.id='".$_temp_detail['parent_id']."') and lower(a.order_status)='shipped' and a.order_date >= ( NOW() - INTERVAL 6 MONTH) ");
...
FROM  `oc_buyback` a
INNER JOIN inv_customers c ON ( a.email = c.email ) 
WHERE (c.parent_id='".$_temp_detail['id']."' or c.id='".$_temp_detail['id']."' ) order by a.buyback_id DESC");
}
else
{
...
FROM  `oc_buyback` a
INNER JOIN inv_customers c ON ( a.email = c.email ) 
WHERE (c.parent_id='".$_temp_detail['parent_id']."' or c.id='".$_temp_detail['parent_id']."') order by a.buyback_id DESC");
}
...
on(LCASE(`ov`.`to_email`) = LCASE(c.email))
WHERE (c.parent_id="'.$_temp_detail['id'].'" or c.id="'.$_temp_detail['id'].'")  ORDER BY ov.date_added DESC';
}
else
{
	$query = 'SELECT 
...
INNER JOIN inv_customers c
on(LCASE(`ov`.`to_email`) = LCASE(c.email))
WHERE (c.parent_id="'.$_temp_detail['parent_id'].'" or c.id="'.$_temp_detail['parent_id'].'") ORDER BY ov.date_added DESC';	
}
$vouchers = $db->func_query($query);
  ...
  $_voucher_balance_query = $db->func_query("SELECT distinct b.voucher_id, a.firstname,a.lastname,a.email,a.account_potential,(b.amount) as total, COALESCE((b.`amount`) + sum(c.`amount`),b.amount) balance FROM `oc_voucher` b LEFT OUTER JOIN `oc_voucher_history` c ON b.`voucher_id` = c.`voucher_id` INNER JOIN inv_customers a on (b.to_email=a.email) where LCASE(b.to_email)='".strtolower($customer_email)."' and b.status=1
    AND date(b.date_added) between '".date('Y-m-d',strtotime('-6 months'))."' and '".date('Y-m-d')."' 
    group by b.voucher_id having balance>0");

  $vouchers_balance = 0.00;
  foreach($_voucher_balance_query as $_voucher_query)
  {
  	$vouchers_balance+=$_voucher_query['balance'];
  }
...
if ($_POST['createUser'] && $_SESSION['c_ua_for_guest'] && $customer_details['type'] == 'guest') {
	...
	$message .= 'https://phonepartsusa.com/index.php?route=account/login' . "<br><br>";
...
if (isset($_GET['email_id'])) {
	...
	sendEmail($emailss['customer_name'], $emailss['customer_email'], $emailss['email_subject'], html_entity_decode($emailss['email_body']));
	// sendEmail($emailss['customer_name'], ADMIN_EMAIL, $emailss['email_subject'], html_entity_decode($emailss['email_body']));
	...
	<div class="div-fixed">
		...
		<div align="center">
			<div class="tabMenu" >
				...
				<input type="button" class="toogleTab" data-tab="tabFreshsales" value="Freshsales Data">
			</div>
			<div class="tabHolder">
					...
					<div id="tabCDetails" class="makeTabs">
						<table width="90%" border="1" cellpadding="10" cellspacing="0" style="border-collapse:collapse;border:1px solid #ddd">
							...
							<tr>
								...
								<input type="text" id="account_potential" value="<?php echo (float)$customer_details['account_potential'];?>"> <a href="#" onclick="updateAccountPotential();">Update</a>
								<br>
								<strong>Weekly Potential: <?php echo '$'.number_format($customer_details['avg_weekly_total'],2);?></strong>
								
									<br>
								<strong>Last Updated: <span id="account_potential_update_user"><?php echo get_username($customer_details['account_potential_update_user']).'</span> (<span id="account_potential_update_date">'.americanDate($customer_details['account_potential_update_date']).'</span>)' ;?></strong>
						...
						<br><br>
						<table width="90%" border="1" cellpadding="10" cellspacing="0" style="border:1px solid #ddd;border-collapse:collapse;">
							<tr>
								...
								<td>
									...
									<?= ($_SESSION['update_customer_email'])? '<a class="button button-info" href="javascript:void(0)" title="Update" onclick="updateEmail(this)">Update</a>': '';?>

									<?= ($_SESSION['update_customer_email'])? '<a class="button button-danger fancybox2 fancybox.iframe"  href="popupfiles/consolidate_customer.php?customer_id='.$customer_email.'">Consolidate</a>': '';?>
							...
							<tr>
								...
								<td colspan="3">	
									<div style="height:200px;overflow-y:scroll;">
											...
											?>
											<div>
										<?php echo $contact['firstname'];?> <?php echo $contact['lastname'];?> <?php

										if($_SESSION['update_customer_email'])
										{
											?>
											<img src="<?php echo $host_path;?>images/cross.png" height="8px" width="8px" title="Remove Consolidation" style="cursor:pointer" onClick="removeConsolidation('<?php echo $contact['email'];?>',this);" />
											<?php
										}
										?><br>  <?php echo $contact['city'];?>,



										 <?php echo $db->func_query_first_cell("SELECT name FROM oc_zone where zone_id='".$contact['zone_id']."'"); ?>, <?php echo $contact['zip'];?>.<br><?php echo linkToProfile($contact['email']); ?>   <hr>
										 </div>
										<?php
									}
								}
							...
							<tr>
								...
								<td>	
									...
									<?php echo $customer_details['tax_id'];?>
								</td>
								<td>Sales Agent:</td>
								<?php
								...
								<td>
								<?php 
								if($_SESSION['update_sales_agent'])
								{
								?>	
								...
								{
									echo get_username($customer_details['user_id']);
								}
								?>
				...
				$source_options = array('Google','Facebook','Bing','Other Online Ad','Magazine','Postcard','Referral','Blog','Discussion Board','Expo/Trade Show');
				sort($source_options);
				
				?>
				
							<select  id="source" name="source" onchange="updateSource('<?php echo $customer_email;?>','hear',this.value)">
								<option value="">Select Option</option>
								<?php
								foreach(($source_options) as $_source)
								{
								?>
									<option <?php echo ($_source==$customer_source)?'selected':'';?>><?php echo $_source;?></option>
								<?php
								}
				...
				?>
							...
							<select  id="business_type" name="business_type" onchange="updateSource('<?php echo $customer_email;?>','business_type',this.value)">
								<option value="">Select Option</option>
								<?php
								foreach(($busines_type_options) as $_source)
								{
								?>
									<option <?php echo ($_source==$customer_business_type)?'selected':'';?>><?php echo $_source;?></option>
								<?php
								}
							...
							<td>Test Account?</td>
							<td>	
									<input type="checkbox" onchange="testAccount(this)" <?php if($customer_details['is_test_account']==1) echo 'checked';?> />
								...
								<td>
									<table width="100%" border="0" align="left">
										...
										<tr>
											<th>Name: </th>
											<td>
												<input type="text" name="shipping_firstname" size="15" value="<?php echo $customer_address['firstname']; ?>" /> 
												<input type="text" name="shipping_lastname" size="15" value="<?php echo $customer_address['lastname']; ?>" />  
											</td>
											 <tr>
												<th>Company</th>
						...
						<?php if ($_SESSION['inv_customer_contact_view']) : ?>
							<br><br>
							<div id="contactsx" style="width:70%;">
								<h3>Contacts</h3>
								<table style="border:1px solid #ddd;border-collapse:collapse;" border="1" width="100%" cellpadding="10" cellspacing="0">
									<thead>
										<tr>
											<th>Name</th>
											<th>Position</th>
					...
					<table width="90%" border="1" cellpadding="10" cellspacing="0" style="border:1px solid #ddd;border-collapse:collapse;">
						...
						<?php endif; ?>
							...
							<div style="max-height:800px;overflow:scroll;width:70%;">
								<table width="100%" border="1" cellpadding="10" cellspacing="0" style="border:1px solid #ddd;border-collapse:collapse;">
									<tr>
										<th></th>
										<th>#</th>
										<th>Added</th>
					...
					<table width="90%" border="1" cellpadding="10" cellspacing="0" style="border:1px solid #ddd;border-collapse:collapse;">
						...
						<table id="list" align="center" border="0" width="70%" cellpadding="10" cellspacing="0" style="border:1px solid #ddd;border-collapse:collapse;">
							<tbody valign="top">
								<tr>
									...
									<td style="border-right: 1px solid;">
										<div>
											...
											<table align="center" border="0" width="100%" cellpadding="10" cellspacing="0" style="border:1px solid #ddd;border-collapse:collapse;">
												<tbody valign="top">
					...
					<div id="tabFreshsales" class="makeTabs">

					
<?php
$to_insert = false;
if(!$customer_details['freshsales_data'])
{
	$to_insert = true;
$curl = curl_init();

curl_setopt_array($curl, array(
  CURLOPT_URL => "https://phonepartsusa.freshsales.io/api/lookup?f=email&entities=lead%2Ccontact&q=".urlencode($customer_details['email']),
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_ENCODING => "",
  CURLOPT_MAXREDIRS => 10,
  CURLOPT_TIMEOUT => 30,
...
$fs_data = json_decode($customer_details['freshsales_data'],true);
// print_r($fs_data);
...
if($f_data)
{
	if($to_insert)
	{
$db->func_query("UPDATE inv_customers SET freshsales_data='".$db->func_escape_string(json_encode($fs_data))."' WHERE TRIM(LOWER(email))='".trim(strtolower($customer_details['email']))."'");
	}
...
?>
					<table width="90%" border="1" cellpadding="10" cellspacing="0" style="border:1px solid #ddd;border-collapse:collapse;">
							<tr>
							<th width="25%">Name:</th>
								<td width="25%" ><?php echo $f_data['display_name'];?></td>
								<th width="25%">Company:</th>
								<td width="25%"><?php echo $f_data['company']['name'];?></td>
							...
							<tr>
								<th>Work Number:</th>
								<td><?php echo $f_data['work_number'];?></td>
								<th>Lead Score:</th>
								<td><?php echo $f_data['lead_score'];?></td>

							</tr>

							<tr>
								<th>Lead Status:</th>
								<td><?php echo $f_data['email_status'];?></td>
								<th>Business Type:</th>
								<td><?php echo $f_data['custom_field']['cf_business_type'];?></td>

							</tr>

							<tr>
								<th>Source:</th>
								<td><?php echo $f_data['custom_field']['cf_how_did_you_hear_about_us'];?></td>
								<th># of Locations:</th>
								<td><?php echo $f_data['custom_field']['cf_of_locations'];?></td>

							</tr>
							<tr>
								<th>Order Frequency:</th>
								<td><?php echo $f_data['custom_field']['cf_how_often_do_you_order'];?></td>
								<th>Years in Business:</th>
								<td><?php echo $f_data['custom_field']['cf_years_in_business'];?></td>

							</tr>
							<tr>
								<th>Total Emails Count:</th>
								<td><?php echo (int)$total_emails;?></td>
								<th>Total Outbound Min:</th>
								<td><?php  echo gmdate("H:i:s", $total_outbound);;?></td>

							</tr>

							<tr>
							<th>Total Inbound Count:</th>
								<td><?php echo (int)$total_inbound;?></td>
								<th>Weekly Repairs Avg:</th>
								<td><?php echo $f_data['custom_field']['cf_average_repairs_per_week'];?></td>
							...
							<td colspan="4" align="center">
								...
								{
									?>
									<button class="button button-info" onclick="convertToContact();">Covert Contact</button>
									<?php
								}
								?>
							</td>
						...
						{
						echo "<b style=\"color:red\">No Data found in Freshsales</b>";
					...
					<table width="90%" border="1" cellpadding="10" cellspacing="0" style="border:1px solid #ddd;border-collapse:collapse;">
						...
						<?php if($vouchers) : ?>
							<br /><br />
							<h2 align="center">Customer Vouchers (Balance: <?php echo '$'.number_format($vouchers_balance,2);?>)</h2>
							<div style="max-height:800px;overflow:scroll;width:70%;">
								<table width="100%" border="1" cellpadding="10" cellspacing="0" style="border:1px solid #ddd;border-collapse:collapse;">
									<tr>
										<th>Code</th>
									...
									$applied_vouchers = $db->func_query("SELECT * FROM oc_voucher_history WHERE voucher_id='".$voucher['voucher_id']."'");
									$user_name='admin';
									if ($voucher['user_id']) {
									...
									{
										$user_name = get_username($voucher_detail['oc_user_id'],true);
									}
			//	$user_name = get_username($voucher_detail['user_id']);
									...
									<tr>
										...
										<?php } ?>
										<td>
										<?php foreach ($applied_vouchers as $app) {
											echo linkToOrder($app['order_id']). ' ('.number_format($app['amount'],2).')<br>';
										} ?>
...
function removeConsolidation(email,obj)
{
	...
	$.ajax({
			...
			type: 'post',
			dataType:'json',
			data: {action: 'remove_consolidation',email:email},
			beforeSend: function () {
			},
			complete: function () {
			},
...
function updateSource(email,type,source)
{
	if(!confirm('Caution: Do you want to update it?'))
	{
		return false;
	}
	...
	$.ajax({
			...
			type: 'post',
			dataType:'json',
			data: {action: 'update_source',email:email,type:type,source:source},
			beforeSend: function () {
			},
			complete: function () {
			},
...
function convertToContact(email,type,source)
{
	if(!confirm('Caution: Do you want to convert the lead into contact ?'))
	{
		return false;
	}
	...
	$.ajax({
			...
			type: 'post',
			dataType:'json',
			data: {action: 'convert_contact',email:email},
			beforeSend: function () {
			},
			complete: function () {
			},
			success: function (json) {
				if(json['error'])
				{
					alert('There is some error converting the lead, please try again or contact admin');
				}
				if(json['success'])
				{
	...
	function testAccount(obj) {
		var status = $(obj).is(':checked');
		$.ajax({
			url: 'customer_profile.php?id=<?php echo $_GET['id'];?>&email=<?=$_GET['email'];?>',
			type: 'post',
			data: {action: 'testAccount',status:status},
			beforeSend: function () {
			},
			complete: function () {
			},
	...
	function updateAccountPotential() {
		
		if(!confirm('Are you sure want to update the Account Potential?'))
		{
			return false;
		}
		$.ajax({
			url: 'customer_profile.php?id=<?php echo $_GET['id'];?>&email=<?=$_GET['email'];?>',
			type: 'post',
			data: {action: 'updateAccountPotential',account_potential:$('#account_potential').val()},
			dataType:'json',
			beforeSend: function () {
			},
			complete: function () {
			...
			success: function (json) {
				if(json['success']==1)
				{
				alert('Account Potential value changed!');
				$('#account_potential_update_user').html(json['account_potential_update_user']);
				$('#account_potential_update_date').html(json['account_potential_update_date']);
				}
				else
				{
