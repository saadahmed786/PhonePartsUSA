...
//Writing query 
$inv_query = "SELECT irs.*, iss.vendor, iss.`date_received`, iss.`date_completed`, iss.package_number FROM `inv_return_shipment_box_items` `irs`, `inv_shipments` `iss` WHERE irs.`shipment_id` = iss.`id` AND shipment_id <> 0  and $where
$orderby";
	...
	<div align="center">
		...
		<table  class="xtable"   align="center" style="width:98%;margin-top: 30px;">
			...
			<tbody>
				<!-- Showing All REcord -->
				<?php
				$total = $db->func_query_first_cell('SELECT  SUM(cost) FROM `inv_return_shipment_box_items` `irs`, `inv_shipments` `iss` WHERE irs.`shipment_id` = iss.`id` AND shipment_id <> 0');
