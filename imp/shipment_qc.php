...
function updateStockLog($sku)
{
	global $db;
		...
		{
			$db->db_exec("update inv_product_inout_stocks SET product_sku='".trim($sku)."',instock_date='".date('Y-m-d H:i:s')."',date_modified='".date('Y-m-d H:i:s')."' where id='".$check['id']."' ");
    ...
    //now add rejected sku to shipments ANd update Product Qty
		if ($_POST['QcComplete']) {
			...
			if ($product['product_sku'] && $received_qty > 0) {
				...
				$check = $db->func_query_first_cell("SELECT is_stock_added FROM inv_shipment_items where shipment_id='".(int)$shipment_id."' and product_sku='".$p_sku."'");


				if(!$check)
				{
					$db->db_exec("UPDATE inv_shipment_items SET is_stock_added=1 where shipment_id='".(int)$shipment_id."' and product_sku='".$p_sku."'");
