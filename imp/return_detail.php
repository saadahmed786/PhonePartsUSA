...
{
	$db->db_exec("UPDATE inv_returns SET is_approved=1,approved_by='".$_SESSION['user_id']."',approved_date='".date('Y-m-d H:i:s')."' where rma_number='".$rma_number."'");

		actionLog('RMA approved for QC');
}
					...
					foreach ( $return_shipment_box_item as $returns_po_item ) {
						if ($box_type == 'RTV'){
							...
							if ($db->func_escape_string($_POST['printerid'])) {
								...
								$_temp_reject_id = str_replace($returns_po_item_insert['product_sku'].'-', '', $returns_po_item_insert['reject_item_id']);
				printLabel('','','','','',$_POST['printerid'],'','15','','',$returns_po_item ['id'],$returns_po_item['reject_item_id'],$returns_po_item ['id']);
	...
	foreach ($return_items as $item) {
		if ($_POST['add_to_box'][$item['id']]) {
			if ($_POST['item_condition'][$item['id']] == 'Item Issue' ) {
				...
				$_temp_reject_id = str_replace($item['sku'].'-', '', $returns_po_item_insert['reject_item_id']);
				printLabel('','','','','',$_POST['printerid'],'','15','','',$_POST['return_item_id'][$item['id']],$_temp_reject_id,$item['sku']);
		...
		<?php if ($_SESSION['message']): ?>
			...
			<div align="center" style="width:90%;margin:0 auto;">
				<form method="post" action="" id="returnForm" onsubmit="return verifyupdates();" enctype="multipart/form-data">
					...
					<h1 style="font-size:14px;color:#0059a0">Created By: 
						...
						<?php
						if($returns['is_approved'])
						{
							?>
							<h1 style="font-size:14px;color:red">Approved By: 
						<?php 
							echo get_username($returns['approved_by']);
						
						?> (<?php echo americanDate($returns['approved_date']); ?>)</h1>
							<?php
						}
						?>
							...
							<td colspan="3"> <table cellpadding="5" style="width:100%">
								<tr>
									...
									<tr>
										...
										<td colspan="3"><?=$manager_name['name'];?></td>
										<td  rowspan="2">
										<?php
										if($_SESSION['returns_qc_approvals']=='1' && $returns['is_approved']==0 && $returns['rma_status']=='Received')
										{
				...
				<table border="0" >
					...
					<?php
					$tracker = $db->func_query_first("SELECT * FROM inv_tracker WHERE tracking_code='".$rma_return['tracking_number']."'");
					if($tracker)
					{
						?>
						<td align="center">
			...
			<p>
				...
				<?php if ($_SESSION['manage_returns'] && in_array($rma_return['rma_status'], array('Received')) ): ?>
					...
					<?php
					echo ($returns['is_approved']==0?'disabled style="background-color:grey;border-color:grey"':'')
					?>
					 />
				<?php endif; ?>		
				<?php if ($_SESSION['return_decision'] && $rma_return['rma_status'] == 'In QC'): ?>		
...
function approveIt()
{
	if(!confirm('Are you sure want to approve this RMA?'))
	{
		return false;
		...
		$.ajax({
			...
			type: "POST",
			data: {action:'approve_it',rma_number:'<?=$rma_number;?>',is_approved:1},
			beforeSend: function () {
