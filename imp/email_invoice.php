...
$vouchers = $db->func_query('SELECT *, `a`.`amount` as `used` FROM `oc_voucher_history` as a, `oc_voucher` as b WHERE a.`voucher_id` = b.`voucher_id` AND a.`' . (($row['store_type']=='web')? 'order_id': 'order_id') . '` = "'. $order_id .'"');
$total_vouchers = 0.00;
