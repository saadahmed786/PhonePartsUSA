...
if(isset($_GET['action']) && $_GET['action']=='add_vendor_shipment')
{
 ...
 {
  $_SESSION['list'][$product_id] = array("qty"=>0);
 }
 header("Location: ".$host_path."vendor_po_create.php");
 exit; 
}
...
// testObject($result2);
  ...
  $html = '
  <strong>Items below ROP<br><a href="'.$host_path.'reorder_settings.php" style="float:left;margin-left:3%" class="fancybox2 fancybox.iframe button">Re-Order Point</a> <input type="button" class="button button-danger" id="add_vendor_shipment"   value="Add Shipment"> '.($_POST['hide_nto']==0?'<a href="'.$host_path.'purchasing_metrics.php?hide_nto=1" class="button button-info" style="float:right;margin-right:3%" >Hide Negative NTO</a>':'<a href="'.$host_path.'purchasing_metrics.php?hide_nto=0" class="button button-info" style="float:right;margin-right:3%" >Show Negative NTO</a>').'</strong> 
            ...
            <table width="100%" class="xtable" cellspacing="0"   align="center" style="margin-top:3px;line-height: 12px">
                  ...
                  foreach($result2 as $result)
                  {
                    ...
                    $issued_qty =  (int)$db->func_query_first_cell("SELECT SUM(b.req_qty) as req_qty from inv_vendor_po a,inv_vendor_po_items b where a.vendor_po_id=b.vendor_po_id and lower(a.status) in ('issued') and lower(trim(sku))='".trim(strtolower($result['model']))."' ");

                    if($_POST['hide_nto']==1 && (ceil( $result['qty_to_be_shipped'])-$issued_qty)<0)
                    {
                      continue;
                    }
                    $html.='<tr>
                    <td><input type="checkbox" class="checkboxes" value="'.$result['product_id'].'"></td>
                    <td>'.$result['tier'].'</td>
                    ...
                    <td>';
                    $vendor_pos = $db->func_query("SELECT SUM(b.req_qty) as req_qty,a.vendor_po_id,a.id,a.status from inv_vendor_po a,inv_vendor_po_items b where a.vendor_po_id=b.vendor_po_id and lower(a.status) in ('estimate','issued') and lower(trim(sku))='".trim(strtolower($result['model']))."' group by a.vendor_po_id ");
                    $z=0;
                    $html.='V: ';
                    $total_ord_qty = 0;
                    foreach($vendor_pos as $vendor_po)
                    {
                      $html.=($z>0?',':'').'<a href="'.$host_path.'vendor_po_view.php?vpo_id='.$vendor_po['id'].'&hide_header=1" class="fancybox3 fancybox.iframe">'.$vendor_po['req_qty'].(strtolower($vendor_po['status'])=='estimate'?'E':'').'</a>';
                      if(strtolower($vendor_po['status'])!='estimate')
                      {

                      $total_ord_qty+=(int)$vendor_po['req_qty'];
                      }
                      $z++;
                    }
                    if(!$vendor_pos)
                    ...
                    $html.='<br>';
                    $shipment_items = $db->func_query("SELECT SUM(b.qty_shipped) as req_qty,a.package_number,a.id from inv_shipments a,inv_shipment_items b where a.id=b.shipment_id and trim(lower(product_sku))='".trim(strtolower($result['model']))."' and lower(status) in ('pending','issued','received','in qc') and a.vendor<>97 group by a.id");
                    $html.='Sh: ';
                     $z=0;
                    foreach($shipment_items as $vendor_po)
                    {
                      $html.=($z>0?',':'').'<a href="'.$host_path.'view_shipment.php?shipment_id='.$vendor_po['id'].'&hide_header=1" class="fancybox3 fancybox.iframe">'.$vendor_po['req_qty'].'</a>';
                      $z++;
                    }
                    ...
                    $html.='</td>
                    <td>'.(ceil( $result['qty_to_be_shipped'])-$total_ord_qty).'</td>
                    </tr>
                    ';
...
if(!$result2)
{
   // $result2 = $db->func_query("SELECT p.price,p.sale_price, p.tier,p.product_id,p.model,(select name from oc_product_description d where p.product_id=d.product_id) as product_name,p.quantity,p.mps,b.outstock_date,b.instock_date from oc_product p,inv_product_inout_stocks b where lower(p.model)=lower(b.product_sku) and Lower(p.model) not like Lower('BKB-MOD-%') and Lower(model) not like Lower('lbb-%') AND p.is_main_sku=1 and p.status = 1 and p.discontinue = 0  and p.is_blowout=0 and p.is_kit = 0 and p.quantity=0  group by p.sku order by p.mps desc");
  $result2 =$db->func_query("SELECT p.price,p.sale_price, p.tier,p.product_id,p.model,(select name from oc_product_description d where p.product_id=d.product_id) as product_name,p.quantity,p.mps, (select concat(b.outstock_date,'~',b.instock_date) from inv_product_inout_stocks b where lower(p.model)=lower(b.product_sku) order by b.outstock_date desc limit 1) as outage_date from oc_product p where Lower(p.model) not like Lower('BKB-MOD-%') and Lower(model) not like Lower('lbb-%') AND p.is_main_sku=1 and p.status = 1 and p.discontinue = 0 and p.is_blowout=0 and p.is_kit = 0 and p.quantity=0 and(select concat(b.outstock_date,'-',b.instock_date) from inv_product_inout_stocks b where lower(p.model)=lower(b.product_sku) order by b.outstock_date desc limit 1) is not null group by p.sku order by p.mps desc");
  $cache->set('purchasing_metrics.lost_sales_oos',$result2);
...
// testObject($result2);
  ...
  $html = '
  <strong>Sale Lost from Inventory Outages</strong>
            ...
            <table width="100%" class="xtable" cellspacing="0"   align="center" style="margin-top:3px;line-height: 12px">
                  ...
                  foreach($result2 as $result)
                  {
                    $outage_date = explode("~", $result['outage_date']);
                    $outstock_date = $outage_date[0];
                    $instock_date = $outage_date[1];
...
if($result['sale_price']>0.00)
{
  $result['price'] = $result['sale_price'];
}

$loss = ceil($missing_qty) * $result['price'];


                    $html.='<tr>
                    <td>'.$result['tier'].'</td>
                    <td>'.americanDate($outstock_date).'</td>
                    <td>'.linkToProduct($result['model'],$host_path).'</td>
...
if(!$result2)
{
	if($_POST['filter_sort_by']=='profit')
	{
		$sort_by = '((sum(b.product_price)-sum(b.product_true_cost*b.product_qty))/sum(b.product_true_cost*b.product_qty))*100';
	}
	else
	{
		$sort_by = 'sum(b.product_price)';
	}
 // echo "select b.product_sku as sku, sum(b.product_qty) as sold_qty,sum(b.product_price) as sold_price,sum(b.product_true_cost*b.product_qty) as sold_cost,(select count(d.sku) from inv_return_items d,inv_returns c where c.id=d.return_id and b.product_sku=d.sku and d.item_condition in ('Item Issue','Item Issue - RTV','Not Tested','Over 60 Days') and date(c.date_qc) BETWEEN '".$_date['date1']."' AND '".$_date['date2']."') as total_return from inv_orders a,inv_orders_items b where a.order_id=b.order_id and lower(a.order_status) in ('shipped','processed','completed') and date(a.order_date) BETWEEN '".$_date['date1']."' AND '".$_date['date2']."' group by b.product_sku order by $sort_by desc limit 100";
   ...
   // $result2 = $db->func_query("select b.product_sku as sku, sum(b.product_qty) as sold_qty,sum(b.product_price) as sold_price,sum(b.product_true_cost*b.product_qty) as sold_cost,(select count(d.sku) from inv_return_items d,inv_returns c where c.id=d.return_id and b.product_sku=d.sku and d.item_condition in ('Item Issue','Item Issue - RTV','Not Tested','Over 60 Days') and date(c.date_qc) BETWEEN '".$_date['date1']."' AND '".$_date['date2']."') as total_return from inv_orders a,inv_orders_items b where a.order_id=b.order_id and lower(a.order_status) in ('shipped','processed','completed') and date(a.order_date) BETWEEN '".$_date['date1']."' AND '".$_date['date2']."' group by b.product_sku order by sum(b.product_price+(b.product_true_cost*b.product_qty)/(b.product_true_cost*b.product_qty)*100) desc limit 100");
...
// testObject($result2);
  ...
  <select id="filter_sort_by" class="filter_sort_by">
  <option value="sold_price" '.($_POST['filter_sort_by']=='sold_price'?'selected':'').'>Sold Price</option>
  <option value="profit" '.($_POST['filter_sort_by']=='profit'?'selected':'').'>Profit %</option>
  </select>
            <table width="100%" class="xtable" cellspacing="0"   align="center" style="margin-top:3px;line-height: 12px">
                  <thead>
                      <tr>
                  ...
                  foreach($result2 as $result)
                  {
                    ...
                    <td> <a href="'.$host_path.'popupfiles/item_chart.php?sku='.strtolower($result['sku']).'" data-tooltip="Show Period Chart of '.$result['sku'].'" class="fancybox3 fancybox.iframe"><i class="fa fa-bar-chart"></i></a> '.getItemName($result['sku']).'</td>
  ...
  <strong>Parts >5% Return Ratio</strong>'.getFilterHtml($_POST['filter_date'],$_POST['custom_date_start'],$_POST['custom_date_end']).'
            <table width="100%" class="xtable" cellspacing="0"   align="center" style="margin-top:3px;line-height: 12px">
                  ...
                  foreach($result2 as $result)
                  {
                    ...
                    <td>'.(int)$result['total_return'].'/'.number_format(($result['total_return']/$result['sold_qty'])*100,2).'%</td>
                    <td>'.(int)$result['sold_qty'].'</td>
                    <td>$'.number_format($result['sold_price'],2).'</td>
                    <td>$'.number_format($result['sold_cost'],2).'</td>
                    <td>$'.number_format($result['sold_price']-$result['sold_cost'],2).'</td>
...
if(isset($_POST['type']) && $_POST['type']=='pending_rtv')
{

  $vendors = $db->func_query("select distinct b.id,b.name,count(*) as total_shipments from inv_rejected_shipments a,inv_users b where a.vendor=b.id and a.status='Pending' and a.is_hidden=0 and a.vendor>0 group by a.vendor");
  $html = '
  <strong>RTV Pending Shipments</strong> 
            <table width="100%" class="xtable" cellspacing="0"   align="center" style="margin-top:3px;line-height: 12px">
                  <thead>
                      <tr>
                  ...
                  foreach($vendors as $vendor)
                  {
                    $_data = $db->func_query_first("select sum(b.cost) as estimated,count(*) as no_of_items from inv_rejected_shipment_items b,inv_rejected_shipments a where a.id=b.rejected_shipment_id and a.vendor='".$vendor['id']."' and deleted=0 and a.status='Pending' and a.is_hidden=0");
                    $total_items+=$_data['no_of_items'];
                    $total_shipments+=$vendor['total_shipments'];
                    $total_estimated+=$_data['estimated'];
                    $html.='<tr>
...
if(isset($_POST['type']) && $_POST['type']=='shipped_rtv')
{
  ...
  <strong>RTV Shipped Shipments</strong> 
            <table width="100%" class="xtable" cellspacing="0"   align="center" style="margin-top:3px;line-height: 12px">
                  ...
                  foreach($vendors as $vendor)
                  {
                    $_data = $db->func_query_first("select sum(b.cost) as estimated,count(*) as no_of_items from inv_rejected_shipment_items b,inv_rejected_shipments a where a.id=b.rejected_shipment_id and a.vendor='".$vendor['id']."' and deleted=0 and a.status='Shipped' and a.is_hidden=0");
                    ...
                    $rtvs = $db->func_query("select distinct package_number,id from inv_rejected_shipments where   status='Shipped' and is_hidden=0 and vendor='".(int)$vendor['id']."' order by date_issued desc");

                    $total_items+=$_data['no_of_items'];
                    
                    $total_estimated+=$_data['estimated'];
                    $html.='<tr>
                    <td style="vertical-align:top">'.$vendor['name'].'</td>
                     ...
                     foreach($rtvs as $rtv)
                    {
                      ...
                      $html.='<a href="'.$host_path.'addedit_rejectedshipment.php?shipment_id='.$rtv['id'].'" >'.$rtv['package_number'].'</a><br>';
                  	...
                  	$html.='</td>';
                    $html.='<td style="vertical-align:top">'.$_data['no_of_items'].'</td>
                    <td style="vertical-align:top">$'.number_format($_data['estimated'],2).'</td>
                    </tr>';
                  }
                  $html.='</tbody><tfoot><tr>
...
<tr>
      ...
      <tr>
          ...
          <td colspan="4" width="" valign="top">
          <div id="pending_rtv" style="height:317px;overflow-y:scroll;overflow-x: hidden;margin-top:5px;text-align: center;">
...
loadData('items_below_rop');
  ...
  function loadData(type)
  {
    ...
    $.ajax({
        ...
        type: 'post',
        data: {type:type,filter_date:$('#'+type+' .filter_date').val(),custom_date_start:$('#'+type+' .custom_date_start').val(),custom_date_end:$('#'+type+' .custom_date_end').val(),'hide_nto':'<?php echo (int)$_GET['hide_nto'];?>',filter_sort_by:$('#'+type+' .filter_sort_by').val()},
        dataType: 'html',
        beforeSend: function() {
  ...
  jQuery(document).ready(function () {
        ...
        jQuery('.fancybox3').fancybox({width: '90%',height:'500px', autoCenter: true,autoSize:false});
      });
...
$(document).on('change','.filter_sort_by',function(e)
  {
  ...
  $(document).on('click','#add_vendor_shipment',function(){

    var product_ids = $('.checkboxes:checked').map(function() {
    return this.value;
}).get().join(',');
    if(!product_ids)
    {
      alert('Please select the Products to be added into Shipment');
      return false;
    }
  
    if(!confirm('Are you sure want to make a Vendor Shipment?'))
    {
      return false;
    }
    

    window.open('<?php echo $host_path;?>purchasing_metrics.php?action=add_vendor_shipment&product_ids='+product_ids);
  })
  </script>