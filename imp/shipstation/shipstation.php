...
class ShipStation{
	...
	public function getOrder($order_id)	{
		...
		$order_id = $order_id[0];
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, "https://ssapi.shipstation.com/orders?customerName=$name&orderNumber=$order_id");
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
		curl_setopt($ch, CURLOPT_HEADER, FALSE);
		curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, FALSE);
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);

		$authtoken = base64_encode("$this->api_key:$this->api_secret");
