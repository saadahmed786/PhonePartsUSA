...
function addOrder($orderObject , $store_name , $prefix = 'NE') {
	...
	//zip
		...
		//if payment is pending for order then it will process again in next cron run
			...
			$order['shipping_amount']   = $orderObject['shippingAmount'];
			...
			if (is_array($products) and sizeof($products) > 0) {
				...
				foreach ($products as $product) {
					...
					$orderItem['product_name']   =getItemName( $product['sku'] );
					$orderItem['product_qty']   = $product['quantity'];
					$orderItem['product_unit'] = $product['unitPrice'];
					$orderItem['product_price'] = $product['unitPrice'] * $product['quantity'];
					$orderItem['product_true_cost']  = getTrueCost($product['sku']);
					$orderItem['dateofmodification'] = date('Y-m-d H:i:s');


					$sub_total = (float)$sub_total + (float)$orderItem['product_price'];
				//check if SKU is KIT SKU
					...
					if($kit_skus){
						...
						for($_i=1;$_i<=$qty;$_i++)
						{
							...
							foreach($kit_skus_array as $kit_skus_row){
								$itemname = getItemName($kit_skus_row);
								$true_cost = getTrueCost($kit_skus_row);
								$_qty =  1;

								$product_unit = $product['unitPrice'];
								$order_price =	$product['unitPrice'];
								if($zz > 0){
									$true_cost = 0.00;
									$_qty =  1;
									$product_unit = 0.00;
								...
								$db->db_exec("insert into inv_orders_items(order_id,order_item_id,product_sku,product_name,product_price,product_qty,product_true_cost,dateofmodification,product_unit)
									values ('".$orderItem['order_id']."','".$orderItem['order_item_id']."','$kit_skus_row','$itemname','$order_price','$_qty','$true_cost','".date('Y-m-d H:i:s')."','".$product_unit."')");
                 ...
                 //   $db->db_exec("update inv_kit_skus SET need_sync = 1 where kit_sku = '$item_sku'");
						...
						$items_true_cost = (float)$items_true_cost + ((float)$orderItem['product_true_cost']*(int)$orderItem['product_qty']);
				...
				$db->db_exec("UPDATE inv_orders SET sub_total='".(float)$sub_total."',items_true_cost='".(float)$items_true_cost."' WHERE order_id='".$order_id."'");
			}
