...
$inv_query="SELECT a.date_added,b.name,sum(a.debit)-sum(a.credit) as amount from inv_accounts_vouchers a,inv_charts b where a.account_code=b.main_code and a.account_code='".$code."' and date(a.date_added)>='".$_GET['start_date']."' and date(a.date_added) <= '".$_GET['end_date']."' group by date(a.date_added) having sum(a.debit)-sum(a.credit)<>0 order by a.date_added";
// echo $inv_query;exit;
  ...
  <div align="center">
    ...
    <table width="98%">
      <tr>
        ...
        ?>
        <tr>
        <td><?php echo americanDate($data[$j]['date_added'],false);?></td>
        <td><?php echo $data[$j]['name'];?></td>
        <td style="color:red"><?php echo ($data[$j]['amount']<0?'($'.number_format($data[$j]['amount']*(-1),2).')':'') ;?></td>
        <td><?php echo ($data[$j]['amount']>0?'$'.number_format($data[$j]['amount'],2):'') ;?></td>
