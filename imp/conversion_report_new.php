...
$pageLink = 'conversion_report_new.php';
...
$inv_query="select distinct (select k.source from oc_customer_source k where k.email=a.email and k.type='hear' limit 1) as listen, a.freshsales_contact_data, (select c.order_date from inv_orders c where a.email=c.email and lower(order_status) in ('on hold','processed','shipped','completed') order by c.order_date desc limit 1) as last_order, a.user_id, a.firstname,a.lastname,a.email,a.company,b.source,(select count(*) from inv_orders c where a.email=c.email and lower(order_status) in ('on hold','processed','shipped','completed')) as count_order,(select sum(c.sub_total+c.tax+c.shipping_amount) from inv_orders c where a.email=c.email and lower(c.order_status) in ('processed','shipped','completed')) as total_ordered,d.order_id,d.order_status,d.order_date from inv_customers a
...
inv_orders d 
on(a.email=d.email)
where   b.type='business_type' $date_query $extra_query  group by a.email $orderby";
if(isset($_GET['test']))
{
  echo $inv_query;
}
      ...
      <tr>
        <td style="vertical-align: top;width: 25%" align="center" >
          ...
          <form  name="order" action="" method="get">
            ...
            {
              ?>
              <input type="button" name="export_csv" value="Export CSV" class="button button-danger"  />
              <?php
            }
            ?>
          </form>
