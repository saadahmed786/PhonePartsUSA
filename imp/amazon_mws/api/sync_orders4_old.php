...
$merchantInfo = $db->func_query_first("Select * from amazon_credential where id=4");
    if(!@$merchantInfo){
    ...
    $majorLastDate = '2015-01-01 20:32:00';
...
require_once '../amazon-config2.php';
...
function getAmazonOrders() {
    ...
    try {
        $amz = new AmazonOrderList("myStoreCA",null,null,'../amazon-config2.php'); //store name matches the array key in the config file
...
function getOrderLineItems($order_id)
{
   // require('../includes/classes.php');
    global $db;
     try {
        $amz = new AmazonOrderItemList("myStoreCA",$order_id,null,null,'../amazon-config2.php'); //store name matches the array key in the config file
       // $amz->setLimits('Modified', $startDate); //accepts either specific timestamps or relative times 
       // $amz->setFulfillmentChannelFilter("MFN"); //no Amazon-fulfilled orders
