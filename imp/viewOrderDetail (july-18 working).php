...
if (!empty($_POST['removeProducts'])) {
	...
	// 	$data = $db->func_query_first('SELECT * FROM `inv_orders_items` WHERE `id` = "'. $item .'"');
	// 	$t_products[] = $data['product_sku'] . ' * ' . $data['product_qty'];
	// 	$s_q = 'SELECT'
	// 	. ' *'
	// 	. ' FROM'
	// 	. ' `oc_order_total`'
	...
	// 	}
	// 	$db->func_query('DELETE FROM `inv_orders_items` WHERE `id` = "'. $item .'"');
...
if ($_POST['action'] == 'save_item') {
	...
	$json = array();
	$oc_record = $db->db_exec("UPDATE oc_order_product SET price='$unit',quantity='$qty', total = '".( $unit * $qty )."' WHERE cast(order_id as char(50))='". $_POST['order_id'] ."' AND model = '". $sku ."'");
	$s_q = 'SELECT'
	. ' *'
	. ' FROM'
	. ' `oc_order_total`'
	...
	if ($record) {
		...
		$log = linkToProduct($sku) ." updated from quantity ". $xData['product_qty'] ." to ". $qty ." in Order #". linkToOrder($_POST['order_id']);
...
if (isset($_POST['update'])) {
	...
	$inputDetails = array(
		...
		'company' => $db->func_escape_string($_POST['company_shipping']),
		...
		'billing_company' => $db->func_escape_string($_POST['company_billing']),
...
if ($status_id) {
	$db->db_exec("update oc_order SET order_status_id = '$status_id' where cast(order_id as char(50)) = '$orderID' OR ref_order_id='".$orderID."'");
}
if($shipping_method)
{
...
if($old['order_status']!=$order_status){
	...
	if ($order_status == 'Processed') {
		...
		$db->db_exec("UPDATE oc_order SET order_status_id=15,date_modified= NOW() WHERE cast(order_id as char(50)) = '".$orderID."' OR ref_order_id='".$orderID."'");
	}
}
...
<h1 style="font-size:14px;color:#0059a0">AGENT: <?= strtoupper($sales_agent); ?></h1>
				...
				<table cellpadding="10" style="border:1px solid #ddd;border-collapse: collapse;" cellspacing="0" width="70%" border="1">
					...
					<?php if (($order['store_type'] == 'web' || $order['store_type']=='po_business') && !$order['ppusa_sync'] && $order_total > $order['paid_price']) { ?>
					<tr class="voucher">
...
if(!$transaction_dets)
						...
						<tr>
							<td width="50%">
								...
								?>
									...
									<tr>
											<th>Company :</th>
											<td><input type="text" name="company_shipping" size="15" value="<?php echo $order['company']; ?>" /> 
										 </td>
									<tr>
										<th>Address : </th>
										<td>
							...
							<table width="100%" border="0" align="left">
								...
								<tr>
											<th>Company</th>
											<td><input type="text" name="company_billing" size="15" value="<?php echo $order['billing_company']; ?>" /> 
				...
				<table align="center" border="1" cellspacing="0" style="border:1px solid #ddd;border-collapse:collapse;" cellpadding="5" width="85%">
					<thead>
						<tr style="font-weight:bold">
							<td rowspan="2" align="center">Item ID</td>
							<td rowspan="2" align="center">SKU</td>
							<td rowspan="2" align="center" style="width: 300px !important;">Item Name</td>
							<td colspan="2" align="center" >Qty <?php if($_SESSION['vendor_po']){ ?> <a href="<?php echo $host_path;?>vendor_po_create.php?order=<?php echo $order['order_id']; ?>" class="fancybox3 fancybox.iframe" >Create PO</a> <?php } ?>
							</td>
							<?php if($_SESSION['display_order_price'] || $_SESSION['login_as'] == 'admin'){ ?>
							...
							if ($_SESSION['display_profit_margin'] || $_SESSION['login_as'] == 'admin') {
								?>
							<td rowspan="2" align="center">% Profit Margin</td>
							<?php
						}
						?>
							<?php if($_SESSION['display_order_price'] || $_SESSION['login_as'] == 'admin'){ ?>
							<td rowspan="2">Line Total</td>
							<?php } ?>
							<?php
							if ($_SESSION['display_cost'] || $_SESSION['login_as'] == 'admin') {
								?>
								<td rowspan="2" style="display: none;" >Line True Cost</td>
								<?php
							}
							?>
							<td rowspan="2">Action <?php if ((strtolower($order['order_status']) == 'estimate' || strtolower($order['order_status']) == 'processed' || strtolower($order['order_status']) == 'on hold' || $_SESSION['login_as'] == 'admin') && ($_SESSION['login_as'] == 'admin' || $_SESSION['add_product_order'])) { ?><a class="additem" href="javascript:void(0);" onclick="addRow();">Add Row</a> <?php } ?></td>
						</tr>
						<tr>
							<th>On Shelf</th>
		...
		$packed_qty = $db->func_query_first_cell("SELECT sum(b.packed_quantity) FROM inv_orders_items b inner join inv_orders a on (a.order_id=b.order_id) and lower(a.order_status) in ('processed','unshipped') and b.is_picked=1 and trim(lower(b.product_sku))='".strtolower(trim($order_item['product_sku']))."'");
						...
						<tr id="pItem_<?= $order_item['id']; ?>">
							<td title="<?php echo ($order_item['product_name'])? $order_item['product_name']: $order_item['product_sku']; ?>"><?php echo ($order_item['order_item_id'])? $order_item['order_item_id']: $order_item['id']; ?></td>
							<td align="center"><div>
					...
					<td>
						<?php if ((strtolower($order['order_status']) == 'estimate' || strtolower($order['order_status']) == 'processed' || strtolower($order['order_status']) == 'on hold' || strtolower($order['order_status']) == 'awaiting fulfillment' || strtolower($order['order_status']) == 'shipped' || $_SESSION['login_as'] == 'admin') && ($_SESSION['login_as'] == 'admin' || $_SESSION['add_product_order'])): ?>
							...
							<a id="edit_btn_<?= $order_item['id']; ?>" href="javascript:void(0)" onClick="editThis('<?= $order_item['id']; ?>')">Edit</a> |  
							<?php } ?>
							<a id="save_btn_<?= $order_item['id']; ?>" style="display: none;" href="javascript:void(0);" onClick="saveThis('<?= $order_item['id']; ?>')">Save</a>
							...
							<!-- <?php } ?> -->
							<a href="javascript:void(0);" onclick="$.fancybox.open( $('#popupReason<?= $order_item['id']; ?>'), {afterClose: function(){}} );">Remove</a>
				...
				<?php }?>
					...
					$total_vouchers_issued = $db->func_query('SELECT SUM(amount) as amount,voucher_id,code FROM `oc_voucher` WHERE  code LIKE "%'. $order['order_id'] .'%"');
					 ?>
					<?php 
					...
					$voucher_html.='<tr class="tr_total_paid" style="display:none;font-weight:normal">
						...
						<td align="right">Voucher(<a target="_blank" href="vouchers_create.php?edit='. $voucher['voucher_id'] .'">'. $voucher['code'] .'</a>, ' . number_format($remain, 2) . '):</td>
						<td>$'.number_format($voucher['used'], 2).'</td>
					</tr>';
					 } 
					 $total_vouchers_used = $total_vouchers_used * (-1);
					 ?>
					<?php foreach ($total_vouchers_issued as $key => $voucher) { 
					...
					if($transaction_dets)
					{
						...
						<tr>
							...
							// $amountDue = ($order_total+$total_vouchers_used) - $order['paid_price'] - ($refunded_amount);
								      ...
								      $amountDue = ($order['sub_total']+$order['tax']+$order['shipping_amount']+$business_fee) - ($order['paid_price']+$total_vouchers_used);
	...
	<br />
	<br />
	<h2 align="center" style="font-size:14px;"> Removed Items </h2><br><br>
	<table align="center" border="1" style="border:1px solid #ddd;border-collapse:collapse;" cellspacing="0" cellpadding="5" width="70%">
		<thead>
			<tr>
		...
		<tbody id="itemholderremoved">
			...
			<tr>
				<td><input type="text" id="delSku<?php echo $iter;?>" value="<?php echo $removedItemDb['item_sku'];?>" class="read_class" style="background-color:#ffffff;border:none;width:110px;" ></td>
				<td><div id="delItemName<?php echo $iter;?>"><?php echo $removedItemDb['item_name'];?></div></td>
				<td><input style="background-color:#ffffff;border:none" id="delTime<?php echo $iter;?>" type="text" value="<?php echo $removedItemDb['date_removed'];?>" class="read_class"></td>
				<td><input style="background-color:#ffffff;border:none" id="delReason<?php echo $iter;?>" type="text" value="<?php echo $removedItemDb['reason'];?>" class="read_class" ></td>
				<td><input style="background-color:#ffffff;border:none" id="delBy<?php echo $iter;?>" type="text" value="<?php echo $removedItemDb['removed_by'];?>" class="read_class"></td>
				<td><input style="background-color:#ffffff;border:none" id="delItemPrice<?php echo $iter;?>" type="text" value="$<?php echo $removedItemDb['item_price'];?>" class="read_class"></td>
...
<?php if($order_shipments):?>
	<br><br><br><br>
	<h2 align="center" style="font-size:14px;"> Shipment Tracking </h2><br>
	<table align="center" border="1" style="border:1px solid #ddd;border-collapse:collapse;" cellspacing="0" cellpadding="5" width="70%">
		<tr>
			<th>Shipping Cost</th>
	...
	<table width="70%">
		<tr>
			...
			<td width="50%" valign="top">
				...
				<h2>Attachments</h2>
				<table border="1" style="border:1px solid #ddd;border-collapse:collapse;" cellspacing="0" cellpadding="10" width="100%">
					<tr>
						<th>Date</th>
						<th>File</th>
						<th>Description</th>
...
<br /><br /> 
<div align="center"> 
	<table border="1" style="border:1px solid #ddd;border-collapse:collapse;" width="70%" cellpadding="10">
		<tr>
			<th>Country Match</th>
			<th>Distance</th>
			<th>IP City</th>
...
function addRowRemoved () {
	...
	data = '<tr>'
	+'<td><input type="text" id="deletedSku'+rem_ind+'" value="" class="read_class" style="background-color:#ffffff;border:none;width:110px;" ></td>'
	+'<td><div id="deletedItemName'+rem_ind+'"></div></td>'
	+'<td><input style="background-color:#ffffff;border:none" id="deletedTime'+rem_ind+'" type="text" value="" class="read_class"></td>'
	+'<td><input style="background-color:#ffffff;border:none" id="deletedReason'+rem_ind+'" type="text" value="" class="read_class" ></td>'
	+'<td><input style="background-color:#ffffff;border:none" id="deletedBy'+rem_ind+'" type="text" value="" class="read_class"></td>'
	+'<td><input style="background-color:#ffffff;border:none" id="deletedPrice'+rem_ind+'" type="text" value="" class="read_class"></td>'
	+'</tr>'
	+'<tr id = "tot_rem_price_row">'
	+'<td colspan ="5" align="right">'
	+'<strong>Total:</strong>'
