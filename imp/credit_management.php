...
if($_POST['action']=='load_cash_funds')
{
  ...
  $rows = $db->func_query("SELECT a.order_date,a.order_id,a.email,a.deposited_date,a.net_amount,deposit_id FROM inv_orders a,inv_orders_details b WHERE a.order_id=b.order_id  and lower(left(b.payment_method,4))='cash' $where  and a.deposit_id>0 ORDER BY a.`deposited_date` desc");
$html='';
  foreach($rows as $row)
  {
    $html.='<tr>';
...
if($_POST['action']=='load_behalf_funds')
{
  ...
  // echo "SELECT * FROM inv_transactions WHERE deposit_id<>0 $where  order by deposit_date desc";exit;
  $rows = $db->func_query("SELECT a.order_date,a.order_id,a.email,a.deposited_date,a.net_amount,a.gross_amount,a.deposit_id FROM inv_orders a,inv_orders_details b WHERE a.order_id=b.order_id  and lower(left(b.payment_method,6))='behalf' $where  and a.deposit_id>0 ORDER BY a.`deposited_date` desc");
$html='';
  foreach($rows as $row)
  {
    $html.='<tr>';
    ...
    $html.='<td>$'.number_format($row['gross_amount'],2).'</td>';
...
if($_POST['action']=='load_card_funds')
{
   $where='';
  if($_POST['filter_transaction_id']!='')
  {
  ...
  // echo "SELECT * FROM inv_transactions WHERE deposit_id<>0 $where  order by deposit_date desc";exit;
  $rows = $db->func_query("SELECT a.order_date,a.order_id,a.email,a.deposited_date,a.net_amount,a.gross_amount,a.deposit_id FROM inv_orders a,inv_orders_details b,oc_order c WHERE a.order_id=b.order_id and c.order_id=a.order_id  and lower(right(c.payment_method,4))='card' $where  and a.deposit_id>0 ORDER BY a.`deposited_date` desc limit 100");
$html='';
  foreach($rows as $row)
  {
    $html.='<tr>';
...
if($_POST['action']=='load_check_funds')
{
  ...
  // echo "SELECT * FROM inv_transactions WHERE deposit_id<>0 $where  order by deposit_date desc";exit;
  $rows = $db->func_query("SELECT a.order_date,a.order_id,a.email,a.deposited_date,a.net_amount,a.gross_amount,a.deposit_id FROM inv_orders a,inv_orders_details b WHERE a.order_id=b.order_id  and lower(left(b.payment_method,5))='check' $where  and a.deposit_id>0 ORDER BY a.`deposited_date` desc");
$html='';
  foreach($rows as $row)
  {
    $html.='<tr>';
...
if($_POST['action']=='load_wire_funds')
{
  ...
  // echo "SELECT * FROM inv_transactions WHERE deposit_id<>0 $where  order by deposit_date desc";exit;
  $rows = $db->func_query("SELECT a.order_date,a.order_id,a.email,a.deposited_date,a.net_amount,a.gross_amount,a.deposit_id FROM inv_orders a,inv_orders_details b WHERE a.order_id=b.order_id  and lower(left(b.payment_method,4)) in ('wire','bank') $where  and a.deposit_id>0 ORDER BY a.`deposited_date` desc");
$html='';
  foreach($rows as $row)
  {
    $html.='<tr>';
...
<body>
  ...
  <div align="center"> 
   ...
   <div align="center" class="tabMenu" >
      ...
      <input type="button" class="toogleTab" data-tab="tabCash" onclick="$('#tab').val('cash');" value="Cash Payments">
      <input type="button" class="toogleTab" data-tab="tabPaypal" onclick="$('#tab').val('paypal');" value="Paypal Accounting">
      <input type="button" class="toogleTab" data-tab="tabBehalf" onclick="$('#tab').val('behalf');" value="Behalf Accounting">
      <input type="button" class="toogleTab" data-tab="tabCard" onclick="$('#tab').val('card');" value="Card Accounting">
      <input type="button" class="toogleTab" data-tab="tabCheck" onclick="$('#tab').val('check');" value="Check Accounting">
      <input type="button" class="toogleTab" data-tab="tabWire" onclick="$('#tab').val('wire');" value="Wire Transfers">
...
<tr>
 ...
 ?>
           ...
           <table width="90%" cellpadding="0" cellspacing="0" class="xtable">
            <tr>
                <td colspan="8" align="center">Date: <input type="text" data-type="date" style="padding:8px 11px" id="filter_transaction_start" value="<?php echo $_GET['filter_transaction_start'];?>"> to <input type="text" data-type="date" style="padding:8px 11px" class="" id="filter_transaction_end" value="<?php echo $_GET['filter_transaction_end'];?>">
                  <input type="button" value="Find" onclick="window.location='credit_management.php?tab=cash&filter_transaction_start='+$('#tabCash #filter_transaction_start').val()+'&filter_transaction_end='+$('#tabCash #filter_transaction_end').val()" class="button">
                </td>
            </tr>
            <tr>

            <td colspan="2" style="font-size:40px;font-weight: bold" align="center" class="undeposited_amount">0.00</td>
            <td colspan="6" align="center"><h3>Undeposited Cash Transactions</h3><small><i>Undeposited Amount: $

            <?php 

            // echo number_format($db->func_query_first_cell("SELECT SUM(a.sub_total+a.shipping_amount+a.tax)+(select COALESCE(sum(c.amount),0) from oc_voucher_history c where c.order_id=a.order_id) as net_amount FROM inv_orders a,inv_orders_details b WHERE a.order_id=b.order_id  and lower(left(b.payment_method,4))='cash' and lower(a.order_status) in ('shipped') and a.deposit_id=0")  ,2);
            echo number_format($db->func_query_first_cell("SELECT SUM(a.paid_price) as net_amount FROM inv_orders a,inv_orders_details b WHERE a.order_id=b.order_id and lower(a.order_status)<>'canceled'  and lower(left(b.payment_method,4))='cash' and a.paid_price>0 and a.deposit_id=0")  ,2);
            ...
            foreach($cashs as $row)
            {
              ...
              ?>
              <tr class="list_items">
              <td><input type="checkbox" name="transaction_id[]" class="undeposited_checkbox" value="<?php echo $row['order_id'];?>" data-value="<?php echo $row['net_amount']+$row['payment_fee'];?>">

              <input type="hidden" name="net_amount[<?php echo $row['order_id'];?>]" value="<?php echo $row['net_amount']+$row['payment_fee'];?>">
              </td>
              <td><?php
              if($row['date_received']!='0000-00-00 00:00:00' && $row['date_received']!='')
              {

               echo americanDate($row['date_received']);
              }
              elseif($row['undeposited_date']!='0000-00-00')
              {
                echo americanDate($row['undeposited_date']);
              }
              else
              {
                echo americanDate($row['order_date']);
              }
              ?>
              </td>
  ...
  <table width="90%" cellpadding="0" cellspacing="0" class="xtable">
            ...
            foreach($deposits as $deposit)
            {
              ...
              <td><a class="fancyboxXZ fancybox.iframe" href="<?php echo $host_path;?>popupfiles/deposit_transactions.php?deposit_id=<?php echo $deposit['deposit_id'];?>&deposit_type=cash"><?php echo $deposit['name'];?></a></td>
 ...
 ?>
  ...
  &nbsp;<input type="button" class="button" value="Add to Deposit" onclick="addToDeposit('Behalf');"></div>
           <table width="90%" cellpadding="0" cellspacing="0" class="xtable">
            <tr>
                <td colspan="8" align="center">Date: <input type="text" data-type="date" style="padding:8px 11px" id="filter_transaction_start" value="<?php echo $_GET['filter_transaction_start'];?>"> to <input type="text" data-type="date" style="padding:8px 11px" class="" id="filter_transaction_end" value="<?php echo $_GET['filter_transaction_end'];?>">
                  <input type="button" value="Find" onclick="window.location='credit_management.php?tab=behalf&filter_transaction_start='+$('#tabBehalf #filter_transaction_start').val()+'&filter_transaction_end='+$('#tabBehalf #filter_transaction_end').val()" class="button">
                </td>
            </tr>
            <tr>

            <td colspan="2" style="font-size:40px;font-weight: bold" align="center" class="undeposited_amount">0.00</td>
            <td colspan="6" align="center"><h3>Undeposited Behalf Transactions</h3><small><i>Undeposited Amount: $<?php echo number_format($db->func_query_first_cell("SELECT sum(a.paid_price) FROM inv_orders a,inv_orders_details b WHERE a.order_id=b.order_id and lower(left(b.payment_method,6))='behalf' and lower(a.order_status)<>'canceled' and a.gross_amount>0 and a.deposit_id=0 and a.undeposited_by>0")  ,2);?></i></small></td>
            </tr>
            ...
            foreach($behalfs as $row)
            {
              ...
              ?>
              <tr class="list_items">
              <td><input type="checkbox" name="transaction_id[]" class="undeposited_checkbox" value="<?php echo $row['order_id'];?>" data-value="<?php echo $row['net_amount'];?>">
              <input type="hidden" name="net_amount[<?php echo $row['order_id'];?>]" value="<?php echo $row['net_amount']; ?>">
              </td>
  ...
  <table width="90%" cellpadding="0" cellspacing="0" class="xtable">
            ...
            foreach($deposits as $deposit)
            {
              ...
              <tr>
              <td><?php echo date('m/d/Y',strtotime($deposit['deposit_date']));?></td>
              <td><a class="fancyboxXZ fancybox.iframe" href="<?php echo $host_path;?>popupfiles/deposit_transactions.php?deposit_id=<?php echo $deposit['deposit_id'];?>&deposit_type=behalf"><?php echo $deposit['name'];?></a></td>
              <td><?php echo $db->func_query_first_cell("SELECT COUNT(*) from inv_orders where deposit_id='".$deposit['deposit_id']."'");?></td>
              <td>
                <?php 
                echo '$'.number_format( (float)$deposit['amount']-(float)$diff ,2);
 ...
 <option value="">Select Deposit #</option>
 <?php
 $open_deposits = $db->func_query("SELECT * FROM inv_deposits WHERE deposit_type='card' and status='open' order by deposit_date desc");
 foreach($open_deposits as $open_deposit)
 {
  ?>
  <option value="<?php echo $open_deposit['deposit_id'];?>"><?php echo $open_deposit['name'];?></option>
 ...
 ?>
  ...
  &nbsp;<input type="button" class="button" value="Add to Deposit" onclick="addToDeposit('Card');"></div>
           <table width="90%" cellpadding="0" cellspacing="0" class="xtable">
            <tr>
                <td colspan="8" align="center">Date: <input type="text" data-type="date" style="padding:8px 11px" id="filter_transaction_start" value="<?php echo $_GET['filter_transaction_start'];?>"> to <input type="text" data-type="date" style="padding:8px 11px" class="" id="filter_transaction_end" value="<?php echo $_GET['filter_transaction_end'];?>">
                  <input type="button" value="Find" onclick="window.location='credit_management.php?tab=card&filter_transaction_start='+$('#tabCard #filter_transaction_start').val()+'&filter_transaction_end='+$('#tabCard #filter_transaction_end').val()" class="button">
                </td>
            </tr>
            <tr>

            <td colspan="2" style="font-size:40px;font-weight: bold" align="center" class="undeposited_amount">0.00</td>
            <td colspan="6" align="center"><h3>Undeposited Card Transactions</h3><small><i>Undeposited Amount: $<?php echo number_format($db->func_query_first_cell("SELECT 
  sum(net_amount) 
FROM
  (SELECT 
    a.order_date,
...
<input type="hidden" name="net_amount[<?php echo $row['order_id'];?>]" value="<?php echo $row['net_amount'];?>">
          ...
          <tr>
            ...
            ?>
            <td colspan="8">
              <em><?php echo $splitPage6->display_count("Displaying %s to %s of (%s)");?></em>
              <div class="pagination" style="float:right">
                <?php echo $splitPage6->display_links(10,$parameters);?>
              </div>
            </td>
          </tr>
        </tfoot>
          ...
          <td colspan="3" width="40%" valign="top" >
          <div style="text-align: right;margin-top:15px;margin-right:15px"><a class="fancybox2 fancybox.iframe button" href="<?php echo $host_path;?>popupfiles/add_bank_deposit.php?type=card">Add Card Deposit</a></div>
  <table width="90%" cellpadding="0" cellspacing="0" class="xtable">
            <tr>
            
            <td colspan="6" align="center"><h3>Card Deposits</h3></td>
            </tr>
            ...
            <tbody>
            <?php
            $deposits = $db->func_query("SELECT * FROM inv_deposits where deposit_type='card' group by name ORDER BY deposit_date desc");
            foreach($deposits as $deposit)
            {
              $diff = $db->func_query_first_cell("SELECT SUM(net_amount) from inv_orders where deposit_id='".$deposit['deposit_id']."'");
              ?>
              <tr>
              <td><?php echo date('m/d/Y',strtotime($deposit['deposit_date']));?></td>
              <td><a class="fancyboxXZ fancybox.iframe" href="<?php echo $host_path;?>popupfiles/deposit_transactions.php?deposit_id=<?php echo $deposit['deposit_id'];?>&deposit_type=card"><?php echo $deposit['name'];?></a></td>
              <td><?php echo $db->func_query_first_cell("SELECT COUNT(*) from inv_orders where deposit_id='".$deposit['deposit_id']."'");?></td>
              <td>
                <?php 
                echo '$'.number_format( (float)$deposit['amount']-(float)$diff ,2);
          ...
          <td colspan="8">
            ...
            <form id="deposited_card_frm">
            <div align="center" style="font-weight:bold">Ref ID: <input type="text" style="padding:8px 11px" name="filter_transaction_id" value=""> Deposit Date: <input type="text" data-type="date" style="padding:8px 11px" name="filter_deposit_start"> to <input type="text" data-type="date" style="padding:8px 11px" name="filter_deposit_end"> Received Date: <input type="text" data-type="date" style="padding:8px 11px" name="filter_received_start"> to <input type="text" data-type="date" style="padding:8px 11px" name="filter_received_end"><br><br><input type="button" value="Find" onclick="depositedCardFunds();" class="button"></div>
            <input type="hidden" name="action" value="load_card_funds">

            </form>


            <table width="90%" cellpadding="0" cellspacing="0" class="xtable" id="deposited_card_table">
 ...
 ?>
  ...
  &nbsp;<input type="button" class="button" value="Add to Deposit" onclick="addToDeposit('Check');"></div>
           <table width="90%" cellpadding="0" cellspacing="0" class="xtable">
            <tr>
                <td colspan="8" align="center">Date: <input type="text" data-type="date" style="padding:8px 11px" id="filter_transaction_start" value="<?php echo $_GET['filter_transaction_start'];?>"> to <input type="text" data-type="date" style="padding:8px 11px" class="" id="filter_transaction_end" value="<?php echo $_GET['filter_transaction_end'];?>">
                  <input type="button" value="Find" onclick="window.location='credit_management.php?tab=check&filter_transaction_start='+$('#tabCheck #filter_transaction_start').val()+'&filter_transaction_end='+$('#tabCheck #filter_transaction_end').val()" class="button">
                </td>
            </tr>
            <tr>

            <td colspan="2" style="font-size:40px;font-weight: bold" align="center" class="undeposited_amount">0.00</td>
            <td colspan="6" align="center"><h3>Undeposited Check Transactions</h3><small><i>Undeposited Amount:$<?php echo number_format($db->func_query_first_cell("SELECT sum(a.paid_price) FROM inv_orders a,inv_orders_details b WHERE a.order_id=b.order_id and lower(left(b.payment_method,5))='check' and lower(a.order_status)<>'canceled' and a.gross_amount>0 and a.deposit_id=0 and a.undeposited_by>0")  ,2);?></i></small></td>
            </tr>
  ...
  <table width="90%" cellpadding="0" cellspacing="0" class="xtable">
            ...
            foreach($deposits as $deposit)
            {
              ...
              <tr>
              <td><?php echo date('m/d/Y',strtotime($deposit['deposit_date']));?></td>
              <td><a class="fancyboxXZ fancybox.iframe" href="<?php echo $host_path;?>popupfiles/deposit_transactions.php?deposit_id=<?php echo $deposit['deposit_id'];?>&deposit_type=check"><?php echo $deposit['name'];?></a></td>
              <td><?php echo $db->func_query_first_cell("SELECT COUNT(*) from inv_orders where deposit_id='".$deposit['deposit_id']."'");?></td>
              <td>
                <?php 
                echo '$'.number_format( (float)$deposit['amount']-(float)$diff ,2);
 ...
 ?>
  ...
  &nbsp;<input type="button" class="button" value="Add to Deposit" onclick="addToDeposit('<?php echo ucfirst($data_type);?>');"></div>
           <table width="90%" cellpadding="0" cellspacing="0" class="xtable">
            <tr>
                <td colspan="8" align="center">Date: <input type="text" data-type="date" style="padding:8px 11px" id="filter_transaction_start" value="<?php echo $_GET['filter_transaction_start'];?>"> to <input type="text" data-type="date" style="padding:8px 11px" class="" id="filter_transaction_end" value="<?php echo $_GET['filter_transaction_end'];?>">
                  <input type="button" value="Find" onclick="window.location='credit_management.php?tab=<?php echo $data_type;?>&filter_transaction_start='+$('#tab<?php echo ucfirst($data_type);?> #filter_transaction_start').val()+'&filter_transaction_end='+$('#tab<?php echo ucfirst($data_type);?> #filter_transaction_end').val()" class="button">
                </td>
            </tr>
            <tr>

            <td colspan="2" style="font-size:40px;font-weight: bold" align="center" class="undeposited_amount">0.00</td>
            <td colspan="6" align="center"><h3>Undeposited <?php echo ucfirst($data_type);?> Transactions</h3><small><i>Undeposited Amount: $<?php echo number_format($db->func_query_first_cell("SELECT sum(a.paid_price) FROM inv_orders a,inv_orders_details b WHERE a.order_id=b.order_id and lower(a.order_status)<>'canceled' and lower(left(b.payment_method,4)) in ('wire','bank') and a.paid_price>0 and a.deposit_id=0 and a.undeposited_by>0")  ,2);?></i></small></td>
            </tr>
  ...
  <table width="90%" cellpadding="0" cellspacing="0" class="xtable">
            ...
            foreach($deposits as $deposit)
            {
              ...
              <tr>
              <td><?php echo date('m/d/Y',strtotime($deposit['deposit_date']));?></td>
              <td><a class="fancyboxXZ fancybox.iframe" href="<?php echo $host_path;?>popupfiles/deposit_transactions.php?deposit_id=<?php echo $deposit['deposit_id'];?>&deposit_type=<?php echo $data_type;?>"><?php echo $deposit['name'];?></a></td>
              <td><?php echo $db->func_query_first_cell("SELECT COUNT(*) from inv_orders where deposit_id='".$deposit['deposit_id']."'");?></td>
              <td>
                <?php 
                echo '$'.number_format( (float)$deposit['amount']-(float)$diff ,2);
     ...
     <table width="98%" cellpadding="10" style="border: 0px solid #585858;"  align="center">
      <tbody>
        ...
        <tr>
          ...
          <form id="deposit_to_frm">
          <input type="hidden" name="remove_tax" value="1">
 <div style="text-align: left;margin-top:15px;margin-left:15px">
 ...
 ?>
  ...
  &nbsp;<input type="button" class="button" value="Add to Deposit" onclick="addToDeposit('<?php echo ucfirst($data_type);?>');"></div>
           <table width="90%" cellpadding="0" cellspacing="0" class="xtable">
            ...
            <td colspan="2" style="font-size:40px;font-weight: bold" align="center" class="undeposited_amount">0.00</td>
            <td colspan="6" align="center"><h3>Undeposited <?php echo ucfirst($data_type);?> Transactions</h3><small><i>Taxation Balance Due: $<?php echo number_format($db->func_query_first_cell("SELECT sum(net_amount) from ( SELECT a.undeposited_date, a.order_date,a.order_id,a.email,a.tax*(-1) as net_amount,a.tax*(-1) as gross_amount,b.payment_method,a.order_status,(select pos_date from oc_order c where a.order_id=c.order_id) as date_received FROM inv_orders a,inv_orders_details b WHERE a.order_id=b.order_id and lower(a.order_status)<>'canceled' and a.tax>0 and refund_tax=0 and a.paid_price>0 and tax_deposit_id=0 union ALL SELECT a.undeposited_date, a.refund_tax_date as order_date,a.order_id,a.email,a.refund_tax as net_amount,a.refund_tax as gross_amount,b.payment_method,a.order_status,(select pos_date from oc_order c where a.order_id=c.order_id) as date_received FROM inv_orders a,inv_orders_details b WHERE a.order_id=b.order_id and lower(a.order_status)<>'canceled' and refund_tax>0 and a.paid_price>0 and tax_deposit_id=0 ) a ")  ,2);?></i></small></td>
            </tr>
          ...
          <tr>
            ...
            ?>
            <td colspan="8">
              <em><?php echo $splitPage_tax->display_count("Displaying %s to %s of (%s)");?></em>
              <div class="pagination" style="float:right">
                <?php echo $splitPage_tax->display_links(10,$parameters);?>
              </div>
            </td>
          </tr>
        </tfoot>
  ...
  <table width="90%" cellpadding="0" cellspacing="0" class="xtable">
            ...
            foreach($deposits as $deposit)
            {
              $diff = $db->func_query_first_cell("SELECT SUM(deposited_tax_amount) from inv_orders where tax_deposit_id='".$deposit['deposit_id']."'");
              ?>
              <tr>
              <td><?php echo date('m/d/Y',strtotime($deposit['deposit_date']));?></td>
              <td><a class="fancyboxXZ fancybox.iframe" href="<?php echo $host_path;?>popupfiles/deposit_transactions.php?deposit_id=<?php echo $deposit['deposit_id'];?>&deposit_type=<?php echo $data_type;?>"><?php echo $deposit['name'];?></a></td>
              <td><?php echo $db->func_query_first_cell("SELECT COUNT(*) from inv_orders where tax_deposit_id='".$deposit['deposit_id']."'");?></td>
              <td>
                <?php 
                echo '$'.number_format( (float)$deposit['amount']+(float)$diff ,2);
                ?>
           ...
           <table width="90%" cellpadding="0" cellspacing="0" class="xtable">
            <tr>
                <td colspan="8" align="center">Date: <input type="text" data-type="date" style="padding:8px 11px" id="filter_transaction_start" value="<?php echo $_GET['filter_transaction_start'];?>"> to <input type="text" data-type="date" style="padding:8px 11px" id="filter_transaction_end" value="<?php echo $_GET['filter_transaction_end'];?>">
                  <input type="button" value="Find" onclick="window.location='credit_management.php?tab=paypal&filter_transaction_start='+$('#tabPaypal #filter_transaction_start').val()+'&filter_transaction_end='+$('#tabPaypal #filter_transaction_end').val()" class="button">
                </td>
            </tr>
            <tr>
...
depositedCardFunds();
  ...
  success: function(json){
    ...
    if (json['success']) {
     alert(json['success']);
     window.location='<?php echo $host_path;?>credit_management.php?tab='+type.toLowerCase()+'&filter_transaction_start=<?php echo $_GET['filter_transaction_start'];?>&filter_transaction_end=<?php echo $_GET['filter_transaction_end'];?>&page=<?php echo ($_GET['page']?$_GET['page']:1);?>';
                      }
                    }
                  });
  ...
  success: function(json){
    ...
    afterClose: function () { // USE THIS IT IS YOUR ANSWER THE KEY WORD IS "afterClose"
        ...
        window.location='<?php echo $host_path;?>credit_management.php?tab='+$('#tab').val()+'&filter_transaction_start=<?php echo $_GET['filter_transaction_start'];?>&filter_transaction_end=<?php echo $_GET['filter_transaction_end'];?>&page=<?php echo $_GET['page'];?>';
