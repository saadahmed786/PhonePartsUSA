...
if (isset($_POST['save']) || isset($_POST['completed']) || isset($_POST['qcdone'])) {
	...
	foreach ($_POST['product_sku'] as $return_item_id => $product_sku) {
		...
		$return_items['comment'] = $_POST['return_comment'][$return_item_id];
...
<tr>
<td align="right"><a class="fancybox3 fancybox.iframe" href="<?=$host_path;?>settings/item_issues.php?popup=1">Add Item Issues</a>
                                  </td>
</tr>
</table>
<?php
										...
										<td>
											...
											if($received_check)
											{
												...
												<b>Date Received: <?php echo americanDate($received_check['date_added']); ?>
													...
													$xuser_id = $received_check['user_id'];
		...
		<tr>
			...
			<td>
				<?php if ($rma_return['rma_status'] == 'Completed' && in_array($return_item['item_condition'], array('Customer Damage'))): ?>
					<input  type="checkbox" name="return_item[<?php echo $return_item['id']; ?>]" class="item_checkbox" value="<?php echo $return_item['id']; ?>" onchange="checkSelectBoxes()" <?php echo ($decisionCheckQuery ? 'disabled=""' : ''); ?> <?= ($return_item['item_condition'] != '' && $return_item['decision'] != '') ? 'disabled=""': '';?> />
