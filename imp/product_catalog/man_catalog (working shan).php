	...
	if ($_POST['action']) {
		...
		if ($array) {
			...
			if ($_POST['action'] == 'loadSubModels') {
				$json['data'] .= '<a href="" onclick="loadNav(this, \'\', \'\', \'\');" class="list-group-item select-all" data-toggle="collapse">Select All</a>';
			}
			$json['data'] .= '</div>';
			$json['data'] .= '</div>';
		}
...
if ($_POST['perform'] == 'loadProducts') {
	...
	if ($array['products']) {
		...
		foreach ($array['products'] as $i => $row) {
			...
			$json['data'] .= '<label>';
			$json['data'] .= '<input type="checkbox" checked="checked" name="pr-'. $row['product_id'] .'" value="' . $row['product_id'] . '">';
			$json['data'] .= '</label>';
			$json['data'] .= '</div>';
			...
			foreach ($catalog->productGrade($row['model']) as $r => $rowr) {
				...
				$json['data'] .= '<input type="checkbox" name="pr-'. $row['product_id'] .'" value="' . $rowr['product_id'] . '">&nbsp;';
				$json['data'] .= '</label>';
				$json['data'] .= '</div>';
...
<body>
	...
	<script type="text/javascript">
		...
		function loadAttr (t) {
			var select = $(t).parent().parent().parent().find('.attrib').attr('class');
			// if ($(t).is(':checked')) {
			// 	$('.selectClass').each(function() {
		...
		function loadProducts(skip, page) {
			if (!cAction || cAction == 'loadModels' || cAction == 'loadSubModels') {
				return false;
			}
