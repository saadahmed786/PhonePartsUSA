...
function getSalesAgent($email){
	global $db;
	$user_id = $db->func_query_first_cell("SELECT user_id from inv_customers where email = '".$email."'");
	if ($user_id != '0') {	
...
function getMinMaxMean($sku,$days){
	global $db;
	$data = $db->func_query_first("SELECT min(b.product_qty) as minimum, max(b.product_qty) as maximum ,AVG(b.product_qty) as average  FROM inv_orders_items b, inv_orders a where a.order_id=b.order_id and b.product_sku='".$sku."' and lower(a.order_status) not in ('on hold','voided','canceled','cancelled','estimate') and a.order_date BETWEEN CURDATE() - INTERVAL $days DAY AND CURDATE() ");
 	//testObject($data);
	return $data;

}
function getProfileId($email, $host_path = '', $extra = '') {
	global $db;
	global $cache;
	$email = trim($email); 
	$customer_email ='';
	...
	if (strpos($email, '@marketplace.amazon.')) {
		...
		//return '<a target="_parent" data-tooltip="'.(int)$my_data['no_of_orders'].' / $'.number_format($my_data['total_amount'],2).' ('.$my_data['customer_group'].')'.'" href="'.$my_data['link'].'">'.$my_data['email'].'</a>';

	}
	return FALSE;
}
function getCountLast30DaysOrderItem($model) {
	global $db;
	$result = $db->func_query_first_cell ( "SELECT count(o.order_id) as count from inv_orders_items op inner join inv_orders o on (op.order_id = o.order_id) where o.order_date >= ( NOW() - INTERVAL 1 MONTH ) AND op.product_sku='$model' and lower(o.order_status) IN( 'processed','shipped')" );
	return $result;
}
function getLast30DaysItemSale($sku){
	global $db;
...
function getLast15DaysItemSale($sku){
	global $db;
...
function getLast30DaysItemReturns($sku){
	global $db;
...
function getVendorPic($vendor_id)
{
	global $db;
	$vendor_detail = $db->func_query_first("SELECT image FROM inv_users WHERE id='".$vendor_id."'");
	if($vendor_detail['image'])
	{
		$image_path = $vendor_detail['image'];

	}
	else
	{
		$image_path = 'https://phonepartsusa.com/image/data/image-coming-soon.jpg';
	}
	return $image_path;
}
...
function getDPSBySku($sku) {
	...
	where ot.product_sku = '$sku'  and lower(o.order_status) in ('shipped','processed','completed') and order_date >= '$start_date' AND order_date <= '$end_date'";
	$product_qty = 0;
...
function addItemToBox($reject_id , $product_sku , $shipment_id = 0, $box_type = 'NTRBox', $reason = false, $order_id = false , $rma_number = false,$cost=0.00, $from){
	...
	$original_date_added = $db->func_query_first_cell("SELECT date_added FROM inv_rejected_shipment_items WHERE reject_item_id = '$reject_id'");
	$returns_po_item_insert ['date_added'] = $original_date_added;
...
function addUpdateProductCost($SKU, $raw_cost, $ex_rate, $shipping_fee = 0,$refurb_cost = 0, $date = false , $vendor_code = 'China Office') {
	global $db;
	...
	if (strlen ( $SKU ) > 0 and $raw_cost > 0) {
		...
		$productCost ['refurb_cost'] = $refurb_cost;
		...
		function getLeastPrice($sku,$customer_group=0)
		{
			...
			$product_detail = getProduct($sku,array('product_id','price','sale_price'));
			$product_id = $product_detail['product_id'];
			
			$general_price = 	(float)$product_detail['price'];
			$sale_price = 	(float)$product_detail['sale_price'];
			
			if($customer_group==0 || $customer_group=='-1')
			{
			...
			if($sale_price>0)
			{
				...
				// echo $customer_group.'-'.$sale_price;exit;
				if($discount_price<=0)
				{
					$old_price = $general_price;
				}
				else
				{
			...
			if($customer_group=='-1' and $sale_price>0)
			{

				$return ='<span style="color:#4986fe; font-size:20px; font-family:\'Lato\';text-decoration:line-through; ">$'.number_format($old_price,2)."</span><span style='color:red;font-family: \"Lato\";font-weight: bold;font-size: 28px;margin-left: 5px;'>$".number_format($price,2)."</span>";
			}
			else
			{

				$return ='<span style="color:#4986fe; font-size:28px; font-weight:bold; font-family:\'Lato\'; ">$'.number_format($price,2)."</span>";
			}
	...
	// insert into products table if not exist
		...
		function getReturnItemId($rma_number , $product_sku,$is_rtv = 0){
			global $db;

			$prefix = $rma_number."-".$product_sku."-";
			if ($is_rtv == 1) {
				$last_number = $db->func_query_first_cell ( "select max(abs(replace(reject_item_id,'$prefix',''))) as reject_item_id from inv_rejected_shipment_items where reject_item_id LIKE '%$prefix%'" );	
			} else {
...
function getComments($order_id,$is_shipstation = 0) {
	global $db;
	...
	foreach ( $comments as $comment ) {
		...
		if ($is_shipstation == 0) {
			...
			if (!stristr($comment['comment'],'Package shipped on')) {
				$result .= "{$comment['comment']}<br/>";	
			}
		}
	}
...
function getWeeklyReturnsBySKUNew($sku,$parameter='')
{
	global $db;
	$year = date('Y');
	$week = date('W');
	...
	for($i=1;$i<=10;$i++)
	{
		...
		$amtOrdered = $db->func_query_first_cell("SELECT SUM(b.product_qty) AS count_sku FROM `inv_orders` a, `inv_orders_items` b WHERE a.`order_id` = b.`order_id` AND 1 = 1 AND b.product_sku = '$sku' AND YEARWEEK(a.order_date,3)='".date('YW',strtotime($return_week[0]))."' ");

		$item_issue= $db->func_query_first_cell("SELECT COUNT(b.sku) as count_sku FROM inv_returns a,inv_return_items b WHERE a.id=b.return_id AND $parameter AND b.sku='".$sku."' AND b.`item_condition` in('Item Issue','Item Issue - RTV') AND YEARWEEK(a.date_qc,3)='".date('YW',strtotime($return_week[0]))."' ");
		$not_tested = $db->func_query_first_cell("SELECT COUNT(b.sku) as count_sku FROM inv_returns a,inv_return_items b WHERE a.id=b.return_id AND $parameter AND b.sku='".$sku."' AND b.`item_condition` = 'Not Tested' AND YEARWEEK(a.date_qc,3)='".date('YW',strtotime($return_week[0]))."' ");
		$defect = ($item_issue+$not_tested)/$amtOrdered *100;
		...
		echo '<td width="10%">'. $db->func_query_first_cell("SELECT COUNT(b.sku) as count_sku FROM inv_returns a,inv_return_items b WHERE a.id=b.return_id AND $parameter AND b.`item_condition` IN ('Good For Stock','Not Tested','Item Issue','Item Issue - RTV','Need To Repair','Over 60 Days','Not PPUSA Part','Shipping Damage') AND b.sku='".$sku."' AND YEARWEEK(a.date_qc,3)='".date('YW',strtotime($return_week[0]))."' ").'</td>';
		echo '<td width="10%">'. number_format($defect,2).'%</td>';
		echo '<td width="10%">'. $item_issue.'</td>';
		echo '<td width="10%">'. $not_tested.'</td>';
		echo '<td width="10%">'. $db->func_query_first_cell("SELECT COUNT(b.sku) as count_sku FROM inv_returns a,inv_return_items b WHERE a.id=b.return_id AND $parameter AND b.sku='".$sku."' AND b.`item_condition` = 'Good For Stock' AND YEARWEEK(a.date_qc,3)='".date('YW',strtotime($return_week[0]))."' ").'</td>';
		echo '<td width="10%">'. $db->func_query_first_cell("SELECT COUNT(b.sku) as count_sku FROM inv_returns a,inv_return_items b WHERE a.id=b.return_id AND $parameter AND b.sku='".$sku."' AND b.`item_condition` = 'Need To Repair' AND YEARWEEK(a.date_qc,3)='".date('YW',strtotime($return_week[0]))."' ").'</td>';
		echo '<td width="10%">'. $db->func_query_first_cell("SELECT COUNT(b.sku) as count_sku FROM inv_returns a,inv_return_items b WHERE a.id=b.return_id AND $parameter AND b.sku='".$sku."' AND b.`item_condition` = 'Over 60 Days' AND YEARWEEK(a.date_qc,3)='".date('YW',strtotime($return_week[0]))."' ").'</td>';
		echo '<td width="10%">'. $db->func_query_first_cell("SELECT COUNT(b.sku) as count_sku FROM inv_returns a,inv_return_items b WHERE a.id=b.return_id AND $parameter AND b.sku='".$sku."' AND b.`item_condition` = 'Not PPUSA Part' AND YEARWEEK(a.date_qc,3)='".date('YW',strtotime($return_week[0]))."' ").'</td>';
		echo '<td width="10%">'. $db->func_query_first_cell("SELECT COUNT(b.sku) as count_sku FROM inv_returns a,inv_return_items b WHERE a.id=b.return_id AND $parameter AND b.sku='".$sku."' AND b.`item_condition` = 'Shipping Damage' AND YEARWEEK(a.date_qc,3)='".date('YW',strtotime($return_week[0]))."' ").'</td>';
...
function getTrueCost($sku,$with_true_cost=1){
	global $db;
	$true_cost = 0.00;
	...
	$cost = $db->func_query_first("SELECT  cost_date,raw_cost,refurb_cost,ex_rate,shipping_fee FROM inv_product_costs WHERE sku='" . $sku . "' ORDER BY id DESC limit 1");
	if($cost)
	{
		if($with_true_cost==1)
		{
		...
		{
			$true_cost = ($cost['raw_cost'] + $cost['shipping_fee']) ;
		}
		$true_cost = $true_cost + $cost['refurb_cost'];
...
function americanDate ($date,$show_time=true) {
...
function linkToShipmentEdit($shipment_id, $host_path = '', $tracking = '', $extra = '') {
	if ($shipment_id) {
		$r = '<a ' . $extra .' href="' . $host_path . 'addedit_shipment.php?shipment_id=' . $shipment_id . '">' . (($tracking)? $tracking: $shipment_id) . '</a>';
	} else {
		$r = 'N/A';
	}
	return $r;
...
function linkToRefurbShipment($shipment_id, $host_path = '', $extra = '') {
	return '<a href="' . $host_path . 'refurbishing/shipment_detail.php?shipment=' . $shipment_id . '">' . $shipment_id . '</a>';
}
...
function linkToVoucher($voucher_number, $host_path = '', $code = '', $extra = '') {
	return '<a href="' . $host_path . 'vouchers_create.php?edit=' . $voucher_number . '" '.$extra.'>' . (($code)? $code: $voucher_number) . '</a>';
}
function getEmoji($email,$size=24,$host_path)
{
	global $db;
	$mood = $db->func_query_first_cell("SELECT customer_mood FROM inv_customers WHERE email='".$email."'");
	$emoji = '';
	if($mood)
	{
		$emoji = '<img src="images/emoji/'.$mood.'.png" style="width:'.$size.'px;margin-left:5px">';
	}
	return $emoji;
}

function linkToProfile($email, $host_path = '', $extra = '',$target = '_parent') {
	global $db;
	global $cache;
	$email = trim($email); 
	$customer_email ='';
	...
	if (strpos($email, '@marketplace.amazon.') || strpos($email, '@seller.sears.') || strpos($email, '@marketplace.newegg.') || strpos($email, '@mail.marketplace.buy.')) {
		return $email;
	} else {
		
		//$my_data = $cache->get('customer.'.$_cache);
		if(!$my_data)
		{
		...
		//$cache->set('customer.'.$_cache,$my_data);
		}




		return '<a target="'.$target.'" data-tooltip="'.(int)$my_data['no_of_orders'].' / $'.number_format($my_data['total_amount'],2).' ('.$my_data['customer_group'].')'.'" href="'.$my_data['link'].'">'.$my_data['email'].'</a>';
...
function sendEmailDetails ($data = array(), $email, $smtp = array(),$attachment='', $attachmentname = '') {
	...
	/*	$email['message'] = '<body style="background:#CCC;">
		...
		$email['message'] = '<table  data-module="noti-3" data-bgcolor="Main BG" width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="currentTable">
		<tbody><tr>
			<td align="center">
																				...
																				<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
																					...
																					<tr>
																						<td data-color="Headline" data-size="Headline" mc:edit="noti-3-1" align="center" style="font-family: \'Open sans\', Arial, sans-serif; color:#FFFFFF; font-size:16px; font-weight: bold;" contenteditable="true" class="editable">
																							<singleline label="headline">'.$email['title'].'</singleline>
																						</td>
																					</tr>
																					<tr>
																						<td height="15"></td>
														...
														<table align="center" width="90%" border="0" cellspacing="0" cellpadding="0">
																	...
																	<table width="100%" align="center" border="0" cellpadding="0" cellspacing="0">
																		<tbody><tr>
																			<td align="center" style="line-height: 0px;">
																				<img editable="" label="image" data-crop="false" src="'.$email['image'].'" alt="img" width="" class="img1" style="display:block; line-height:0px; font-size:0px; border:0px;" mc:edit="noti-3-4">
																			</td>
																		</tr>
																	</tbody></table>
																</td>
															...
															<tr>
																<td data-link-style="text-decoration:none; color:#3b3b3b;" data-link-color="Title Link" data-color="Title" data-size="Title" mc:edit="noti-3-5" align="center" style="font-family: \'Open Sans\', Arial, sans-serif; font-size: 22px;color:#414a51;font-weight: bold;line-height: 28px;">
																	<singleline label="Headline">'.$numberTitle.': '.$numberValue.'</singleline>
																</td>
															</tr>
															<!--end headline-->	
															<tr>
															...
															<tr>
																<td data-link-style="text-decoration:none; color:#447BE8;" data-link-color="Content Link" data-color="Content" data-size="Content" mc:edit="noti-3-6" align="left" style="font-family: \'Open sans\', Arial, sans-serif; color:#7f8c8d; font-size:14px; line-height: 14px;">
																	<multiline label="content">'.str_replace('\\', '', str_replace(PHP_EOL, '', $email['message'])).'</multiline>
																</td>
															</tr>
															<!--end content-->
															<tr>
																	...
																	<table data-bgcolor="Button" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#39CB59" class="textbutton" style="border-radius:4px;">
																		<tbody><tr>
																			<td data-link-style="text-decoration:none; color:#ffffff;" data-link-color="Button Link" data-size="Button" mc:edit="noti-3-7" height="40" align="center" style="font-family: \'Open sans\', Arial, sans-serif; color:#FFFFFF; font-size:14px;padding-left: 25px;padding-right: 25px;font-weight: bold; ">
																				<a href="https://phonepartsusa.com" style="text-decoration: none; color:#FFFFFF;" data-color="Button Link">
		...
		$dataEmail['email_body'] = $db->func_escape_string(htmlentities($email['message'],ENT_QUOTES));
		...
		// echo htmlentities($email['message']);exit;
...
function whiteList($order, $hold = 0, $array = 0) {
	...
	$skus = $db->func_query("SELECT product_sku,product_qty FROM inv_orders_items where order_id='".$order['order_id']."'");
	$sort_array = array();
	foreach($skus as $sku)
	{
	...
	$authentic_store=false;
	if ($order['is_manual']==1 || $order['store_type'] == 'ebay' || $order['store_type'] == 'rakuten' || $order['store_type'] == 'newegg' || $order['store_type'] == 'newsears' || $order['store_type'] == 'opensky' || $order['store_type'] == 'wish' || $order['store_type'] == 'po_business' || strpos(trim(strtolower($order['email'])), '@marketplace.amazon.')) {
		$check = 4;
	...
	if (stristr(trim(strtolower($order['email'])), '@protonmail.com') !== false) {
		$blackListCustomer = 1;
	}
	...
	if (!$blackListCustomer && $check == 0) {
		...
		if (($ship_meth == 'usps first class' || $ship_meth == 'usps priority mail' || $ship_meth == 'ups/fedex ground' || $ship_meth == 'standard shipping' || $ship_meth == 'per item shipping rate') && $order['order_price'] < 30.00 && $order['store_type']!='bigcommerce') {
			$check += 1;
			...
			$checkArr[3] = 'Un-Matched Shipment Method with Order Total - ' . $ship_meth;
		}
	}
	...
	if($check>=4 && $hold==1 && $valid==true)
	{
		if(strtolower($order['order_status'])!='shipped' && strtolower($order['order_status'])!='canceled' && strtolower($order['order_status'])!='cancelled' && strtolower($order['order_status'])!='voided' && strtolower($order['order_status'])!='completed' && strtolower($order['order_status'])!='declined' && $order['order_status']!='On Hold'  )
		{


		//makeLedger($order['order_id'],$sort_array,$_SESSION['user_id'],'not_picked');
		}
	}

	if ($check >= 4 && $hold == 1 && $valid == true && $order['order_status'] == 'On Hold') {
		$db->db_exec("UPDATE inv_orders SET order_status='Processed' WHERE order_id='".$order['order_id']."'");
		$db->db_exec("UPDATE inv_orders_items SET ostatus='processed' WHERE order_id='".$order['order_id']."'");
		$db->db_exec("UPDATE oc_order SET order_status_id='15',date_modified='".date('Y-m-d H:i:s')."' WHERE cast(`order_id` as char(50))='".$order['order_id']."' OR ref_order_id='".$order['order_id']."'");
		

		//makeLedger($order['order_id'],$sort_array,$_SESSION['user_id'],'reserved_not_picked');
		$addcomment = array();
	...
	if ($check < 4 && $hold == 1 && $order['first_order_verification'] == 0) {
		...
		if(strtolower($order['order_status'])!='shipped' && strtolower($order['order_status'])!='canceled' && strtolower($order['order_status'])!='cancelled' && strtolower($order['order_status'])!='voided' && strtolower($order['order_status'])!='completed' && strtolower($order['order_status'])!='declined' && strtolower($order['order_status'])=='processed' && $order['is_manual']!=1  )
		{
		...
		// close the connection, release resources used
			...
			if ($order['order_status'] == 'Canceled' || $order['order_status'] == 'Cancelled') {
				$db->db_exec("UPDATE inv_orders SET order_status='Canceled', first_order_verification = '1' WHERE order_id='".$order['order_id']."'");
				$db->db_exec("UPDATE inv_orders_items SET ostatus='canceled' WHERE order_id='".$order['order_id']."'");
				$db->db_exec("UPDATE oc_order SET order_status_id='7',date_modified='".date('Y-m-d H:i:s')."' WHERE cast(`order_id` as char(50))='".$order['order_id']."' OR ref_order_id='".$order['order_id']."'");
			} else if (($order['order_status'] != 'Canceled' || $order['order_status'] != 'Cancelled') && strtolower($order['order_status'])=='processed') {
				$db->db_exec("UPDATE inv_orders SET order_status='On Hold', first_order_verification = '1' WHERE order_id='".$order['order_id']."'");
				$db->db_exec("UPDATE inv_orders_items SET ostatus='on hold' WHERE order_id='".$order['order_id']."'");
				$db->db_exec("UPDATE oc_order SET order_status_id='21',date_modified='".date('Y-m-d H:i:s')."' WHERE cast(`order_id` as char(50))='".$order['order_id']."' OR ref_order_id='".$order['order_id']."'");	



				//makeLedger($order['order_id'],$sort_array,$_SESSION['user_id'],'reserved');
			}
		...
		// $dataEmail['email'] = $order['email'];
			$dataEmail['email'] = 'faheemm@phonepartsusa.com';
			...
			$addcomment['order_id'] = $order['order_id'];
			if (($order['order_status'] != 'Canceled' || $order['order_status'] != 'Cancelled') && strtolower($order['order_status'])=='processed'){
				$order_history_id = $db->func_array2insert("oc_order_history", $addcomment);
			}
		}
	...
	if ($order['order_price'] > 100.00 && $total_orders == 0 && $authentic_store==false) {
		$db->db_exec("UPDATE inv_orders SET order_status='On Hold',first_order_verification='1' WHERE order_id='".$order['order_id']."'");
		$db->db_exec("UPDATE inv_orders_items SET ostatus='on hold' WHERE order_id='".$order['order_id']."'");
		$db->db_exec("UPDATE oc_order SET order_status_id='21',date_modified='".date('Y-m-d H:i:s')."' WHERE cast(`order_id` as char(50))='".$order['order_id']."' OR ref_order_id='".$order['order_id']."'");
		$addcomment = array();
		$addcomment['date_added'] = date('Y-m-d H:i:s');
		$addcomment['user_id'] = '43';
		$addcomment['comment'] = 'Customers First Order Above $100<br>Order is Set to On Hold VIA Verification Process';
		$addcomment['order_id'] = $order['order_id'];
		$order_history_id = $db->func_array2insert("oc_order_history", $addcomment);
...
function logIP($user_id)
{
	...
	// $data['ip_address'] = ($user_id==43?$db->func_query_first_cell('select ip_address from inv_login_logs where user_id=83 order by log_id desc limit 1'):$_SERVER['REMOTE_ADDR']);
...
function noImage($name, $host_path, $path,$cache=1,$width=50,$height=50) {
	$path = str_replace('/imp', '', $path);
	// return str_replace('//imp.', '//', $host_path) . 'image/' . $name;
	if (file_exists($path . 'image/' . $name) && trim($name)!='') {
		if($cache==0)
		{
			return str_replace('//imp.', '//', $host_path) . 'image/' . $name;
		...
		else
		{
			...
			$name = str_replace('data/', 'cache/data/', $name);
			$name = str_replace('impskus/', 'cache/impskus/', $name);
			$name = str_replace('.jpg', '-'.$width.'x'.$height.'.jpg', $name);
			$name = str_replace('.png', '-'.$width.'x'.$height.'.png', $name);
			// str_replace(search, replace, subject)
			// $name = str_replace('.jpg', '-50x50.jpg',$name);
			// $name = str_replace('.png', '-50x50.png',$name);
			...
			{
				return str_replace('//imp.', '//', $host_path) . 'image/cache/data/image-coming-soon-'.$width.'x'.$height.'.jpg';
			}
		}
	}
...
function orderTotal($order_id, $update = false) {
	...
	if($order['state'] == 'Nevada' && !$db->func_query_first_cell("SELECT dis_tax FROM inv_customers WHERE email='" . $order['email'] . "'")) {
	...
	if($order_id=='565133')
	{
		// echo $db->func_query_first_cell("SELECT dis_tax FROM inv_customers WHERE email='" . $order['email'] . "'");exit;
	}
	...
	$profit = $order_total - $sub_total['truecost'] - $tax;

	if ($update) {	
		$db->db_exec('UPDATE inv_orders SET `order_price` = "' . round($order_total, 2) . '",sub_total="'.(float)$sub_total['amount'].'",items_true_cost="'.$sub_total['truecost'].'",tax="'.(float)$tax.'", `profit` = "' . round($profit, 2) . '" WHERE order_id = "' . $order_id . '"');
		if ($order['store_type'] == 'web') {
			$db->func_query("UPDATE oc_order_total set value = '" . $sub_total['amount'] . "', text = '$" . $sub_total['amount'] . "' WHERE order_id = '$order_id' AND code = 'sub_total'");
...
/*function printLabel ($id, $sku, $box_number, $reason, $order_id, $printer, $source, $font = 12,$img='', $rejLabel='') {
	global $host_path;
	global $path;
	require_once($path.'inc/barcode/BarcodeGenerator.php');
	require_once($path.'inc/barcode/BarcodeGeneratorPNG.php');
	...
	$body .= ($box_number)?'<tr><td class="idheading">'. $box_number .'</td></tr>': '';
	...
	$body .= ($img)?'<tr><td class="heading"><img style="width:170px;height:63px" src="data:image/png;base64,' . base64_encode($generator->getBarcode($sku, $generator::TYPE_CODE_128)) . '"></td></tr>': '';
	$body .= ($rejLabel)?'<tr><td class="heading"><img style="width:170px;height:63px" src="data:image/png;base64,' . base64_encode($generator->getBarcode($sku, $generator::TYPE_CODE_128)) . '"></td></tr>': '';
	$body .= '</table>';
	$body .= '</div>';
	$footer = '</body>';
	$footer .= '</html>';
	...
	printNodePDF($pdf, 'Label print for return', $printer);
	return $pdf;
}*/

function printLabel ($id, $sku, $box_number, $reason, $order_id, $printer, $source, $font = 12,$img='', $rejLabel='',$barcode_label='',$top_text='',$second_text='') {
	//error_reporting(E_ALL);
	//ini_set('display_errors', 1);
	global $host_path;
	global $path;
	// require_once ($path.'inc/Barcode.php');
	// $Barcode = new Barcode();
	// $Barcode->setType('C128');
	// $Barcode->setSize(110,200,0);
	// $Barcode->hideCodeType();
	// $Barcode->setCode($sku);
	// $Barcode->setText('');
	// $rand_num=rand();
	// $file =$path.'images/user/' . $sku . ''.$rand_num.'.png';
	...
	$body .= ($reason)?'<tr><td class="heading">'. $reason .'</td></tr>': '';
	$body .= ($order_id)?'<tr><td class="heading">'. $order_id .'</td></tr>': '';
	$body .= ($top_text)?'<tr><td class="idheading">'. $top_text .'</td></tr>': '';
	$body .= ($second_text)?'<tr><td class="headingMain">'. $second_text .'</td></tr>': '';

	// New Barcode Library Used
	// $body .= ($img)?'<tr><td class="heading"><img style="width:16px" src="'.$host_path.'images/user/'. $sku . ''.$rand_num.'.png" alt="barcode"></td></tr>': '';
	// $body .= ($rejLabel)?'<tr><td class="heading"><img style="width:16px" src="'.$host_path.'images/user/'. $sku . ''.$rand_num.'.png" alt="barcode"></td></tr>': '';
	$body .= ($img)?'<tr><td class="heading"><img style="width:170px;height:63px" src="data:image/png;base64,' . base64_encode($generator->getBarcode($sku, $generator::TYPE_CODE_128)) . '"></td></tr>': '';
	$body .= ($rejLabel)?'<tr><td class="heading"><img style="width:170px;height:63px" src="data:image/png;base64,' . base64_encode($generator->getBarcode($sku, $generator::TYPE_CODE_128)) . '"></td></tr>': '';

	$body .= ($barcode_label)?'<tr><td class="heading"><img style="width:170px;height:50px" src="data:image/png;base64,' . base64_encode($generator->getBarcode($barcode_label, $generator::TYPE_CODE_128)) . '"></td></tr>': '';
	// Old Barcode Library
...
function getOCItemPrice($product_id)
{
	global $db;
	$product_query = $db->func_query_first("SELECT * FROM oc_product p LEFT JOIN oc_product_description pd ON (p.product_id = pd.product_id) WHERE p.product_id = '" . (int)$product_id . "' AND pd.language_id = '1' AND p.date_available <= NOW() AND p.status = '1'");
	...
	function makeLedger($order_id,$skus,$user_id=0,$action='reserved',$description='',$notes='')
	{
		global $db;
		
		$db->db_exec("START TRANSACTION");
		foreach($skus as $sku => $quantity)
		{
			if($description=='')
			{
				$description = ledgerDescription($action);
			}

			$on_hand = $db->func_query_first_cell("select quantity FROM oc_product WHERE trim(lower(model))='".strtolower(trim($sku))."' ");
			// 		$reserved = $db->func_query_first_cell("SELECT sum(b.product_qty)  FROM inv_orders_items b inner join inv_orders a on (a.order_id=b.order_id) and lower(a.order_status)='on hold'  and trim(lower(b.product_sku))='".strtolower(trim($sku))."'");
			...
			// 		$picked = $db->func_query_first_cell("SELECT sum(b.picked_quantity) FROM inv_orders_items b inner join inv_orders a on (a.order_id=b.order_id) and lower(a.order_status) in ('processed','unshipped','on hold') and b.is_picked=1 and a.is_packed=0 and trim(lower(b.product_sku))='".strtolower(trim($sku))."'");
			// 		$packed = $db->func_query_first_cell("SELECT sum(b.packed_quantity) FROM inv_orders_items b inner join inv_orders a on (a.order_id=b.order_id) and lower(a.order_status) in ('processed','unshipped') and b.is_picked=1 and a.is_packed=1 and trim(lower(b.product_sku))='".strtolower(trim($sku))."'");

			$query=$db->db_exec("INSERT INTO inv_product_ledger SET order_id='".$db->func_escape_string($order_id)."',sku='".$db->func_escape_string($sku)."',quantity='".(int)$quantity."',action='".$action."',description='".$db->func_escape_string($description)."',notes='".$db->func_escape_string($notes)."',user_id='".(int)$user_id."',date_added='".date('Y-m-d H:i:s')."',on_hand='".(int)$on_hand."'");
		}
	...
	function getInventoryDetail($sku)
	{
		global $db;
		
		$product_info = $db->func_query_first("SELECT quantity,prefill,prefill_shipment FROM oc_product WHERE TRIM(LOWER(model))='".strtolower(trim($sku))."'");
	...
	function ledgerDescription($action)
	{
		switch ($action) {
			case 'reserved':
			...
			break;
			case 'not_picked':
			$description = 'Saved as Not Picked.';
			break;
			case 'reserved_not_picked':
			$description = 'Reserved &rarr; Not Picked.';
			break;
	...
	function getCountries($limit = true)
	{
		...
		if($limit)
		{
			$where =' country_id = 38 or country_id=223';
		}
		$data = $db->func_query("SELECT * FROM oc_country where $where order by name asc");
		return $data;
	}
	function getCountry($country_id)
	{
		global $db;
		$row = $db->func_query_first_cell("SELECT name FROM oc_country WHERE country_id='".(int)$country_id."'");
		return $row;
	}
	function getStates($limit = true)
	{
		global $db;
		$where = ' 1 = 1 ';
		if($limit)
		...
		$data = $db->func_query("SELECT * FROM oc_zone where $where order by name asc");
		return $data;
	}
	...
	{
		global $db;
		$row = $db->func_query_first_cell("SELECT name FROM oc_zone WHERE zone_id='".(int)$zone_id."'");
		return $row;
	}
	function initCompetitorLinks($sku)
	{
		global $db;
		$db->db_exec("INSERT INTO inv_product_price_scrap SET sku='".$sku."',date_updated='".date('Y-m-d H:i:s')."',is_new=1,type='mobile_sentrix',url=''");
		$db->db_exec("INSERT INTO inv_product_price_scrap SET sku='".$sku."',date_updated='".date('Y-m-d H:i:s')."',is_new=1,type='fixez',url=''");
		$db->db_exec("INSERT INTO inv_product_price_scrap SET sku='".$sku."',date_updated='".date('Y-m-d H:i:s')."',is_new=1,type='mengtor',url=''");
		$db->db_exec("INSERT INTO inv_product_price_scrap SET sku='".$sku."',date_updated='".date('Y-m-d H:i:s')."',is_new=1,type='mobile_defenders',url=''");
		$db->db_exec("INSERT INTO inv_product_price_scrap SET sku='".$sku."',date_updated='".date('Y-m-d H:i:s')."',is_new=1,type='etrade_supply',url=''");
		$db->db_exec("INSERT INTO inv_product_price_scrap SET sku='".$sku."',date_updated='".date('Y-m-d H:i:s')."',is_new=1,type='maya_cellular',url=''");
		$db->db_exec("INSERT INTO inv_product_price_scrap SET sku='".$sku."',date_updated='".date('Y-m-d H:i:s')."',is_new=1,type='lcd_loop',url=''");
		$db->db_exec("INSERT INTO inv_product_price_scrap SET sku='".$sku."',date_updated='".date('Y-m-d H:i:s')."',is_new=1,type='parts_4_cells',url=''");
		$db->db_exec("INSERT INTO inv_product_price_scrap SET sku='".$sku."',date_updated='".date('Y-m-d H:i:s')."',is_new=1,type='cell_parts_hub',url=''");
	}

	function updateOnShelfQty($sku,$quantity)
	{
		global $db;
		// echo $quantity;exit;
		// echo "SELECT p.model, ( (SELECT COALESCE(sum(b.picked_quantity) - sum(b.packed_quantity),0) FROM inv_orders_items b where b.ostatus in ('processed','unshipped','on hold') and b.is_picked=1 and b.opacked=0 and b.product_sku=p.model) + (SELECT COALESCE(sum(b.packed_quantity),0) FROM inv_orders_items b where b.ostatus in ('processed','unshipped','on hold') and b.is_packed=1 and b.product_sku=p.model)) as picked_packed from oc_product p where trim(lower(p.model))='".trim(strtolower($sku))."'";exit;
		$row=$db->func_query_first("SELECT p.model, ( (SELECT COALESCE(sum(b.picked_quantity) - sum(b.packed_quantity),0) FROM inv_orders_items b where b.ostatus in ('processed','unshipped','on hold') and b.is_picked=1 and b.opacked=0 and b.product_sku=p.model) + (SELECT COALESCE(sum(b.packed_quantity),0) FROM inv_orders_items b where b.ostatus in ('processed','unshipped','on hold') and b.is_packed=1 and b.product_sku=p.model)) as picked_packed from oc_product p where trim(lower(p.model))='".trim(strtolower($sku))."'");
		// echo "UPDATE oc_product SET quantity='".((int)$quantity+(int)$row['picked_packed'])."' where trim(lower(sku))='".trim(strtolower($sku))."'";exit;
		$db->db_exec("START TRANSACTION");

		$query = $db->db_exec("UPDATE oc_product SET quantity='".((int)$quantity+(int)$row['picked_packed'])."',date_modified='".date("Y-m-d H:i:s")."' where trim(lower(model))='".trim(strtolower($sku))."'");
	...
	function add_accounting_voucher($data)
	{
		global $db;
		// file_put_contents("accounts.log","Data: ".json_encode($data)."\n",FILE_APPEND);
		$type = $data['type'];
		
		$data['debit'] = ($data['debit']<0?$data['debit']*(-1):$data['debit']);	
		$data['credit'] = ($data['credit']<0?$data['credit']*(-1):$data['credit']);	

		$insert = array();

		$insert['description'] = $db->func_escape_string($data['description']);
		$insert['debit'] = (float)$data['debit'];
		$insert['credit'] = (float)$data['credit'];
		
		$insert['order_id'] = $db->func_escape_string($data['order_id']);
		$insert['buyback_id'] = (int)$data['buyback_id'];
		$insert['customer_email'] = $db->func_escape_string($data['customer_email']);
		if($data['contra_account_code'])
		{
		$insert['contra_account_code'] = get_account_head($data['contra_account_code']);
			
		}
		$insert['user_id'] = (int)$_SESSION['user_id'];
		$insert['company_id'] = 0;
		// $insert['date_added'] = date('Y-m-d H:i:s',$data['date_added']);
		$insert['date_added'] = $data['date_added'];
		if(!$data['account_code'])
		{
			$insert['account_code'] = get_account_head($data['type']);
			
		}
		else
		{

			$insert['account_code']  = $data['account_code'];
		}
		
		$insert['account_name'] = $db->func_query_first_cell("SELECT name FROM inv_charts where main_code='".$db->func_escape_string($insert['account_code'])."'");

		return $db->func_array2insert("inv_accounts_vouchers",$insert);

	}

	function get_account_head($type)
	{
	...
	function addVoucher($order_id,$method,$amount,$reference='',$payment_fee=0.00,$net_amount=0.00,$date='')
	{	
		// file_put_contents("vouchers.txt","Add Voucher: ".json_encode($voucher)."\n",FILE_APPEND);
		if($date=='')
		{
		...
		global $db;
		$voucher = array();
		$voucher['order_id'] = $db->func_escape_string($order_id);
		$voucher['reference'] = $db->func_escape_string($reference);
		$voucher['method']=$db->func_escape_string($method);
		$voucher['amount'] = (float)$amount;
		$voucher['fee'] = (float)$payment_fee;
		$voucher['net_amount'] = (float)$net_amount;
		$voucher['date_added'] = $date;
		$voucher['user_id'] = (int)$_SESSION['user_id'];
		return $db->func_array2insert("inv_vouchers",$voucher);



	}
	function map_payment_method($method,$ref='')
	{
		$return = '';
		switch($method)
		{
		...
		if(trim($ref))
		{
			if($method=='paypal' || $method=='payflow')
			{
				$return = '<a href="javascript:void(0)" data-tooltip="'.$return.'">'.$ref.'</a> ';
			}
