...
function addToRejectedShipment($product_sku, $qty, $shipment_id,$reason) {
	global $db;
	...
	if (! $last_id) {
		...
		$rejcetedShipment ['package_number'] = 'RJ-' . rand();
...
function getAvgProductCost($sku, $date = '')
{
	...
	$rows = $db->func_query("SELECT DISTINCT  DATE(cost_date) as cost_date,raw_cost,ex_rate,shipping_fee FROM inv_product_costs WHERE sku='".$sku."' $date ORDER BY id DESC limit 3");
	$avg = 0.00;
	$i=0;
	foreach($rows as $row)
	{
...
function whiteList($order, $hold = 0, $array = 0) {
	...
	if ($check >= 3 && $hold == 1 && $order['first_order_verification'] == 1 && $order['order_status'] == 'On Hold') {
	...
	if ($check < 3 && $hold == 1 && $order['first_order_verification'] == 0) {
...
function addBoxMoveLog ($cID, $nID, $iID) {
 ...
 function moveItemToLBB ($reject_id, $lbbBoxId,$oemQty, $oemPrice, $nonOemQty, $nonOemPrice) {
...
function printLabel ($id, $sku, $box_number, $reason, $order_id, $printer, $source, $font = 12) {
	$fontid = 12;
	if (strlen($id) > 25) {
		$fontid = 10;
	}
	...
	$body .= ($sku)?'<tr><td class="heading">'. $sku .'<sup>'. $source .'</sup></td></tr>': '';
