...
class Database{
	...
	public function select_db($db){
		@mysqli_select_db($this->conn , $db) || die("Could not connect to SQL db - " . mysql_error());
	}
	...
	public function db_query($query) {
		...
		$result = mysqli_query($this->conn , $query) or die(self::db_error($query));
		return $result;
	}
	...
	private function db_error($query) {
		$mysql_error = mysqli_errno($this->conn)." : ".mysqli_error($this->conn);
		// $msg  = "Site        : ".$_SERVER['HTTP_HOST']."<br />";
		// $msg .= "Remote IP   : ".$_SERVER['REMOTE_ADDR']."<br />";
		$msg="URL: ".$_SERVER['REQUEST_URI']."<br />";
		$msg .= "SQL query   : $query<br />";
		// $msg .= "Error code  : ".mysqli_errno($this->conn)."<br/>";
		$msg .= "Description : ".mysqli_error($this->conn);

		mail("xaman.riaz@gmail.com","IMP db error","$query, $mysql_error, $msg");
	...
	public function func_array2insert ($tbl, $arr, $is_replace = false) {
		if($tbl=='inv_orders')
		{
			//echo ($is_replace?"REPLACE":"INSERT")." INTO $tbl (" . implode(", ", array_keys($arr)) . ") VALUES ('" . implode("', '", $arr) . "')";exit;
		}
	...
	public function func_escape_string($str){
