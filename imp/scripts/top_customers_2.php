...
if(isset($_GET['agent']))
{
	$agent = trim(strtolower($_GET['agent']));
}
...
$filename = 'top_ordering_customer_2-'.$page.'.csv';
$fp = fopen($filename, "w");
...
$rows = $db->func_query("SELECT count(*) as count,lower(trim(email)) as email,sum(order_price) as order_price FROM inv_orders  where date(order_date) between '".$first."' and '".$second."'  and lower(order_status) in ('processed','shipped') and store_type not like '%amazon%' GROUP by email having sum(order_price)>=500  order by 3 desc limit ".$limit_start.", ".$total);
foreach($rows as $row)
{
	...
	if($agent)
	{
		if($agent!=trim(strtolower($customer_data['name'])))
		{
			continue;
		}
	}
	...
	foreach($months as $month)
	{
		$order_data =  $db->func_query_first("SELECT sum(order_price) as order_price FROM inv_orders  where month(order_date)='".$month['month']."' and year(order_date)='".$month['year']."' and lower(trim(email))='".$row['email']."'  and lower(order_status) in ('processed','shipped') and store_type not like '%amazon%' ");
		$rowData[] = (float)$order_data['order_price'];
	}
