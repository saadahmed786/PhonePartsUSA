	...
	$_order_id = $_POST['order_id'];
				...
				foreach ($codes as $code) {
					...
					if ($orderTotal > 0) {
						...
						$vouch_id = addVoucher($_order_id,'store_credit',$used*(-1),linkToVoucher($voucher['voucher_id'],'', $voucher['code']));

						// Accounting Voucher

					$accounts = array();
					$accounts['description'] = $voucher['code'].' Applied @ Order # '.$_order_id;
					$accounts['credit'] = 0.00;
					$accounts['debit'] = ($used<0?$used*(-1):$used);
					$accounts['order_id'] = $_order_id;
					$accounts['customer_email'] = $voucher['to_email'];
					...
					$accounts['description'] = $voucher['code'].' Applied @ Order # '.$_order_id;
					$accounts['debit'] = 0.00;
					$accounts['credit'] = ($used<0?$used*(-1):$used);
					$accounts['order_id'] = $_order_id;
					$accounts['customer_email'] = $voucher['to_email'];
					$accounts['type']='sales';
					...
					add_accounting_voucher($accounts); // store credit applied
						...
						// End accounting voucher




						$db->db_exec("UPDATE inv_vouchers SET voucher_id='".(int)$voucher['voucher_id']."',description='Credit Usage' where id='".$vouch_id."'");
...
if(isset($_GET['action']) && $_GET['action']=='removeVoucher')
{
	...
	$voucher_data = $db->func_query_first_cell("SELECT * FROM oc_voucher where voucher_id='".$check['voucher_id']."'");
	...
	$vouch_id = addVoucher($_GET['order'],'store_credit',$check['amount']*(-1),'<a href="vouchers_create.php?edit='.$check['voucher_id'].'">'.$db->func_query_first_cell("SELECT code FROM oc_voucher WHERE voucher_id='".$check['voucher_id']."'").'</a>');

	$db->db_exec("UPDATE inv_vouchers SET voucher_id='".(int)$check['voucher_id']."',description='Credit Removed' where id='".$vouch_id."'");


	// Accounting Voucher

					$accounts = array();
					$accounts['description'] = $voucher_data['code'].' Removed from Order #'.$_GET['order'];
					$accounts['debit'] = 0.00;
					$accounts['credit'] = ($check['amount']<0?$check['amount']*(-1):$check['amount']);
					$accounts['order_id'] = $_GET['order'];
					$accounts['customer_email'] = $voucher_data['to_email'];
					$accounts['type']='store_credit';
					$accounts['contra_account_code'] = 'sales';
					$accounts['date_added'] = date('Y-m-d H:i:s');
					...
					$accounts['description'] = $voucher_data['code'].' Removed from Order #'.$_GET['order'];
					$accounts['credit'] = 0.00;
					$accounts['debit'] = ($check['amount']<0?$check['amount']*(-1):$check['amount']);
					$accounts['order_id'] = $_GET['order'];
					$accounts['customer_email'] = $voucher_data['to_email'];
					$accounts['type']='sales';
					$accounts['contra_account_code'] = 'store_credit';
...
if(strtolower($_POST['order_status'])=='shipped')
{
	$db->db_exec("UPDATE inv_orders SET ship_date='".date('Y-m-d H:i:s')."' WHERE order_id='".$orderID."'");
}
...
if(isset($_GET['debug']))
{
	...
	$_tax = $order['tax'];
	$total_vouchers = $db->func_query_first_cell('SELECT SUM(`a`.`amount`) AS `used` FROM `oc_voucher_history` as a, `oc_voucher` as b WHERE a.`voucher_id` = b.`voucher_id` AND ' . (($order['store_type'] == 'web')? 'cast(a.order_id as char(50))': 'cast(a.order_id as char(50))') . ' = "'. $order['order_id'] .'"');
...
{
	$total_vouchers_used+=$voucher['used'];
}
		...
		<div align="center" style="clear:both">
			...
			<form method="post" action="" id="xfrm">
					...
					// echo "SELECT a.name FROM inv_users a,inv_customers b where a.id=b.user_id and trim(b.email)='".trim($order['email'])."'";
						...
						if ($sales_agent) { ?>
<h1 style="font-size:12px;color:#0059a0">Agent: <?= strtoupper($sales_agent); ?></h1>
						<?php } ?>
						<?php
						...
						{
							?>
							<h2 style="font-size:12px;color:#0059a0"><?php echo $inv_status;?></h2>
							<?php
						}
						$_deposit_id = array();
						$is_deposited = false;
						if($order['deposit_id']!=0)
						{
							$is_deposited = true;
							$checks = $db->func_query_first_cell("SELECT deposit_id from inv_transactions where order_id='".$order['order_id']."'");
							if($checks)
							{
								$is_deposited = true; 
							}
						...
						if(strtolower($order['order_status'])!='canceled' && $_SESSION['credit_management_report']==1)
						{
						
							?>
							<!-- <h2 style="font-size:12px;color:red">(<?php echo ($is_deposited?'Deposited':'Undeposited');?>)</h2> -->
							<?php
						}
						?>
				...
				<table cellpadding="10" style="border:1px solid #ddd;border-collapse: collapse;" cellspacing="0" width="70%" border="1">
					...
					<tr>
						<th class="no_display">Sub Store Type </th>
						<td class="no_display"> <?= $order['sub_store_type'] ?> </td>
					...
					<tr>
						...
						<td>
							<?php
								...
								$refunded_amount = $db->func_query_first_cell("SELECT SUM(amount) from inv_transactions where order_id='".$orderID."' and amount<0");
								$refunded_amount = 0.00;
							if ($refunded_amount < 0) {
								      	$refunded_amount = $refunded_amount * -1;
								      }
							// $amountDue = ($order_total+$total_vouchers_used) - $order['paid_price'] - ($refunded_amount);
								      // echo $total_vouchers_used;exit;
								      if(isset($_GET['debug']))
								      {
								      	// var_dump($voucher_issued_amount.'aaa');
								      }
								      $amountDue = round(($order['sub_total']+$order['tax']+$order['shipping_amount']+$business_fee)  - ($order['paid_price']+$total_vouchers_used-$total_vouchers_issued ),2);

								     
								      
							     if (round($order_total - $refunded_amount - $order['refunded_amount'],2) == 0.00 || $free_checkout == 'Free Checkout') {
								      	$amountDue = 0;	
								      }
								      $_payments = array();
								      if($new_module)
								      {
								      	$store_credit_paid = $db->func_query_first_cell("SELECT SUM(amount) from inv_vouchers where order_id='".$db->func_escape_string($order['order_id'])."' and method='store_credit'");
								      	$amount_paid = $db->func_query_first_cell("SELECT SUM(amount) from inv_vouchers where order_id='".$db->func_escape_string($order['order_id'])."' and method<>'store_credit'");
								      	
								      	$_payments = $db->func_query("SELECT * from inv_vouchers where order_id='".$db->func_escape_string($order['order_id'])."' and amount<>0.00");

								      	$amountDue = (($order['sub_total']+$order['tax']+$order['shipping_amount']+$business_fee) + (float)$store_credit_paid ) - $amount_paid;

								      	if(isset($_GET['debug']))
								      	{
								      		echo $amount_paid.'-'.$store_credit_paid;
								      	}
								      	// echo $amountDue;exit;
								      }
									...
									// if((strtolower($order['payment_method'])=='check' || strtolower($order['payment_method'])=='behalf' || strtolower($order['payment_method'])=='wire transfer' || strtolower(substr($order['payment_method'], 0,4))=='cash') && $order['gross_amount']==0.00 )

								      ?>

								      <?php
								      $is_deposited_check = false;

								      // if(($amountDue>0.00 && strtolower($order['order_status'])!='canceled') || ($order['payment_method']=='Cash or Credit at Store Pick-Up' && $order['order_status']=='Processed') )
									if($amountDue>0)
									{
										$is_deposited_check = true;
									?>
									...
									<a href="<?php echo $host_path;?>popupfiles/make_payment_order_new.php?order=<?php echo $order['order_id']; ?>" class="fancybox2 fancybox.iframe button button-danger" >Make Payments</a>
										<?php
									}
									else
									{
									...
									<a href="<?php echo $host_path;?>popupfiles/make_payment_order.php?order=<?php echo $order['order_id']; ?>" class="fancybox4 fancybox.iframe button button-danger" >Make Payment</a>
										<?php
									}
									?>
								...
								else
								{
									...
									echo '* '.($order['payment_source']=='Payflow'?$order['payment_source']:$order['payment_method']).' ';
									}
								}
					...
					//{
						if(!$transaction_dets)
						{
							$transaction_dets = $db->func_query("SELECT transaction_id,amount,deposit_id,net_amount FROM inv_transactions WHERE order_id = '".$orderID."'");
...
if(!$transaction_dets)
						...
						if($transaction_dets)
						{
							...
							foreach($transaction_dets as $transaction_det)
							{
								...
								$transaction_id .= $transaction_det['transaction_id'].' ($'. (($transaction_det['amount'])? number_format($transaction_det['amount'],2): number_format($order['paid_price'],2)).')';


								if($_SESSION['credit_management_report'])
								{

								if($transaction_det['deposit_id'])
								{
										$transaction_id.= ' <strong> (<a href="'.$host_path.'popupfiles/deposit_transactions.php?deposit_id='.$transaction_det['deposit_id'].'&deposit_type=paypal" target="_blank">'.$db->func_query_first_cell("SELECT name from inv_deposits where deposit_id='".(int)$transaction_det['deposit_id']."'").'</a>)</strong><br>';
								}
								else
								{
									$transaction_id.= ' <strong>(Undeposited)</strong><br>';
								}
							}
						//print_r($transaction_det);
							...
							if($new_module)
							{
								...
								{
									echo $z.') '.map_payment_method($_payment['method'],$_payment['reference']).' ($'.number_format($_payment['amount'],2).') (Undeposited)<br>';
									$z++;
								}
							}
					//}
						...
						?>
							...
							if(!$new_module)
							{
								if($is_deposited_check==false )
								{
									if($_SESSION['credit_management_report']==1)
									{
										if(strtolower(substr($order['payment_method'],0, 4))!='cash' && $cash_paid>0)
										{
											echo '<br>* Cash ';
										}

										if(strtolower(substr($order['payment_method'],0, 4))!='card' && $card_paid>0)
										{
											echo '<br>* Card ';
										}
									...
									if($order['deposit_id']>0)
									{
										...
										echo ' ('.'$'.number_format($order['net_amount'],2).') <strong>(<a href="'.$host_path.'popupfiles/deposit_transactions.php?deposit_id='.$order['deposit_id'].'&deposit_type=cash" target="_blank">'.$db->func_query_first_cell("SELECT name from inv_deposits where deposit_id='".(int)$order['deposit_id']."'").'</a>)</strong><br>';
									}
									else
									{
									echo " <strong>(Undeposited)</strong><br>";
									}
									}
								}
							}
					...
					?>
						...
						<?php if ($is_po || $order['store_type'] == 'web'):?>
							...
							<tr>
								<th class="no_display">Payment Source :</th>
								<td class="no_display">
									<br>	
								...
								<?php if ($is_po): ?>
									...
									?>
								<th>PO # :</th>
								<td colspan="3"><input type="text" name="po_order_number" id="po_order_number" value="<?php echo $order['po_order_number']; ?>" /></td>
								<?php endif; ?>
							</tr>
				...
				<div align="left">
					...
					if($_SESSION['can_lock_price'])
									...
									<strong style="position: absolute;margin-left:110px;margin-top:10px">Lock Prices </strong><label class="switch" style="margin-left:190px"><input type="checkbox" <?php echo ($order['lock_prices']==1?'checked':'');?> name="lock_prices"><div class="slider round"><!--ADDED HTML --><span class="on">ON</span><span class="off">OFF</span><!--END--></div></label>
				...
				<table align="center" border="1" cellspacing="0" style="border:1px solid #ddd;border-collapse:collapse;" cellpadding="5" width="95%" >
					<thead>
		...
		success: function(json) {
			...
			if (json['success']) {
				...
				parent.$.fancybox.close();
					...
					{
						if(!confirm('Are you sure want to update the Payment method?'))
						{
							return false;
						}
				...
				<?php }?>
					...
					<tr>
						...
						<td>$<?= number_format($_tax, 2) ?>
								<?php
								if( $_tax>0 && $_SESSION['login_as']=='admin')
								{
									?>
									<img src="images/cross.png" style="cursor:pointer;width:9px" title="Refund Tax" onClick="refundTax();" >
									<?php
								}
								?>
					...
					<?php if ($_SESSION['login_as'] == 'admin'): ?>
						...
						<?php
						if($order['transaction_fee']>0 or $order['payment_fee'])
						{
							?>
							<tr>
								<td align="right" style="color:blue">Transaction Fee:</td>
								<td style="color:blue">-$<?= number_format(($order['transaction_fee']?$order['transaction_fee']:$order['payment_fee']), 2) ?></td>
							</tr>
							<?php
						}
						?>
					...
					<tr>
							...
							<td>$<?= number_format($order['sub_total']+$order['tax']+$order['shipping_amount']+$business_fee+$voucher_issued_amount, 2); ?></td>
						</tr>

					<?php
					if(strpos(strtolower($order['payment_method']), 'cash') !== false && strtolower($order['order_status'])=='shipped' && $order['payment_method']!= 'Cash On Delivery'  && strtolower($order['store_type'])!='po_business' )
					{
						// echo 'herezaman';exit;
			...
			// $order['paid_price'] = $order_total;
					...
					if($transaction_dets)
					{
							...
							if($transaction_det['deposit_id'])
							 {
							 	...
							 	$paid_html.='<tr class="tr_total_paid" style="display:none;font-weight:normal;color:green">
							 <td align="right">(<a href="'.$host_path.'popupfiles/deposit_transactions.php?deposit_id='.$transaction_det['deposit_id'].'">'.$db->func_query_first_cell("SELECT name from inv_deposits where deposit_id='".(int)$transaction_det['deposit_id']."'").'</a>)</td>
							 	<td>$'.number_format($transaction_det['net_amount'],2).'</td>
							 	</tr>';
						...
						$paid_amount = (float)$paid_amount + (float)$cash_paid ;
						 }
						 if ($card_paid != 0.0000){
						$paid_html.='<tr class="tr_total_paid" style="display:none;font-weight: normal;">
							<td style="color: green;" align="right" colspan="2">
						...
						$paid_amount = (float)$paid_amount + (float)$card_paid ;
						}
						 if ($change_due != 0.0000){
						$paid_html.='<tr class="tr_total_paid" style="display:none;font-weight: normal;">
						...
						$paid_amount = (float)$paid_amount - (float)$change_due ;
						}
							 ...
							 ?>
								...
								<td>$<?php echo number_format(($paid_amount+$total_vouchers_used)-($refunded_amount+$order['refunded_amount']),2);?></td>
								<!-- <td>$<?php echo number_format($db->func_query_first_cell("SELECT SUM(amount) from inv_vouchers where order_id='".$order['order_id']."'"),2);?></td> -->
							 </tr>
							 ...
							 <?php $order['paid_price'] = ($paid_amount-$refunded_amount-$order['refunded_amount']);?>
							...
							// echo $order['deposit_id'].'zaman';exit;
							// print_r($order);exit;
						 ?>
						<tr style="cursor: pointer;background-color: #dfd"  onclick="$('.tr_total_paid').toggle()">
							<td align="right">
						...
						?>
						 ...
						 <?php
							 ...
							 if($order['deposit_id'])
							 {
							 	...
							 	?>
							 	<tr class="tr_total_paid" style="display:none;font-weight:normal">
							 	<td align="right">Deposit # <a href="<?php echo $host_path;?>popupfiles/deposit_transactions.php?deposit_id=<?php echo $order['deposit_id'];?>&deposit_type=cash"><?php echo $db->func_query_first_cell("SELECT name from inv_deposits where deposit_id='".(int)$order['deposit_id']."'");?></a></td>
							 	<td>$<?php echo number_format($order['net_amount'],2);?></td>
							 	</tr>
							 	<?php
							 }
							 ?>
					...
					else
					{
						...
						$order['paid_price'] = (float)$db->func_query_first_cell("SELECT SUM(amount) from inv_vouchers where order_id='".$order['order_id']."' and method<>'store_credit'");

						$order['paid_price']+= (float)$db->func_query_first_cell("SELECT SUM(amount*(-1)) from inv_vouchers where order_id='".$order['order_id']."' and method='store_credit' and amount<0.00"); 
						?>
						<tr style="cursor: pointer;background-color: #dfd"  onclick="$('.tr_total_paid').toggle()">
							<td align="right" >
								Total Paid:
							...
							<td>$<?php echo number_format($order['paid_price'], 2); ?></td>
						</tr>
						<?php
						...
						foreach($_payments as $_payment)
						{
								...
								if($_payment['method']=='store_credit')
								{
									continue;
								}
						?>
						<tr class="tr_total_paid" style="display:none">
						<td align="right"><?php echo map_payment_method($_payment['method'],$_payment['reference']);?>
							<?php
							// if($_payment['method']=='store_credit')
							// {
							// 	echo ($_SESSION['vouchers_update']==1?' <a href="'.$host_path.'viewOrderDetail.php?order='.$_GET['order'].'&action=removeVoucher&voucher_history_id='.$_payment['voucher_history_id'].'"><img src="images/cross.png" title="Remove Voucher" /></a>';
							// }
							?>

						</td>
						<td>$<?php echo number_format($_payment['amount'],2);?></td>
						</tr>
						<?php
					}
					...
					foreach ($vouchers as $key => $voucher) { 
						...
						<tr>
							...
							<?php 
							// disabled amount due as calculated above
							/*
							$free_checkout = $db->func_query_first_cell('SELECT payment_method from oc_order where order_id = "'. $orderID .'"');
							if ($refunded_amount < 0) {
								      	$refunded_amount = $refunded_amount * -1;
							     ...
							     if (round($order_total - $refunded_amount - $order['refunded_amount'],2) == 0.00 || $free_checkout == 'Free Checkout') {
								      ...
								      */
							?>
							<td>$<?= number_format(round($amountDue, 2), 2); ?><input type="hidden" value="<?php echo $amountDue; ?>" name="amount_due" />
						</td>
						</tr>
				</table>
			</td>
...
function refundTax()
{
  ...
  //    'href' : '#refund_tax',
  //    'content':'<h2>Refund Tax</h2><table style="width:400px" cellpadding="5" cellspacing="5" border="0"><tr><td>Tax ID:</td><td><input type="text"></td></tr><tr><td colspan="2" align="center"><input type="button" value="Refund"></td></tr></table>'
  // });

  $('#remove_btn_selected').attr('href', '<?= $linkToPay; ?>' + rmProducts+ '&reasons='+rmReason+'&remove_tax=1');
									$('#remove_btn_selected').click();
}
