...
if ($_POST['QCUpdate'] || $_POST['QcComplete']) {
    ...
    //now add rejected sku to shipments ANd update Product Qty
		if ($_POST['QcComplete']) {
			...
			if ($qc_shipment['rejected'] > 0) {
				...
				addToRejectedShipment($product['product_sku'], $product['rejected'], $shipment_id, $product['rejected_reason'],($product['product_price']+$per_item_shipping_cost));
			...
			if ($product['product_sku'] && $received_qty > 0) {
				...
				$productArray = array('quantity' => ($received_qty + $db->func_query_first_cell("SELECT quantity FROM oc_product WHERE model = '$p_sku'")));
				$db->func_array2update("oc_product", $productArray, " sku = '". $p_sku ."'");
			}

			if ($product['grade_a'] && $product['grade_a_qty'] > 0) {
				$p_sku = $product['grade_a'];
				$productArray = array('quantity' => ($product['grade_a_qty'] + $db->func_query_first_cell("SELECT quantity FROM oc_product WHERE model = '$p_sku'")));
				$db->func_array2update("oc_product", $productArray, " sku = '". $p_sku ."'");
			}

			if ($product['grade_b'] && $product['grade_b_qty'] > 0) {
				$p_sku = $product['grade_b'];
				$productArray = array('quantity' => ($product['grade_b_qty'] + $db->func_query_first_cell("SELECT quantity FROM oc_product WHERE model = '$p_sku'")));
				$db->func_array2update("oc_product", $productArray, " sku = '". $p_sku ."'");
			}

			if ($product['grade_c'] && $product['grade_c_qty'] > 0) {
				$p_sku = $product['grade_c'];
				$productArray = array('quantity' => ($product['grade_c_qty'] + $db->func_query_first_cell("SELECT quantity FROM oc_product WHERE model = '$p_sku'")));
				$db->func_array2update("oc_product", $productArray, " sku = '". $p_sku ."'");
			}

			if ($product['grade_d'] && $product['grade_d_qty'] > 0) {
				$p_sku = $product['grade_d'];
				$productArray = array('quantity' => ($product['grade_d_qty'] + $db->func_query_first_cell("SELECT quantity FROM oc_product WHERE model = '$p_sku'")));
				$db->func_array2update("oc_product", $productArray, " sku = '". $p_sku ."'");
			}
  ...
  // logRejectItem(count($_POST['products']),' items added from Shipment <a href="'.$host_path.'view_shipment.php?shipment_id='.$shipment_id.'">' . $to . '</a> by ','',$to);
    ...
    //save shipment cost
        ...
        // echo 'Current Cost: '.$current_cost.'<br>';

        // echo 'New Qty: '.$new_qty.'<br>';

        // echo 'New Cost: '.$new_cost.'<br><br>===============================<br>';
