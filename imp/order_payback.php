...
if ($_POST['order_id'] ) {
    ...
    if (!$_POST['reimb']) {
        ...
        $vouch_id = addVoucher($order_id,'store_credit',($_POST['amount']),linkToVoucher($id,'', $vouch['code']));
        $db->db_exec("UPDATE inv_vouchers SET voucher_id='".(int)$id."',description='Credit Issued' where id='".$vouch_id."'");


                    $accounts = array();
                    $accounts['description'] = $vouch['code'].' Issued against Order #'.$order_id;
                    $accounts['credit'] = 0.00;
                    $accounts['debit'] = $_POST['amount'];
                    $accounts['order_id'] = $order_id;
                    $accounts['customer_email'] = $vouch['to_email'];
                    if($remove_tax)
                    {
                        $accounts['type']='tax';
        ...
        $db->db_exec("UPDATE inv_orders SET voucher_issued_amount=voucher_issued_amount+".(float)$_POST['amount']." where order_id='".$order_id."' ");

        if($remove_tax)
        {
            $db->db_exec("UPDATE inv_orders SET refund_tax='".(float)$_POST['amount']."',refund_tax_type='store_credit',refund_tax_date='".date('Y-m-d H:i:s')."' where order_id='".$order_id."' ");            
        }
     ...
     if($remove_tax)
            {
                $db->db_exec("UPDATE inv_customers SET tax_id='".$db->func_escape_string($_POST['tax_id'])."',dis_tax=1 where lower(email)='".strtolower($order_details['email'])."'");

                $db->db_exec("UPDATE inv_orders SET tax=0.00 where order_id='".$order_id."'");
                orderTotal($order_id, true);

                // $vouch_id = addVoucher($order_id,'store_credit',($_POST['amount']*(-1)),linkToVoucher($id,'', $vouch['code']));
        // $db->db_exec("UPDATE inv_vouchers SET voucher_id='".(int)$id."',description='Credit Issued' where id='".$vouch_id."'");
        ...
        <div align="center">
            <form action="" id="myFrm" method="post">
                ...
                <table align="center" border="1" cellpadding="10" cellspacing="0" style="border:1px solid #585858;border-collapse:collapse;width:90%">
                    ...
                    <tr style="<?php echo ($remove_tax?'display:none':'');?>">
                        <td>Item(s)</td>
        ...
        var reimbursement = 0;
                   ...
                   // alert($('input[name=method]').val());return false;
                    ...
                    $.ajax({
                        ...
                        url: url,
                        type: "POST",
                        data: {itemrem:'<?php echo (int)$item_remove;?>',order_id: '<?php echo $order_info['order_id']; ?>', refundShipping: refundShipping, reason: $('#reason').val(), items: '<?php echo $items; ?>', amount: $('input[name=price]').val(), reimb: reimbursement,voucher_items_reasons: '<?php echo $voucher_items_reasons; ?>', title: $('input[name="title"]').val(), subject: $('input[name="subject"]').val(), canned_id: $('select[name="canned_id"]').val(), comment: CKEDITOR.instances.comment.getData(),remove_tax:'<?php echo (int)$remove_tax;?>',tax_id:$('input[name=tax_id]').val()},
                        dataType: "json",
                        beforeSend: function () {
