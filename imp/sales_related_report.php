...
if(isset($_POST['action']) && $_POST['action']=='load_comment')
{
  ...
  $rows = $db->func_query("SELECT customer_name,email from inv_orders where lower(order_status) in ('processed','shipped','completed') and email not like '%@marketplace.amazon%' and month(order_date)='".$month."' and year(order_date)='".$year."' group by email having sum(sub_total+shipping_amount+tax)>'".$deno."' and sum(sub_total+shipping_amount+tax)<='".$deno_end."'");
...
loadData('monthly_report');
  ...
  {
    $('.load-comments-'+k+i).show();

    if($('.load-comments-'+k+i).html()!='')
    {
      return false;
    }
     $.ajax({
        ...
        beforeSend: function() {
          ...
          $('.load-comments-'+k+i).html('<img src="images/loading.gif" height"16" width="16" />');
          // $('#'+type).html('<img src="images/loading.gif" height"100" width="100" />');
        },  
        complete: function() {
        },      
