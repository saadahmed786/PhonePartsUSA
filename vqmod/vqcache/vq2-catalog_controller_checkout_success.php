...
class ControllerCheckoutSuccess extends Controller { 
	public function index() { 
		...
		// print_r($this->data['orderDetails']);exit;
			...
			$order_check = $this->db->query("SELECT * FROM ".DB_PREFIX."order WHERE trim(lower(email))='".trim(strtolower($orderDetails['email']))."' ");
			if($order_check==1)
			{
				$this->load->model('account/customer');
				$business_type_check = $this->db->query("SELECT source FROM ".DB_PREFIX."customer_source WHERE trim(lower(email))='".trim(strtolower($orderDetails['email']))."' and `type`='business_type'");
					$business_source_check = $this->db->query("SELECT source FROM ".DB_PREFIX."customer_source WHERE trim(lower(email))='".trim(strtolower($orderDetails['email']))."' and `type`='hear'");

						$business_type = $business_type_check->row['source'];
						$business_source = $business_source_check->row['source'];
				$data = array();
			$data['lead_source_id'] = '31845';
			$data['time_zone'] ='Pacific Time';
			$data['owner_id'] = '21225';
			$data['work_number'] = '+1'.$OrderDetails['telephone'];
			$data['first_name'] = ucfirst($OrderDetails['firstname']);
			$data['last_name'] = ucfirst($OrderDetails['lastname']);
			$data['email'] = strtolower($OrderDetails['email']);
			$data['company']['name']  = $OrderDetails['shipping_company'];
			$data['custom_field']['cf_business_type'] = $business_type;
			$data['custom_field']['cf_how_did_you_hear_about_us'] = $business_source;
			$this->model_account_customer->addFSLead($data);	
			}
