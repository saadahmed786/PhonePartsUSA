...
// Create new db if needed for authnet admin tools
	...
	// print_r($_SESSION);exit;		
		...
		$this->data['main_banner2'] = $this->model_tool_image->resize('new_site/home/ces-2019-web-banner.jpg', 960, 391); // main banner2
		// $this->data['main_banner2'] = 'https://phonepartsusa.com/image/new_site/home/ces-2019-web-banner.jpg';
		...
		//$allProducts = (array_key_exists('shop', $this->request->get) && ($this->request->get['shop'] == 'shop'));
			...
			$this->data['banner_overlay']='';
			if($this->customer->isLogged())
			{
				$last_order = $this->db->query("SELECT a.order_id,b.tracking_number from ".DB_PREFIX."order a,inv_shipstation_transactions b where a.order_id=b.order_id and a.order_status_id=3 and b.voided=0 and a.shipping_code='multiflatrate.multiflatrate_0'  and a.customer_id='".$this->customer->getId()."' order by a.date_added desc limit 1");

				$order_details = $last_order->row;

				$last_tracking = $this->db->query("SELECT b.status from inv_tracker a,inv_tracker_status b where a.tracker_id=b.tracker_id and a.tracking_code=''".$order_details['tracking_number']."' order by b.id desc limit 1");
				$_last = $last_tracking->row;
				if($this->customer->getEmail()=='phonepartusa@gmail.com')
				{
				if($_last=='pre_transit' or $_last=='in_transit')
				{
					$this->data['banner_overlay']='Order # '.$order_details['order_id'].' is not yet delivered';
				}
			}
			}
