...
$this->data['text_your_address_billing'] = $this->language->get('text_your_address_billing');
		...
		if (file_exists(DIR_TEMPLATE . (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/checkout/guest.tpl')) {
			$this->template = (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/checkout/guest.tpl';
		} else {
	...
	public function validate() {
		...
		if (!$json) {
			...
			$check = $this->db->query("SELECT email from oc_customer_source where email='".$this->db->escape($this->request->post['email'])."'");
			...
			$_data = array();
			$_data['lead_source_id'] = '31845';
			$_data['time_zone'] ='Pacific Time';
			$_data['owner_id'] = '4067';
			$_data['work_number'] = '+1'.$this->request->post['telephone'];
			$_data['first_name'] = ucfirst($this->request->post['firstname']);
			$_data['last_name'] = ucfirst($this->request->post['lastname']);
			$_data['email'] = strtolower($this->request->post['email']);
			$_data['company']['name']  = $this->request->post['company'];
			$_data['custom_field']['cf_business_type'] = $this->request->post['business_type'];
			$_data['custom_field']['cf_how_did_you_hear_about_us'] = $this->request->post['source'];
			$this->model_account_customer->addFSLead($_data);
			}
			...
			$this->db->query("INSERT INTO inv_customers SET firstname='".$this->db->escape($this->request->post['firstname'])."',lastname='".$this->db->escape(($this->request->post['lastname']))."',telephone='".$this->db->escape($this->request->post['telephone'])."',email='".$this->db->escape(strtolower($this->request->post['email']))."',customer_group='Default',no_of_orders=0,total_amount=0.00,last_order='0000-00-00 00:00:00',customer_id='0',date_added=NOW(),is_business_customer=0,company='".$this->db->escape($this->request->post['company'])."'");
			}
			...
			$this->session->data['guest']['telephone_2'] = $this->request->post['telephone_2'];
    ...
    public function guestInfoCheckout(){
