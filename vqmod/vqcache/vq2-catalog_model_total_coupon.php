		...
		if (isset($this->session->data['coupon'])) {
			...
			if ($coupon_info) {
				...
				foreach ($this->cart->getProducts() as $product) {
					...
					if($coupon_info['filter_discount']) {
						if($product['discount_type'] == 1) {
							$status = false;
						}
					}
					
					if($coupon_info['filter_special']) {
					...
					if(!empty($coupon_info['category_products'])) {
						if($coupon_info['filter_category']) {
							if(in_array($product['product_id'], $coupon_info['category_products'])) $status = false;
						} else {
							if(!in_array($product['product_id'], $coupon_info['category_products'])) $status = false;
						}
					}
					...
					if(!empty($coupon_info['manufacturer_products'])) {
						if($coupon_info['filter_manufacturer']) {
							if(in_array($product['product_id'], $coupon_info['manufacturer_products'])) $status = false;
						} else {
							if(!in_array($product['product_id'], $coupon_info['manufacturer_products'])) $status = false;
						}
					}
				...
				if($coupon_info['type'] == 'P' && $coupon_info['maximum_amount']>0 && $discount_total>$coupon_info['maximum_amount'])
							{
								$discount_total = $coupon_info['maximum_amount'];
							}				
