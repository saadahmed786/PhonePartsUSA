...
class ModelAccountOrder extends Model {
	...
	public function getOrder($order_id) {
		$order_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order` WHERE order_id = '" . (int)$order_id . "' AND LOWER(email) = LOWER('" . $this->customer->getEmail() . "') AND order_status_id > '0'");
		...
		if ($order_query->num_rows) {
			...
			$transaction = $this->db->query("SELECT * FROM `inv_order_shipments` WHERE order_id = '" . (int)$order_id . "'");
	$shipping_cost = $this->db->query("SELECT `shipping_cost` from `inv_orders_details` WHERE order_id = '" . (int)$order_id . "'");
			...
			return array(
				...
				'shipping_cost'         	=> $shipping_cost->row['shipping_cost'],
				...
				'tracking_id'             => $transaction->row['tracking_number'],
				'tracking_carrier'		  	=> $transaction->row['service_code']
			);
		} else {
			return false;	
		}
	...
	public function getOrderAll($order_id) {
		$order_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order` WHERE order_id = '" . $order_id . "'  AND order_status_id > '0'");
	...
	public function getOrders($start = 0, $limit = 20) {
		...
		// $query = $this->db->query("SELECT o.order_id, o.firstname, o.lastname, os.name as status, o.shipping_address_1, o.shipping_zone, o.shipping_country, o.shipping_city, o.shipping_postcode, o.date_added, o.total, o.currency_code, o.currency_value, ios.tracking_number,ios.service_code FROM `" . DB_PREFIX . "order` o LEFT JOIN " . DB_PREFIX . "order_status os ON (o.order_status_id = os.order_status_id) LEFT JOIN inv_order_shipments ios ON (o.order_id = ios.order_id) WHERE LOWER(o.email) = LOWER('" . $this->customer->getEmail() . "') AND o.order_status_id > '0' AND os.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY o.order_id DESC LIMIT " . (int)$start . "," . (int)$limit);

		$query = $this->db->query("SELECT o.order_id, o.firstname, o.lastname, (select name from ".DB_PREFIX."order_status os WHERE os.order_status_id=o.order_status_id) as status, o.shipping_address_1, o.shipping_zone, o.shipping_country, o.shipping_city, o.shipping_postcode, o.date_added, o.total, o.currency_code, o.currency_value, (select tracking_number FROM inv_order_shipments ios where ios.order_id=o.order_id and voided=0 limit 1) as tracking_number,(select service_code FROM inv_order_shipments ios where ios.order_id=o.order_id and voided=0 limit 1) as service_code FROM `" . DB_PREFIX . "order` o  WHERE LOWER(o.email) = LOWER('" . $this->customer->getEmail() . "') AND o.order_status_id > '0' ORDER BY o.order_id DESC LIMIT " . (int)$start . "," . (int)$limit);
	
		return $query->rows;
	}

	public function getTracking($order_id) {
		$query = $this->db->query("SELECT * from inv_shipstation_transactions where order_id = '". (int)$order_id ."' ORDER BY voided DESC");
		if(!$query->row)
		{
			$query = $this->db->query("SELECT * FROM inv_label_data WHERE order_id='".(int)$order_id."'");
		}
		$tracking = array();
		$i = 0;
		$results = $query->rows;
		foreach ($results as $shipping) {
				$tracker = $this->db->query("SELECT * FROM inv_tracker WHERE tracking_code='".$shipping['tracking_number']."'");
				$tracking[$i] = $tracker->row;
				$_emp = $this->db->query("SELECT * FROM inv_tracker_status WHERE tracker_id='".$tracker->row['tracker_id']."' order by id desc");
				$tracking[$i]['tracking_info'] = $_emp->rows;
				$tracking[$i]['service_code'] = $shipping['service_code'];
				$i++;
		}
		if($order_id='555711')
		{
	...
	public function getOrderSubTotal($order_id) {
		$query = $this->db->query("SELECT total AS subtotal FROM `" . DB_PREFIX . "order` WHERE order_id = '" . (int)$order_id . "'");
