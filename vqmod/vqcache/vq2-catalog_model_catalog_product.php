...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		$product_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.date_available <= NOW() AND p.status = '1' AND ( (p.quantity > '0') OR (p.quantity <= '0' and p.location <> '1')) AND ( (p.quantity > '0') OR (p.quantity <= '0' and p.location <> '1')) AND p.visibility='1'");
		...
		$query = $this->db->query("SELECT DISTINCT  p.*,pd.*,p2s.*,m.*,p.quantity+p.prefill-((SELECT COALESCE(sum(b.product_qty) - sum(b.picked_quantity),0) FROM inv_orders_items b where b.ostatus in ('processed','on hold') and b.product_sku=p.model) + (SELECT COALESCE(sum(b.picked_quantity) - sum(b.packed_quantity),0) FROM inv_orders_items b where b.ostatus in ('processed','unshipped','on hold') and b.is_picked=1 and b.opacked=0 and b.product_sku=p.model) + (SELECT COALESCE(sum(b.packed_quantity),0) FROM inv_orders_items b where b.ostatus in ('processed','unshipped','on hold') and b.is_packed=1 and b.product_sku=p.model) - (SELECT COALESCE(sum(b.product_qty),0) FROM inv_orders_items b where b.ostatus='on hold' and b.is_picked=0 and b.is_packed=0 and b.product_sku=p.model)) as quantity, pd.name AS name, p.is_main_sku, p.image, p.sale_price, m.name AS manufacturer, (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$customer_group_id . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special, (SELECT points FROM " . DB_PREFIX . "product_reward pr WHERE pr.product_id = p.product_id AND customer_group_id = '" . (int)$customer_group_id . "') AS reward, (SELECT ss.name FROM " . DB_PREFIX . "stock_status ss WHERE ss.stock_status_id = p.stock_status_id AND ss.language_id = '" . (int)$this->config->get('config_language_id') . "') AS stock_status, (SELECT wcd.unit FROM " . DB_PREFIX . "weight_class_description wcd WHERE p.weight_class_id = wcd.weight_class_id AND wcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS weight_class, (SELECT lcd.unit FROM " . DB_PREFIX . "length_class_description lcd WHERE p.length_class_id = lcd.length_class_id AND lcd.language_id = '" . (int)$this->config->get('config_language_id') . "') AS length_class, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, (SELECT COUNT(*) AS total FROM " . DB_PREFIX . "review r2 WHERE r2.product_id = p.product_id AND r2.status = '1' GROUP BY r2.product_id) AS reviews, p.sort_order FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) LEFT JOIN " . DB_PREFIX . "manufacturer m ON (p.manufacturer_id = m.manufacturer_id) WHERE p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND ( (p.quantity > '0') OR (p.quantity <= '0' and p.location <> '1')) AND ( (p.quantity > '0') OR (p.quantity <= '0' and p.location <> '1')) and p.visibility=1 AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");
	...
	public function getProductGrades($main_sku) {
		$query = $this->db->query("SELECT product_id,price,item_grade FROM oc_product WHERE main_sku = '". $this->db->escape($main_sku) ."' AND quantity <> '0' AND status = '1' order by product_id");
		return $query->rows;
	}
	...
	public function getProductQuality($sku) {
		...
		$query = $this->db->query("SELECT f.* FROM inv_device_product AS a, inv_device_manufacturer AS b, inv_device_device AS c, inv_device_model AS d, inv_device_attrib AS e, inv_attr AS f, inv_attribute_group AS g WHERE a.device_product_id = b.device_product_id AND b.device_manufacturer_id = c.device_manufacturer_id AND c.device_device_id = d.device_device_id AND d.device_model_id = e.device_model_id AND e.attrib_id = f.id AND f.attribute_group_id = g.id AND g.name = 'Quality' AND a.sku = '".$this->db->escape($sku)."' GROUP BY e.attrib_id");
...
// echo "SELECT a.`device_product_id` FROM `inv_device_product` `a`, `oc_product` `b`, `oc_product_description` `c`, `inv_device_class` `d`, `inv_classification` `e` WHERE a.`sku` = b.`model` AND b.`product_id` = c.`product_id` AND a.`device_product_id` = d.`device_product_id` AND d.`class_id` = e.`id` AND b.`is_main_sku` = '1' AND b.`status` = '1' AND b.`product_id` ='".$product_id."' ";exit;		
		...
		if($device_product_id)
		{
			...
			$manufacturer_id = $this->db->query("select * from inv_device_manufacturer where device_product_id='".(int)$device_product_id['device_product_id']."'")->row;
			if($manufacturer_id)
			{
				$device_device = $this->db->query("SELECT group_concat( b.device_id ) as device_id FROM inv_device_manufacturer a,inv_device_device b WHERE a.`device_manufacturer_id`=b.`device_manufacturer_id` AND a.`manufacturer_id`='" .(int) $manufacturer_id['manufacturer_id'] . "' AND a.`device_product_id`='" . (int) $device_product_id['device_product_id'] . "'")->row;
		  		...
		  		{
		  			$selected_models = $this->db->query("SELECT group_concat(mo.model_id) as model_ids FROM `inv_device_manufacturer` m INNER JOIN `inv_device_device` d ON (m.`device_manufacturer_id` = d.`device_manufacturer_id`) INNER JOIN `inv_device_model` mo ON (d.`device_device_id` = mo.`device_device_id`) WHERE m.manufacturer_id='".(int)$manufacturer_id['manufacturer_id']."' AND m.device_product_id='".(int)$device_product_id['device_product_id']."' AND d.device_id in (".$device_device['device_id'].")")->row;
	...
	public function getProducts($data = array()) {
		...
		if (!$product_data) {
			...
			$sql .= " WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND ( (p.quantity > '0') OR (p.quantity <= '0' and p.location <> '1')) AND ( (p.quantity > '0') OR (p.quantity <= '0' and p.location <> '1')) AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'"; 
...
public function getProductSpecials($data = array()) {
	...
	$sql = "SELECT DISTINCT ps.product_id, (SELECT AVG(rating) FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = ps.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating FROM " . DB_PREFIX . "product_special ps LEFT JOIN " . DB_PREFIX . "product p ON (ps.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND ( (p.quantity > '0') OR (p.quantity <= '0' and p.location <> '1')) AND ( (p.quantity > '0') OR (p.quantity <= '0' and p.location <> '1')) AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) GROUP BY ps.product_id";
...
public function getLatestProducts($limit) {
	...
	if (!$product_data) { 
		$query = $this->db->query("SELECT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND ( (p.quantity > '0') OR (p.quantity <= '0' and p.location <> '1')) AND ( (p.quantity > '0') OR (p.quantity <= '0' and p.location <> '1')) AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY p.date_added DESC LIMIT " . (int)$limit);
...
public function getPopularProducts($limit) {
	...
	$query = $this->db->query("SELECT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND ( (p.quantity > '0') OR (p.quantity <= '0' and p.location <> '1')) AND ( (p.quantity > '0') OR (p.quantity <= '0' and p.location <> '1')) AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY p.viewed, p.date_added DESC LIMIT " . (int)$limit);
...
public function getBestSellerProducts($limit) {
	...
	if (!$product_data) { 
		...
		$query = $this->db->query("SELECT op.product_id, COUNT(*) AS total FROM " . DB_PREFIX . "order_product op LEFT JOIN `" . DB_PREFIX . "order` o ON (op.order_id = o.order_id) LEFT JOIN `" . DB_PREFIX . "product` p ON (op.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE o.order_status_id > '0' AND p.status = '1' AND ( (p.quantity > '0') OR (p.quantity <= '0' and p.location <> '1')) AND ( (p.quantity > '0') OR (p.quantity <= '0' and p.location <> '1')) AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' GROUP BY op.product_id ORDER BY total DESC LIMIT " . (int)$limit);
...
public function getProductRelated($product_id) {
	...
	$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_related pr LEFT JOIN " . DB_PREFIX . "product p ON (pr.related_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pr.product_id = '" . (int)$product_id . "' AND p.status = '1' AND ( (p.quantity > '0') OR (p.quantity <= '0' and p.location <> '1')) AND ( (p.quantity > '0') OR (p.quantity <= '0' and p.location <> '1')) AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");
...
public function getTotalProductSpecials() {
	...
	$query = $this->db->query("SELECT COUNT(DISTINCT ps.product_id) AS total FROM " . DB_PREFIX . "product_special ps LEFT JOIN " . DB_PREFIX . "product p ON (ps.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND ( (p.quantity > '0') OR (p.quantity <= '0' and p.location <> '1')) AND ( (p.quantity > '0') OR (p.quantity <= '0' and p.location <> '1')) AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND ps.customer_group_id = '" . (int)$customer_group_id . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW()))");
...
public function getProductClass($sku) {
	...
	$query = $this->db->query("SELECT c.*, d.name as main_category FROM inv_device_product AS a, inv_device_class AS b, inv_classification AS c, inv_main_classification AS d WHERE a.device_product_id = b.device_product_id AND b.class_id = c.id AND c.main_class_id = d.id AND sku = '".$this->db->escape($sku)."'");
...
public function getProductIDbySku($sku) {
	// echo "SELECT product_id FROM " . DB_PREFIX . "product WHERE model = '". $this->db->escape($sku) ."'";exit;
	$query = $this->db->query("SELECT product_id FROM " . DB_PREFIX . "product WHERE model = '". $this->db->escape($sku) ."'");
	return $query->row['product_id'];
}

public function getProductSteps($id)
{
	$query = $this->db->query("SELECT * FROM inv_product_repair_guide WHERE product_sku = '". $this->db->escape($id) ."'");
	return $query;
}

public function getProductQuestion($id)
{
	$query = $this->db->query("SELECT * FROM oc_product_question WHERE product_sku = '". $this->db->escape($id) ."'");
	return $query;
}
