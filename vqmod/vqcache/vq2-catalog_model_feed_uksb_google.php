...
class ModelFeedUKSBGoogle extends Model {
	...
	public function getTotalProducts() {
		$customer_group_id = $this->config->get('config_customer_group_id');
		$customer_group_id='1633';
		$sql = "SELECT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) LEFT JOIN " . DB_PREFIX . "uksb_google_merchant_products uksbp ON (p.product_id = uksbp.product_id) WHERE p.price > 0 AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND pd.description != '' AND p.status = '1' AND ( (p.quantity > '0') OR (p.quantity <= '0' and p.location <> '1')) AND p.date_available <= NOW() AND uksbp.g_on_google = 1 AND (uksbp.g_expiry_date > NOW() OR uksbp.g_expiry_date = '') AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' GROUP BY p.product_id";
