...
$this->data['customer_profiling'] = $customer_profiling;
	...
	public function DoExpressCheckoutPayment(){
		...
		if (!isset($this->session->data['order_id'])) {
			$this->log->write("PAYFLOW EXPRESS ERROR (A): No session order id found");

			echo '<html><head><script type="text/javascript">';
			echo 'alert("No session order id found");';
			echo 'window.location="' . (HTTPS_SERVER  . 'index.php?route=checkout/checkout&nc=1') . '";';
  ...
  "RETURNURL" => $store_url. 'payflow_callback',
  // "SILENTPOSTURL"=>$store_url. 'payflow_callback',
  "CANCELURL" => $store_url. 'index.php?route=checkout/cart',
  "ERRORURL" => $store_url. 'payflow_callback',
  "URLMETHOD"=>"POST",
...
// This example uses hardcoded values for simplicity.
  ...
  "BILLTOSTREET" => $this->session->data['newcheckout']['address_1'],
  "BILLTOCITY" => $this->session->data['newcheckout']['city'],
  "BILLTOSTATE" => ($order_info['payment_iso_code_2'] != 'US') ? $this->model_localisation_zone->getZone($this->session->data['newcheckout']['zone_id'])['name'] : $this->model_localisation_zone->getZone($this->session->data['newcheckout']['zone_id'])['code'],
  "BILLTOZIP" => $this->session->data['newcheckout']['postcode'],
  ...
  "SHIPTOSTATE" => $this->model_localisation_zone->getZone($order_info['shipping_zone_id'])['code'],
  ...
  "SHIPTOCOUNTRY" => $order_info['shipping_iso_code_2']
		...
		foreach ($this->cart->getProducts() as $product) {
			...
			$request2['L_NAME' . $i] 	= html_entity_decode($product['name'], ENT_QUOTES, "ISO-8859-1");
			$request2['L_NUMBER' . $i] 	= $product['model'];
			$request2['L_DESC' . $i] 	= (substr(html_entity_decode($description, ENT_QUOTES, "ISO-8859-1"), 0, 110) . '...');
			$request2['L_AMT' . $i ] 	= $price;
			$request2['L_QTY' . $i ] 	= $product['quantity'];
			$i++;
...
if ($response['RESULT'] != 0) {
  $this->pre($response, "Payflow call failed");
  exit(0);
} else { 
  $securetoken = $response['SECURETOKEN'];
  $securetokenid = $response['SECURETOKENID'];
}
//print_r($response).'aa';exit;
$this->redirect('https://payflowlink.paypal.com?SECURETOKEN='.$securetoken.'&SECURETOKENID='.$securetokenid.'&MODE=LIVE');
	...
	public function send() {
      ...
      // End Fix for payment method swap
		...
		// $invnum = html_entity_decode($this->session->data['order_id'], ENT_QUOTES, 'UTF-8');

		// if ($invnum != '')  $payment_data_array['INVNUM'] = $invnum;
		$payment_data_array['INVNUM'] = $this->session->data['order_id'];
		...
		if (in_array($response_data['RESULT'],array('0','126','104'))) {
            ...
            $redirect=($this->config->get('config_gts_status')) ? $this->url->link('checkout/success', '', 'SSL') : $this->url->link('checkout/success');
...
public function callback()
{
	...
	if (!in_array($response_data['RESULT'],array('0','126','104'))) {
			...
			echo '<html><head><script type="text/javascript">';
			echo 'alert("'.($response_data['RESPMSG']).'");';
			echo 'window.location="' . (HTTPS_SERVER  . 'index.php?route=checkout/checkout&nc=1') . '";';
			echo '</script></head></html>';
			exit;
		// $this->redirect($this->url->link('checkout/checkout','nc=1'));
		// $response_data = $this->request->post;
	}
	...
	if (isset($response_data['RESPMSG']) && $response_data['RESULT']=='1000' && preg_match('/10001.*Timeout/',$response_data['RESPMSG'])) {
		...
		if (in_array($response_data['RESULT'],array('0','126','104'))) {
			...
			if (!$ppat_order_query->num_rows) {
				...
				$this->db->query("INSERT INTO " . DB_PREFIX . "payflow_admin_tools SET `order_id` = '" . $this->session->data['order_id'] . "', transaction_id = '" . $response_data['PNREF'] . "',pp_transaction_id='".$response_data['PPREF']."', amount = '" .(float)$response_data['AMT']. "', authorization_id = '" .$this->db->escape($response_data['AUTHCODE']). "',avsaddr='".$response_data['AVSADDR']."',avszip='".$response_data['AVSZIP']."',cvv2match='".$response_data['CVV2MATCH']."'");
...
private function run_payflow_call($params) {
...
private function pre($arr, $heading = NULL) {
  if (!empty($heading)) {
