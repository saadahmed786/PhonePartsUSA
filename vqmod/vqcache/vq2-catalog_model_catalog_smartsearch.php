...
class ModelCatalogSmartsearch extends Model {
	...
	public function smartsearch($data = array()) {
		...
		$select_sql .= " WHERE p.date_available <= NOW() AND p.status = '1' AND ( (p.quantity > '0') OR (p.quantity <= '0' and p.location <> '1')) AND ( (p.quantity > '0') OR (p.quantity <= '0' and p.location <> '1')) AND p2s.store_id = '" . $store_id . "'";
