...
$this->data['text_your_address_billing'] = $this->language->get('text_your_address_billing');
		...
		if (file_exists(DIR_TEMPLATE . (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/checkout/register.tpl')) {
			$this->template = (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/checkout/register.tpl';
		} else {
	...
	public function validate() {
		...
		if (!$json) {
			$this->model_account_customer->addCustomer($this->request->post,1);
			// echo 'here';exit;
