        ...
        <div class="span7">
            ...
            <div class="pull-right">
              <div class="top_menu">  
                  <ul> 
                      <li>
                          <a class="fancybox.ajax orders_list" href="index.php?route=pos/pos/orders&token=<?= $token ?>&picked_up_orders=false" class="">
            ...
            <div class="input_wrapper discount_wrapper">
                ...
                <div class="column_2">
                 ...
                 <div class="column_2">
                  <button class="command-button" id="cancel_order" style="background: none repeat scroll 0 0 green;
    color: white;
        ...
        <div class="grid">
            <div class="row" style="display:none">                
                ...
                <div class="span4">
                    ...
                    <div data-role="input-control" class="input-control text">
                        ...
                        <input type="" id="item_selected" value="">
                        
                        <input type="" id="removed_items" name="removed_items" value="">
            ...
            <div class="row div_split_payment" style="display:none">
                ...
                <div class="span5">
                      ...
                      <input type="text" name="split_card" value="0"  onChange="changeSplitAmount()"/>                      
                   </div>    
            ...
            <div class="row" id="tr-round" style="display:none">
                ...
                <div class="span1">
                   <div data-role="input-control" class="input-control">
                      <input type="checkbox" name="round_value" checked  onChange="roundFunction()" /> 
                   </div>                          
                </div>
...
<script type="text/javascript">
	...
	/*if($('select[name=payment_method]').val()=='Cash'){
		$('input[name=paid]').val(Math.round($('#xtotal').val()));
		}
                        else
                        {
	...
	function changeSplitAmount()
	{
		...
		var card = parseFloat($('input[name=split_card]').val());
...
$('#order').click(function(e){
	...
	if($('#xpayment_method').val()=='paid')
	{
		...
		if($('#xpayment_type').val()=='PayPal' || $('#xpayment_type').val()=='Paypal Express')
		{
...
function update_cart($products, $total_data,is_combine,order_id){
    var html  = '';
    for(var i=0; i< $products.length; i++){
        ...
        html += '<td><a class="cart_remove" id="product-'+i+'" data-key="'+$products[i]['key']+'" data-order="'+order_id+'"><i class=" icon-cancel-2"></i></a></td>';
        html += '</tr>';
    }
...
else
{
    ...
    for(var i=0; i < $total_data.length; i++){
        ...
        html += '<div><b>'+$total_data[i].title +'</b><br><span id="'+($total_data[i].code=='total'?'main_total':($total_data[i].code=='sub_total'?'main_sub_total':''))+'">'+$total_data[i].text+'</span></div>';
    }
    html += '</div>';
	//alert(html);
 ...
 var main_sub_total =  $('.total_wrapper #main_total').html();
		...
		$('#main_sub_total').html('$'+main_sub_total);
		$('#main_total').html('$'+main_total);
		
		
		$('input[name=paid]').val(main_total);
		$('#xtotal').val(main_total);
	}
}
...
function voucher(){
 $.post('index.php?route=pos/pos/voucher&token=<?php echo $token; ?>',{ voucher: $('#voucher').val(),order_id: $('input[name="order_id"]').val() }, function(data){        
        var json = JSON.parse(data);
...
$('.order_list .edit').live('click', function(){
    ...
    $.get('index.php?route=pos/pos/getOrder&order_id='+$(this).attr('data-order-id')+'&is_combine='+is_combine+'&token=<?php echo $token; ?>',function(data){
		...
		update_cart(data['products'], data['total_data'],is_combine,order_id);
