<?php 
class ModelModuleExcelportcustomergroup extends ModelModuleExcelport {
	public function importXLSCustomerGroups($language, &$allLanguages, $file, $importLimit = 100) {
		...
		$chunkFilter = new CustomReadFilter(array('CustomerGroups' => array('A', ($progress['importedCount'] + 2), 'AM', (($progress['importedCount'] + $importLimit) + 1))), true); 
		
		$madeImports = false;
		$objReader = new PHPExcel_Reader_Excel2007();
		$objReader->setReadFilter($chunkFilter);
		$objReader->setReadDataOnly(true);
		$objReader->setLoadSheetsOnly(array("CustomerGroups"));
		...
		$map = array(
			...
			'description'			=> 2,
			'approval'				=> 3,
			'company_id_display'	=> 4,
			'company_id_required'	=> 5,
			'tax_id_display' 		=> 6,
			'tax_id_required'		=> 7,
			'sort_order'			=> 8
		);
		...
		do {
			...
			if (!empty($customer_group_name)) {
				...
				$customer_group_company_id_display = $customerGroupSheetObj->getCell(PHPExcel_Cell::stringFromColumnIndex($source[0] + $map['company_id_display']) . ($source[1]))->getValue() == 'Enabled' ? 1 : 0;
				
				$customer_group_company_id_required = $customerGroupSheetObj->getCell(PHPExcel_Cell::stringFromColumnIndex($source[0] + $map['company_id_required']) . ($source[1]))->getValue() == 'Enabled' ? 1 : 0;
				
				$customer_group_tax_id_display = $customerGroupSheetObj->getCell(PHPExcel_Cell::stringFromColumnIndex($source[0] + $map['tax_id_display']) . ($source[1]))->getValue() == 'Enabled' ? 1 : 0;
				
				$customer_group_tax_id_required = $customerGroupSheetObj->getCell(PHPExcel_Cell::stringFromColumnIndex($source[0] + $map['tax_id_required']) . ($source[1]))->getValue() == 'Enabled' ? 1 : 0;
				...
				$customer_group = array(
					...
					'company_id_display' => $customer_group_company_id_display,
					'company_id_required' => $customer_group_company_id_required,
					'tax_id_display' => $customer_group_tax_id_display,
					'tax_id_required' => $customer_group_tax_id_required,
	...
	public function exportXLSCustomerGroups($language, $destinationFolder = '', $customerGroupNumber, $export_filters = array()) {
		...
		$dataValidations = array(
			...
			array(
				'type' => 'list',
				'field' => $generals['company_id_display'],
				'data' => array(0,2,0,3),
				'range' => '',
			),
			array(
				'type' => 'list',
				'field' => $generals['company_id_required'],
				'data' => array(0,2,0,3),
				'range' => '',
			),
			array(
				'type' => 'list',
				'field' => $generals['tax_id_display'],
				'data' => array(0,2,0,3),
				'range' => '',
			),
			array(
				'type' => 'list',
				'field' => $generals['tax_id_required'],
				'data' => array(0,2,0,3),
				'range' => '',
			)
		);
		...
		if ($customer_groups->num_rows > 0) {
			foreach ($customer_groups->rows as $myCustomerGroupIndex => $row) {
				...
				$row['company_id_display'] = empty($row['company_id_display']) ? 'Disabled' : 'Enabled';
				$row['company_id_required'] = empty($row['company_id_required']) ? 'Disabled' : 'Enabled';
				$row['tax_id_display'] = empty($row['tax_id_display']) ? 'Disabled' : 'Enabled';
				$row['tax_id_required'] = empty($row['tax_id_required']) ? 'Disabled' : 'Enabled';
	...
	public function addCustomerGroup($customer_group_id = '', $data, &$allLanguages) {
		...
		if (version_compare(VERSION, '1.5.3', '<')) {
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "customer_group SET ".(!empty($customer_group_id) ? "customer_group_id = '" . (int)trim($customer_group_id) . "', " : "")."approval = '" . (int)$data['approval'] . "', company_id_display = '" . (int)$data['company_id_display'] . "', company_id_required = '" . (int)$data['company_id_required'] . "', tax_id_display = '" . (int)$data['tax_id_display'] . "', tax_id_required = '" . (int)$data['tax_id_required'] . "', sort_order = '" . (int)$data['sort_order'] . "'");
