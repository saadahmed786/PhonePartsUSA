<?php 
class ModelModuleExcelportoption extends ModelModuleExcelport {
	public function importXLSOptions($language, &$allLanguages, $file) {
		...
		$objReader->setLoadSheetsOnly(array("Options"));
		...
		do {
			...
			if (!empty($option_name) && !empty($option_type)) {
				...
				do {
					$option_value_name = trim(strval($optionsSheetObj->getCell(PHPExcel_Cell::stringFromColumnIndex($source[0] + $option_map['option_value_name']) . ($source[1]))->getValue()));
					
					if (!empty($option_value_name) || $option_value_name === '') {
						$option_values[] = array(
				...
				} while ((!empty($option_value_name) || $option_value_name === '') && empty($option_name_for_next_option_value));
