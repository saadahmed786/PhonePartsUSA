<?php 
class ModelModuleExcelportattribute extends ModelModuleExcelport {
	public function importXLSAttributes($language, &$allLanguages, $file) {
		...
		$objReader->setLoadSheetsOnly(array("Attributes", "Attribute Groups"));
		...
		do {
			$safe_mode = ini_get('safe_mode'); if ((empty($safe_mode) || strtolower($safe_mode) == 'off' ) && function_exists('set_time_limit') && stripos(ini_get('disable_functions'), 'set_time_limit') === FALSE && stripos(ini_get('disable_functions'), 'set_time_limit') === FALSE) set_time_limit(60);
