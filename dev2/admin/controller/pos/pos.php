...
class ControllerPosPos extends Controller {
	...
	public function get_total() {
      ...
      public function updateSalesTax(){
        ...
        $perform = $this->request->post['perform'];
        $order_total = $this->request->post['order_total'];
        $order_tax = $this->request->post['order_tax'];
        $order_tax_text = '$'.round($order_tax,2);
        if ($perform == 'add') {
         $this->db->query("INSERT INTO ".DB_PREFIX."order_total SET order_id='".$order_id."',code='tax',title='Tax 8.1000',text='".$order_tax_text."',value='".$order_tax."',sort_order='5'");
         $this->db->query("UPDATE inv_orders SET order_price ='".$order_total."' WHERE order_id='" . (int) $order_id . "'");
         $this->db->query("INSERT INTO ".DB_PREFIX."order_history SET order_id='".$order_id."',comment='".$comment."',date_added = NOW()");
         //$this->db->query("UPDATE ".DB_PREFIX."order SET comment ='".$comment."' WHERE order_id='" . (int) $order_id . "'");
         $json = array();
         $json['success'] = 'Sales Tax Added';
         ...
         exit;
          ...
          //$this->db->query("UPDATE ".DB_PREFIX."order SET comment ='".$comment."' WHERE order_id='" . (int) $order_id . "'");
          $json = array();
          $json['success'] = 'Sales Tax Removed';
          echo json_encode($json);
          exit; 
        }
      }
...
//End firefox detection
        ...
        // $cash = $this->model_pos_pos->get_today_cash($this->user->getId());
        // $card = $this->model_pos_pos->get_today_card($this->user->getId());
        // $paypal = $this->model_pos_pos->get_today_paypal($this->user->getId());
        $drawer = $this->model_pos_drawer->getUserDrawer($this->user->getUserInfo('user_id'));
        $this->data['close_drawer_id'] = $drawer['close_drawer_id'];
        $this->data['drawer_name'] = $drawer['name'];
        $cash = $this->model_pos_drawer->getCloseDrawerValue('expected', $this->data['close_drawer_id']);
        $card = $this->model_pos_drawer->getCloseDrawerValue('credit_card_total', $this->data['close_drawer_id']);
        $paypal = $this->model_pos_drawer->getCloseDrawerValue('paypal_total', $this->data['close_drawer_id']);


        if (!$drawer['close_drawer_id'] && !$this->user->getUserInfo('pos_manager')) {
        	$this->session->data['error_warning'] = 'No drawer has been assigned. Please check with your manager.';
        	$this->logout();
        	exit;
        }

        $largest_id=$this->db->query("SELECT MAX(id) as id FROM inv_returns");
        $largest_id = $largest_id->row;
       // echo $largest_id['id'];exit;
        $order_id=$this->db->query("SELECT order_id as order_id FROM inv_returns WHERE id='".$largest_id['id']."'");
        $order_id = $order_id->row;
        //echo $order_id['order_id'];

        $payment_method=$this->db->query("SELECT payment_method as payment_method FROM oc_order WHERE order_id='".$order_id['order_id']."'");
        $payment_method = $payment_method->row;
       // echo $payment_method['payment_method'];

        $price=$this->db->query("SELECT price as price FROM inv_return_decision WHERE return_id='".$largest_id['id']."'");
        $price = $price->row;
       // echo $price['price'];

         $this->data['cash'] = $this->currency->format($cash);
         $this->data['cashexp'] = $cash;

        if($payment_method['payment_method']=='Card,Cash' || $payment_method['payment_method']=='Card/Cash' || 
          $payment_method['payment_method']=='Card' || $payment_method['payment_method']=='Debit Card / Credit Card'){

        $newcash=$cash-$price['price'];  

        $this->data['cash'] = $this->currency->format($newcash);
        $this->data['cashexp'] = $newcash;

}

        $this->data['cardtotal'] = $card;
        $this->data['paypaltotal'] = $paypal;

        $this->data['card'] = $this->currency->format($card);
        $this->data['paypal'] = $this->currency->format($paypal);
        ...
        $this->data['userpos'] = $this->user->getUserInfo('firstname') . ' ' . $this->user->getUserInfo('lastname');
        $this->data['drawers'] = ($this->user->getUserInfo('pos_manager'))? $this->model_pos_drawer->getDrawers() : array();

        $this->data['pos_users'] = $this->model_pos_drawer->getAvailableUsers();

        foreach ($this->data['drawers'] as $dkey => $drawer) {
        	$user = $this->model_user_user->getUser($drawer['user_id']);
        	$this->data['drawers'][$dkey]['user_name'] = $user['firstname'] . ' ' . $user['lastname'];
        }
    ...
    public function updateList() {
        ...
        $json['payment_type'] = ($order_info['payment_method'] == 'Cash or Credit at Store Pick-Up' ? 'Unpaid' : 'Paid');
        ...
        $customer_order_info = array(
        	...
        	'customer_email_encoded' => base64_encode($order_info['email']),
        ...
        //if($order_info['payment_method']=='Cash or Credit at Store Pick-Up') $order_info['total'] = round($order_info['total']);


        $json['xtotal'] = round($xtotal, 2);
...
//END add order 
      ...
      public function editOrder() {
		...
		$cardPaid = (float)$this->request->post['cardPaid'];
		$cashPaid = (float)$this->request->post['cashPaid'];
		...
		foreach ($order_ids as $order_id) {
			...
			$paid_status = ($order_info['payment_method'] == 'Cash or Credit at Store Pick-Up' ? 'unpaid' : 'paid');
			...
			$data['shipping_country_id'] = $order_info['shipping_country_id'];
			$data['shipping_zone_id'] = $order_info['shipping_zone_id'];
			$data['auth_qc'] = (int)$this->request->post['auth_qc'];
			$data['auth_manager'] = (int)$this->request->post['auth_manager'];
        	// $data['shipping_method'] = 'Pickup From Store';
        	// $data['shipping_code'] = 'pickup.pickup';
        	...
        	// Display prices
          	...
          	foreach ($total_data as $key => $o_total) {
          		if ($o_total['code'] == 'total') {
          			$totalAmount = $o_total['value'];
          		}
          	}
			...
			*/
            	...
            	foreach ($removed_data as $item_removed) {
            		if ($item_removed) {
            			...
            			if ($item_removed[4] == $order_id) {
            				...
            				$negPrice = str_replace('$', '', $data1['total']);
            				$order_info['total'] = (float) $order_info['total'] - (float) $negPrice;
                    ...
                    $this->load->model('localisation/return_reason');
                    $reasonx = $this->model_localisation_return_reason->getReturnReason($data1['reason_id']);
                    $reson = $reasonx['name'];
                    $rdata = array('order_id' => $data1['order_id'],
                      'item_sku' => $__sku['model'],
                      'item_name' => $__sku['name'],
                      'date_removed' => date('Y-m-d H:i:s'),
                      'reason' => $reson,
                      'removed_by' => $this->user->getUserName()
                      );
                    $this->model_sale_order->removeItemFromOrderIMP($rdata);
            ...
            if($order_ids_count>1) {
              $data['pos_total'] = $order_info['total'];
            }
            ...
            $drawer_paypal = 0.00;
            if ($payment_method == 'Cash,Card') {
            	$drawer_card += $this->request->post['split_card'];
            	$drawer_cash += $data['pos_total'] - $drawer_card;
            }
            if ($payment_method == 'Card') {
              // $drawer_card += $order_info['total'];
            	$drawer_card += $data['pos_total'];
            }
            if ($payment_method == 'Cash') {
            	$drawer_cash += $data['pos_total'];
            }
            if ($payment_method == 'PayPal' || $payment_method == 'Paypal Express') {
            	$drawer_paypal += $data['pos_total'];
            }


            $this->load->model('pos/drawer');
            $close_drawer_id = $this->model_pos_drawer->getUserDrawer($this->user->getUserInfo('user_id'))['close_drawer_id'];

            if ((int)$order_info['user_id'] != (int)$this->user->getUserInfo('user_id') && $this->request->post['is_cancel'] != 'cancel') {
            	$this->model_pos_drawer->updateCloseDrawerCash($drawer_cash, $drawer_card, $drawer_paypal, $close_drawer_id);          
            }


              // If order is combined pos_total is assumed that customer paid full order amount

            // end condition

            $data['card_paid'] = $cardPaid;
            $data['cash_paid'] = $cashPaid;
            $data['change_due'] = $changeDue;
            $data['paypal_paid'] = 0.0000;
            if (!$cardPaid > 0 && !$cashPaid > 0) {
            	$data['paypal_paid'] = $totalAmount;
            }
          ...
          $this->load->model('pos/slip');

          $slipData = $this->model_pos_slip->createSlip($order_idx, $data);
          $dir = 'invoice/';
          $pdf = $this->model_pos_slip->createPdf($slipData, $dir);
          
          $this->model_pos_slip->savePdf($order_idx, $pdf);

          if ($this->request->post['is_cancel'] != 'cancel' && $this->request->post['print_recipt'] == 'true') {
          	$this->model_pos_slip->emailInvoice($order_idx);
          	$this->printSlip($order_ids[0]);
          	// $this->printSlip($order_ids[0]);

          }else if($this->request->post['is_cancel'] != 'cancel'){
          	// $this->printSlip($order_ids[0]);
          }
          $this->response->setOutput(json_encode($json));
      ...
      public function voucher() {
        ...
        if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
            ...
            foreach ($total_data as $key => $value) {
              $total_data[$key]['value'] = round($value['value'], 2);
              $total_data[$key]['text'] = '$' . str_replace('-', '', $total_data[$key]['value']);
            }
      ...
      public function change() {
      	if ($_POST['req'] == md5($_GET['token'])) {
      		$password = $_POST['passwrod'];
      		$id = $this->user->getId();
      		if ($_POST['password'] == $_POST['password1'] && $id) {
      			$this->load->model('user/user');
      	...
      	echo json_encode(array('msg' => 'Password didn\'t match'));
      	exit;
      }
        ...
        foreach ($rows as $row) {
        	if (!in_array($row['order_id'], $prohibited_ids)) {
        		...
        		foreach ($order_products as $product) {
        			$this->data['rows'][$i]['products'][] = array('model' => $product['model'], 'quantity' => $product['quantity'], 'name' => $product['name'], 'product_id' => $product['product_id']);
        		}
      ...
      public function ordersAJAX() {
        ...
        foreach ($rows as $row) {
        	...
        	$order_products = $this->model_sale_order->getOrderProducts($row['order_id']);
        	$row['decodeemail'] = $row['email'];
        	$row['email'] = base64_encode($row['email']);
        	$row['date_modified'] = date('m-d-Y h:i A',strtotime($row['date_modified']));
...
//        $this->model_sale_order->backUpOrder($this->request->get['order_id']); // taking backup
        ...
        $this->load->model('pos/pos');
        $customer_info = $this->model_pos_pos->getCustomer($order_info['customer_id']);
        $_customer_group = $this->model_pos_pos->getCustomerGroup($customer_info['customer_group_id']);
        $_customer_tax = $this->model_pos_pos->getCustomerTax($customer_info['customer_id']);

        if ($customer_info) {
        	$customer_detail = array('customer_id' => $customer_info['customer_id'],'customer_tax' => $_customer_tax['dis_tax'],'customer_group' => $_customer_group['name'],'customer_email' => $customer_info['email'], 'date' => date('m/d/y h:i A'), 'customer_name' => $customer_info['firstname'] . ' ' . $customer_info['lastname']);
...
//     }
      ...
      public function sendPrint () {
      	...
      	$order_idx = $this->request->post['order_id'];
      	$this->printSlip($order_idx);
      }

      public function printSlip($orders)	{
      	
      	$this->load->model('pos/slip');
      	$this->model_pos_slip->printNodeSlip($orders);

      }

      public function printSlipAjax()	{
      	$orders = implode(',', $this->request->post['selected']);
      	
      	if ($orders) {
      		$this->printSlip($orders);
      	}
    ...
    ////////////////////////////////// Close Drawer ////////////////////////////////
      ...
      public function closeDrawer() {
       ...
       // exit;
      	...
      	/* getting values from close drawer form*/ 

      	$pennies_count=$this->request->post['pennies_count'];

      	$pennies_value=$this->request->post['pennies_value'];

      	$nickles_count=$this->request->post['nickles_count'];
      	$nickles_value=$this->request->post['nickles_value'];

      	$dimes_count=$this->request->post['dimes_count'];
      	$dimes_value=$this->request->post['dimes_value'];

      	$quarters_count=$this->request->post['quarters_count'];
      	$quarters_value=$this->request->post['quarters_value'];

      	$halfdollars_count=$this->request->post['halfdollars_count'];
      	$halfdollars_value=$this->request->post['halfdollars_value'];


      	$ones_count=$this->request->post['ones_count'];
      	$twos_count=$this->request->post['twos_count'];
      	$fives_count=$this->request->post['fives_count'];
      	$tens_count=$this->request->post['tens_count'];
      	$twenties_count=$this->request->post['twenties_count'];
      	$fifties_count=$this->request->post['fifties_count'];
      	$hundred_count=$this->request->post['hundreds_count'];


      	$ones_value=$this->request->post['ones_value'];
      	$twos_value=$this->request->post['twos_value'];
      	$fives_value=$this->request->post['fives_value'];
      	$tens_value=$this->request->post['tens_value'];
      	$twenties_value=$this->request->post['twenties_value'];
      	$fifties_value=$this->request->post['fifties_value'];
      	$hundred_value=$this->request->post['hundreds_value'];

      	$cash_total=$this->request->post['cashtotal'];
      	$starting_cash=$this->request->post['starting_cash'];
      	$deposit_total=$this->request->post['deposit_total'];
      	$expected=$this->request->post['expected'];
      	$over_short=$this->request->post['over_short'];
      	$credit_card_total=$this->request->post['credit_card_total'];
      	$paypal_total=$this->request->post['paypal_total'];
      	$closed_by=$this->request->post['closed_by'];
      	$oc_closed_by_id=$this->request->post['oc_closed_by_id'];
      	$close_drawer_id=$this->request->post['close_drawer_id'];
      	$msg=$this->request->post['drawer_msg'];

      	$this->db->query("UPDATE ".DB_PREFIX."close_drawer SET closed_by='". $closed_by ."', oc_closed_by_id='". $oc_closed_by_id ."', pennies_count='".$pennies_count."',pennies_value='".$pennies_value."',nickles_count='".$nickles_count."',nickles_value='".$nickles_value."',dimes_count='".$dimes_count."',dimes_value='".$dimes_value."',quarters_count='".$quarters_count."',quarters_value='".$quarters_value."',half_dollars_count='".$halfdollars_count."',half_dollars_value='".$halfdollars_value."',one_dollar_value='".$ones_value."',two_dollar_value='".$twos_value."',five_dollar_value='".$fives_value."',ten_dollar_value='".$tens_value."',twenty_dollar_value='".$twenties_value."',fifty_dollar_value='".$fifties_value."',hundred_dollar_value='".$hundred_value."',cash_total='".$cash_total."',deposit_total='".$deposit_total."',expected='".$expected."',over_short='".$over_short."',starting_cash='".$starting_cash."',date='". date('Y-m-d H:i:s') ."',credit_card_total='".$credit_card_total."',paypal_total='".$paypal_total."',ones_count='".$ones_count."',twos_count='".$twos_count."',fives_count='".$fives_count."',tens_count='".$tens_count."',twenties_count='".$twenties_count."',fifties_count='".$fifties_count."',hundreds_count='".$hundred_count."', msg='". $msg ."' WHERE id='". $close_drawer_id ."'");
      	$this->db->query("UPDATE ".DB_PREFIX."drawer SET status='Close' WHERE close_drawer_id='". $close_drawer_id ."'");


      	$this->drawerSlip($close_drawer_id);

      }


      public function drawerSlip($drawerID) {
      	$this->load->model('pos/slip');
      	$pdf = $this->model_pos_slip->createDrawerSlip($drawerID);
      	$this->model_pos_slip->printNodePDF($pdf, 'Drawer Close Receipt Print - '.$this->user->userHavePermission('firstname').' '.$this->user->userHavePermission('lastname'));
      }

      public function printDrawerSlip($close_drawer_id = 0) {
      	if ($close_drawer_id == 0) {
      		$close_drawer_id = $this->request->post['close_drawer_id'];
      	}
      	$this->load->model('pos/drawer');
      	$pdf = $this->model_pos_drawer->getCloseDrawerValue('pdf', $close_drawer_id);
      	$this->model_pos_slip->printNodePDF($pdf, 'Drawer Close Receipt Print - '.$this->user->userHavePermission('firstname').' '.$this->user->userHavePermission('lastname'));
      }
    ...
    /////////////////////////////////End Close Drawer /////////////////////////////
      ...
      public function openStore () {
      	...
      	$this->load->model('pos/pos');
      	$this->model_pos_pos->storeStatusSet($store);
      }

      public function drawerAssignUser () {
      	$drawer_id=$this->request->post['drawer_id'];
      	$user_id=$this->request->post['user_id'];
      	$starting_cash=$this->request->post['starting_cash'];
      	$this->load->model('pos/drawer');
      	$this->model_pos_drawer->drawerAssignUser($drawer_id, $user_id, $starting_cash);
      }

      public function closeDrawerManager () {
      	$drawer_id=$this->request->post['drawer_id'];
      	$user_id=$this->request->post['user_id'];
      	$this->load->model('pos/drawer');
      	$this->model_pos_drawer->drawerAssignUser($drawer_id, $user_id);
      }

      public function loadCloseDrawer () {
      	$close_drawer_id=$this->request->post['close_drawer_id'];
      	$this->load->model('pos/drawer');
      	$this->load->model('pos/pos');

      	$return = $this->model_pos_drawer->getAssignDrawer($close_drawer_id);

      // $return['expected'] = $this->model_pos_pos->get_today_cash($return['user_id']);

      // $return['credit_card_total'] = $this->model_pos_pos->get_today_card($return['user_id']);
      // $return['paypal_total'] = $this->model_pos_pos->get_today_paypal($return['user_id']);
