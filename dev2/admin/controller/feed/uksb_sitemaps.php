...
class ControllerFeedUksbSitemaps extends Controller {
	...
	public function index() {
		...
		$this->language->load('feed/uksb_sitemaps');
		...
		$this->data['data_feed_url'] = HTTP_CATALOG;
		$this->data['data_feed'] = 'index.php?route=feed/uksb_sitemaps/google&store=0';
		$this->data['data_feed2'] = 'index.php?route=feed/uksb_sitemaps/all&store=0';
		...
		if($this->model_setting_store->getTotalStores()>0){
			...
			foreach($stores as $store){
				$this->data['data_feed_url'] .= '^'.$store['url'];
				$this->data['data_feed'] .= '^index.php?route=feed/uksb_sitemaps/google&store='.$store['store_id'];
				$this->data['data_feed2'] .= '^index.php?route=feed/uksb_sitemaps/all&store='.$store['store_id'];
			}
		}
