...
class ControllerSaleRma extends Controller { 
	...
	public function index() {
		...
		$item_issues = $this->db->query("select * from inv_reasonlist ORDER BY `name`");
	...
	public function insert()
	{
		...
		$printer = $this->request->post['printer'];
		...
		$data['auth_qc'] = $this->request->post['auth_qc'];
		$data['auth_manager'] = $this->request->post['auth_manager'];
		if($order_info['ref_order_id'])
		{
		...
		{	
			...
			$order_product = $this->model_sale_order->getOrderProduct($order_id,$product_detail['product_id']);
			$data = array();
			$data['discount_amount'] = (($order_product['price'] / 100) * (float)$this->request->post['discount_per']);
			$data['product_id'] = $product_detail['product_id'];
			$data['product_title'] = $product_detail['name'];
			$data['sku'] = $product_detail['sku'];
			$data['price'] = (float)$order_product['price'] - $data['discount_amount'];
			$data['quantity'] = 1;
			...
			$data['printer'] = $printer[$key];
			$data['decision'] = $decision[$key];
			$data['discount_per'] = $this->request->post['discount_per'];
			$data['restocking'] = $this->request->post['restocking'];
			$data['restocking_grade'] = $this->request->post['restocking_grade'];
			...
			if ($data['decision'] == 'Issue Refund') {
			$this->db->query("UPDATE ".DB_PREFIX."close_drawer SET expected = expected -'". (float)$data['price'] ."'  WHERE id='".(int)$drawer['close_drawer_id']."'");
			}
		...
		$drawer = $this->model_pos_drawer->getUserDrawer($this->user->getUserInfo('user_id'));
			if ($data['decision'] == 'Issue Refund') {
			$this->db->query("UPDATE ".DB_PREFIX."close_drawer SET expected = expected -'". (float)$total_tax ."'  WHERE id='".(int)$drawer['close_drawer_id']."'");
			}
		...
		$slipData = $this->model_pos_slip->createRMASlip($return_id);

		$dir = 'invoice/';

		$pdf = $this->model_pos_slip->createPdf($slipData, $dir);

		$this->model_pos_slip->saveRMAPdf($return_id, $pdf);

		if ($decision[0] != 'Issue Replacement') {

			$this->model_pos_slip->printNodePDF($pdf, 'RMA Receipt Print - '.$this->user->userHavePermission('firstname').' '.$this->user->userHavePermission('lastname'));
	...
	public function verifyPins () {
		$this->load->model('sale/rma');
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			$result = $this->model_sale_rma->verifyPins($this->request->post);
	...
	public function issue_credit()
	{
        ...
        //New Voucher Reasoning code by Gohar
		$rma_number = $this->request->get['rma_number'];
		$rma_query_1 = $this->db->query("SELECT * FROM inv_returns WHERE rma_number = '" . $rma_number . "'");
    	$rma = $rma_query_1->row;
    	$rma_query_2 = $this->db->query("SELECT * FROM inv_return_items WHERE return_id = '" . $rma['id'] . "'");
    	$rma_items = $rma_query_2->rows;
    	$voucher_items_reasons = '';
    	foreach ($rma_items as $item) {
    		$voucher_items_reasons .= $item['sku'].'('.$item['item_condition'].' - '.$item['item_issue'].');';
    	}
		...
		foreach($products as $product)
		{
			...
			if($product_info)
			{
				...
				$item_detail.='SKU: '.$product_info['model'].', Title:'.$product_info['name'].', Qty: 1, Price: '.$product_info['price'].'<br>';
		...
		$this->db->query("INSERT INTO inv_voucher_details SET voucher_id='".$voucher_id."',order_id='".$order_id."',rma_number='".$this->request->post['rma_number']."',is_pos=1,oc_user_id='".$this->user->getId()."',item_detail='".$item_detail."'");
		foreach ($rma_items as $item) {
				$vproduct = array();
				$vproduct['voucher_id'] = $voucher_id;
				$vproduct['order_id'] =$order_id;
				$vproduct['rma_number'] =$this->request->post['rma_number'];
				$vproduct['sku'] = $item['sku'];
				$vproduct['price'] = $item['price'];
				$vproduct['reason'] = '('.$item['item_condition'].'-'.$item['item_issue'].')';
				$v_item = $this->model_sale_voucher->addVoucherProduct($vproduct);
				

		}
		if($voucher_id)
		{			
			echo 'Voucher #: '.$code.' has been generated and sent';
	...
	public function refund_invoice()
	{
		...
		foreach($query->rows as $row)
		{
			...
			$data['order_total'][] = array(
				'code'=>$row['code'],
				'title'=>$row['title'],
				'text'=>$this->currency->format($value*(-1), $order_info['currency_code'], $order_info['currency_value']),
				'value'=>$value*(-1),
				'sort_order'=>$row['sort_order']
				)	;
