 ...
 else
 {
	$isOld = false; 
		...
		if($product_details['payment_code'] == 'pp_standard' or $product_details['payment_code']=='authorizenet_aim' or $product_details['payment_code']=='paypal_express')
		{
			...
			if(!$orderHistories and $product_details['order_status_id']!=$allowedShippingMethod){ echo 'Store Credit cannot be issued for this Order~error';exit;}
		...
		else if($product_details['payment_code']=='cod')
		{
			...
			if($product_details['order_status_id']!=$allowedShippingMethod) {echo 'For Cash at Store Pickup Payments, the order must be shipped~error';exit;}
			
		}
		else
		{
			echo 'The Store Credit can only be generated for Paypal, Authorize.net &amp; Cash at Pickup Payment Methods~error';exit;	
