...
class ModelOneCheckoutCheckout extends Model {	
	...
	public function create($data) {
		...
		$this->insertSurvey($order_id, $data['onecheckout_survey_option']);

		return $order_id;
	}
	
	public function insertSurvey($order_id, $survey_option) {
		if($order_id && $survey_option) {
			$this->db->query("DELETE From " . DB_PREFIX . "order_survey WHERE order_id = '" . (int)$order_id . "'");
			$this->db->query("INSERT INTO " . DB_PREFIX . "order_survey SET order_id = '" . (int)$order_id . "', option_value = '" . $this->db->escape($survey_option) . "'");
		}
	}
