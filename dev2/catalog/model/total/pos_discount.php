...
class ModelTotalPosDiscount extends Model {
	public function getTotal(&$total_data, &$total, &$taxes) {
		if (isset($this->session->data['pos_discount'])) {
			...
			if ($discount['code'] == 'pos_discount') {
				if ((0-$discount['value']) > $total) {
					$amount = -$total;	
				} else {
					$amount = $discount['value'];	
				}				
			} elseif ($discount['code'] == 'pos_discount_subtotal' || $discount['code'] == 'pos_discount_total') {
