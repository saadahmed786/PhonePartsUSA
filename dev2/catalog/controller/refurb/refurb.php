...
class ControllerRefurbRefurb extends Controller {
	...
	public function index() {
		...
		$this->document->setTitle('LCD Refurbishment Program');
		$this->load->model('account/address');
		$this->load->model('refurb/refurb');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validate())) {
			...
			$refurb_id =  $this->model_refurb_refurb->saveData($this->request->post);
			$this->session->data['refurb_id'] = $refurb_id;
			if(isset($this->request->post['theme']) and $this->request->post['theme']=='2')
			{
				$this->redirect($this->url->link('refurb/refurb/printoutnew'));
			}
			else
			{
				
				$this->redirect($this->url->link('refurb/refurb/printout'));
			}
			exit;
		}
		...
		$this->data['action'] = $this->url->link('refurb/refurb');
		$addresses = array();
		$addresses = $this->model_account_address->getAddresses();
		$this->data['addresses'] = $addresses;

		$general = $this->model_refurb_refurb->getGeneralDetails();
		$products = $this->model_refurb_refurb->getProducts();
		$this->data['description'] = $this->model_refurb_refurb->getGradeDesc();
		...
		$this->data['breadcrumbs'][] = array(
			'text' => 'LCD Refurbishment',
			'href' => $this->url->link('refurb/refurb', '', 'SSL'),
			'separator' => $this->language->get('text_separator')
			);

		$this->data['heading_title'] = 'LCD Refurbishment Program';



		$this->template = (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/refurb/refurb.tpl';
	...
	{
		$this->load->model('refurb/refurb');
		$refurb_id = $this->session->data['refurb_id'];	
		if(!$refurb_id)
		{
			$this->redirect($this->url->link('refurb/refurb'));
			exit;	
		}
		...
		$detail = $this->model_refurb_refurb->getRefurbDetail($this->session->data['refurb_id']);
		$products = $this->model_refurb_refurb->getRefurbProducts($this->session->data['refurb_id']);
		$address = $this->model_refurb_refurb->getAddress($this->session->data['refurb_id']);
		$this->data['detail'] = $detail;
		$this->data['products'] = $products;
		$this->data['address'] = $address;


		$barcode_image = '<barcode type="C128A" value="Refurb:' . $detail['shipment_number'] . '" label="label" style="width:50mm; height:8mm; color: #000; font-size: 3mm"></barcode>';
		$html = '
...
//unlink('temp.pdf');
				...
				$html2 = 'Thank you for submitting your LCD Refurbishment Request'."<br>";
       // $html2.='Please read our returns policy (<a href="http://phonepartsusa.com/returns-or-exchanges">http://phonepartsusa.com/returns-or-exchanges</a>) and note the following:';
				...
				$mail->setSubject(html_entity_decode("LCD Refurbishment", ENT_QUOTES, 'UTF-8'));
				$mail->addAttachment(DIR_IMAGE . 'returns/' . $filename . '.pdf');
				$mail->setHtml($html2);
				// $mail->send();


		$this->template = (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/refurb/thank_you.tpl';
...
//unlink('temp.pdf');
       ...
       // $html2.='Please read our returns policy (<a href="http://phonepartsusa.com/returns-or-exchanges">http://phonepartsusa.com/returns-or-exchanges</a>) and note the following:';
					...
					<div class="selet-item-inner">
						...
						<div class="print-inner">
							...
							<div class="product-detail">
								<a class="return-btn" href="javascript:void(0)" onclick="printThis();"><img src="image/print-icon.png" alt="print-icon">Print Refurb Label</a>
			...
			<script>
				...
				function printThis()
				{
					var mywindow = window.open("", "Refurb Print", "height=400,width=600");
					mywindow.document.write("<html><head><title>Refurb Print</title>");
...
public function ajaxshipment() {
		...
		$this->template = (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/refurb/ajax_shipping_address.tpl';
