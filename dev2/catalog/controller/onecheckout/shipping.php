...
class ControllerOneCheckoutShipping extends Controller {
  	...
  	public function index() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			$this->data['text_not_shipping_method'] = $this->language->get('text_not_shipping_method');		

			$this->data['insurance_fee'] = $this->getChild('onecheckout/insurance_fee');
			...
			} elseif($this->config->has('onecheckout_shipping')) {//new
				
				$this->load->model('shipping/' . $this->config->get('onecheckout_shipping'));
				$defaultmethod = $this->{'model_shipping_' . $this->config->get('onecheckout_shipping')}->getQuote($shipping_address); 
				if ($defaultmethod) {
					$this->data['code'] = $defaultmethod['quote'][key($defaultmethod['quote'])]['code'];
				} else {
					$this->data['code'] = '';
				}
			...
			if (file_exists(DIR_TEMPLATE . (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/onecheckout/shipping.tpl')) {
				$this->template = (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/onecheckout/shipping.tpl';
			} else {
