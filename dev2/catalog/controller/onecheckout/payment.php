...
class ControllerOneCheckoutPayment extends Controller {
  	...
  	public function index() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			$this->data['text_checkout_comment'] = $this->language->get('text_checkout_comment');
			...
			} elseif($this->config->has('onecheckout_payment')) {//new
				
				$this->load->model('payment/' . $this->config->get('onecheckout_payment'));
				$defaultmethod = $this->{'model_payment_' . $this->config->get('onecheckout_payment')}->getMethod($payment_address, $total); 
				if ($defaultmethod) {
					$this->data['code'] = $defaultmethod['code'];
				} else {
					$this->data['code'] = '';
				}
			...
			if (file_exists(DIR_TEMPLATE . (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/onecheckout/payment.tpl')) {
				$this->template = (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/onecheckout/payment.tpl';
			} else {
