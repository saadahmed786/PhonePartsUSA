...
class ControllerPaymentGoogleCheckout extends Controller {
	public function index() {
		...
		if ($order_info) {		
			...
			if (file_exists(DIR_TEMPLATE . (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/payment/google_checkout.tpl')) {
				$this->template = (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/payment/google_checkout.tpl';
			} else {
