...
class ControllerModuleNe extends Controller {
	...
	protected function index($setting) {
		...
		if (($this->config->get($this->_name . '_show_unloginned_subscribe') == '1' && !$this->customer->isLogged()) || $this->config->get($this->_name . '_show_unloginned_subscribe') == '2') {
			...
			if ($is_modal) {
				if (file_exists(DIR_TEMPLATE . (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/stylesheet/ne/modal.css')) {
					$this->document->addStyle(basename(DIR_APPLICATION) . '/view/theme/' . (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/stylesheet/ne/modal.css');
				} else {
				...
				if (file_exists(DIR_TEMPLATE . (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/' . $this->_name . '/modal.tpl')) {
					$this->template = (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/' . $this->_name . '/modal.tpl';
				} else {
					$this->template = 'default/template/' . $this->_name . '/modal.tpl';
				}
			} else {
				if (file_exists(DIR_TEMPLATE . (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/stylesheet/ne/subscribe.css')) {
					$this->document->addStyle(basename(DIR_APPLICATION) . '/view/theme/' . (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/stylesheet/ne/subscribe.css');
				} else {
					$this->document->addStyle(basename(DIR_APPLICATION) . '/view/theme/default/stylesheet/ne/subscribe.css');
				}

				if (file_exists(DIR_TEMPLATE . (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/' . $this->_name . '/subscribe.tpl')) {
					$this->template = (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/' . $this->_name . '/subscribe.tpl';
				} else {
