...
class ControllerAccountAddress extends Controller {
	...
	private function getList() {
		...
		if ($this->session->data['warehouse']) {
			$results[0] = $this->model_account_address->getAddress($this->customer->getAddressId());
		} else {
		...
		if (file_exists(DIR_TEMPLATE . (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/account/address_list.tpl')) {
			$this->template = (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/account/address_list.tpl';
		} else {
	...
	private function getForm() {
		...
		if (file_exists(DIR_TEMPLATE . (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/account/address_form.tpl')) {
			$this->template = (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/account/address_form.tpl';
		} else {
	...
	private function validateForm() {
		...
		if ((utf8_strlen($this->request->post['address_1']) < 3) || (utf8_strlen($this->request->post['address_1']) > 128) || !preg_match("/^[a-zA-Z0-9\,\.\'\`?: ]*$/", $this->request->post['address_1'])) {
		...
		if ((utf8_strlen($this->request->post['password']) != '') && ((utf8_strlen($this->request->post['password']) < 4) || (utf8_strlen($this->request->post['password']) > 20))) {
