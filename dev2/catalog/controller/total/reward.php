...
class ControllerTotalReward extends Controller {
	public function index() {
		...
		if ($points && $points_total && $this->config->get('reward_status')) {
			...
			if (file_exists(DIR_TEMPLATE . (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/total/reward.tpl')) {
				$this->template = (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/total/reward.tpl';
			} else {
