<?php 
class ControllerCatalogAccessories extends Controller {
	public function index() {
		...
		$this->document->setTitle("Accessories");
		$this->data['heading_title'] = "Accessories";
		...
		if (isset($this->request->get['path'])) {
			...
			foreach ($breadcrumbs as $breadcrumb) {
				...
				$this->data['breadcrumbs'][] = array (
					...
					'href'      => $this->url->link('catalog/accessories', 'path=' . $path),
					);
		...
		$this->data['module'] = ($this->request->get['route'])? explode('/', $this->request->get['route'])[1]: 'accessories';
		...
		if ($main_class_id) {
			...
			$__manufacturers = $this->cache->get('catalogmanufacturers2.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id'));
			...
			$this->cache->set('catalogmanufacturers2.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id'), $__manufacturers);
		}
			...
			foreach ($rows as $key => $row) {
				$products[] = array (
					...
					'href' => $this->url->link('catalog/accessories', 'path=' . $row['id']),
					);
			}
		...
		if ($main_class_id) {
			...
			if (file_exists(DIR_TEMPLATE . (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/catalog/accessories.tpl')) {
				$this->template = (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/catalog/accessories.tpl';
			} else {
				$this->template = 'default/template/catalog/accessories.tpl';	
			}
		}
