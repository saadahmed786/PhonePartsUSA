<?php 
class ControllerCatalogRefurbishing extends Controller {
	public function index() {
		$this->language->load('catalog/refurbishing');
		
		$this->load->model('catalog/catalog');
		
		$this->load->model('tool/image');
		$this->document->setTitle("Refurbishing");
		$this->data['heading_title'] = "Refurbishing";
		...
		if (isset($this->request->get['path'])) {
			...
			foreach ($breadcrumbs as $breadcrumb) {
				...
				$this->data['breadcrumbs'][] = array (
					...
					'href'      => $this->url->link('catalog/refurbishing', 'path=' . $path),
					);
		...
		$this->data['module'] = ($this->request->get['route'])? explode('/', $this->request->get['route'])[1]: 'refurbishing';
		...
		if ($main_class_id) {
			...
			$__manufacturers = $this->cache->get('catalogrefurbishing.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id'));
			...
			$this->cache->set('catalogrefurbishing.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id'), $__manufacturers);
		}
			...
			{
			$this->data['class_id'] = $this->request->get['class_id'];
			}
			...
			foreach ($rows as $key => $row) {
				$products[] = array (
					...
					'href' => $this->url->link('catalog/refurbishing', 'path=' . $row['id']),
					);
			}
		...
		if ($main_class_id) {
			...
			if (file_exists(DIR_TEMPLATE . (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/catalog/refurbishing.tpl')) {
				$this->template = (($this->session->data['temp_theme'])? $this->session->data['temp_theme'] : $this->config->get('config_template')) . '/template/catalog/refurbishing.tpl';
			} else {
				$this->template = 'default/template/catalog/refurbishing.tpl';	
			}
		}
	...
	public function loadFilterProducts() {
		...
		if(!$productFilter['class_id'] || !$productFilter['manufacturers'] || !$_sub_models)
		{
			// echo 'undefined filters';exit;
		}
			// echo $sub_models;exit;
