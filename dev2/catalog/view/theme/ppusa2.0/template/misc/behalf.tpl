...
<div class="tab-inner pd60" id="behalf-payment-element">
  ...
  window.behalfPaymentReady = function() {
    ...
    var checkoutContext = {
      ...
      "paymentDetails" : {
        ...
        "shippingAmount": <?php echo (float)$sub_total;?>,
  ...
  if(eventData.paymentStatus === "approved") {
   ...
   $.ajax({
        ...
        type: 'post',
        data: 'email='+encodeURIComponent('<?php echo $email;?>')+'&paymentStatus='+encodeURIComponent(eventData.paymentStatus)+'&paymentToken='+encodeURIComponent(eventData.paymentToken+'&behalfBuyerId='+event.behalfBuyerId),
        dataType: 'json',
        success: function(json) {
