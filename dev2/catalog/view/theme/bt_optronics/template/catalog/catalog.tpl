	...
	<div class="ui equal width grid">
		...
		<div class="row" id="MainMenu">
			...
			<div class="row marginNull">
				...
				<div class="eight wide column">
					<div class="row marginNull getShareLink">
						<a onclick="getShareLink();">Get Shareable Link</a>
					</div>
				</div>
			</div>
		</div>
				...
				<div id="products" class="ui grid" style="text-align: center;">
					<div class="ui sixteen column">
						<div class="ui segment">
							<h2>Choose a device to view compatibile parts and accessories</h2>
						</div>
					</div>
				</div>
			</div>
...
<script type="text/javascript">
	...
	function loadAttr (t) {
				...
				$.ajax({
					...
					if (json['attributes']) {
						...
						for (var i = 0; i < loop; i++) {
							...
							attributes += '<label>';
							attributes += '<input type="checkbox" data-val-id="'+ $(t).val() + '-' + json['attributes'][i].id +'" value="'+ json['attributes'][i].id +'" onChange="loadProducts(1, 1)"> ' + json['attributes'][i].name.ucFirst();
							attributes += '</label>';
							attributes += '</div>';
							attributes += '</div>';
						}
			...
			if (productsx) {
				...
				temp_group_id = 1633;
					...
					for (var x = 0; x < priceGroup.length; x++) {
						...
						products += '<tr class="groupPrice group-'+ priceGroup[x].customer_group_id +'" '+ ((temp_group_id != priceGroup[x].customer_group_id)? 'style="display: none;"': '') +'>';
						...
						if (temp_group_id == priceGroup[x].customer_group_id && priceGroup[x].quantity == '1') {
							if (intQty) {
					...
					if(productsx[i].is_kit==1)
					{
						price = productsx[i].price;
					}

					if (priceGroup.length == 0 || productsx[i].is_kit==1 ) {
						text = 'Price'
						products += '<tr>';
						products += '<td><span>' + text + '</span></td>';
		...
		function changePrice (group_id, qty, product_id) {
			...
			var data_sku = $('.product-'+product_id).attr('data-sku');
			if (data_sku.substr(0,7)=='APL-001' || data_sku.substr(0,4)=='SRN-' || data_sku.substr(0,7)=='TAB-SRN' ) {
				group_id = 1633;
			}			
