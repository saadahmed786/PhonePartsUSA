...
$_query = "select * from inv_return_shipment_boxes where $where and box_type = 'NotTestedBox' order by date_added desc";
$count_query = "select count(id) as total from inv_return_shipment_boxes where $where and box_type = 'NotTestedBox'";

$splitPage   = new splitPageResults($db , $_query , 25 , "boxes/not_tested.php",$page ,  $count_query);
...
foreach($rma_returns as $index => $rma_return){
	...
	if($sku_str){
		...
		$products = $db->func_query("select product_id from oc_product where sku in ($sku_str)");
		...
		if($product_ids){
			...
			$rma_returns[$index]['items_cost'] = $db->func_query_first_cell("select sum(price) from oc_product_discount where product_id IN ($product_ids_str) AND customer_group_id = '8' and quantity = 1");
		}
	}
	else{
...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <body>
		...
		<div align="center">
			<a href="<?php echo $host_path;?>boxes/createbox.php?type=NotTestedBox&return=not_tested">Create new Box</a> <br /><br />
					   ...
					   <td style="width:50px;"><?php echo $k+1;?></td>			   		
			   		   ...
			   		   <td>
			   		       <?php if(($rma_return['status'] == 'Issued' && $_SESSION['edit_pending_shipment']) || $_SESSION['login_as'] == 'admin'):?>	
                              		<a href="<?php echo $host_path;?>boxes/boxes_edit.php?box_id=<?php echo $rma_return['id']?>&return=not_tested">Edit</a>
                           <?php endif;?>	
                          
                           <?php if($_SESSION['delete_shipment']):?>
                          		|
                          		<a href="<?php echo $host_path;?>boxes/deletebox.php?action=delete&box_id=<?php echo $rma_return['id']?>&return=not_tested" onclick="if(!confirm('Are you sure ?')){ return false; }">Delete</a>
                           <?php endif;?>
			   		   </td>
			   		</tr>
			   <?php endforeach;?>
