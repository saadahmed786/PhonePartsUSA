...
if ($_POST['MoveItemIssue']) {
	if (count($_POST['reject_ids']) > 0) {
		...
		header("Location:$host_path/boxes/over_days.php");
		exit;
	} else {
...
where $where and box_type = 'O60DBox' order by date_added desc";
...
$inv_return_shipment_box_id = $db->func_query_first_cell("select id from inv_return_shipment_boxes where box_number LIKE '%O60DBox%' and status = 'Issued'");
if(!$inv_return_shipment_box_id){
	$return_shipment_boxes_insert = array ();
	$return_shipment_boxes_insert ['box_number'] = getReturnBoxNumber ( 0, 'O60DBox' );
	$return_shipment_boxes_insert ['box_type']   = 'O60DBox';
...
<head>
	...
	<title>Over 60 Days Items</title>
...
<body>
	...
	<h2 align="center">Over 60 Days Items</h2>
			...
			<table class="data" border="1" style="border-collapse:collapse;" width="94%" cellspacing="0" align="center" cellpadding="3">
				...
				<?php foreach($ntr_items as $k => $ntr_item):?>
					<tr>
						...
						<td><?php echo linkToShipment($ntr_item['shipment_id'], $host_path, $ntr_item['shipment_number']); ?></td>
						<td align="center">
						...
						<td><?php echo linkToOrder($ntr_item['order_id'], $host_path);?></td>
