...
if($_POST['MoveLBB']){
	if(count($_POST['reject_ids']) > 0){
		foreach($_POST['reject_ids'] as $reject_id){
			moveItemToLBB($reject_id,$_POST['shipment_id'][$reject_id],$_POST['condition'][$reject_id],'1','0','0','GFS',$_POST['lbb_sku'][$reject_id]);
		}
...
if($_POST['MoveReject']){
	if(count($_POST['reject_ids']) > 0){
		foreach($_POST['reject_ids'] as $reject_id){
			// $from = "GFS";
			// $to = "MoveToReject";
			// logRejectItem($reject_id, 'Moved back to Rejected Item PO from GFS by ',$from, $to);
			returnMoveToRJ($reject_id, $_POST['reject_reason'][$reject_id], $_POST['shipment_id'][$reject_id], 'GFS');
		}
...
elseif($_POST['save'] || $_POST['Complete']){ 
	...
	if($_POST['Complete'] && $_SESSION['edit_received_shipment']){
		...
		//new shipment code gohar
		if ($_GET['return'] == 'return_to_stock') {

		//$checkExist = $db->func_query_first_cell("select id from inv_shipments where package_number = '".$shipment['box_number']."'");
		$box_date = $db->func_query_first_cell("select date_added from inv_return_shipment_boxes where box_number = '".$shipment['box_number']."'");
			$shipment_name = 'RMA-GFS-'.date('m-d-Y',strtotime($box_date));
			$checkExist = $db->func_query_first_cell("select id from inv_shipments where package_number = '".$shipment_name."' AND status = 'Issued'");
		if (!$checkExist) {
			$shipment_data = array();
			$shipment_data['package_number'] = $shipment_name;
			$shipment_data['ex_rate'] = '1';
			$shipment_data['carrier'] = 'In House';
			$shipment_data['status'] = 'Issued';
			$shipment_data['vendor'] = '97';
			...
			$inv_shipment_id = $checkExist;
		}
		foreach($reject_item_ids as $id => $reject_id){
			$sku = $db->func_escape_string($_POST['sku'][$reject_id]);
			$product_id = $db->func_query_first_cell("SELECT product_id FROM oc_product WHERE sku='".$sku."'");
			$item_cost = $db->func_escape_string($_POST['item_cost'][$reject_id]);
			$rma_number = $db->func_escape_string($_POST['rma_number'][$reject_id]);
			$shipment_item = array();
			$shipment_item['product_id']  = $product_id;
			$shipment_item['product_sku'] = $sku;
			$shipment_item['unit_price']  = $item_cost;
			$shipment_item['cu_po']  = $rma_number;
			$shipment_item['shipment_id'] = $inv_shipment_id;
			$shipment_item['qty_shipped'] = 1;
			$db->func_array2insert("inv_shipment_items",$shipment_item);
			//$check_item = $db->func_query_first("select * from inv_shipment_items where shipment_id = '".$inv_shipment_id."' AND product_sku = '".$sku."'");
			//if ($check_item['id']) {

			//	$db->func_array2update("inv_shipment_items",$shipment_item,"id = '".$check_item['id']."'");
			//}else {
		//	}
		}
		}
		
		$db->db_exec("update inv_return_shipment_boxes SET status = 'Completed',fb_added = '1'  where id = '$box_id'");
		...
		<div align="center">
			<form method="post" action="">
				...
				<div>
					<table>
						<tr>
							...
							<td>&nbsp;</td>
							<?php if($_GET['return'] != 'shipping_damage' && $_GET['return'] != 'customer_issue_box'):?>
								<td>
									Select Box:
				...
				<div align="center">
					...
					<?php if($_GET['return'] == 'not_tested' || $_GET['return'] == 'customer_issue_box'):?>
						<input type="submit" name="MoveGFS" value="Move to GFS" />
					<?php endif;?>
					
					<?php if($box_detail['status'] != 'Completed' && $_SESSION['edit_received_shipment'] && $_GET['return'] != 'customer_issue_box'):?>
				...
				<div>	
					<?php if($products):?>
						<table id="table1" style="width:100% !important;" cellspacing="0" cellpadding="5px" border="1" align="center" style="border-collapse:collapse;">
							<thead>
								<tr>
									<th width="6%">#</th>
									<th width="6%">Added</th>
									...
									<th width="6%">Claim #</th>
										<?php if($_SESSION['display_refund_amount']){?>
									<th width="6%">Carrier Refund Amount</th>
										<?php } ?>
									<?php } ?>
									<th width="6%">Source</th>
									<?php if($_GET['return'] == 'item_issue') { ?>
									<th width="6%">Issue</th>
									<?php } ?>
							...
							<tbody>
								...
								?>
								<tr class="return_product list_items return_<?php echo $product['return_item_id'];?>" data-sku="<?php echo $product['product_sku'];?>" data-return-id="<?php echo $product['return_item_id'];?>">
									<td>
										<input type="checkbox" class="selection" name="reject_ids[]" value="<?php echo $product['return_item_id'];?>" />
									</td>
									...
									if($product['shipment_id'])
									{
										...
										<input type="hidden" value="<?php echo $product['shipment_id'];?>" name="order_id[<?php echo $product['return_item_id'];?>]" />
										<?php
									}
									else
									{
										...
										<td align="center">
											...
											<input type="hidden" value="<?php echo $product['order_id'];?>" name="order_id[<?php echo $product['return_item_id'];?>]" />
										</td>
										<?php
									}
									?>
									<td align="center">
									    <input  type="hidden" name="rma_number[<?php echo $product['return_item_id']?>]" value="<?php echo $product['rma_number'];?>" />
									...
									<input  type="hidden" name="item_cost[<?php echo $product['return_item_id']?>]" value="<?php echo $product['cost'];?>" />
									...
									<?php if($_GET['return'] == 'item_issue') { ?>
									<td align="center"><?=$db->func_query_first_cell('SELECT item_issue FROM inv_product_issues WHERE product_sku = "'. $product['product_sku'] .'"');?></td>
									<?php } ?>
									
									<?php 
								$rtsItemReason = $db->func_query_first_cell("select name from inv_rj_reasons where id = '".
									$db->func_escape_string($product['reason'])."'");
								?>
									
									
									<td align="center">
										<input type="hidden" value="<?php echo $product['product_sku'];?>" name="sku[<?php echo $product['return_item_id'];?>]" />
