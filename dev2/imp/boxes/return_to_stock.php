...
if((int)$_GET['id'] and $_GET['action'] == 'ignore'){
	$box_id = (int)$_GET['id'];
	$db->db_exec("update  inv_return_shipment_boxes SET ignored = '0' where id = '$box_id'");
	
	$_SESSION['message'] = "Good For Stock Box status is ignored";
	header("Location:boxes/return_to_stock.php");
	exit;
}
...
$_query = "select * from inv_return_shipment_boxes where $where and box_type = 'GFSBox' order by date_added desc";
$count_query = "select count(id) as total from inv_return_shipment_boxes where $where and box_type = 'GFSBox'";

$splitPage   = new splitPageResults($db , $_query , 25 , "return_to_stock.php",$page ,  $count_query);
...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
	 ...
	 <title>Good For Stock boxes</title>
  ...
  <body>
		<div align="center"> 
		 ...
		 <h2 align="center">Good For Stock Boxes</h2>
		...
		<div align="center">
			<a href="<?php echo $host_path;?>boxes/createbox.php?type=GFSBox&return=return_to_stock">Create new Box</a> <br /><br />
		    ...
		    <table class="data" border="1" style="border-collapse:collapse;" width="94%" cellspacing="0" align="center" cellpadding="3">
			   ...
			   <?php foreach($rma_returns as $k => $rma_return):?>
					   ...
					   <td style="width:50px;"><?php echo $k+1;?></td>			   		
			   		   ...
			   		   <td align="center"><?php echo ($rma_return['fb_added'] == 1) ? 'Yes' : 'No';?></td>
                          
                       <td align="center">
                          	<?php if($shipment['ignored'] == 1):?>
                          		<a href="boxes/return_to_stock.php?action=ignore&id=<?php echo $rma_return['id']?>" onclick="if(!confirm('Are you sure?')){ return false; }">Upload Again</a>
                          	<?php endif;?>	
                       </td>
                          
			   		   <td>
			   		       <?php if(($rma_return['status'] == 'Issued' && $_SESSION['edit_pending_shipment']) || $_SESSION['login_as'] == 'admin'):?>	
                              		<a href="<?php echo $host_path;?>boxes/boxes_edit.php?box_id=<?php echo $rma_return['id']?>&return=return_to_stock">Edit</a>
                              		|
                           <?php endif;?>	
                           
                           <?php if($rma_return['status'] == 'Issued' && $_SESSION['qc_shipment']):?>
                              		<a href="<?php echo $host_path;?>boxes/rts_qc.php?box_id=<?php echo $rma_return['id']?>">
                              	 		QC Shipment
                              		</a>
                              		|
                           <?php endif;?>
                           ...
                           <?php if($_SESSION['delete_shipment']):?>
                          		<a href="<?php echo $host_path;?>boxes/deletebox.php?action=delete&box_id=<?php echo $rma_return['id']?>&return=return_to_stock" onclick="if(!confirm('Are you sure ?')){ return false; }">Delete</a>
                           <?php endif;?>
			   		   </td>
			   		</tr>
			   <?php endforeach;?>
