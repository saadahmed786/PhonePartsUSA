...
if($_POST['print']){ 
	foreach ($_POST['reject_ids'] as $reject_id) {
		...
		printLabel($reject_id,$_POST['sku'][$reject_id],$shipment_name,$ntrItemReason,$_POST['package'][$reject_id],$_POST['printer_id'],'','','','');
...
elseif($_POST['MoveRTS']){
	if(count($_POST['reject_ids']) > 0){
		foreach($_POST['reject_ids'] as $reject_id){
			...
			$item_cost = (float)$_POST['product_cost'][$reject_id];
			$from = $db->func_escape_string($_POST['package_number']);
			addItemToBox($reject_id , $_POST[$reject_id] , $_shipment_id , 'RTSBox' , $reason,false,false,$item_cost,$from);
			$db->db_exec("update inv_rejected_shipment_items set deleted =1 where reject_item_id = '".$reject_id."' and rejected_shipment_id = '$shipment_id' ");
		}
		$_SESSION['message'] = 'Selected Item(s) has been successfully moved to RTS';
	}	
	else{
...
$boxes = $db->func_query("select id , package_number from inv_rejected_shipments where id != '$shipment_id' order by date_added DESC");
$shipments = $db->func_query('SELECT * FROM `inv_shipments` WHERE vendor = "'. $shipment_detail['vendor'] .'" AND status = "Pending"');
if ($_POST['status'] == 'Shipped' && $shipment_detail['status'] != 'Shipped') {
				...
				<?php if($shipment_detail['status'] != 'Completed') { ?>
					...
					<form method="post" action="">
						...
						<div>
							<table>
								<tr>
									<?php if (($_SESSION['login_as'] == 'employee' || $_SESSION['login_as'] == 'admin') && $shipment_detail['status'] == 'Pending' || $shipment_detail['status'] == 'Shipped' || $shipment_detail['status'] == 'Received' || $shipment_detail['status'] == 'Completed' ) { ?>
								...
								echo createField("carrier", "carrier" , "select" , $shipment_detail['carrier'] , $carriers, 'onclick="$(\'.carrier\').val($(this).val());" class="carrier" readonly');
										...
										<td required>
											...
											if ($vtx) {
												...
												echo '<input readonly type="text" required class="tracking_number" style="display: none;" name="tracking_number" onkeyup="$(\'.tracking_number\').val($(this).val());" value="' . $shipment_detail['tracking_number'] . '"/>';
													...
													if($shipment_detail['status'] == 'Shipped'){
										...
										<tr>
											...
											<?php if ($_SESSION['login_as'] == 'employee'){ //&& $shipment_detail['status'] == 'Shipped' ) { ?>
												...
												<?php if (!$vtx) { ?>
													<?php if(($_SESSION['login_as'] == 'employee' && $shipment_detail['status'] == 'Pending') && $shipment_detail['status'] != 'Completed'){ ?>
													<td>
														Rejected PO:
								...
								//echo ($shipment_detail['carrier']);
											...
											<div align="center">
												...
												<?php if (!$vtx) { ?>
													...
													<input type="submit" name="MoveRTS" value="Move to RTS" onclick="if(!confirm('Are you sure?')){ return false; }" />&nbsp&nbsp&nbsp&nbsp
													<?php }?>
													<?php } ?>
													<?php if (($_SESSION['login_as'] == 'admin' || ($vtx && $shipment_detail['status'] != 'Shipped')) && $shipment_detail['status'] != 'Pending' && $shipment_detail['status'] != 'Completed') { ?>
														...
														<div>	
															<?php if($products):?>
																<table id="table1" width="80%" cellspacing="0" cellpadding="5px" border="1" align="center" style="border-collapse:collapse; border-left: 0px; border-right:0px;">
																	<thead>
																	<tr data-no="1">
																							<td style="border-left: 0px; border-right:0px;" colspan="11" align="center"><h2 style="padding-top: 10px;">RECEIVED ITEM(s)</h2></td>
																						</tr>
																		<tr>
																			...
																			<th>Date Added</th>
																			<th>Date Updated</th>
																			<?php if (($vtx || $_SESSION['login_as'] == 'admin') && $shipment_detail['status'] == 'Shipped' ) { ?>
																				...
																				<tbody>
																					...
																					<?php if ($product['received'] != $products[($pkey - 1 )]['received'] && $pkey) { ?>
																						...
																						<thead>
																						<tr>
																						<td style="border-left: 0px; border-right:0px;" colspan="11" align="center"><h2 style="padding-top: 10px;">NOT RECEIVED ITEM(s)</h2></td>
																						</tr>
																						<tr>
																						<th>#</th>
																						<th>Shipment Number</th>
																						...
																						<?php } ?>
																						<tr class="list_items">
																							<?php if (($_SESSION['login_as'] == 'employee' && $shipment_detail['status'] == 'Completed' )|| ($_SESSION['login_as'] == 'employee' && $shipment_detail['status'] == 'Shipped' )) { ?>
																								<td>
																								...
																								<?php } else{ ?>
																									...
																									<?php if (($_SESSION['login_as'] == 'employee' || $_SESSION['login_as'] == 'admin') && ($shipment_detail['status'] == 'Shipped' || $shipment_detail['status'] == 'Received' || $shipment_detail['status'] == 'Completed') ) { ?>
																										<td align="center">
																															...
																															<select name="reason[<?php echo $product['reject_item_id']?>]">
																																...
																																<?php if($_SESSION['display_cost']):?>
																																	<td align="center">
																																		...
																																		<input type="hidden" value="<?php echo $product['cost'];?>" name="product_cost[<?php echo $product['reject_item_id'];?>]" />
										...
										<table>
										<tr>
										<?php if ($_SESSION['login_as'] == 'employee' && $shipment_detail['status'] == 'Pending' || $shipment_detail['status'] == 'Shipped' ) { ?>
										<td>Vendor:</td>
										<td readonly>
											<?php
											if ($vtx) {
																																						...
																																						<div align="center">
																																							...
																																							<?php if (($_SESSION['login_as'] == 'employee' && $shipment_detail['status'] != 'Completed' &&  $shipment_detail['status'] != 'Pending') && $_SESSION['login_as'] != 'employee') { ?>
																																								...
																																								<?php if($_SESSION['login_as'] == 'admin' && $shipment_detail['status'] == 'Pending'){?>
																																									...
																																									<input type="submit" name="MoveRTS" value="Move to RTS" onclick="if(!confirm('Are you sure?')){ return false; }" />&nbsp&nbsp&nbsp&nbsp
																																									<?php } ?>
																																									<?php if ($_SESSION['login_as'] == 'employee' && $shipment_detail['status'] != 'Completed' && $shipment_detail['status'] != 'Shipped' ) { ?>
