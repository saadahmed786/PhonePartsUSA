...
if($_POST['save']) { 
	...
	foreach($reasons as $id => $reason){
		...
		printLabel($item['reject_item_id'], $item['product_sku'], (($item['vendor_po_id'])? $item['vendor_po_id']: $shipmentName), $db->func_query_first_cell( "select name from inv_rj_reasons where id = '". $item['reject_reason'] ."'" ), $item['shipment_id'], $_POST['printerid']);
...

	{
		...
		$to = $db->func_query_first_cell( "select package_number from inv_rejected_shipments where id = '".$_x."'" );



		$from = $db->func_query_first_cell('SELECT package_number from inv_shipments WHERE id="'.$_GET['id'].'"');



		logRejectItem($_val,' items added from Shipment <a href="'.$host_path.'view_shipment.php?shipment_id='.$_GET['id'].'">' . $from . '</a> by ','',$to);
	...
	if ($_POST['ntr_reason']) {
		...
		foreach($reasons as $id => $reason){
			...
			printLabel($item['return_item_id'], $item['product_sku'], $shipmentName, $item['reason'], (($item['vendor_po_id'])? $item['vendor_po_id']: 'Stock'), $_POST['printerid']);
...
$printers = array(
	...
	<body>
		...
		<div align="center">
			...
			<form method="post" action="" enctype="multipart/form-data">
				...
				<p>
					...
					<input class="selection" type="checkbox" id="select_all_reason" onchange="checkboxReason()">Select Reason For All Items</input>

					<?php $reasons = $db->func_query("SELECT * FROM inv_rj_reasons group by name")?>

					<select onchange="changeReasons()" style="display: none;" id="generic_reasons">
									...
									<?php for($j=0;$j<$product['qty_rejected'];$j++): ?>
										...
										<tr>
											...
											<td align="center">
												...
												<select class="generic" required="" name="reason[<?php echo $product['id']?>][<?php echo $j?>]">
		...
		<script type="text/javascript">
			...
			function checkboxReason(){

				if($('#select_all_reason').prop("checked")==true){

					$('#generic_reasons').show();

				} else if($('#select_all_reason').prop("checked")==false) {

					$('#generic_reasons').hide();
			...
			function changeReasons(){
				...
				var reason = $('#generic_reasons').val();
