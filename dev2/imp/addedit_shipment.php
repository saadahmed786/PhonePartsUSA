...
$vendors = $db->func_query("select id , name as value from inv_users where group_id = 1 order by lower(name)");
$carriers = array(
...
if($_POST['save'] || $_POST['IssueShipment'] || $_POST['ReceiveIt']){
	...
	$shipment['customs_duties'] = (float)$_POST['customs_duties'];
	...
	if($shipment_id){
		...
		if($checkExist){
			...
			addComment('shipment',array('id' => $shipment_id, 'comment' => $status_comment));
	...
	if($shipment_id) {
		...
		foreach($_POST['products'] as $product) {
			...
			$shipment_item['refurb_cost'] = $product['refurb_cost'];
			if($_SESSION['display_cost']){
		...
		foreach($_POST['new_products'] as $product){
			...
			// echo $shipment_item['product_sku'];exit;
	...
	if($_POST['ReceiveIt'] && $_SESSION['edit_received_shipment']){
		...
		$vendor_po_id = $db->func_query_first_cell("SELECT vendor_po_id from inv_shipments where id='".$shipment_id."'");

		if($vendor_po_id)
		{
	$details = $db->func_query_first("SELECT * FROM inv_vendor_po WHERE vendor_po_id = '$vendor_po_id'");;
			$vpo_id = (int)($details['id']);
	...
	$shipment_data = $db->func_query_first("SELECT a.ex_rate, SUM(b.qty_received) as qty_received,sum(b.unit_price * b.qty_received) as unit_price,(select sum(c.shipping_cost) from inv_shipments c where c.vendor_po_id=a.vendor_po_id) as shipping_cost from inv_shipments a,inv_shipment_items b where a.id=b.shipment_id and a.vendor_po_id='".$vendor_po_id."'");
		...
		$uppdate_skus_vendor =  $db->func_query("SELECT * FROM inv_shipment_items WHERE shipment_id='$shipment_id' and is_new=1 and is_trello_updated=0 ");
	...
	// update vendor
		...
		$update_skus =  $db->func_query("SELECT * FROM inv_shipment_items WHERE shipment_id='$shipment_id'");
		foreach($update_skus as $update_sku)
		{
			$check = $db->func_query_first_cell("SELECT vendor from inv_product_vendors where product_sku='".$db->func_escape_string($update_sku['product_sku'])."' and vendor='".(int)$_POST['vendor']."'");
			if((int)$_POST['vendor']!=0 && (int)$_POST['vendor']!=97 && (int)$_POST['vendor']!=46 )
			{
				$check = $db->func_query_first_cell("SELECT vendor from inv_product_vendors where product_sku='".$db->func_escape_string($update_sku['product_sku'])."' and vendor='".(int)$_POST['vendor']."'");
				$db->db_exec("UPDATE oc_product SET vendor='".($db->func_query_first_cell("select lower(name) from inv_users where id='".(int)$_POST['vendor']."' order by 1 desc"))."' where model='".$update_sku['product_sku']."'");
				if(!$check)
				{


			//$db->db_exec("DELETE FROM inv_product_vendors where product_sku='".$update_sku['product_sku']."'");
					$db->db_exec("INSERT INTO inv_product_vendors SET vendor='".(int)$_POST['vendor']."',product_sku='".$db->func_escape_string($update_sku['product_sku'])."'");
				}
			}
...
if($shipment_id){
	...
	foreach($shipment_items as $product_id => $shipment_item){
		$list[$product_id] = array(	"cu_po"=>$shipment_item['cu_po'],
			...
			"refurb_cost" => $shipment_item['refurb_cost'],		
...
//$parameters[] = "dir=$dir";
$sort = @$_GET['sort'];
if(!$sort || !in_array($sort,array("model","quantity"))){
	$sort = 'model';
}
//$parameters[] = "sort=$sort";
if(in_array($sort,array("model","quantity"))){
	$dsort = "p.$sort";
}
else{
	$dsort = "p.$sort";
}
$inv_query   = "select p.product_id , p.model, p.quantity+p.prefill-((SELECT COALESCE(sum(b.product_qty) - sum(b.picked_quantity),0) FROM inv_orders_items b where b.ostatus in ('processed','on hold') and b.product_sku=p.model) + (SELECT COALESCE(sum(b.picked_quantity) - sum(b.packed_quantity),0) FROM inv_orders_items b where b.ostatus in ('processed','unshipped','on hold') and b.is_picked=1 and b.opacked=0 and b.product_sku=p.model) + (SELECT COALESCE(sum(b.packed_quantity),0) FROM inv_orders_items b where b.ostatus in ('processed','unshipped','on hold') and b.is_packed=1 and b.product_sku=p.model) ) as quantity, p.status, p.mps , p.image , pd.name,p.prefill from 
oc_product p inner join oc_product_description pd on (p.product_id = pd.product_id) 
where p.product_id in ($product_ids) order by $dsort $dir";
// echo $inv_query;exit;
...
<head>
	...
	<script type="text/javascript">
		...
		function removeFromList(product_id , is_new,sku){
			if(!confirm('Are you sure want to remove the item from the list and shipment?'))
			{
				return false;
			}
			jQuery.ajax({
				url: 'inc/ajax.php?action=removeFromList&product_id='+product_id+'&is_new='+is_new+'&shipment_id=<?php echo $_GET['shipment_id'];?>&sku='+sku,
	...
	<div align="center">
		<form id="frm" method="post" action="">
			...
			<table width="90%" cellpadding="5">
				<tr>
					<td>Vendor:</td>
					<td><?php if($_SESSION['display_vendor'])
						{
							if($shipment_detail['is_lbb']==1)
							{ 
								echo createField("servicer", "servicer" , "select" , $shipment_detail['servicer'] , $servicers, 'required=""');
							}
							else
							{
								echo createField("vendor", "vendor" , "select" , $shipment_detail['vendor'] , $vendors, 'required=""');
							}
						...
						?></td>
						<td>Ex. Rate:</td>
						<td><?php if($_SESSION['display_exrate'])
							{
								?>
						...
						<td>Shipment No.:</td>
						<td><input type="text" name="package_number" value="<?php echo $shipment_detail['package_number'];?>" required /></td>

						<td colspan="2"><input onclick="rfCheck();" id="rf_checkbox" type="checkbox" name="refurb_check"> Contains Refurbished Items</td>
					</tr>
					<tr>
						<td>Carrier:</td>
						<td><?php echo createField("carrier", "carrier" , "select" , $shipment_detail['carrier'] , $carriers);?></td>
						<td>Tracking #:</td>
						<td><input type="text" name="tracking_number" value="<?php echo $shipment_detail['tracking_number'];?>"  /></td>
						<td>Shipping Cost:</td>
						<td><?php if($_SESSION['display_cost'])
							{
								?>
						...
						<td><input type="text" name="customs_duties" value="<?php echo $shipment_detail['customs_duties'];?>"  /></td>
					...
					<tr>
						...
						if($_SESSION['display_vendor'])
						{
							?>
							<td>ETA:</td>
							<td><input type="date" name="eta" value="<?php echo ($shipment_detail['eta']?date('Y-m-d',strtotime($shipment_detail['eta'])) :'');?>" ></td>
							<?php
						}
						?>
					</tr>
					<tr>
						<td colspan="5" align="right">
							<input type="submit" class="button button-info" style="font-size:15px;padding:12px 27px;" name="save" value="Save" />
						</td>
						<td colspan="3" align="right">
							...
							<?php } ?>
							<?php
							if($_SESSION['login_as']=='admin' || $_SESSION['shipment_invoice'])
							{
								?>
								<a href="shipment_invoice.php?shipment_id=<?= $shipment_id; ?>&action=view" target="_blank" class="button">Download PDF</a>
								<?php
							}
							?>
						</td>
				...
				<div>	
					<?php if($products):?>
						<table width="90%" cellspacing="0" cellpadding="5px" border="1" align="center" style="border-collapse:collapse;">
							<thead>
								<tr>
									...
									<th align="center">New Item</th>
									<th>Image</th>
									<th><a href="addedit_shipment.php?sort=model&dir=<?php echo $ddir;?>&<?=$parameters;?>&page=1">SKU</a></th>
									<th>Name</th>
									<th><a href="addedit_shipment.php?<?=$parameters;?>&sort=quantity&dir=<?php echo $ddir;?>&page=1">In Stock</a></th>
									<th>Ref #</th>
									<th>Qty Shipped</th>
									<th>Qty Received</th>
									<?php
							...
							<tbody>
								<?php $i = $splitPage->display_i_count(); $tot = 0;?>
								<?php foreach($products as $product){
									$tot = $tot + 1;?>
									<?php if($list[$product['product_id']]['qty_received'] == $list[$product['product_id']]['qty']){ ?>
									<tr class="row_<?php echo $product['product_id'];?> <?php echo ($list[$product['product_id']]['rejected'] == 1)? 'rejected': ''; ?>" >
										<?php } else if (($list[$product['product_id']]['qty_received'] != $list[$product['product_id']]['qty']) && $list[$product['product_id']]['qty_received'] ){ ?>
										<tr style="background-color: #EEE8AA" class="row_<?php echo $product['product_id'];?> <?php echo ($list[$product['product_id']]['rejected'] == 1)? 'rejected': ''; ?>" >
											<?php } else { ?>
											<tr style="background-color: #FFB6C1" class="row_<?php echo $product['product_id'];?> <?php echo ($list[$product['product_id']]['rejected'] == 1)? 'rejected': ''; ?>" >
												<?php } ?>
												...
												?>
												<?php if($_SESSION['display_cost']):?>
													<?php $cost_queries = $db->func_query("SELECT DISTINCT  DATE(cost_date) as cost_date,raw_cost,shipping_fee,ex_rate FROM inv_product_costs WHERE sku='".$product['model']."' ORDER BY id DESC LIMIT 3"); ?>
													<td align="center"> <?php
														if($cost_queries)
														{
															$kk = 0;
														...
														<?php $true_cost_without = $true_cost_without + ($list[$product['product_id']]['price']* $list[$product['product_id']]['qty_received']); ?>
											<?php //$totalPrice += $previous_costx * $list[$product['product_id']]['qty'];
											$totalPrice += $list[$product['product_id']]['price'] * $list[$product['product_id']]['qty']; ?>
											<?php //$totalPriceR += $previous_costx * $list[$product['product_id']]['qty_received'];
											$totalPriceR += $list[$product['product_id']]['price'] * $list[$product['product_id']]['qty_received']; ?>
											<?php $totalItems += $list[$product['product_id']]['qty']; ?>
											<?php $totalItemsR += $list[$product['product_id']]['qty_received']; ?>
											<?php } ?>
										</td>
										<td align="center"><input style="width: 50px;" type="text" name="products[<?php echo $i?>][price]" value="<?php echo $list[$product['product_id']]['price']?>" /><br>
											<div style="display: none;" class="rf_cost" >
												<input style="width: 60px; background-color:#D3D3D3;" align="center" value="<?php echo $list[$product['product_id']]['refurb_cost']?>" placeholder="Item Cost" type="text" name="products[<?php echo $i?>][refurb_cost]" />
											</div></td>
											<?php $cost_difference =  (float)$list[$product['product_id']]['price'] - (float)$previous_costx ; ?>
											<td align="center"><span class="tag <?php echo ($cost_difference<0?'green-bg':($cost_difference>0?'red-bg':''));?>"><?php echo ($cost_difference>0?'+':'');?><?php echo number_format($cost_difference,2);?></span></td>
										<?php endif;?>		
										<td align="center">
											<?php if ($list[$product['product_id']]['rejected'] == 0) { ?>
											<a href="javascript://" onclick="removeFromList(<?php echo $product['product_id'];?>,0,'<?php echo $product['model'];?>');">X</a>
											<?php } ?>
										</td>
									</tr>
									<?php $i++;?>
									...
									<?php foreach($newlist as $new_item_id => $newItem){
										$tot = $tot+1;?>
										<?php if($newItem['qty_received'] == $newItem['qty']){ ?>
										<tr id="row_<?php echo $new_item_id;?>">
											...
											<tr style="background-color: #FFB6C1" id="row_<?php echo $new_item_id;?>">
												<?php } else if (($newItem['qty_received'] != $newItem['qty']) && $newItem['qty_received'] )  { ?>
												<tr style="background-color: #EEE8AA" id="row_<?php echo $new_item_id;?>">
													...
													<?php if($_SESSION['display_cost']):?>
														...
														//$c = $previous_costx;
															...
															?>
															<?php 
															$true_cost_without = $true_cost_without + ($newItem['price']* $newItem['qty_received']);
											//$totalPrice += $previous_costx * $newItem['qty'];
															$totalPrice += $newItem['price'] * $newItem['qty']; ?>
											<?php //$totalPriceR += $previous_costx * $newItem['qty_received'];
											$totalPriceR += $newItem['price'] * $newItem['qty_received']; ?>
									...
									<td></td>
									<td align="center">
										<a href="javascript://" onclick="removeFromList(<?php echo $new_item_id;?> , '1','<?php echo $newItem['sku'];?>');">X</a>
									</td>
								</tr>
								<?php $i++; ?>
								<?php } ?>
						...
						<div align="center" style=";margin-top:5px;">
							...
							if(($shipment_detail['status']=='Issued' || $shipment_detail['Received']) && $_SESSION['qc_shipment'])
							{
								?>
								<a href="<?php echo $host_path;?>popupfiles/prefill.php?shipment_id=<?php echo $shipment_id;?>" class="button button-danger fancyboxX4 fancybox.iframe" >Pre-Fill Qty</a>
								<?php
							}
							?>
							...
							<?php if($shipment_id AND $shipment_detail['status'] == 'Issued' && $_SESSION['edit_received_shipment']):?>
								...
								<button type="submit" style="margin-left:70%" name="ReceiveIt" class="button" value="ReceiveIt" onclick="if(!confirm('Are you sure?')){ return false; }">
									Save And Receive It
								</button>
		...
		<script>
			...
			$('#new_ordered_cost').text('<?php echo number_format($totalPrice, 2); ?>');
			$('#new_received_cost').text('<?php echo number_format($totalPriceR, 2); ?>');
			$('#new_received_shipping').text('<?php echo $shipping_cost; ?>');
			$('#new_received_total_vc').text('<?php echo $rtt; ?>');
			$('#new_received_exrate').text('<?php echo number_format($ex_rate, 2);; ?>');
			$('#new_received_total').text('$<?php echo number_format($rter, 2); ?>');
		...
		<script>
			function ValidateIt()
			{
				...
				else if($('input[name=tracking_number]').val().length<=4 && $('select[name=carrier]').val()!='In House' )
				{
			...
			function rfCheck()
			{
				if($('#rf_checkbox'). prop("checked") == true){
					$('.rf_cost').show();
