...
class AmazonAPI {
	...
	public function __construct($market_place_id , $aws_access_key = '' , $aws_secret_key = '') {
		$config = array (
		...
		if($aws_secret_key){
			...
			$this->aws_secret_key = AWS_SECRET_ACCESS_KEY;
		}
		
		$this->service = new MarketplaceWebService_Client ( $this->aws_access_key, $this->aws_secret_key, $config, APPLICATION_NAME, APPLICATION_VERSION );
		
		$this->market_place_id = $market_place_id;
		$this->marketplaceIdArray = array (
			"Id" => array ( $market_place_id ) 
		);
	}
public function RefundOrderXml($data, $merchant_id) {
...
<Component>
<Type>Principal</Type>
<Amount currency="USD">'.round($product['price'],2).'</Amount>
</Component>
<Component>
<Type>Shipping</Type>
<Amount currency="USD">'.round($data['shipping_fee'],2).'</Amount>
</Component>
<Component>
<Type>Tax</Type>
...
return $xml;
	...
	public function InventoryXml($products, $merchant_id, $OperationType = 'Update') {
		$xml = '<?xml version="1.0" encoding="utf-8" ?>
	...
	public function getProductPrice($merchant_id, $product_sku , $aws_access_key = '' , $aws_secret_key = '') {
		if($aws_access_key){
			$this->aws_access_key = $aws_access_key;
		}
		else{
		...
		$service = new MarketplaceWebServiceProducts_Client ( $this->aws_access_key, $this->aws_secret_key, APPLICATION_NAME, APPLICATION_VERSION, $config );
