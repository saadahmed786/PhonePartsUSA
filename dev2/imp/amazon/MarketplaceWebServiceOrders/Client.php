...
class MarketplaceWebServiceOrders_Client implements MarketplaceWebServiceOrders_Interface
{
	...
	private function _reportAnyErrors($responseBody, $status, $responseHeaderMetadata, Exception $e =  null)
	{
		...
		if (!is_null($responseBody) && strpos($responseBody, '<') === 0) {
			...
			$responseBody, $errorMatcherThree)) {
				...
				require_once ('MarketplaceWebServiceOrders/Exception.php');
				$ex = new MarketplaceWebServiceOrders_Exception(array('Message' => 'Internal Error', 'StatusCode' => $status, 'XML' => $responseBody, 'ResponseHeaderMetadata' => $responseHeaderMetadata));
			}
		} else {
			require_once ('MarketplaceWebServiceOrders/Exception.php');
			$ex = new MarketplaceWebServiceOrders_Exception(array('Message' => 'Internal Error', 'StatusCode' => $status, 'XML' => $responseBody, 'ResponseHeaderMetadata' => $responseHeaderMetadata));
	...
	{
		return gmdate("Y-m-d\TH:i:s.\\0\\0\\0\\Z", ( time() +  (1*60*60) ) );
	}
