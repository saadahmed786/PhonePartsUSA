...
if ($_GET['submit'] == 'Search') {
	...
	foreach ($_GET as $key => $value) {
		if ($value) {
			if ($_SESSION['user_id'] == 0 && $key == 'vendor') {
				$filter[] = $key . ' = "' . $value . '"';
			} else {
		...
		<table width="90%" cellpadding="10" border="1"  align="center">
			...
			<tbody>
				...
				<tr>
					...
					<td><?= linkToVPO($row['id'], $host_path, 'View'); ?> | <a href="vendor_po.php?action=delete&vpo_id=<?php echo $row['id'];?>&vpo_name=<?php echo $row['vendor_po_id'];?>" onclick="if(!confirm('Are you sure ?')){ return false; }">Delete</a></td>
				</tr>
				<?php } ?>
			</tbody>
		</table>
