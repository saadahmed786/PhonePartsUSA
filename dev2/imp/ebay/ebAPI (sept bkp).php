	...
	public function ManageOrder($orderObject) {
		...
		if ($orderObject) {
			...
			if (!$orderExist) {
				...
				$db->db_exec("insert into inv_orders(order_id,order_date,order_price,paid_price,order_status,email,store_type,record_number,customer_name,dateofmodification)

                             values ('$OrderID','" . $order_date . "','$OrderTotal','$OrderTotal','$OrderStatus','$Email','ebay','$SellingManagerSalesRecordNumber','" . $CustName . "','" . date('Y-m-d H:i:s') . "')");
				...
				foreach ($orderObject->TransactionArray->Transaction as $transaction) {
					...
					$db->db_exec("insert into inv_orders_items(order_id,order_item_id,product_sku,product_unit,product_price,product_qty,product_true_cost,dateofmodification)

                                  values ('$OrderID','$transaction_id','$product_sku','".$product_unit."','$product_price','$qty','$true_cost','" . date('Y-m-d H:i:s') . "')");
	...
	public function getMyeBaySelling($usertoken, $page = 1, $siteId = 0) {
		...
		if ($responseObj) {
			...
			if (count($errors) > 0) {
				...
				mail("vipin.garg12@gmail.com", "IMP error", $errorMsg);
