...
//Old Paypal check commented Gohar 
...
// Parse results
	...
	if (isset($response['RESULT']) && $response['RESULT'] == 0) {
		...
		foreach (explode(",", $_POST['items']) as $item) {
			...
			$data['return_item_id'] = $return_info['id'];
		...
		$log = 'Rma # '. linkToRma($_POST['return_id']) .' Refund has been issued of amount $'.number_format((float)$_POST['amount'],2) . ' for '. rtrim($logsku, ',');
		actionLog($log);
	} else {
...
function paypal_refund() {
	...
	if (!$json['error']) {
        ...
        //
          ...
          if (!$query) {
          	$another_query=$db->func_query_first("SELECT transaction_id FROM inv_transactions WHERE order_id='".$_POST['order_id']."' AND is_mapped=1 and amount >0 limit 1 ");
          	if(!$another_query)
          	{
          	$json['error'] = 'Error: Order data not found';
          	...
          	{
          		$transactionID = urlencode($another_query['transaction_id']);
