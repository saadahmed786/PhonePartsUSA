...
$_query = "Select p.product_id,p.sku, p.quantity, p.price,p.retail_price ,p.is_kit,p.vendor,p.classification_id,p.sub_classification_id,p.status,p.sale_price
		   from oc_product p 
		    where p.is_main_sku = 1 limit $start,$limit";
            
$products = $db->func_query($_query);
if(isset($_POST['file_name']) && trim($_POST['file_name']!=''))
{
    $filename = changeImageName($_POST['file_name']).'.csv';
}
else
{
