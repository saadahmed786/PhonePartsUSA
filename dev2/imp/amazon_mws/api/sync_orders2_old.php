...
$merchantInfo = $db->func_query_first("Select * from amazon_credential where id=1");
    if(!@$merchantInfo){
    ...
    $majorLastDate = '2016-01-13 20:32:00';
...
function getAmazonOrders() {
    ...
    try {
        $amz = new AmazonOrderList("myStore2"); //store name matches the array key in the config file
...
function getOrderLineItems($order_id)
{
   // require('../includes/classes.php');
    global $db;
     try {
        $amz = new AmazonOrderItemList("myStore2"); //store name matches the array key in the config file
       // $amz->setLimits('Modified', $startDate); //accepts either specific timestamps or relative times 
       // $amz->setFulfillmentChannelFilter("MFN"); //no Amazon-fulfilled orders
