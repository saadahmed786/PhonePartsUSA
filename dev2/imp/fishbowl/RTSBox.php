...
$script = $db->func_query_first("select * from scripts where name != 'GFSBox' and status = 1");
...
$db->db_exec("update scripts SET status = 1 , last_time = '".date('Y-m-d H:i:s')."' where name = 'GFSBox'");
...
$db->db_exec("update scripts SET status = 0 where name = 'GFSBox'");
