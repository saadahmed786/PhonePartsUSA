...
$customers2 = $cache->get('sales_dashboard.customers2.'.$cache_data);
  ...
  // $customers2 = $db->func_query("SELECT b.id, a.id,a.firstname,a.lastname,a.email,(select avg(d.total) from inv_customer_data_summary d where d.email=a.email and d.type='order') as avg_weekly_total,max(b.date) as last_ordered from inv_customers a, inv_customer_data_summary b where trim(LOWER(a.email))=trim(lower(b.email)) and b.type='order' and $sql_condition and yearweek(b.date,3)='".$last2Week."' and b.email not in (select c.email from inv_customer_data_summary c where a.email=c.email and yearweek(c.date,3) in ('".$currentWeek."','".$lastWeekNumber."'))   group by a.email");
     ...
     $customers2 = $db->func_query("SELECT a.firstname,a.lastname,a.email,b.total,a.account_potential as avg_weekly_total,a.last_contacted,a.city,a.state,a.longitude,a.latitude from inv_customers a, inv_customer_data_summary b where trim(LOWER(a.email))=trim(lower(b.email)) and b.type='order' and $sql_condition and yearweek(b.date,3)='".$lastWeekNumber."' and a.email not in (select c.email from inv_customer_data_summary c where a.email=c.email and yearweek(c.date,3)='".$currentWeek."') group by a.email");
...
$customers3 = $db->func_query("SELECT b.id, a.id,a.firstname,a.lastname,a.email,a.account_potential as avg_weekly_total,max(b.date) as last_ordered,a.company,a.city,a.state,a.last_contacted,a.longitude,a.latitude from inv_customers a, inv_customer_data_summary b where trim(LOWER(a.email))=trim(lower(b.email)) and b.type='order' and $sql_condition  and b.email not in (select c.email from inv_customer_data_summary c where a.email=c.email and yearweek(c.date,3) in ('".$currentWeek."','".$lastWeekNumber."'))   group by a.email");
