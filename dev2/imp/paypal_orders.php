...
if(isset($_POST['action']) && $_POST['action']=='mapbox')
{
	...
	$check_order = $db->func_query_first("SELECT order_id, store_type, paid_price, order_price FROM inv_orders where order_id='".$order_id."' ");
	...
	else
	{
		...
		if($is_multi==0)
		{
			...
			// if ($check_order['store_type'] == 'po_business') {
				$db->db_exec("UPDATE inv_orders SET paid_price=$paid_price, payment_detail_1='$transaction_id' WHERE order_id='$ref_id'");
			// }
			...
			// if ($check_order['store_type'] == 'po_business') {
				$db->db_exec("UPDATE inv_orders SET paid_price=$paid_price, payment_detail_2='$transaction_id' WHERE order_id='$ref_id'");
			// }
