...
if ($_POST['update_billing_address']) {
	if ($_POST['address_from_order'] == '1') {
		$order_id = $_POST['address_order_id'];
		$update_array = array ();
		$update_array['bill_address1'] = $_POST['bill_address1'];
		$update_array['bill_address2'] = $_POST['bill_address2'];
		$update_array['billing_company'] = $_POST['bill_company'];
		$update_array['bill_city'] = $_POST['bill_city'];
		$update_array['bill_zip'] = $_POST['bill_zip'];
		$update_array['bill_country'] = $_POST['bill_country'];
		$update_array['bill_state'] = $_POST['bill_state'];
		$db->func_array2update("inv_orders_details",$update_array,"order_id = '$order_id'");
		$_SESSION['message'] = 'Address Changed';
		header("Location: $host_path/customer_profile.php?id=".$_GET['id']."&email=".$_GET['email']);
		exit;
		...
		$po_id = $_POST['po_address_id'];
		$update_array = array ();
		$update_array['address1'] = $_POST['bill_address1'];
		$update_array['address2'] = $_POST['bill_address2'];
		$update_array['company_name'] = $_POST['bill_company'];
		$update_array['city'] = $_POST['bill_city'];
		$update_array['zip'] = $_POST['bill_zip'];
		$update_array['state'] = $_POST['bill_state'];
		$db->func_array2update("inv_po_customers",$update_array,"id = '$po_id'");
		$_SESSION['message'] = 'Address Changed';
		header("Location: $host_path/customer_profile.php?id=".$_GET['id']."&email=".$_GET['email']);
		exit;
		...
		$check = $db->func_query_first_cell("SELECT address_id FROM oc_buyback WHERE shipment_number ='".$lbb_shipment."'");
		if ($check <= 0) {
			$update_array['address_1'] = $_POST['bill_address1'];
			$update_array['city'] = $_POST['bill_city'];
			$update_array['postcode'] = $_POST['bill_zip'];
			$state = $db->func_query_first_cell ("SELECT zone_id FROM oc_zone WHERE lower(name)  LIKE '%".strtolower($_POST['bill_state'])."%'");
			$update_array['zone_id'] = $state;
			$db->func_array2update("oc_buyback",$update_array,"shipment_number = '$lbb_shipment'");
			$_SESSION['message'] = 'Address Changed';
		header("Location: $host_path/customer_profile.php?id=".$_GET['id']."&email=".$_GET['email']);
		exit;
			...
			$update_array['address_1'] = $_POST['bill_address1'];
			$update_array['address_2'] = $_POST['bill_address2'];
			$update_array['city'] = $_POST['bill_city'];
			$update_array['postcode'] = $_POST['bill_zip'];
			$state = $db->func_query_first_cell ("SELECT zone_id FROM oc_zone WHERE lower(name)  LIKE '%".strtolower($_POST['bill_state'])."%'");
			$update_array['zone_id'] = $state;
...
$total_debit = 0.00;
$total_credit = 0.00;
$total3month_credit = 0.00;
$total3month_debit = 0.00;
$total6month_credit = 0.00;
$total6month_debit = 0.00;
$k = 0;
$temp_order = array();
foreach($orders as $order)
{
...
foreach($temp_order as $i => $order) { 
	...
	if ($orderDate > $orderDate3Month) {
		$total3month_credit+=$credit;
		$total3month_debit+=$debit;
	}

	if ($orderDate > $orderDate6Month) {
		$total6month_credit+=$credit;
		$total6month_debit+=$debit;
	}
	$payStatus = 0;
	...
	if ($order_total > $order['paid_price'] && $payStatus) {
	...
	if($_SESSION['display_cost']) {
		...
		$shipping_charges = round($db->func_query_first_cell('SELECT SUM(`value`) FROM `oc_order_total` WHERE cast(`order_id` as char(50)) = "'. $order['order_id'] .'" AND `code` = "shipping"'),2);
	...
	<div class="div-fixed">
		...
		<div align="center">
			<div class="tabMenu" >
				...
				<input type="button" class="toogleTab" data-tab="tabOrders" value="Orders History">
				...
				<form method="post" action="" enctype="multipart/form-data">
					<div id="tabCDetails" class="makeTabs">
						<table width="70%" border="1" cellpadding="10" cellspacing="0" style="border-collapse:collapse;">
							...
							<tr>
								...
								<td>
									...
									<?= ($_SESSION['login_as'] == 'admin')? '<a href="javascript:void(0)" title="Merge" onclick="$(\'.mergeEmail\').toggle();">Merge Email</a>': '';?>
								</td>
							...
							<tr>
								...
								<td>
									Sale: <b>$<?php echo number_format($total_debit,2);?> / $<?php echo number_format($total_credit,2);?> </b>
									<br>
									Returns: <b>$<?php echo $returns_total; ?></b> <span style="margin-left: 30px;"> Return Percent: <b><?php echo number_format((($returns_total / $total_debit) * 100), 2); ?>%</b> </span>
									<hr>
									Sale: <b>$<?php echo number_format($total6month_debit,2);?> / $<?php echo number_format($total6month_credit,2);?> </b>
									<br>
									Returns: <b>$<?php echo $returns6_total; ?></b> <span style="margin-left: 30px;"> Return Percent: <b><?php echo number_format((($returns6_total / $total6month_debit) * 100), 2); ?>%</b> </span>
									<hr>
									Sale: <b>$<?php echo number_format($total3month_debit,2);?> / $<?php echo number_format($total3month_credit,2);?> </b>
									<br>
									Returns: <b>$<?php echo $returns3_total; ?></b> <span style="margin-left: 30px;"> Return Percent: <b><?php echo number_format((($returns3_total / $total3month_debit) * 100), 2); ?>%</b> </span>
								</td>
								<?php } else { ?>
								<td>
								</td><td></td>
						...
						<table width="70%" border="1" cellpadding="10" cellspacing="0" style="border-collapse:collapse;">
							<tr>
								<?php if ($customer_details['type'] == 'po' || $customer_details['type'] == 'lbb') { ?>
								<input type="hidden" name="address_from_order" value="0">
								<input type="hidden" name="address_customer_id" value="<?php echo $customer_id; ?>">
								<?php if ($customer_details['type'] == 'po'){ ?>
								<input type="hidden" name="is_po_address" value="1">
								<input type="hidden" name="po_address_id" value="<?php echo $customer_details['id']; ?>">
								<td>
									<table width="100%" border="0" align="left">
										...
										<tr>
											<th>Name: </th>
											<td>
												<input type="text" name="shipping_firstname" readonly="" size="15" value="<?php echo $customer_details['firstname']; ?>" /> 
												<input type="text" name="shipping_lastname" readonly size="15" value="<?php echo $customer_details['lastname']; ?>" />  
											</td>
											 <tr>
												<th>Company</th>
												<td><input type="text" name="shipping_company" size="15" value="<?php echo $customer_details['company_name']; ?>" /></td>
											</tr>
											<tr>
												<th>Address : </th>
												<td>
													<input type="text" name="shipping_address1" size="15" value="<?php echo $customer_details['address1'];?>" /> 
													<input type="text" name="shipping_address2" size="15" value="<?php echo $customer_details['address2'];?>" /> 
												</td>
											</tr>
											<tr>
												<th>City : </th>
												<td>
													<input type="text" name="shipping_city" size="15" value="<?php echo $customer_details['city']; ?>" /> 
												</td>
											</tr>
											<tr>
												<th>State : </th>
												<td>
													<input type="text" name="shipping_state" size="15" value="<?php echo $customer_details['state']; ?>" /> 
												</td>
											</tr>
											<tr>
												<th>Country : </th>
												<td>
													<input type="text" name="shipping_country" size="15" value="<?php echo $customer_details['country']; ?>" /> 
												</td>
											</tr>
											<tr>
												<th>Zip : </th>
												<td><input type="text" name="shipping_zip" size="15" value="<?php echo $customer_details['zip']; ?>" /> 
												</td>
											</tr>
											<tr><td colspan="2" align="center"><input type="submit" name="update_shipping_address" class="submit" value="Update Address"></td></tr>
										</table>
									</td>
									<td>
										<table width="100%" border="0" align="right">
											...
											<tr>
												<th>Name : </th>
												<td>
													<input type="text" name="bill_firstname" readonly size="15" value="<?php echo $customer_details['firstname']; ?>" /> 
													<input type="text" name="bill_lastname" readonly size="15" value="<?php echo $customer_details['lastname']; ?>" />  
												</td>
												 <tr>
													<th>Company</th>
													<td><input type="text" name="bill_company" size="15" value="<?php echo $customer_details['company_name']; ?>" /></td>
												</tr>
												<tr>
													<th>Address : </th>
													<td>
														<input type="text" name="bill_address1" size="15" value="<?php echo $customer_details['address1'];?>" /> 
														<input type="text" name="bill_address2" size="15" value="<?php echo $customer_details['address2'];?>" /> 
													</td>
												</tr>
												<tr>
													<th>City : </th>
													<td>
														<input type="text" name="bill_city" size="15" value="<?php echo $customer_details['city']; ?>" /> 
													</td>
												</tr>
												<tr>
													<th>State : </th>
													<td>
														<input type="text" name="bill_state" size="15" value="<?php echo $customer_details['state']; ?>" /> 
													</td>
												</tr>
												<tr>
													<th>Country : </th>
													<td>
														<input type="text" name="bill_country" size="15" value="<?php echo $customer_details['country']; ?>" /> 
													</td>
												</tr>
												<tr>
													<th>Zip : </th>
													<td><input type="text" name="bill_zip" size="15" value="<?php echo $customer_details['zip']; ?>" /> 
													</td>
												</tr>
												<tr><td colspan="2" align="center"><input type="submit" class="submit" name="update_billing_address" value="Update Address"></td></tr>
											</table>
										</td>
										<?php } else { ?>
										<input type="hidden" name="is_lbb_address" value="1">
										<input type="hidden" name="lbb_address_id" value="<?php echo $customer_details['shipment_number']; ?>">
										<td>
									<table width="100%" border="0" align="left">
										<h3 align="center">Default Shipping Address </h3>
										<tr>
											<th>Name: </th>
											<td>
												<input type="text" name="shipping_firstname" size="15" value="<?php echo $customer_details['firstname']; ?>" /> 
												<input type="text" name="shipping_lastname" size="15" value="<?php echo $customer_details['lastname']; ?>" />  
											</td>
											<tr>
											 	<th>Company</th>
												<td><input type="text" name="shipping_company" size="15" value="<?php echo $customer_details['company_name']; ?>" /></td>
											...
											<tr>
												<th>Address : </th>
												<td>
													<input type="text" name="shipping_address1" size="15" value="<?php echo $customer_details['address_1'];?>" />  
												</td>
											</tr>
											<tr>
												<th>City : </th>
											...
											<tr>
												<th>State : </th>
												<td>
												<?php $state = $db->func_query_first("SELECT * FROM oc_zone WHERE zone_id ='". $customer_details['zone_id'] ."'"); ?>
													<input type="text" name="shipping_state" size="15" value="<?php echo $state['name']; ?>" /> 
												</td>
											</tr>
											<tr>
												<th>Country : </th>
											...
											<tr>
												<th>Zip : </th>
												<td><input type="text" name="shipping_zip" size="15" value="<?php echo $customer_details['postcode']; ?>" /> 
												</td>
											</tr>
											<tr><td colspan="2" align="center"><input type="submit" name="update_shipping_address" class="submit" value="Update Address"></td></tr>
										</table>
									...
									<td>
										<table width="100%" border="0" align="right">
											...
											<tr>
												<th>Name : </th>
												<td>
													<input type="text" name="bill_firstname" size="15" value="<?php echo $customer_details['firstname']; ?>" /> 
													<input type="text" name="bill_lastname" size="15" value="<?php echo $customer_details['lastname']; ?>" />  
												</td>
												 <tr>
													<th>Company</th>
													<td><input type="text" name="bill_company" size="15" value="<?php echo $customer_details['company_name']; ?>" /></td>
												...
												<tr>
													<th>Address : </th>
													<td>
														<input type="text" name="bill_address1" size="15" value="<?php echo $customer_details['address_1'];?>" /> 
													</td>
												</tr>
												<tr>
													<th>City : </th>
												...
												<tr>
													...
													<td>
												<?php $state = $db->func_query_first("SELECT * FROM oc_zone WHERE zone_id ='". $customer_details['zone_id'] ."'"); ?>
													<input type="text" name="bill_state" size="15" value="<?php echo $state['name']; ?>" /> 
												...
												<tr>
													<th>Zip : </th>
													<td><input type="text" name="bill_zip" size="15" value="<?php echo $customer_details['postcode']; ?>" /> 
													</td>
												</tr>
												<tr><td colspan="2" align="center"><input type="submit" class="submit" name="update_billing_address" value="Update Address"></td></tr>
											</table>
										...
										<?php } ?>
										<?php } else { ?>
										<input type="hidden" name="address_from_order" value="1">
										<input type="hidden" name="address_order_id" value="<?php echo $orders[0]['order_id'];?>">
										<td>
											<table width="100%" border="0" align="left">
												<h3 align="center">Default Shipping Address </h3>
												<tr>
													...
													<tr>
														<th>Company</th>
														<td><input type="text" name="shipping_company" size="15" value="<?php echo $orders[0]['company'];?>" /></td>
													</tr>
													<tr>
														<th>Address : </th>
														<td>
															<input type="text" name="shipping_address1" size="15" value="<?php echo $orders[0]['address1'];?>" /> 
															<input type="text" name="shipping_address2" size="15" value="<?php echo $orders[0]['address2'];?>" /> 
														</td>
													</tr>
													<tr>
														<th>City : </th>
														<td>
															<input type="text" name="shipping_city" size="15" value="<?php echo $orders[0]['city']; ?>" /> 
														</td>
													</tr>
													<tr>
														<th>State : </th>
														<td>
															<input type="text" name="shipping_state" size="15" value="<?php echo $orders[0]['state']; ?>" /> 
														</td>
													</tr>
													<tr>
														<th>Country : </th>
														<td>
															<input type="text" name="shipping_country" size="15" value="<?php echo $orders[0]['country']; ?>" /> 
														</td>
													</tr>
													<tr>
														<th>Zip : </th>
														<td>	<input type="text" name="shipping_zip" size="15" value="<?php echo $orders[0]['zip']; ?>" /> 
														</td>
													</tr>
													<tr><td colspan="2" align="center"><input type="submit" class="submit" name="update_shipping_address" value="Update Address"></td></tr>
												</table>
												<!-- <a type="submit" class="button " id="editAddress" name="editAddress" onclick="$.fancybox.open( $('#edit_address'), {afterClose: function(){}} )"/>Edit Address</a> -->
											</td>
											<td>
												<table width="100%" border="0" align="right">
													...
													<tr>
														...
														<tr>
															<th>Company</th>
															<td><input type="text" name="bill_company" size="15" value="<?php echo $orders[0]['billing_company'];?>" /></td>
														</tr>
														<tr>
															<th>Address : </th>
															<td>
																<input type="text" name="bill_address1" size="15" value="<?php echo $orders[0]['bill_address1'];?>" /> 
																<input type="text" name="bill_address2" size="15" value="<?php echo $orders[0]['bill_address2'];?>" /> 
															</td>
														</tr>
														<tr>
															<th>City : </th>
															<td>
																<input type="text" name="bill_city" size="15" value="<?php echo $orders[0]['bill_city']; ?>" /> 
															</td>
														</tr>
														<tr>
															<th>State : </th>
															<td>
																<input type="text" name="bill_state" size="15" value="<?php echo $orders[0]['bill_state']; ?>" /> 
															</td>
														</tr>
														<tr>
															<th>Country : </th>
															<td>
																<input type="text" name="bill_country" size="15" value="<?php echo $orders[0]['bill_country']; ?>" /> 
															</td>
														</tr>
														<tr>
															<th>Zip : </th>
															<td><input type="text" name="bill_zip" size="15" value="<?php echo $orders[0]['bill_zip']; ?>" /> 
															</td>
														</tr>
														<tr><td colspan="2" align="center"><input type="submit" class="submit" name="update_billing_address" value="Update Address"></td></tr>
													</table>
						...
						<table width="70%" border="0">
							<tr>
								...
								<td align="center">
									...
									<?php $cols = array('payment_firstname','payment_lastname','payment_address_1','payment_address_2','payment_city','payment_zone','payment_country','payment_postcode');
									...
									<?php foreach($billing_addresses as $address){ ?>
										<option data-fname='<?php echo $address['payment_firstname'];?>' data-lname='<?php echo $address['payment_lastname'];?>' data-ad1='<?php echo $address['payment_address_1'];?>' data-ad2='<?php echo $address['payment_address_2'];?>' data-city='<?php echo $address['payment_city'];?>' data-zone='<?php echo $address['payment_zone'];?>' data-postcode='<?php echo $address['payment_postcode'];?>' data-country='<?php echo $address['payment_country'];?>' value="" ><?php echo $address['payment_address_1'].', '.$address['payment_address_2'].', '.$address['payment_city'].', '.$address['payment_zone'].', '.$address['payment_postcode'].', '.$address['payment_country']; ?> </option>
										<?php } ?>
									</select>
								</td>
							</tr>
								...
								<table style="border-collapse:collapse;" border="1" width="100%" cellpadding="10" cellspacing="0">
									<tbody>
										<tr>
											...
											<td style="background-color: black;color: white">$<?= number_format($total_credit,2); ?></td>
											<th style="background-color: black;color: white">Payments Total:</th>
											<td style="background-color: black;color: white">$<?= number_format($total_debit,2); ?></td>
					...
					<div id="tabReturns" class="makeTabs">
						...
						?>
						<br /><br />
						<h2 align="center">Return Privileges</h2>
