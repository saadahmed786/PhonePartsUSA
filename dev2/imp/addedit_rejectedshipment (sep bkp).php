...
elseif($_POST['MoveNTR']){
	if(count($_POST['reject_ids']) > 0){
		foreach($_POST['reject_ids'] as $reject_id){
			...
			$item_cost = (float)$_POST['item_cost'][$reject_id];
			$from = $db->func_escape_string($_POST['package_number']);
			addItemToBox($reject_id , $_POST[$reject_id] , $_shipment_id , 'NTRBox' , $text,false,false,$item_cost,$from);

			$db->db_exec("update inv_rejected_shipment_items set deleted =1 where reject_item_id = '".$reject_id."' and rejected_shipment_id = '$shipment_id' limit 1");
		}
	}	
	else{
...
elseif($_POST['MoveRTS']){
	if(count($_POST['reject_ids']) > 0){
		foreach($_POST['reject_ids'] as $reject_id){
			...
			$item_cost = (float)$_POST['item_cost'][$reject_id];
			$from = $db->func_escape_string($_POST['package_number']);
			addItemToBox($reject_id , $_POST[$reject_id] , $_shipment_id , 'RTSBox' , $text,false,false,$item_cost,$from);
...
where rejected_shipment_id = '$shipment_id' and si.deleted=0 ";
...
<body>
	...
	<div align="center">
		...
		<form method="post" action="">
			...
			<div align="center">
				<br />
				<input type="submit" name="print" value="Print" onclick="if(!confirm('Are you sure?')){ return false; }" /> &nbsp&nbsp&nbsp&nbsp
				...
				<?php if($products):?>
					<table width="80%" cellspacing="0" cellpadding="5px" border="1" align="center" style="border-collapse:collapse;">
						...
						<tbody>
							...
							<tr>
								<td>
									<input class="selection" type="checkbox" name="reject_ids[<?php echo $product['reject_item_id'];?>]" value="<?php echo $product['reject_item_id'];?>" />
							...
							<?php if($shipment_detail['status'] == 'Shipped') {?>
								<td align="center">
									...
									<!-- <input type="text" name="reason[<?php echo $product['reject_item_id']?>]" value="<?php echo $product['reject_reason']?>" /> -->
								...
								<?php if ($received) { ?>
								<td align="center">
									<input type="checkbox" <?php echo (($product['received'])? 'checked="checked"': '');?> <?php echo (($shipment_detail['status'] != 'Received')? 'disabled=""': ''); ?> name="received[<?php echo $product['reject_item_id'];?>]" value="1" />
								</td>
								<?php } ?>
								<?php if (!$vtx) { ?>
								<?php if($shipment_detail['status'] != 'Completed'){?>
					...
					<script type="text/javascript">
						function funcBtnChanger(){
							...
							{
								case 'Pending':
								changeVal='Save And Shipped Shipment';
								xval = 'Shipped';
								break;
								case 'Shipped':
					...
					<?php
					$log= $db->func_query("SELECT * FROM inv_rj_shipment_items_log WHERE `from` = '".$shipment_detail['package_number']."'"); 
						...
						<table align="center" border="1" cellspacing="0" cellpadding="5" width="70%">
							...
							<tbody>
								...
								<tr>
									...
									<td><?php echo $logg['date_added']; ?></td>
