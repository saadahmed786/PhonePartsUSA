...
//orignal
$refunds = $db->func_query("SELECT v.voucher_id as v_id,v.amount,v.date_added,v.code,v.to_email,v.is_manual as manual,v.user_id as user,v.reason_id as manual_reason_id,v.voucher_items_reasons as regular_reasons ,vd.* FROM `oc_voucher` v LEFt JOIN `inv_voucher_details` vd on (v.voucher_id = vd.voucher_id) WHERE v.status = '1' AND DATE(v.date_added) = DATE(( NOW() - INTERVAL 1 DAY ))  ORDER BY v.`date_added` desc");

//$refunds = $db->func_query("SELECT v.amount,v.date_added,v.code,v.to_email,v.is_manual as manual,v.user_id as user,vp.*,vd.* FROM `oc_voucher` v LEFt JOIN `inv_voucher_details` vd on (v.voucher_id = vd.voucher_id) inner join inv_voucher_products vp on (vp.voucher_id = v.voucher_id) WHERE v.status = '1' AND DATE(v.date_added) =DATE(( NOW() - INTERVAL 1 DAY ))  ORDER BY v.`date_added` desc");
//testObject($refunds);

//$refunds = $db->func_query("SELECT v.amount,v.date_added,v.code,v.to_email,v.is_manual as manual,v.user_id as user ,vd.* FROM `oc_voucher` v LEFt JOIN `inv_voucher_details` vd on (v.voucher_id = vd.voucher_id) WHERE v.status = '1' AND DATE(v.date_added) in ('2017-07-25','2017-07-26','2017-07-27','2017-07-28','2017-07-29') ORDER BY v.`date_added` desc");
//testObject($refunds);

//$headers = array("Date Added","Ref RMA","Ref Order/Shipment ID","Voucher Code","Total Amount","Item Amount","Item SKU & Name","Sent To","Source","User","Reason");
...
$filename = "refund_reports/Store Credit Report-".$date.".csv";
...
//$mail->addAddress('gohar.chattha@gmail.com', 'Saad Ahmed');
$mail->Subject = ('Store Credits Report - PhonePartsUSA');
$mail->Body = 'Store Credits generated Yesterday. ';
$mail->IsHTML(true);
