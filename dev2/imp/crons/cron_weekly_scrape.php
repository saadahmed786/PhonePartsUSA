...
$rows = $db->func_query("SELECT * FROM inv_product_scrape_prices where DATE(date_updated)<>'".date('Y-m-d')."' AND DATE(date_updated)<>'".date('Y-m-d',strtotime('-1 days'))."'   LIMIT 25");
foreach($rows as $row)
{
...
$price = (float)str_replace($to_be_replaced,'',$scraped_data);
	if($price)
	{
		...
		echo $row['sku'].'--'.$price."<br>";
	}
	else
	{
		$db->db_exec("UPDATE inv_product_scrape_prices SET price='0.00',date_updated='".date("Y-m-d H:i:s")."',site_updated=1 WHERE id='".$row['id']."'");	
		echo $row['sku'].'--0.00<br>';
	}
}
?>
