...
// echo curl_proxy('https://www.mobilesentrix.com/lg-g3-lcd-no-frame-white');exit;
if ($db->func_query_first_cell('SELECT COUNT(id) FROM inv_product_price_scrap WHERE updated = "0" and url<>""') == 0) {
	// exit;
	$db->db_exec ("UPDATE inv_product_price_scrap SET updated = '0' where url<>''");
}
if(!isset($_POST['sku']))
{
	$rows = $db->func_query("SELECT * FROM inv_product_price_scrap where updated = '0' and url<>'' LIMIT 20");
}
else
{
...
foreach($rows as $row) {
	...
	if ($row['type'] == 'fixez') {
		...
		$scrap = scrape_between($scrap, '<div class="col-xs-6 mainPrice">', '<div class="col-xs-6">');  
		$scrap = scrape_between($scrap, '<span class="price">', '</span>');  
	...
	$db->func_array2update("inv_product_price_scrap", array('date_updated' => date("Y-m-d H:i:s"), 'updated' => '1'), "id = '". $row['id'] ."'");
	$output .= '---------------------<br>';
}
if(isset($_POST['sku']))
{
