...
$products = $db->func_query("SELECT product_id,model as sku FROM oc_product WHERE is_inout_updated=0 and status=1 limit 50");
if(isset($_GET['action']) && $_GET['action']=='reset')
{
	$db->db_exec("UPDATE oc_product SET is_inout_updated=0");
	echo 'Reset';
	exit;
}
     ...
     foreach($kit_skus_array as $product)
     {
       $sku = $product;         
		...
		$_temp =  $db->func_query_first("SELECT SUM(b.product_qty) as qty_sold,AVG(b.product_true_cost) as avg_cost,SUM(b.product_price) as total_sold 
			FROM inv_orders a 
			INNER JOIN inv_orders_items b on (a.order_id=b.order_id)
			
		 AND b.product_sku='".$sku."'  and date(a.order_date)='$today' AND LOWER(a.order_status) IN ( 'processed',  'shipped',  'completed',  'unshipped')");
		//$po_fba_orders =  $db->func_query_first("SELECT b.product_qty as qty_sold,AVG(b.product_true_cost) as avg_cost,SUM(b.product_price) as total_sold FROM inv_orders a,inv_orders_items b WHERE a.order_id=b.order_id AND b.product_sku='".$sku."'  and date(a.order_date)='$today' AND LOWER(a.order_status) IN ( 'processed',  'shipped',  'completed',  'unshipped')");
		...
		if($qty_received || $qty_sold || $avg_cost || $avg_price || $total_sold)
		{
			...
			$db->db_exec("UPDATE oc_product SET is_inout_updated=1 WHERE model='".$_product['sku']."'");
		}
		else
		{
			$db->db_exec("UPDATE oc_product SET is_inout_updated=1 WHERE model='".$_product['sku']."'");
