...
page_permission("device_type");
$page = 'device_type.php';
$mode = $_GET['mode'];

if($mode == 'edit'){
	$id = (int)$_GET['id'];
	$result = $db->func_query_first("select * from inv_d_type where id = '$id'");
}
if($mode=='delete')
{
	$id = (int)$_GET['id'];
	$db->db_exec("DELETE FROM inv_d_type WHERE id='".$id."'");	
...
if($_POST['add']){
	...
	if($id){
		...
		$db->func_array2update("inv_d_type",$array,"id = '$id'");
	}
	else{
		
		$id = $db->func_array2insert("inv_d_type",$array);
	}
		...
		<div align="center">
			<div align="center" style="display:none">
			 ...
			 <form action="" method="post" enctype="multipart/form-data">
			 	<h2> <a href="devices_settings.php" title="back">&laquo;</a> Device Type</h2>
			    <table align="center" border="1" width="60%" cellpadding="10" cellspacing="0" style="border:1px solid #585858;border-collapse:collapse;">
			         
			         
			           <tr>
			             <td>Code:</td>
			         	 <td><input type="text" name="code" value="<?php echo @$result['code'];?>" required /></td>
			         </tr>
			        <tr>
			             <td>Name:</td>
         ...
         <?php
		 $lists = $db->func_query("SELECT * FROM inv_d_type ORDER BY name");
