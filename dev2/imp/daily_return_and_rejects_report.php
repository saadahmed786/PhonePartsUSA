...
$types = array(
	...
	'CIB' => 'Customer Damage/Not PPUSA Part',
	'SDBox' => 'Shipping Damage',
	'NTRBox' => 'Need To Repair',
	'rejected_items' => 'Rejects',
	'unprocessed_items' => 'Unprocessed Return Items'
	);
if ($_POST['create']) {
	...
	//testObject($rejected_items);
	//echo "SELECT ri.*,r.rma_number,r.date_added as date_received inv_return_items ri INNER JOIN inv_returns r on (r.id = ri.return_id) WHERE DATE(r.date_added) >= '$start' AND DATE(r.date_added) <= '$end' AND r.rma_status = 'Received' AND r.source = 'storefront'  ORDER BY DESC";exit;
	$_query = "SELECT ri.*,r.rma_number,r.date_added as date_received,r.source as rma_source,r.order_id FROM inv_return_items ri INNER JOIN inv_returns r on (r.id = ri.return_id) WHERE DATE(r.date_added) >= '$start' AND DATE(r.date_added) <= '$end' AND r.rma_status = 'Received' AND r.source = 'storefront'  ORDER BY r.date_added DESC";
	$returns_unprocessed_items = $db->func_query($_query);
	//testObject($returns_unprocessed_items );
	if($gfs=='0'){
	$productsData = array('ItemIssueBox' => $ItemIssueBox, 'NotTestedBox' => $NotTestedBox, 'GFSBox' => $GFSBox, 'CIB' => $CIB, 'SDBox' => $SDBox, 'NTRBox' => $NTRBox, 'rejected_items' => $rejected_items, 'unprocessed_items' => $returns_unprocessed_items );
	} else{
		$productsData = array('ItemIssueBox' => $ItemIssueBox, 'NotTestedBox' => $NotTestedBox, 'CIB' => $CIB, 'SDBox' => $SDBox, 'NTRBox' => $NTRBox, 'rejected_items' => $rejected_items, 'unprocessed_items' => $returns_unprocessed_items );
	}
	// $productsData = array();
	$height = 10;
	$html .= '<style>';
	...
	foreach ($productsData as $key => $products) {
		...
		if ($key == 'unprocessed_items') {
			$tableHead = '<tr class="head"> <th class="small">Date Received</th><th class="small">RMA</th> <th class="order">OrderID</th><th class="return_id">Return ID</th> <th>SKU</th> <th class="title">Title</th> <th class="return_id">Reason</th><th>Released</th> <th>Received</th>  </tr>';	
		}else{
		...
		foreach ($products as $it) {
				...
				}else if ($it['rejected_shipment_id']){
					$vendor = get_username($db->func_query_first_cell("SELECT `vendor` FROM inv_rejected_shipments WHERE id = '". $it['rejected_shipment_id'] ."'"));
				} 
				else{
					$vendor = 'N/A';
				}
			...
			//$vendor = $db->func_query_first_cell("SELECT `vendor` FROM oc_product a, oc_product_description b WHERE a.product_id = b.product_id AND sku = '". $it['product_sku'] ."'");
				if ($key == 'rejected_items') {
					...
					if ($it['rma_number']) {
						if ($storefront == '1'){
							$it['source'] = $db->func_query_first_cell( "SELECT source from inv_returns where source = 'storefront' AND rma_number = '". $it['rma_number'] ."'" );
						} else {
				...
				if ($key == 'unprocessed_items') {
					$html .= '<tr class="data"> <td class="small">'.americanDate($it['date_received']).'</td><td class="small">'.$it['rma_number'].'<br><small>('.$it['rma_source'].')</small></td><td class="order">'.$it['order_id'].'</td> <td class="return_id">'. (($it['return_item_id']) ? $it['return_item_id']: 'N/A') .'</td> <td>'.$it['sku'].'</td> <td class="title">'.$it['title'].'</td> <td class="return_id">'.$it['return_code'].'</td><td>&nbsp;</td> <td>&nbsp;</td> </tr>';
					$height += 37;
				}else if ($key == 'rejected_items' && $it['source']) {
					...
					$height += 37;
				} else if ($key != 'rejected_items' || ($key == 'rejected_items' && $storefront == '0' )){
