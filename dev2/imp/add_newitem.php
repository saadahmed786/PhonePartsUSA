...
if($_POST['add']){
	if($_POST['title']){
		...
		if ($_POST['sku_type']) {
			// print_r((( $db->func_query_first_cell('select model from oc_product where model like "'. $_POST['sku_type'] .'%" order by model DESC;'))));exit;
			...
			// $_POST['sku'] = (( $db->func_query_first_cell('select model from oc_product where model like "'. $_POST['sku_type'] .'%" order by model DESC;')));
			$_POST['sku'] = (( $db->func_query_first_cell('select max(abs(replace(model,"'.$_POST['sku_type'].'",""))) as new_sku from oc_product where model like "'. $_POST['sku_type'] .'%" order by model DESC;')));
			$_POST['sku'] = explode("-", $_POST['sku']);
			$_POST['sku'] = end($_POST['sku']);
			// echo $_POST['sku'];exit;
