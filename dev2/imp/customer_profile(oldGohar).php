...
if($_POST['action']=='updateSalesAgent')
{
	...
	$db->db_exec("UPDATE inv_customers SET user_id='".$user_id."' WHERE email='".$customer_email."' ");	
...
foreach($temp_order as $i => $order) { 
	...
	$orderData .= '<td width="250px;">'.($order['manual']?'':''.getComments($order['order_id'])).'</td>';

	$orderData .= '<td>'.($order['manual']?'':''.getAttachments($order['order_id'])).'</td>';
	if($_SESSION['display_cost']) {
		$orderData .= '<td>$'.number_format($credit, 2).'</td>';	
		$orderData .= '<td>$'.number_format($debit,2).'</td>';
		$orderData .= '<td>$'.number_format($balance,2).'</td>';	
	}
	$orderData .= '</tr>';
		...
		<div align="center">
			...
			<div class="tabHolder">
					...
					<div id="tabCDetails" class="makeTabs">
						<table width="70%" border="1" cellpadding="10" cellspacing="0" style="border-collapse:collapse;">
							...
							<tr>
								...
								<td>Customer Name:</td>
								<td><b><?php echo $customer_details['firstname']." ".$customer_details['lastname'];?></b></td>
							</tr>
						...
						<table width="70%" border="1" cellpadding="10" cellspacing="0" style="border-collapse:collapse;">
							<tr>
								...
								<td>
									<b>Latest Shipping Address</b> <br /><br />
								...
								<td>
									<b>Lattest Billing Address</b> <br /><br />
						...
						<div align="left" style="width:70%">
							...
							<a class="fancybox2 fancybox.iframe" href="popupfiles/billing_addresses.php?customer_id=<?php echo $customer_id;?>">Billing:__</a>   
							&nbsp; 
							<a class="fancybox2 fancybox.iframe" href="popupfiles/shipping_addresses.php?customer_id=<?php echo $customer_id;?>">Shipping:__</a>
						</div>
						<br><br>
						<?php } ?>
								...
								<table style="border-collapse:collapse;" border="1" width="100%" cellpadding="10" cellspacing="0">
									<tbody>
										<tr>
											...
											<th>Balance:</th>
											<td><span id="span_balance">$<?= number_format($balance,2); ?></span></td>
										</tr>
			  ...
			  <?php $i=$i+1; ?>
				...
				<a class="button" href="customer_profile.php?customer_id=<?php echo $address1[$i]['customer_id'];?>&address=<?php echo $address1[$i]['address_id'];?>"  name="save_changes" >Save Changes</a>
				...
				success: function(json){
					if (json['success']) {
						$.ajax({
							...
							type: 'POST',
							dataType: 'json',
							data: {sendEmailPassword: 'yes', password: json['password']},
						})
						.always(function(json) {						
							alert(json['message']);
						});
