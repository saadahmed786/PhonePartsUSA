...
{
  $query = "select count(x.rec_$start) rec_$start,x.monthx,x.yearx from ( select count(*) rec_$start,order_date,month(order_date) monthx,year(order_date) yearx from inv_orders where lower(order_status) in ('processed','shipped','completed') and email not like '%@marketplace.amazon%' group by lower(email),month(order_date),year(order_date) having sum(sub_total+shipping_amount+tax)>$start and sum(sub_total+shipping_amount+tax)<=$end order by order_date desc) x group by x.monthx,x.yearx order by x.order_date desc limit 13";
  return $query;
}
                  ...
                  $sum = 0;
                    foreach($denom as $deno)
                    {
                     ...
                     if(!isset($result[$i]['rec_'.$deno]))
                     {
                      ...
                      $html.='<td>'.round($result[$i]['rec_'.$deno]).'</td>';
...
if(isset($_POST['type']) and $_POST['type']=='80_sale')
{
  ...
  //$rows = $db->func_query("select  a.email,sum(a.order_price) as order_price1 from inv_orders a where a.order_date > CURRENT_DATE - INTERVAL 30 DAY  AND LOWER(a.order_status) in ('processed','shipped','completed') group by a.email having order_price1>500 order by order_price1 desc limit 100");
    ...
    $rows = $db->func_query("select  a.email,sum(a.sub_total+a.shipping_amount+a.tax) as order_price1 from inv_orders a where  a.order_date > CURRENT_DATE - INTERVAL 30 DAY  AND LOWER(a.order_status) in ('processed','shipped','completed') and lower(a.email) not like '%@phonepartsusa.com%' group by a.email having order_price1>500 order by order_price1 desc limit 60");
