...
$perission = 'customers';
$pageName = 'Privilege';
$pageLink = 'addedit_privileges.php';
...
if ($_POST['update']) {
	...
	$_POST['privilege'] = str_replace(' ', '_', strtolower($_POST['name']));
...
$rows = $db->func_query("SELECT privilege_id as id, name, privilege_type_id as type_id FROM $table order by privilege_id DESC");
$types = $db->func_query("SELECT privilege_type_id as id, name FROM inv_privilege_type order by privilege_type_id DESC");
			...
			<tr>
				<td width="50%">
					<div class="contain_list">
						<table id="list" align="center" border="0" width="100%" cellpadding="10" cellspacing="0" style="border:0px solid #585858;border-collapse:collapse;">
							...
							<tr type-id="<?php echo $row['type_id']; ?>" data-id="<?php echo $row['id']; ?>">
				...
				<td width="50%">
					<table align="center" border="0" width="100%" cellpadding="10" cellspacing="0" style="border:0px solid #585858;border-collapse:collapse; text-align: right;">
						<tr>
							<td>
							<select id="type" column-name="privilege_type_id" style="width: 100%;">
						...
						<tr>
							<td>
								...
								<input type="hidden" column-name="privilege_id" name="id" />
							</td>
						</tr>
						<tr>
		...
		function loadReasons () {
			...
			.always(function(json) {
				...
				for (var i = 0; i < json.length; i++) {
					html += '<tr data-id="' + json[i]['id'] + '" type-id="' + json[i]['type_id'] + '"><td>' + json[i]['name'] + '</td></tr>';
				}
				$('#list').html(html);
			});
		}
