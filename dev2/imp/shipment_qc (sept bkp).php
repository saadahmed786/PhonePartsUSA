	...
	foreach($_POST['products'] as $product){
		...
		if ($_POST['QcComplete']) {
			...
			if($qc_shipment['rejected'] > 0){
				...
				addToRejectedShipment($product['product_sku'], $product['rejected'] , $shipment_id,$product['rejected_reason']);
		...
		foreach($_POST['products'] as $product){
			...
			// echo 'New Cost: '.$new_cost.'<br><br>===============================<br>';
...

				{
					...
					$db->db_exec("INSERT INTO inv_avg_cost SET sku='".$product['product_sku']."',cost='".getTrueCost($product['product_sku'])."',true_cost='".getTrueCost($product['product_sku'])."',date_added='".date('Y-m-d H:i:s')."'"); }
			...
			if($rejected || $ntr){
				...
				$last_id = $db->func_query_first_cell("select id from inv_rejected_shipments where vendor = '". $shipment_detail['vendor'] ."' and status = 'Issued'");
									...
									<tr class="row_<?php echo $product['product_sku'];?>">
										...
										<td align="center">
											...
											<div class="product_<?php echo $product['product_sku'];?>_row" <?php if($product['accept_all'] == 1 || is_null($product['accept_all'])):?> style="display:none;" <?php endif;?>>
												...
												<input type="text" size="5" name="products[<?php echo $i?>][rejected]" value="<?php echo $product['rejected']?>" /><br><br>
										...
										<td align="center">
											...
											<div class="product_<?php echo $product['product_sku'];?>_row" <?php if($product['accept_all'] == 1 || is_null($product['accept_all'])):?> style="display:none;" <?php endif;?>>
												...
												<input type="text" size="5" name="products[<?php echo $i?>][ntr]" value="<?php echo $product['ntr']?>" /><br><br>
