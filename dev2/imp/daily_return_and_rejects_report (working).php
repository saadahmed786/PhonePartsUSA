...
if ($_POST['create']) {
	...
	$today = date('m/d/Y');
	$_query = "SELECT si . *, s.box_number, s.box_type from inv_return_shipment_box_items si inner join inv_return_shipment_boxes s ON (s.id = si.return_shipment_box_id) WHERE date(si.date_added) >= '$start' AND date(si.date_added) <= '$end' AND box_type = 'ItemIssueBox' order by `source` DESC, si.date_added DESC";

	$_query1 = "SELECT si . *, s.box_number, s.box_type from inv_return_shipment_box_items si inner join inv_return_shipment_boxes s ON (s.id = si.return_shipment_box_id) WHERE date(si.date_added) >= '$start' AND date(si.date_added) <= '$end' AND box_type = 'ItemIssueBox' order by `source` DESC, si.date_added DESC";

	$ItemIssueBox = $db->func_query($_query);
	$_query = "SELECT si . *, s.box_number, s.box_type from inv_return_shipment_box_items si inner join inv_return_shipment_boxes s ON (s.id = si.return_shipment_box_id) WHERE date(si.date_added) >= '$start' AND date(si.date_added) <= '$end' AND box_type = 'NotTestedBox' order by `source` DESC, si.date_added DESC";
	$NotTestedBox = $db->func_query($_query);
	$_query = "SELECT si . *, s.box_number, s.box_type from inv_return_shipment_box_items si inner join inv_return_shipment_boxes s ON (s.id = si.return_shipment_box_id) WHERE date(si.date_added) >= '$start' AND date(si.date_added) <= '$end' AND box_type = 'RTSBox' order by `source` DESC, si.date_added DESC";
	$RTSBox = $db->func_query($_query);
	$_query = "SELECT si . *, s.box_number, s.box_type from inv_return_shipment_box_items si inner join inv_return_shipment_boxes s ON (s.id = si.return_shipment_box_id) WHERE date(si.date_added) >= '$start' AND date(si.date_added) <= '$end' AND box_type = 'NTRBox' order by `source` DESC, si.date_added DESC";
	$NTRBox = $db->func_query($_query);
	$_query = "SELECT si . *, s.box_number, s.box_type from inv_return_shipment_box_items si inner join inv_return_shipment_boxes s ON (s.id = si.return_shipment_box_id) WHERE date(si.date_added) >= '$start' AND date(si.date_added) <= '$end' AND box_type = 'CIB' order by `source` DESC, si.date_added DESC";
	$CIB = $db->func_query($_query);
	$_query = "SELECT si . *, s.box_number, s.box_type from inv_return_shipment_box_items si inner join inv_return_shipment_boxes s ON (s.id = si.return_shipment_box_id) WHERE date(si.date_added) >= '$start' AND date(si.date_added) <= '$end' AND box_type = 'SDBox' order by `source` DESC, si.date_added DESC";
	...
	$productsData = array('ItemIssueBox' => $ItemIssueBox, 'NotTestedBox' => $NotTestedBox, 'RTSBox' => $RTSBox, 'CIB' => $CIB, 'SDBox' => $SDBox, 'NTRBox' => $NTRBox, 'rejected_items' => $rejected_items );
	// $productsData = array();
	$height = 10;
	$html .= '<style>';
