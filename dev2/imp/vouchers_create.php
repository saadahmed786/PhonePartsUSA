	...
	<div align="center">
		...
		<form action="" method="post" onsubmit="return formsubmit();" enctype="multipart/form-data">
			...
			<table align="center" border="1" width="60%" cellpadding="10" cellspacing="0" style="border:1px solid #585858;border-collapse:collapse;">
				<tr>
					<th>Amount</th>
					<td><input type="text" onkeyup="allowFloat(this);" name="amount" value="<?= ($voucher_id) ? number_format((float)$voucher['amount'], 2, '.', ''): '';?>" /></td>
				</tr>  
				<tr>
					<th>Voucher code</th>
				...
				<tr>
					...
					<td>
						<select required name="update_reason">
							...
							<?php
							 $reasons = $db->func_query('SELECT * FROM inv_voucher_reasons where reason_type = "Update" order by id desc'); ?>
							 <option value="">Select</option>
							<?php foreach ($reasons as $reason) { ?>
