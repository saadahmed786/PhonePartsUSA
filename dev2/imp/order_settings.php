...
$perission = 'order_reasons';
$pageName = 'Order Reason';
$pageLink = 'order_settings.php';
$pageCreateLink = false;
$pageSetting = false;
$table = '`inv_order_reasons`';
...
<body>
			...
			<table align="center" border="1" width="60%" cellpadding="10" cellspacing="0" style="border:1px solid #585858;border-collapse:collapse;">
				...
				<tr>
					...
					<div style="display: none;">
						<?php $types = array('order', 'item' );?>
						<select name="type">
							<?php foreach ($types as $type) { ?>
								<option value="<?= $type; ?>" <?= ($editData['type'] == $type)? 'selected="selected"': ''; ?>><?= $type; ?></option>
							<?php } ?>
						</select>
					</div>
					</td>
	...
	<div>
		<table align="center" border="1" width="60%" cellpadding="10" cellspacing="0" style="border:1px solid #585858;border-collapse:collapse;">
			...
			<tbody>
				<?php foreach ($db->func_query('SELECT * FROM `inv_order_reasons`') as $i => $row) { ?>
				<tr>
					<td><?= ($i) + 1; ?></td>
					<td><?= $row['name']; ?></td>
					<td><?= $row['type']; ?></td>
