...
$pageName = 'Charge Back Reason';
$pageLink = 'chargeback_settings.php';
$pageCreateLink = false;
$pageSetting = false;
$table = '`inv_chargeback_settings`';
...
<body>
	...
	<div>
		<table align="center" border="1" width="60%" cellpadding="10" cellspacing="0" style="border:1px solid #585858;border-collapse:collapse;">
			...
			<tbody>
				<?php foreach ($db->func_query('SELECT * FROM `inv_chargeback_settings`') as $i => $chargeback) { ?>
				<tr>
					<td><?= ($i) + 1; ?></td>
					<td><?= $chargeback['name']; ?></td>
					<td>
						<a href="<?= $pageLink .'?edit='. $chargeback['id']; ?>">Edit</a>
						<a href="<?= $pageLink .'?delete='. $chargeback['id']; ?>">Delete</a>
					</td>
				</tr>
				<?php } ?>
